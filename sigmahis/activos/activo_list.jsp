...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  sql = "select codigo_detalle, nvl(descripcion,'---') as descripcion from tbl_con_detalle_otro where cod_compania="+(String)session.getAttribute("_companyId")+appendFilter+" order by codigo_detalle";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
 rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_con_detalle_otro where cod_compania="+(String)session.getAttribute("_companyId")+appendFilter);
...
document.title = 'Prueba - '+document.title;

function add()
{
	abrir_ventana('../activos/activo_config.jsp');
}

function edit(id)
{
	abrir_ventana('../activos/activo_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../activos/print_list_activos.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
