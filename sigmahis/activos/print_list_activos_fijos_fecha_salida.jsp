...
if (!fechaInicial.equalsIgnoreCase("") && !fechaFinal.equalsIgnoreCase("")){
   appendFilter += "  and trunc(a.final_garantia) between to_date('"+fechaInicial+"','dd/mm/yyyy') and to_date('"+fechaFinal+"','dd/mm/yyyy') ";
} 
else if (!fechaFinal.equalsIgnoreCase("")){
   appendFilter += "  and trunc(a.final_garantia) <= to_date('"+fechaFinal+"','dd/mm/yyyy')";
}
else if (!fechaInicial.equalsIgnoreCase("")){
   appendFilter += "  and trunc(a.final_garantia) >= to_date('"+fechaInicial+"','dd/mm/yyyy')";
}

sql="select a.secuencia, a.entrada_codigo, a.cuentah_activo, a.cuentah_espec, b.descripcion detalleOtros, a.estatus, decode(a.estatus,'ACTI','ACTIVO','RETIR','INACTIVO') estatusDsp, a.tipo_activo, a.cod_provee, a.cuentah_activo||'-'||a.cuentah_espec||'-'||a.cuentah_detalle||'-'||b.descripcion listado_activo , to_char(a.fecha_de_entrada,'dd/mm/yyyy') fecha_entrada, a.observacion, a.cod_articulo, a.cod_clase, a.cod_flia, a.porcentaje, nvl(a.placa,a.placa_nueva) placa, c.descripcion unidad_ejec, decode(a.tipo_activo,'I','INMUEBLE','B','BIEN','T','TERRENO') tipo, nvl(a.valor_deprem,0) valor_deprem , nvl(a.acum_deprec,0) acum_deprec, nvl(a.acum_deprem,0) acum_deprem, nvl(a.valor_actual,0) valor_actual, nvl(a.valor_inicial,0) valor_inicial, a.vida_estimada,a.cuentah_activo||'---'||d.descripcion as cuentaEspecificacion, to_char(a.final_garantia,'dd/mm/yyyy') f_garantia from tbl_con_activos a, tbl_con_detalle_otro b, tbl_sec_unidad_ejec c,tbl_con_especificacion d where a.compania="+(String)session.getAttribute("_companyId")+appendFilter+" and a.compania = b.cod_compania(+) and a.cuentah_detalle = b.codigo_detalle(+) and a.compania = c.compania(+) and a.ue_codigo = c.codigo(+) and a.cuentah_Activo=d.cta_control(+)  and a.cuentah_espec=d.codigo_espec order by a.fecha_de_entrada , to_number(a.secuencia)";

 al = SQLMgr.getDataList(sql);
 
 String _xtraSubtitle = "";
 if (!fechaInicial.trim().equals("") && !fechaFinal.trim().equals("")) _xtraSubtitle = "Desde "+fechaInicial+"   Hasta "+fechaFinal;
 else if (!fechaInicial.trim().equals("")) _xtraSubtitle = "Desde "+fechaInicial;
 else if (!fechaFinal.trim().equals("")) _xtraSubtitle = "Hasta "+fechaFinal;
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String xtraSubtitle = _xtraSubtitle;
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.addCols(" "+cdo.getColValue("f_garantia"),0,1);
