...
sql = "select a.cuentah_activo, a.cuentah_espec, a.cuentah_detalle, a.secuencia, a.observacion, nvl(d.valor_act_ant,0) valor_act_ant, nvl(d.monto_depre,0) monto_depre, nvl(d.monto_depre_mejora,0) m_depre_mej, nvl(d.valor_activo_act,0) valor_activo, nvl(d.valor_mejora_ant,0) valor_mejora_ant, nvl(d.valor_mejor_act,0) valor_mejor_act, (nvl(valor_act_ant,0) -( nvl(monto_depre,0)+ nvl(monto_depre_mejora,0)) ) valor_actual, d.cd_mes, o.descripcion from tbl_con_activos a,	tbl_con_deprec_mensual d, tbl_con_detalle_otro o where a.secuencia = d.activo_sec and a.compania = "+(String) session.getAttribute("_companyId")+" and d.compania = a.compania and d.cd_ano = nvl("+anio+",d.cd_ano) and d.cd_mes = nvl("+mes+",0) and d.activo_sec >= to_number(nvl("+desde+",d.activo_sec)) and d.activo_sec <= to_number(nvl("+hasta+",d.activo_sec)) and o.codigo_detalle = a.cuentah_detalle and o.cod_compania = a.compania order by  o.descripcion, to_number(a.secuencia)";
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
 ...
 pc.setNoColumnFixWidth(dDetalle);
	...
	pc.createTable();
		...
		pc.addCols("CUENTA GASTO DE DEPRECIACION ",1,4);	
		pc.addCols(" ",1,1);
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.setVAlignment(0);
		 ...
		 pc.setNoColumnFixWidth(dDetalle); 
			...
			pc.addCols(" "+cdo.getColValue("cta1_gast_depre"),2,1);
			pc.addCols(" "+cdo.getColValue("cta2_gast_depre"),2,1);
			pc.addCols(" "+cdo.getColValue("ctas3"),2,1);
			pc.addCols(" "+cdo.getColValue("cta4_gast_depre"),2,1);
			pc.addCols(" ",1,1);
	...
	else
	{
		...
		pc.addCols(" Total: "+cdo1.getColValue("totales"),2,7);
		pc.addCols(" ",0,1);		
		pc.addCols(" ",1,8);
		pc.addCols(" ",1,8);
			
		pc.addCols("NOMBRE DE LA CUENTA",0,1);
		pc.addCols("CUENTA DEPRECIACION ACUMULADA",1,4);	
	...
	{
	CommonDataObject cdo3 = SQLMgr.getData(sqlU);
		pc.addCols(" Total: "+cdo3.getColValue("total"),2,7);
		pc.addCols(" ",0,1);		
