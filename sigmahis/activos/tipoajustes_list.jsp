...
900085  VER LISTA DE TIPOS DE AJUSTES
900086  IMPRIMIR LISTA DE TIPOS DE AJUSTES
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  {
    appendFilter += " where upper(cod_ajuste) like '%"+request.getParameter("codigo").toUpperCase()+"%'";
    searchOn = "cod_ajuste";
  ...
  sql = "SELECT cod_ajuste as codigo, descripcion FROM tbl_con_tipo_ajuste "+appendFilter+" order by cod_ajuste, descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_con_tipo_ajuste "+appendFilter);
...
document.title = 'Tipo Ajustes - '+document.title;

function add()
{
  abrir_ventana('tipoajustes_config.jsp');
}

function edit(id)
{
  abrir_ventana('tipoajustes_config.jsp?mode=edit&id='+id);
}

function printList()
{
  abrir_ventana('print_list_tipo_ajustes.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
      ...
      {
        %>
      <a href="javascript:add()" class="Link00">[ Registrar Nuevo Tipo de Ajuste ]</a>
        <%
     }
      %>
      </td>
