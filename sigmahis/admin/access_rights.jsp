...
if (url != null && !url.trim().equals("")) appendFilter += " and upper(v.name||decode(v.qs,null,nvl(x.qs,''),'?'||v.qs||decode(x.qs,null,'','&'||x.qs))) like '%"+url.toUpperCase()+"%'";
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	if (change == null)
	{
		sql = "select lpad(entitlement_code,8,'0') as entitlement_code from tbl_sec_profile_entitlements where profile_id="+id+"";
		...
		for (int i=0; i<al.size(); i++)
		{
			...
			cdo.setTableName("tbl_sec_profile_entitlements");
			cdo.addColValue("profile_id",id);
...
//	sql = "select substr(lpad(x.entitlement_code,8,'0'),0,2) as module_id, lpad(x.entitlement_code,8,'0') as entitlement_code, x.entitlement_desc, y.name as module_name from tbl_sec_entitlements x, tbl_sec_module y where substr(lpad(x.entitlement_code,8,'0'),0,2)=lpad(y.id,2,'0') and x.entitlement_code!=0"+appendFilter+" order by 1, 2";
	if (request.getParameter("module") != null)
	{
	sql = "select substr(lpad(z.entitlement_code,8,'0'),0,2) as module_id, lpad(z.entitlement_code,8,'0') as entitlement_code, z.entitlement_desc, y.name as module_name, nvl(v.name||decode(v.qs,null,nvl(x.qs,''),'?'||v.qs||decode(x.qs,null,'','&'||x.qs)),' ') as url from tbl_sec_entitlements z, tbl_sec_module y, tbl_sec_page_entitlement x, tbl_sec_pages v where substr(lpad(z.entitlement_code,8,'0'),0,2)=lpad(y.id,2,'0') and z.entitlement_code!=0 and z.entitlement_code=x.entitlement_code(+) and x.page_id=v.id(+)"+appendFilter+" order by 5, 1, 2";
	//al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	al = SQLMgr.getDataList(sql);
	rowCount = CmnMgr.getCount("select count(*) from tbl_sec_entitlements z, tbl_sec_module y, tbl_sec_page_entitlement x, tbl_sec_pages v where substr(lpad(z.entitlement_code,8,'0'),0,2)=lpad(y.id,2,'0') and z.entitlement_code!=0 and z.entitlement_code=x.entitlement_code(+) and x.page_id=v.id(+)"+appendFilter);
	}
...
<tr class="TextHeader">
	<td colspan="2" align="center"><cellbytelabel>PERFIL</cellbytelabel>: <%=cdo.getColValue("profile_name")%></td>
</tr>
<tr class="TextFilter">
	...
	<td colspan="2">
		...
		<cellbytelabel>URL</cellbytelabel>
		<%=fb.textBox("url","",false,false,false,40,"Text10",null,null)%>
		<%=fb.submit("go","Ir")%>
	</td>
	<%=fb.formEnd()%>
...
<%=fb.hidden("url",url).replaceAll(" id=\"url\"","")%>
		<tr class="TextPager">
			<td colspan="4" align="right">
				...
				<%=fb.submit("saveT","Guardar",true,false)%>
		...
		<tr class="TextHeader01" align="center">
			<td colspan="3">Derecho de Acceso</td>
			<td>URL</td>
		</tr>
<%
String moduleName = "";
...
%>
		<tr class="TextHeader01">
			<td colspan="4">
				<%=fb.checkbox("module"+cdo.getColValue("module_id"),cdo.getColValue("module_id"),false,false,null,null,"onClick=\"javascript:checkAll('"+cdo.getColValue("module_id")+"')\"")%>
...
%>
		...
		<%=fb.hidden("entitlement"+i,cdo.getColValue("entitlement_code"))%>
		<tr class="TextRow01">
			<td width="4%" align="right"><%=fb.checkbox("entitlement_check"+i,"y",vAccess.contains(cdo.getColValue("entitlement_code")),false,null,null,(cdo.getColValue("entitlement_code").trim().endsWith("00"))?"":"onClick=\"javascript:checkParent(this,'"+parent+"')\"")%></td>
			<td width="9%" align="center"><label for="entitlement_check<%=i%>"<%=style%>>[<%=cdo.getColValue("entitlement_code")%>]</label></td>
			<td width="55%"><label for="entitlement_check<%=i%>"<%=style%>><%=cdo.getColValue("entitlement_desc")%></label></td>
			<td width="32%"><%=(cdo.getColValue("url").trim().equals(""))?"* DERECHO DE ACCESO SIN ASIGNAR PAGINA *":((cdo.getColValue("entitlement_code").trim().endsWith("00"))?cdo.getColValue("url"):"")%></td>
		</tr>
<%
...
%>
		...
		<tr class="TextPager">
			<td colspan="4" align="right">
				...
				<%=fb.submit("saveB","Guardar",true,false)%>
...
else
{
	...
	if (SQLMgr.getErrCode().equals(MessageCode.SUCCESS)) SQLMgr.execute("call sp_sec_save_profile_menu_tree("+id+")",false,true);
