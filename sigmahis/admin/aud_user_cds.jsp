...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	sbSql.append(", (select descripcion from tbl_cds_centro_servicio where codigo = z.codigo) as descripcion");
	sbSql.append(", (select descripcion from tbl_cds_centro_servicio where codigo = nvl(z.p_codigo,z.codigo)) as p_descripcion");
	sbSql.append(", (select (select nombre from tbl_sec_compania where codigo = compania_unorg) from tbl_cds_centro_servicio where codigo = z.codigo) as compania");
	sbSql.append(", (select (select nombre from tbl_sec_compania where codigo = compania_unorg) from tbl_cds_centro_servicio where codigo = nvl(z.p_codigo,z.codigo)) as p_compania");
	sbSql.append(" from (");

	for (int i=0; i<audschema.length; i++) {
		if (i>0) sbSql.append(" union all ");
		sbSql.append("select user_id, cds as codigo, aud_timestamp, aud_webuser_ip, aud_action");
		sbSql.append(", lag(cds) over (partition by user_id, cds order by user_id, cds, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_codigo");
		sbSql.append(" from ");
		sbSql.append(audschema[i].replaceAll("@@","."));
		sbSql.append("sec_user_cds a where user_id = ");
		sbSql.append(id);
		if (!audUser.trim().equals("")) { sbSql.append(" and upper(aud_webuser_ip) like '%"); sbSql.append(audUser); sbSql.append("%'"); }
		if (!code.trim().equals("")) { sbSql.append(" and cds = ").append(code); }
		if (!descrip.trim().equals("")) {	sbSql.append(" and exists (select null from tbl_cds_centro_servicio where codigo = a.cds and upper(descripcion) like '%").append(descrip.toUpperCase()).append("%')"); }
	}
...
document.title = 'Auditoría de CDS del Usuario - '+document.title;
var xHeight=0;
parent.tLoaded[2]=true;
...
<tr class="TextFilter">
	<td>
		<cellbytelabel>CDS</cellbytelabel>
...
%>
				<tr class="TextHeader01"><td colspan="6" align="center"><label class="Text14Bold SpacingText">CDS --> <%=cdo.getColValue("descripcion")%></label></td></tr>
<% } %>
