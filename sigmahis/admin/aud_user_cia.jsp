...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	if (audAction.equalsIgnoreCase("INS")) {
		...
		sbFilter.append(" and aud_action = 'UPD' and ( codigo <> nvl(p_codigo,-999) or status <> nvl(p_status,'-') )");
	} else if (audAction.equalsIgnoreCase("DEL")) {
		sbFilter.append(" and aud_action = 'DEL'");
	} else {
		sbFilter.append(" and ( codigo <> nvl(p_codigo,-999) or status <> nvl(p_status,'-')/**/ or aud_action in ('INS','DEL') )");
	}
	...
	sbSql.append("select z.user_id, z.codigo, z.status, decode(z.status,'A','ACTIVO','I','INACTIVO',z.status) as estado, nvl(z.p_codigo,z.codigo) as p_codigo, nvl(z.p_status,z.status) as p_status, decode(nvl(z.p_status,z.status),'A','ACTIVO','I','INACTIVO',nvl(z.p_status,z.status)) as p_estado");
	sbSql.append(", to_char(z.aud_timestamp,'dd/mm/yyyy hh24:mi:ss') as aud_date, z.aud_webuser_ip as aud_user, z.aud_action");
	sbSql.append(", (select nombre from tbl_sec_compania where codigo = z.codigo) as descripcion");
	sbSql.append(", (select nombre from tbl_sec_compania where codigo = nvl(z.p_codigo,z.codigo)) as p_descripcion");
	sbSql.append(" from (");

	for (int i=0; i<audschema.length; i++) {
		if (i>0) sbSql.append(" union all ");
		sbSql.append("select user_id, company_id as codigo, status, aud_timestamp, aud_webuser_ip, aud_action");
		sbSql.append(", lag(company_id) over (partition by user_id, company_id order by user_id, company_id, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_codigo");
		sbSql.append(", lag(status) over (partition by user_id, company_id order by user_id, company_id, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_status");
		sbSql.append(" from ");
		sbSql.append(audschema[i].replaceAll("@@","."));
		sbSql.append("sec_user_comp a where user_id = ");
		sbSql.append(id);
		if (!audUser.trim().equals("")) { sbSql.append(" and upper(aud_webuser_ip) like '%"); sbSql.append(audUser); sbSql.append("%'"); }
		if (!code.trim().equals("")) { sbSql.append(" and company_id = ").append(code); }
		if (!descrip.trim().equals("")) {	sbSql.append(" and exists (select null from tbl_sec_compania where codigo = a.company_id and upper(nombre) like '%").append(descrip.toUpperCase()).append("%')"); }
	}

	sbSql.append(") z");
	if (sbFilter.length() != 0) sbSql.append(sbFilter.replace(0,4," where"));
	sbSql.append(" order by user_id, codigo, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)");
...
document.title = 'Auditoría de Compañía del Usuario - '+document.title;
var xHeight=0;
parent.tLoaded[6]=true;
...
%>
				<tr class="TextHeader01"><td colspan="6" align="center"><label class="Text14Bold SpacingText">COMPA&Ntilde;IA --> <%=cdo.getColValue("descripcion")%></label></td></tr>
<% } %>
