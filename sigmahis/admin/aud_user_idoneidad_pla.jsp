...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	if (audAction.equalsIgnoreCase("INS")) {
		...
		sbFilter.append(" and aud_action = 'UPD' and ( codigo <> nvl(p_codigo,-999) or compania <> nvl(p_compania,-999) or idoneidad <> nvl(p_idoneidad,'---') or registro <> nvl(p_registro,'---') or folio <> nvl(p_folio,'---') )");
	} else if (audAction.equalsIgnoreCase("DEL")) {
		sbFilter.append(" and aud_action = 'DEL'");
	} else {
		sbFilter.append(" and ( codigo <> nvl(p_codigo,-999) or compania <> nvl(p_compania,-999) or idoneidad <> nvl(p_idoneidad,'---') or registro <> nvl(p_registro,'---') or folio <> nvl(p_folio,'---')/**/ or aud_action in ('INS','DEL') )");
	}
	...
	sbSql.append("select z.user_id, z.codigo, z.idoneidad, z.folio, z.registro, nvl(z.p_codigo,z.codigo) as p_codigo, nvl(z.p_idoneidad,z.idoneidad) as p_idoneidad, nvl(z.p_folio,z.folio) as p_folio, nvl(z.p_registro,z.registro) as p_registro");
	sbSql.append(", to_char(z.aud_timestamp,'dd/mm/yyyy hh24:mi:ss') as aud_date, z.aud_webuser_ip as aud_user, z.aud_action");
	sbSql.append(", (select nombre from tbl_sec_compania where codigo = z.compania) as compania");
	sbSql.append(", (select nombre from tbl_sec_compania where codigo = nvl(z.p_compania,z.compania)) as p_compania");
	sbSql.append(" from (");

	for (int i=0; i<audschema.length; i++) {
		if (i>0) sbSql.append(" union all ");
		sbSql.append("select emp_id as user_id, compania, codigo, idoneidad, folio, registro, aud_timestamp, aud_webuser_ip, aud_action");
		sbSql.append(", lag(compania) over (partition by emp_id, compania, codigo order by emp_id, compania, codigo, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_compania");
		sbSql.append(", lag(codigo) over (partition by emp_id, compania, codigo order by emp_id, compania, codigo, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_codigo");
		sbSql.append(", lag(idoneidad) over (partition by emp_id, compania, codigo order by emp_id, compania, codigo, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_idoneidad");
		sbSql.append(", lag(folio) over (partition by emp_id, compania, codigo order by emp_id, compania, codigo, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_folio");
		sbSql.append(", lag(registro) over (partition by emp_id, compania, codigo order by emp_id, compania, codigo, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_registro");
		sbSql.append(" from ");
		sbSql.append(audschema[i].replaceAll("@@","."));
		sbSql.append("pla_idoneidad a where emp_id = ");
		sbSql.append(id);
		if (!audUser.trim().equals("")) { sbSql.append(" and upper(aud_webuser_ip) like '%"); sbSql.append(audUser); sbSql.append("%'"); }
		if (!code.trim().equals("")) { sbSql.append(" and codigo = ").append(code); }
	}

	sbSql.append(") z");
	if (sbFilter.length() != 0) sbSql.append(sbFilter.replace(0,4," where"));
	sbSql.append(" order by user_id, compania, codigo, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)");
...
document.title = 'Auditoría de Idoneidad del Usuario - '+document.title;
var xHeight=0;
parent.tLoaded[10]=true;
...
<tr class="TextFilter">
	<td>
		...
		<%=fb.textBox("code",code,false,false,false,20,100,null,null,null)%>
...
<div id="_cContent" class="ContainerContent">
				...
				<tr class="TextHeader" align="center">
					<td width="2%">&nbsp;</td>
					<td width="25%"><cellbytelabel>Idoneidad</cellbytelabel></td>
					<td width="24%"><cellbytelabel>Registro</cellbytelabel></td>
					<td width="24%"><cellbytelabel>Folio</cellbytelabel></td>
					<td width="8%"><cellbytelabel>Aud. Fecha</cellbytelabel></td>
					<td width="17%"><cellbytelabel>Aud. Usuario</cellbytelabel></td>
				</tr>
<% if (request.getParameter("audUser") == null) { %>
				<tr class="TextRow01 RedText" align="center"><td colspan="6">I N T R O D U Z C A &nbsp;&nbsp;&nbsp; P A R A M E T R O S &nbsp;&nbsp;&nbsp; D E &nbsp;&nbsp;&nbsp; B U S Q U E D A</td></tr>
<% } else if (al.size() == 0) { %>
				<tr class="TextRow01 RedText" align="center"><td colspan="6">B U S Q U E D A &nbsp;&nbsp;&nbsp; S I N &nbsp;&nbsp;&nbsp; R E S U L T A D O S</td></tr>
<% } %>
<%
String g = "";
...
%>
				<tr class="TextHeader01"><td colspan="6" align="center"><label class="Text14Bold SpacingText">CODIGO --> <%=cdo.getColValue("codigo")%></label></td></tr>
<% } %>
				<tr class="TextRow00<%=(!cdo.getColValue("aud_action").equalsIgnoreCase("UPD")?" RedTextBold":"")%>">
					...
					<td align="center"><%=cdo.getColValue("p_idoneidad")%></td>
					<td align="center"><%=cdo.getColValue("p_registro")%></td>
					<td align="center"><%=cdo.getColValue("p_folio")%></td>
				...
				<tr class="TextRow01 RedTextBold">
					<td><%=(cdo.getColValue("p_compania").equals(cdo.getColValue("compania")))?"":cdo.getColValue("compania")%></td>
					<td align="center"><%=(cdo.getColValue("p_idoneidad").equals(cdo.getColValue("idoneidad")))?"":cdo.getColValue("idoneidad")%></td>
					<td align="center"><%=(cdo.getColValue("p_registro").equals(cdo.getColValue("registro")))?"":cdo.getColValue("registro")%></td>
					<td align="center"><%=(cdo.getColValue("p_folio").equals(cdo.getColValue("folio")))?"":cdo.getColValue("folio")%></td>
				</tr>
<%
