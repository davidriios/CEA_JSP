...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	sbSql.append(", (select profile_name from tbl_sec_profiles where profile_id = z.codigo) as descripcion");
	sbSql.append(", (select profile_name from tbl_sec_profiles where profile_id = nvl(z.p_codigo,z.codigo)) as p_descripcion");
	sbSql.append(" from (");

	for (int i=0; i<audschema.length; i++) {
		if (i>0) sbSql.append(" union all ");
		sbSql.append("select user_id, profile_id as codigo, aud_timestamp, aud_webuser_ip, aud_action");
		sbSql.append(", lag(profile_id) over (partition by user_id, profile_id order by user_id, profile_id, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_codigo");
		sbSql.append(" from ");
		sbSql.append(audschema[i].replaceAll("@@","."));
		sbSql.append("sec_user_profile a where profile_id != 0 and user_id = ");
		sbSql.append(id);
		if (!audUser.trim().equals("")) { sbSql.append(" and upper(aud_webuser_ip) like '%"); sbSql.append(audUser); sbSql.append("%'"); }
		if (!code.trim().equals("")) { sbSql.append(" and profile_id = ").append(code); }
		if (!descrip.trim().equals("")) {	sbSql.append(" and exists (select null from tbl_sec_profiles where profile_id = a.profile_id and upper(profile_name) like '%").append(descrip.toUpperCase()).append("%')"); }
	}
...
document.title = 'Auditoría de Perfil del Usuario - '+document.title;
var xHeight=0;
parent.tLoaded[1]=true;
function doAction(){xHeight=objHeight('_tblMain');resizeFrame();}
function resizeFrame(resetHeight){resetFrameHeight(document.getElementById('_cMain'),xHeight,200);}
function audit(profileId){abrir_ventana('../admin/audit_profile_entitlement.jsp?id='+profileId);}
</script>
<style type="text/css">
<!--
...
<tr class="TextFilter">
	<td>
		<cellbytelabel>Perfil</cellbytelabel>
...
%>
				<tr class="TextHeader01"><td colspan="5" align="center"><label class="Text14Bold SpacingText">PERFIL --> <a href="javascript:audit(<%=cdo.getColValue("p_codigo")%>)" class="Link01"><%=cdo.getColValue("descripcion")%></a></label></td></tr>
<% } %>
