...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	if (audAction.equalsIgnoreCase("INS")) {
		...
		sbFilter.append(" and aud_action = 'UPD' and ( codigo <> nvl(p_codigo,-999) or compania <> nvl(p_compania,-999) )");
	} else if (audAction.equalsIgnoreCase("DEL")) {
		sbFilter.append(" and aud_action = 'DEL'");
	} else {
		sbFilter.append(" and ( codigo <> nvl(p_codigo,-999) or compania <> nvl(p_compania,-999)/**/ or aud_action in ('INS','DEL') )");
	}
	...
	sbSql.append("select z.user_id, z.codigo, nvl(z.p_codigo,z.codigo) as p_codigo");
	sbSql.append(", to_char(z.aud_timestamp,'dd/mm/yyyy hh24:mi:ss') as aud_date, z.aud_webuser_ip as aud_user, z.aud_action");
	sbSql.append(", (select descripcion from tbl_sal_habitacion where compania = z.compania and codigo = z.codigo) as descripcion");
	sbSql.append(", (select descripcion from tbl_sal_habitacion where compania = nvl(z.p_compania,z.compania) and codigo = nvl(z.p_codigo,z.codigo)) as p_descripcion");
	...
	sbSql.append(", (select (select descripcion from tbl_cds_centro_servicio where codigo = unidad_admin) from tbl_sal_habitacion where compania = z.compania and codigo = z.codigo) as cds");
	sbSql.append(", (select (select descripcion from tbl_cds_centro_servicio where codigo = unidad_admin) from tbl_sal_habitacion where compania = nvl(z.p_compania,z.compania) and codigo = nvl(z.p_codigo,z.codigo)) as p_cds");
	sbSql.append(" from (");

	for (int i=0; i<audschema.length; i++) {
		if (i>0) sbSql.append(" union all ");
		sbSql.append("select user_id, compania, habitacion as codigo, aud_timestamp, aud_webuser_ip, aud_action");
		sbSql.append(", lag(compania) over (partition by user_id, compania, habitacion order by user_id, compania, habitacion, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_compania");
		sbSql.append(", lag(habitacion) over (partition by user_id, compania, habitacion order by user_id, compania, habitacion, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_codigo");
		sbSql.append(" from ");
		sbSql.append(audschema[i].replaceAll("@@","."));
		sbSql.append("sec_user_quirofano a where user_id = ");
		sbSql.append(id);
		if (!audUser.trim().equals("")) { sbSql.append(" and upper(aud_webuser_ip) like '%"); sbSql.append(audUser); sbSql.append("%'"); }
		if (!code.trim().equals("")) { sbSql.append(" and habitacion = '").append(code).append("'"); }
		if (!descrip.trim().equals("")) {	sbSql.append(" and exists (select null from tbl_sal_habitacion where compania = a.compania and codigo = a.habitacion and upper(descripcion) like '%").append(descrip.toUpperCase()).append("%')"); }
	}
...
document.title = 'Auditoría de Quirófano del Usuario - '+document.title;
var xHeight=0;
parent.tLoaded[9]=true;
...
<div id="_cContent" class="ContainerContent">
				...
				<tr class="TextHeader" align="center">
					...
					<td width="21%"><cellbytelabel>Compa&ntilde;&iacute;a</cellbytelabel></td>
					<td width="21%"><cellbytelabel>Centro Servicio</cellbytelabel></td>
					<td width="8%"><cellbytelabel>Aud. Fecha</cellbytelabel></td>
					<td width="17%"><cellbytelabel>Aud. Usuario</cellbytelabel></td>
				</tr>
<% if (request.getParameter("audUser") == null) { %>
				<tr class="TextRow01 RedText" align="center"><td colspan="7">I N T R O D U Z C A &nbsp;&nbsp;&nbsp; P A R A M E T R O S &nbsp;&nbsp;&nbsp; D E &nbsp;&nbsp;&nbsp; B U S Q U E D A</td></tr>
<% } else if (al.size() == 0) { %>
				<tr class="TextRow01 RedText" align="center"><td colspan="7">B U S Q U E D A &nbsp;&nbsp;&nbsp; S I N &nbsp;&nbsp;&nbsp; R E S U L T A D O S</td></tr>
<% } %>
<%
String g = "";
...
%>
				<tr class="TextHeader01"><td colspan="7" align="center"><label class="Text14Bold SpacingText">QUIROFANO --> <%=cdo.getColValue("descripcion")%></label></td></tr>
<% } %>
				<tr class="TextRow00<%=(!cdo.getColValue("aud_action").equalsIgnoreCase("UPD")?" RedTextBold":"")%>">
					...
					<td><%=cdo.getColValue("p_compania")%></td>
					<td><%=cdo.getColValue("p_cds")%></td>
				...
				<tr class="TextRow01 RedTextBold">
					...
					<td><%=(cdo.getColValue("p_cds").equals(cdo.getColValue("cds")))?"":cdo.getColValue("cds")%></td>
				</tr>
<%
