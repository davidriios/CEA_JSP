...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	sbSql.append(", (select descripcion from tbl_sec_unidad_ejec where compania = z.compania and codigo = z.codigo) as descripcion");
	sbSql.append(", (select descripcion from tbl_sec_unidad_ejec where compania = nvl(z.p_compania,z.compania) and codigo = nvl(z.p_codigo,z.codigo)) as p_descripcion");
	...
	for (int i=0; i<audschema.length; i++) {
		...
		sbSql.append("select user_id, compania, ua as codigo, aud_timestamp, aud_webuser_ip, aud_action");
		sbSql.append(", lag(compania) over (partition by user_id, compania, ua order by user_id, compania, ua, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_compania");
		sbSql.append(", lag(ua) over (partition by user_id, compania, ua order by user_id, compania, ua, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_codigo");
		sbSql.append(" from ");
		sbSql.append(audschema[i].replaceAll("@@","."));
		sbSql.append("sec_user_ua a where user_id = ");
		sbSql.append(id);
		if (!audUser.trim().equals("")) { sbSql.append(" and upper(aud_webuser_ip) like '%"); sbSql.append(audUser); sbSql.append("%'"); }
		if (!code.trim().equals("")) { sbSql.append(" and ua = ").append(code); }
		if (!descrip.trim().equals("")) {	sbSql.append(" and exists (select null from tbl_sec_unidad_ejec where compania = a.compania and codigo = a.ua and upper(descripcion) like '%").append(descrip.toUpperCase()).append("%')"); }
	}
...
document.title = 'Auditoría de UA del Usuario - '+document.title;
var xHeight=0;
parent.tLoaded[3]=true;
...
%>
				<tr class="TextHeader01"><td colspan="6" align="center"><label class="Text14Bold SpacingText">UNIDAD --> <%=cdo.getColValue("descripcion")%></label></td></tr>
<% } %>
