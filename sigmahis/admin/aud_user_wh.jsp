...
if (whType == null || whType.trim().equals("")) throw new Exception("El Tipo de Almacén no es válido. Por favor intente nuevamente!");
if (code == null ) code = "";
...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	sbSql.append(", (select descripcion from tbl_inv_almacen where compania = z.compania and codigo_almacen = z.codigo) as descripcion");
	sbSql.append(", (select descripcion from tbl_inv_almacen where compania = nvl(z.p_compania,z.compania) and codigo_almacen = nvl(z.p_codigo,z.codigo)) as p_descripcion");
	...
	for (int i=0; i<audschema.length; i++) {
		...
		sbSql.append("select user_id, compania, almacen as codigo, aud_timestamp, aud_webuser_ip, aud_action");
		sbSql.append(", lag(compania) over (partition by user_id, compania, almacen order by user_id, compania, almacen, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_compania");
		sbSql.append(", lag(almacen) over (partition by user_id, compania, almacen order by user_id, compania, almacen, aud_timestamp, decode(aud_action,'INS',1,'UPD',2,3)) as p_codigo");
		sbSql.append(" from ");
		sbSql.append(audschema[i].replaceAll("@@","."));
		sbSql.append("sec_user_almacen a where user_id = ");
		sbSql.append(id);
		...
		if (!code.trim().equals("")) { sbSql.append(" and almacen = ").append(code); }
		if (!descrip.trim().equals("")) {	sbSql.append(" and exists (select null from tbl_inv_almacen where compania = a.compania and codigo_almacen = a.almacen and upper(descripcion) like '%").append(descrip.toUpperCase()).append("%')"); }
	}
	...
	int tId = 0;
	String tDesc = "";
	if (whType.equalsIgnoreCase("UA")) {
		tId = 4;
		tDesc = "UA";
	} else if (whType.equalsIgnoreCase("CDS")) {
		tId = 5;
		tDesc = "CDS";
	} else if (whType.equalsIgnoreCase("INV")) {
		tId = 7;
		tDesc = "Inventario";
	}
%>
...
document.title = 'Auditoría de Almacenes <%=tDesc%> del Usuario - '+document.title;
var xHeight=0;
parent.tLoaded[<%=tId%>]=true;
...
%>
				<tr class="TextHeader01"><td colspan="6" align="center"><label class="Text14Bold SpacingText">ALMACEN --> <%=cdo.getColValue("descripcion")%></label></td></tr>
<% } %>
