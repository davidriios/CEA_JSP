...
if (!code.trim().equals("")) appendFilter += " and upper(code) like '%"+code.toUpperCase()+"%'";
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	sql = "select id, code, name, decode(status,'A','ACTIVO','I','INACTIVO') as status from tbl_sec_department where code!='0'"+appendFilter+" order by name";
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) from tbl_sec_department where code!='0'"+appendFilter);
...
document.title = 'Administración de Departamento- '+document.title;
function add(){abrir_ventana('../admin/reg_department.jsp');}
function edit(id){abrir_ventana('../admin/reg_department.jsp?mode=edit&id='+id);}
function printList(){abrir_ventana('../admin/print_list_department.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');}
</script>
</head>
...
<tr>
	<td align="right">&nbsp;<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel>Registrar Nuevo Departamento</cellbytelabel> ]</a></authtype></td>
</tr>
<tr>
	<td>
