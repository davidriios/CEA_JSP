...
sql = "select s.id, s.comentario, s.usuario_creacion, s.usuario_modificacion, to_char(s.fecha_creacion,'dd/mm/yyyy') fecha_creacion , to_char(s.fecha_modificacion,'dd/mm/yyyy') fecha_modificacion, s.other1, s.other2, s.other3, s.other4, s.other5, decode(s.estado,'A','ACTIVO','INACTIVO') as estado from tbl_admin_seguimientos s where s.compania = "+compania+" and s.ref_type = '"+refType+"' and s.ref_id = '"+refId+"'"+(!id.trim().equals("")?" and s.id = "+id:"")+" order by s.fecha_creacion desc";

al = SQLMgr.getDataList(sql);

java.util.Hashtable iType = new java.util.Hashtable();

iType.put("CAJA_TURNO","\n(CIERRE CAJA - Turno#"+refId+" - Caja:"+cajaText+")");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String subtitle = "COMENTARIOS";
	String xtraSubtitle = ""+iType.get(refType);
