...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	else
	{
		...
		sql = "select id, description, display_area, status from tbl_sec_doc_type where id="+id;
...
document.title="Mantenimiento de Tipo Documento - "+document.title;
</script>
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="ADMINISTRACION - MANTENIMIENTO - TIPO DOCUMENTO"></jsp:param>
</jsp:include>
...
<%=fb.hidden("id",id)%>
		...
		<tr class="TextRow01" >
			...
			<td width="35%"><%=fb.textBox("description",cdo.getColValue("description"),true,false,false,45)%></td>
		</tr>
		<tr class="TextRow01" >
			<td><cellbytelabel>Secci&oacute;n</cellbytelabel></td>
			<td><%=fb.select("display_area","P=PACIENTE,X=EXPEDIENTE,A=ADMISION,H=RECURSOS HUMANOS,C=CONTABILIDAD,O=GERENCIA DE OPERACIONES,G=GERENCIA GENERAL",cdo.getColValue("display_area"))%></td>
...
else
{
  ...
  cdo.setTableName("tbl_sec_doc_type");
  cdo.addColValue("description",request.getParameter("description"));
  cdo.addColValue("display_area",""+request.getParameter("display_area"));
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/admin/doc_type_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/admin/doc_type_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/admin/doc_type_list.jsp';
<%
	}
