...
try {citasSopAdm = java.util.ResourceBundle.getBundle("issi").getString("citasSopAdm");}catch(Exception e){ citasSopAdm = "S";}
...
if (cds.trim().equalsIgnoreCase("")) {
	...
	if (!UserDet.getUserProfile().contains("0")) {
		sbFilter.append(" and a.centro_servicio in (");
		if (session.getAttribute("_cds") != null) sbFilter.append(CmnMgr.vector2numSqlInClause((Vector) session.getAttribute("_cds")));
...
else if(!status.trim().equals("")) {sbFilter.append(" and a.estado = '"); sbFilter.append(status); sbFilter.append("'"); }
...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	if (request.getParameter("cds") != null) {
		...
		sbSql.append("select nvl(a.fecha_ingreso,a.fecha_creacion) as sort_date, to_char(b.fecha_nacimiento,'dd/mm/yyyy') as fechaNacimiento, a.codigo_paciente as codigoPaciente, a.secuencia as noAdmision, /*to_char(nvl(a.fecha_ingreso,a.fecha_creacion),'dd/mm/yyyy')*/nvl(to_char(a.fecha_ingreso,'dd/mm/yyyy'),' ') as fechaIngreso, a.estado, nvl(to_char(a.fecha_egreso,'dd/mm/yyyy'),' ') as fechaEgreso, a.categoria, a.tipo_admision as tipoAdmision, b.id_paciente as pasaporte, b.vip, decode(b.pasaporte,null,b.provincia||'-'||b.sigla||'-'||b.tomo||'-'||b.asiento,b.pasaporte) as cedulaPamd, a.compania, a.pac_id as pacId, b.nombre_paciente as nombrePaciente, (select nombre_corto from tbl_adm_categoria_admision where codigo = a.categoria) as categoriaDesc, (select descripcion from tbl_adm_tipo_admision_cia where categoria = a.categoria and codigo = a.tipo_admision and compania = a.compania) as tipoAdmisionDesc, a.centro_servicio as centroServicio, (select descripcion from tbl_cds_centro_servicio where codigo = a.centro_servicio) as centroServicioDesc, (select cds from tbl_adm_atencion_cu where pac_id = a.pac_id and secuencia = a.adm_root and rownum = 1) as area/*es el cds para expediente*/, nvl((select cama from tbl_adm_atencion_cu where pac_id = a.pac_id and secuencia = a.adm_root and rownum = 1),' ') as cama, nvl((select estado from tbl_adm_atencion_cu where pac_id = a.pac_id and secuencia = a.secuencia and rownum = 1),'X') as status, a.medico, a.conta_cred as contaCred, b.edad as key,a.adm_root as admRoot,get_sec_comp_param(a.compania,'VER_CAMA_EST_ADM') revisadoAdmision, a.observ_ayuda as observAyuda, a.motivo_Anulacion motivoAnulacion, a.usuario_anulacion usuarioAnulacion, to_char(a.fecha_anulacion,'dd/mm/yyyy hh12:mi:ss am') fechaAnulacion ");


		sbSql.append(", (select case when count(*) > 0 then 'Y' else 'N' end from tbl_adm_beneficios_x_admision  be where be.pac_id = a.pac_id and be.admision= a.secuencia and be.convenio_sol_emp ='S' and be.estado = 'A' and be.prioridad = 1) as convenioSolEmp ");
		sbSql.append(", (select case when count(*) > 0 then '' else 'S' end from tbl_adm_cama_admision where pac_id = a.pac_id and admision = a.secuencia and fecha_final is null ) as fechaFinal,to_char(b.f_nac,'dd/mm/yyyy') as fechaNacimientoAnt ");
	...
	if(rowCount==0) pVal=0;
		...
		issi.admin.CommonDataObject cdoEGY = SQLMgr.getData("select get_sec_comp_param("+(String)session.getAttribute("_companyId")+",'CDS_EGY') as cds_egy, get_sec_comp_param("+(String)session.getAttribute("_companyId")+",'ADM_USAR_HORA') as adm_usar_hora from dual");
...
function printList(p){
 if (p=='PDF') abrir_ventana('../admision/print_list_admision.jsp?appendFilter=<%=IBIZEscapeChars.forURL(sbFilter.toString())%>&dobleCob=<%=dobleCob%>&fp=<%=admUsarHora?"CUSTOM":""%>');
	 ...
	 abrir_ventana('../cellbyteWV/report_container.jsp?reportName=admision/print_list_admision.rptdesign&filter=<%=IBIZEscapeChars.forURL(sbFilter.toString())%>&pCtrlHeader=true&dobleCob=<%=dobleCob%>');
 }
}
...
function goOption(option)
{
	...
	else if(option==0)abrir_ventana('../admision/admision_config_new.jsp?citasSopAdm=<%=citasSopAdm%><%=preventPopup?"&preventPopup=Y":""%>');
...
H=?
*/
			if((estado=='I' || estado == 'N')&&(option==1||option==3/*||option==5*/||option==6||option==7||option==8||option==10||option==11||option==12||option==13||option==33||option==34))CBMSG.warning('No se puede ejecutar la acción para una admisión con estado:'+statusAdm+'!');
			else if((estado == 'P')&&(option==5||option==6||option==7||option==8||option==10||option==11||option==12||option==13||option==15||option==33))CBMSG.warning('No se puede ejecutar la acción para una admisión con estado:'+statusAdm+'!');
			else if(option==1)abrir_ventana('../admision/admision_config_new.jsp?mode=edit<%=preventPopup?"&preventPopup=Y":""%>&pacId='+pacId+'&noAdmision='+noAdmision+'&cds='+cds+'&citasSopAdm=<%=citasSopAdm%>');
			else if(option==2){
							if (categoria =="2" && cdsAdm == "<%=cdsEGY%>")
			...
			else if(option==5)
			{
				...
				{
					/*if(eval('document.result.estado'+k).value=='A' || eval('document.result.estado'+k).value=='E')abrir_ventana('../admision/solicitud_beneficio.jsp?pac_id='+pacId+'&admision='+noAdmision);
					else{ CBMSG.warning('Solo para pacientes activos y en Espera, se mostraran los datos para consulta..');
					 abrir_ventana('../admision/solicitud_beneficio.jsp?pac_id='+pacId+'&admision='+noAdmision+'&mode=view');}*/
...
function view(pacId,noAdmision){abrir_ventana('../admision/admision_config_new.jsp?mode=view&pacId='+pacId+'&noAdmision='+noAdmision);}
...
function getPatientDetails(k)
{
	...
	{
		asegDesc=getDBData('<%=request.getContextPath()%>','y.nombre','(select * from tbl_adm_beneficios_x_admision where pac_id='+pacId+' and admision='+noAdmision+' and prioridad=1 and nvl(estado,\'A\')=\'A\') z, tbl_adm_empresa y','z.empresa=y.codigo','');
	}
...
function ListConsentimiento(pacId,noAdmision){
	...
	//window.open(val);
	<%if(consetimiento.trim().equals("0")){%>showPopWin('../common/sel_consentimiento_hpp.jsp?pacId='+pacId+'&noAdmision='+noAdmision,winWidth*.75,winHeight*.65,null,null,'');<%}else{%>showPopWin('../common/sel_consentimiento.jsp?pacId='+pacId+'&noAdmision='+noAdmision,winWidth*.75,winHeight*.65,null,null,'');<%}%>
}
var xHeight=0;
...
<tr>
	<td align="right">
		...
		<authtype type='3'><a href="javascript:goOption(0)" class="hint hint--top" data-hint="Crear Nueva Admisión"><img height="<%=iconHeight%>" width="<%=iconWidth%>" class="ImageBorder" onMouseOver="javascript:mouseOver(this,0)" onMouseOut="javascript:mouseOut(this,0)" src="../images/crear_nueva_admision.png"></a></authtype>
		...
		<authtype type='53'><a href="javascript:goOption(7)" class="hint hint--top" data-hint="Cargos / Devoluciones de Materiales"><img height="<%=iconHeight%>" width="<%=iconWidth%>" class="ImageBorder" onMouseOver="javascript:mouseOver(this,7)" onMouseOut="javascript:mouseOut(this,7)" src="../images/cargos_devoluciones_de_materiales.png"></a></authtype>
...
<%=fb.hidden("onlySol",onlySol)%>
			...
			%>
				...
				<%=fb.select("status","AE=ACTIVA Y EN ESPERA,A=ACTIVA,P=PRE ADMISIONES,S=ESPECIAL,E=ESPERA,I=INACTIVO,N=ANULADA",status,false,false,0,"Text10",null,null,null,"T")%>
			</td>
		</tr>
		<tr class="TextFilter">
			<td width="38%">
...
<div id="_cContent" class="ContainerContent">
		...
		<tr class="TextHeader" align="center">
			<td width="15%"><cellbytelabel id="1">&Aacute;rea</cellbytelabel></td>
			<td width="5%"><cellbytelabel id="17">Cat.</cellbytelabel></td>
			<td width="7%"><cellbytelabel id="5">Fecha Nac.</cellbytelabel></td>
			...
			<td width="22%"><cellbytelabel id="8">Paciente</cellbytelabel></td>
			<td width="7%"><cellbytelabel id="19">Fecha Ingreso</cellbytelabel></td>
			<td width="7%"><cellbytelabel id="20">Fecha Egreso</cellbytelabel></td>
...
%>
		...
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="center">
			 <span class="observAyuda" title="" data-i="<%=i%>"><%=estado%></span>
			</td>
			<td align="center"><a href="javascript:ListConsentimiento(<%=adm.getPacId()%>,<%=adm.getNoAdmision()%>)" class="Link02Bold" onMouseOver="setoverc(this,'Link01Bold')" onMouseOut="setoutc(this,'Link02Bold')"><cellbytelabel id="21">Consentimiento</cellbytelabel></a></td>
