...
if (!(SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"400015") || SecMgr.checkAccess(session.getId(),"500006"))) throw new Exception("Usted no tiene los suficientes Derechos de Acceso a esta página.");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	if (mode.equalsIgnoreCase("add"))
	{
		...
		cdo.addColValue("catCode",cdoType.getColValue("catCode"));
		cdo.addColValue("catDesc",cdoType.getColValue("catDesc"));
		cdo.addColValue("tipoCode",cdoType.getColValue("tipoCode"));
		cdo.addColValue("tipoDesc",cdoType.getColValue("tipoDesc"));
	}
	else
	{
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<%=fb.hidden("tipoCode",cdo.getColValue("tipoCode"))%>
				...
				<tr class="TextRow01">
					<td><cellbytelabel id="2">Tipo</cellbytelabel></td>
					<td><%=cdo.getColValue("tipoCode")%> - <%=cdo.getColValue("tipoDesc")%></td>
				</tr>						
				<tr class="TextRow01">
					<td width="12%"><cellbytelabel id="3">C&oacute;digo</cellbytelabel></td>
...
else
{
  ...
  if (mode.equalsIgnoreCase("add"))
  {
	cdo.addColValue("categoria",request.getParameter("catCode")); 
	cdo.addColValue("tipo",request.getParameter("tipoCode"));
	cdo.setAutoIncCol("codigo");
	cdo.setAutoIncWhereClause("categoria="+catCode+" and tipo="+tipoCode);

	SQLMgr.insert(cdo);
  }
  else
  {
    cdo.setWhereClause("categoria="+catCode+" and tipo="+tipoCode+" and codigo="+request.getParameter("codigo"));
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/admision/clasific_admision_list.jsp?catCode="+catCode+"&tipoCode="+tipoCode))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/admision/clasific_admision_list.jsp?catCode="+catCode+"&tipoCode="+tipoCode)%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/admision/clasific_admision_list.jsp?catCode=<%=catCode%>&tipoCode=<%=tipoCode%>';
<%
	}
%>
	window.close();
