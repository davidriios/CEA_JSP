...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else
  {
	...
	{*/
		sql = "select  id,codigo as code, descripcion as name,rango as rango,decode(estatus,'A','Activo','I','Inactivo') as estatus from tbl_cds_cat_diagenfermera"+appendFilter+" order by descripcion";
		al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
		rowCount = CmnMgr.getCount("select count(*) from tbl_cds_cat_diagenfermera"+appendFilter);
	//}
...
function add()
{
	abrir_ventana('../admision/enfcategoriadiagnostico_config.jsp');
}

function edit(id)
{
	abrir_ventana('../admision/enfcategoriadiagnostico_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../admision/print_list_cat_enfdiagnotisco.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
...
<table align="center" width="100%" cellpadding="0" cellspacing="0">
	...
	<% if (!fp.equalsIgnoreCase("diag") && !fp.equalsIgnoreCase("rCatDiag")) {%>
			<authtype type='3'><a href="javascript:add()" class="Link00">[<cellbytelabel id="1">Registrar Nueva Categor&iacute;a de Diagn&oacute;stico Enfermera</cellbytelabel>]</a></authtype>
			<%}%>
		</td>
  </tr>
	<tr>
...
%>
				...
				<td width="22%">
					...
					<%=fb.textBox("code",code,false,false,false,10)%>
					<%=fb.submit("go","Ir")%>
			  </td>
				<%=fb.formEnd()%>
...
<%=fb.formStart(true)%> 
		...
		<tr class="TextHeader" align="center">
			<td width="3%">&nbsp;</td>
			<td width="11%"><cellbytelabel id="2">C&oacute;digo</cellbytelabel></td>
			<td width="42%"><cellbytelabel id="9">Descripci&oacute;n</cellbytelabel></td>
			<td width="15%"><cellbytelabel id="10">Rango</cellbytelabel></td>
			<td width="19%"><cellbytelabel id="4">Estatus</cellbytelabel></td>
			<td width="10%">&nbsp;</td>
		</tr>
<%
...
<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
	<%}%>
			...
			<td align="center"><%=cdo.getColValue("estatus")%></td>
			<td align="center">
<%
if(!fp.equalsIgnoreCase("diag") && !fp.equalsIgnoreCase("rCatDiag")){
%>
			<authtype type='4'><a href="javascript:edit(<%=cdo.getColValue("id")%>)" class="Link02Bold" onMouseOver="setoverc(this,'Link01Bold')" onMouseOut="setoutc(this,'Link02Bold')"><cellbytelabel id="10">Editar</cellbytelabel></a></authtype>
<%
}
%>
			</td>
...
%>
				...
				<%=fb.formEnd()%>
				<td width="40%"><cellbytelabel id="6">Total Registro(s)</cellbytelabel> <%=rowCount%></td>
				<td width="40%" align="right"><cellbytelabel id="7">Registros desde</cellbytelabel> <%=pVal%> <cellbytelabel id="8">hasta</cellbytelabel> <%=nVal%></cellbytelabel></td>
<%
