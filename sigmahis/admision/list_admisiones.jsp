...
String secuencia="", codigo = "", nombre= "", pacId= "", tipoDoc= "",cedula= "";
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
	...
	if (request.getParameter("secuencia") != null && !request.getParameter("secuencia").equals("")){
		appendFilter += " and aa.secuencia = "+request.getParameter("secuencia");

		searchOn = "aa.secuencia";
	...
	if (request.getParameter("pid") != null && !request.getParameter("pid").equals("")){
		appendFilter += " and aa.pac_id = "+request.getParameter("pid");

		searchOn = "aa.pac_id";
		pacId = request.getParameter("pid");
		searchVal = request.getParameter("pid");
		searchType = "1";
		searchDisp = "PID";
		doSearch = true;
	}
	if (request.getParameter("cedula") != null && !request.getParameter("cedula").equals("")){
		appendFilter += " and aa.cedula like '%"+request.getParameter("cedula")+"%'";

		searchOn = "aa.cedula";
		cedula = request.getParameter("cedula");
		searchVal = request.getParameter("cedula");
		searchType = "1";
		searchDisp = "Cédula/Pasaporte";
		doSearch = true;
	}
	
	if (request.getParameter("nombre") != null && !request.getParameter("nombre").equals("")){
		appendFilter += " and upper(aa.nombre) like '%"+request.getParameter("nombre").toUpperCase()+"%'";
		searchOn = "aa.nombre";
	...
	sql = "select aa.*, (select count(a.pac_id) from tbl_adm_documentos_admision a where a.pac_id = aa.pac_id and a.admision = aa.secuencia) has_scan_doc from(select decode (b.tipo_id_paciente,'P',b.pasaporte,b.provincia || '-' || b.sigla || '-' || b.tomo || '-' || b.asiento) cedula , to_char(c.fecha_egreso, 'dd/mm/yyyy') fecha_egreso, to_char(c.fecha_ingreso, 'dd/mm/yyyy') fecha_ingreso, b.codigo, b.primer_nombre||decode(b.segundo_nombre,null,'',' '||b.segundo_nombre)||decode(b.primer_apellido,null,'',' '||b.primer_apellido)||decode(b.segundo_apellido,null,'',' '||b.segundo_apellido)||decode(b.sexo,'F',decode(b.apellido_de_casada,null,'',' '||b.apellido_de_casada)) as nombre, c.secuencia, c.pac_id, to_char(b.f_nac,'dd/mm/yyyy') as fecha_nacimiento, c.codigo_paciente from tbl_adm_paciente b, tbl_adm_admision c where c.estado not in('I','N','C') and b.pac_id = c.pac_id and c.compania = " + (String) session.getAttribute("_companyId") +") aa "+appendFilter+"  order by aa.nombre, aa.fecha_nacimiento, aa.secuencia desc";
...
document.title = 'Admisi&oacute;n - '+document.title;

function showDocs(pacId, noAdmision){
  showPopWin('../admision/list_doc_escan.jsp?pacId='+pacId+'&noAdmision='+noAdmision,winWidth*.65,winHeight*.65,null,null,'');
}
</script>
</head>
...
%>
				...
				<%=fb.hidden("fg",fg)%>
				<td>
				    <cellbytelabel>PID</cellbytelabel>
					<%=fb.textBox("pid",pacId,false,false,false,5)%>&nbsp;
					<cellbytelabel id="1">No. Admisi&oacute;n</cellbytelabel>
					<%=fb.textBox("secuencia",secuencia,false,false,false,5)%>&nbsp;
					<cellbytelabel>Nombre</cellbytelabel>
					<%=fb.textBox("nombre",nombre,false,false,false,40)%>&nbsp;
					<cellbytelabel>C&eacute;dula/Pas.</cellbytelabel>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
		...
		<tr class="TextHeader" align="center">
			...
			<td width="15%"><cellbytelabel>C&eacute;d./Pas.</cellbytelabel></td>
...
%>
		...
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="center">
			<%if(cdo.getColValue("has_scan_doc")!=null && !cdo.getColValue("has_scan_doc").equals("")&&!cdo.getColValue("has_scan_doc").equals("0")){%>
			<a class="Link00Bold" href="javascript:showDocs('<%=cdo.getColValue("pac_id")%>','<%=cdo.getColValue("secuencia")%>')">Ver</a>
			</td>
			<%}%>
		</tr>
<%
