...
issi.admin.ISSILogger.setSession(session);
	...
	String p_mes  	= request.getParameter("mes8");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String subtitle = "REPORTE DE PACIENTES EGRESADOS ";
...
for (int i=1; i<=12; i++){ 
	...
	sql = "select count(*) total from tbl_adm_admision where categoria=1 and trunc(fecha_egreso)>= to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy')and trunc(fecha_egreso)<=last_day (to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy'))";
...
{
	sql = "select count(*) total from tbl_adm_admision where categoria=1 and trunc(fecha_egreso)>= to_date('01/'||lpad("+p_mes+",2,0)||'/"+p_anio+"','dd/mm/yyyy')and trunc(fecha_egreso)<=last_day (to_date('01/'||lpad("+p_mes+",2,0)||'/"+p_anio+"','dd/mm/yyyy'))";
