...
//--------------Patient info ----------------------------------------//
sql = " select p.nombre_paciente nombrePaciente, decode(p.tipo_id_paciente, 'P',p.pasaporte,p.provincia||'-' ||p.sigla||'-' ||p.tomo||'-' ||p.asiento) cedula, (select decode ( extra_logo_status,null,' ','0',' ',decode(extra_logo_path,null, ' ', extra_logo_path)) from tbl_param_consentimientos where id = "+idConsent+") extra_logo, nvl(r.nombre,p.NOMBRE_PACIENTE) as nombre_responsable, nvl(r.identificacion,decode(p.tipo_id_paciente, 'P',p.pasaporte,p.provincia||'-' ||p.sigla||'-' ||p.tomo||'-' ||p.asiento)) as responsable_id, decode(m.sexo,'M','AL DR. ','A LA DRA. ')||m.primer_nombre||decode(m.segundo_nombre,null,'',' '||m.segundo_nombre)||decode(m.primer_apellido,null,'',' '||m.primer_apellido)||decode(m.segundo_apellido,null,'',' '||m.segundo_apellido)||decode(m.sexo,'F',decode(m.apellido_de_casada,null,'',' '||m.apellido_de_casada)) as nombre_med ,nvl(getPrecioUltCama("+compania+","+pacId+","+noAdmision+"),' ') as precio_hab from vw_adm_paciente p, tbl_adm_responsable r,tbl_adm_admision a,tbl_adm_medico m Where p.pac_id = r.pac_id(+) and p.pac_id = a.pac_id and a.medico = m.codigo  and p.pac_id="+pacId+" and a.secuencia = "+noAdmision+" and r.admision(+)="+noAdmision+" and r.estado(+) ='A' ";

al = SQLMgr.getDataList("select ( select coalesce(observacion,nombre) from tbl_cds_diagnostico where codigo=a.diagnostico ) as diagnosticos from tbl_adm_diagnostico_x_admision a where a.pac_id="+pacId+" and a.admision="+noAdmision+" order by a.orden_diag");

for (int d = 0; d<al.size(); d++){
  cdo = (CommonDataObject)al.get(d);
  strDiag += cdo.getColValue("diagnosticos")+", ";
}
...
// {
	...
	pc.addCols("AUTORIZACION PARA HOSPITALIZACION  Y TRATAMIENTO MEDIO Y/O QUIRURGICO",0,tblMain.size());

	pc.setFont(10,0);
	pc.addCols(" ", 0, tblMain.size());
	pc.addCols("POR ESTE MEDIO AUTORIZO ** "+cdo.getColValue("nombre_med")+" ** ",4,tblMain.size());
	pc.addCols(" ", 0, tblMain.size());
	pc.addCols(" - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ", 0, tblMain.size(),14.0f);
	
	pc.addCols(" ", 0, tblMain.size());

	pc.addCols("Y A LOS OTROS MEDICOS QUE PUDIERAN SER CONSULTADOS, PARA QUE INTERVENGAN EN EL DIAGNOSTICO Y/O TRATAMIENTO DE:",4,tblMain.size());
	
	pc.setFont(10,1);
	pc.addCols(strDiag, 0, tblMain.size(),30.0f);
	...
	pc.addCols("O CUALQUIER OTRA CONDIION QUE SE PUDIERA PRESENTAR EN EL TRANSCURSO DE MI HOSPITALIZACION",4,tblMain.size());
	
	pc.addCols("\n\n\t\t\t\t\t\tEN CASO DE QUE SE PRESENTEN CONDICIONES QUE PUEDAN REQUERIR PROCEDIEMIENTOS ADICIONALES. LOS AUTORIZO A REALIZAR LOS QUE CONSIDEREN RECOMENDABLES. DE IGUAL MANERA DOY MI CONSENTIMIENTO QUE SE ME ADMINISTRE  ANESTESIA, EN CASO DE QUE FUERA NECESARIO. AUTORIZO EXPRESAMENTE AL (LOS) MEDICOS (S) QUE ME ATENDIO Y/O AL HOSPITAL '"+_comp.getNombre()+"' A SUMINISTRAR A SOLICITUD DE MI COMPAÑÍA ASEGURADORA. CUALQUIER INFORMACION SOBRE MI ESTADO DE SALUD Y/O TRATAMIENTO RECIBIDO.",3,tblMain.size());
	
	pc.addCols("\n\n\t\t\t\t\t\tIGUALMENTE AUTORIZO SE ME ADMITA EN CUARTO CON TARIFA DE B./ "+(cdo.getColValue("precio_hab")!=null && !cdo.getColValue("precio_hab").equals("")?cdo.getColValue("precio_hab"):"...................")+" POR DIA. EN CASO DE NO EFECTUASE UN REEMBOLSO AL HOSPITAL '"+_comp.getNombre()+"', DE MI CUENTA DE HOSPITALIZACION, POR UNA COMPAÑÍA ASEGURADORA O POR ALGUN PLAN MEDICO HOSPITALARIO, ME COMPROMETO A CANCELAR EL SALDO TOTAL DE MI CUENTA AL TERMINAR LA ATENCION EN EL HOSPITAL.",3,tblMain.size());
	...
	pc.addCols("\n\n\n\nFECHA: "+cDateTime,4,tblMain.size());
