...
//control imgae
		...
		pc.addCols("1.", 0, 1);
		pc.addCols("Reconocemos que el hecho de ser admitido en cualquier forma en el Hospital "+_comp.getNombre()+" o en cualquiera de sus entidades afiliadas, para recibir tratamiento, procedimientos médicos o quirúrgicos, generarán gastos que se reflejarán y se nos comunicarán en un estado de cuenta, el cual desde este momento nos comprometemos a cubrir en su totalidad.", 3, 3);
		pc.addCols("\n", 1, dHeader.size());

		pc.addCols("2.", 0, 1);
		pc.addCols("Nos comprometemos a que en este caso de utilizar un seguro de hospitalización, cualquier suma que éste seguro no cubra, será pagada por nosotros una vez nos sea notificada, por parte del Hospital "+_comp.getNombre()+" o por la Compañía de Seguros, la suma adeudada.", 3, 3);
		pc.addCols("\n", 1, dHeader.size());

		pc.addCols("3.", 0, 1);
		pc.addCols("Nos comprometemos a hacer todos los abonos al estado de cuenta que nos solicite el Hospital "+_comp.getNombre()+", durante la estadía de EL PACIENTE en sus instalaciones.", 3, 3);
		pc.addCols("\n", 1, dHeader.size());
		...
		pc.addCols("5.", 0, 1);
		pc.addCols("En caso de no estar en capacidad para cubrir los gastos ocasionados durante la hospitalización, expresamente autorizamos al Hospital "+_comp.getNombre()+" a trasladar al PACIENTE a una institución pública de salud.", 3, 3);
		pc.addCols("\n", 1, dHeader.size());

		pc.addCols("6.", 0, 1);
		pc.addCols("De requerir traslado a otra institución por cualquier motivo, cancelaremos el saldo del estado de cuenta antes del traslado.", 3, 3);
		pc.addCols("\n", 1, dHeader.size());

		pc.addCols("7.", 0, 1);
		pc.addCols("Por este medio autorizamos expresamente y de manera irrevocable al Hospital "+_comp.getNombre()+", sus representantes, funcionarios o agentes a solicitar información e investigar nuestro historial de crédito en cualquier agencia de información de datos y de referencias crediticias, en cualquier momento y cuantas veces lo requiera, sin solicitarnos autorización cada vez que lo haga.", 3, 3);
		pc.addCols("\n", 1, dHeader.size());

		pc.addCols("8.", 0, 1);
		pc.addCols("Autorizamos al Hospital "+_comp.getNombre()+" a proporcionar nuestro historial de crédito a cualquier agente económico y agencia de información de datos y de referencias crediticias (APC) según su criterio.", 3, 3);
		pc.addCols("\n", 1, dHeader.size());

		pc.addCols("9.", 0, 1);
		pc.addCols("Reconocemos que el Hospital "+_comp.getNombre()+", sus representantes, funcionarios o agentes no serán responsables por errores en los datos existentes en nuestro historial de crédito ni por posibles daños y perjuicios que lo contenido en el mismo pueda ocasionar.", 3, 3);
		pc.addCols("\n", 1, dHeader.size(),30.2f);

		pc.addCols("10.", 0, 1);
		pc.addCols("Autorizamos al Hospital "+_comp.getNombre()+" a suministrar a la Compañía de Seguros correspondientes, toda la información médica u otra información relacionada a mi Historial Médico requerido para el análisis de cualquier reclamación para el pago por los servicios brindados. Esta autorización se extiende a la autorización para reproducir fotocopias a petición de la Compañía de Seguros correspondientes.", 3, 3);
		pc.addCols("\n", 1, dHeader.size());

		pc.addCols("11.", 0, 1);
		pc.addCols("Por este medio reconozco que he recibido el documento de los Derechos y Responsabilidades del Paciente. Entiendo que existe personal profesional disponible para explicar este documento.", 3, 3);
		pc.addCols("\n", 1, dHeader.size());
	   ...
	   //Testigo 2
		...
		pc.addCols("Para el HOSPITAL "+_comp.getNombre()+"\n\n", 0, dHeader.size(), 30.2f);
		...
		pc.addCols("\n\n\nFiador Solidario: \n\n\nPor este medio me comprometo irrevocablemente ante el Hospital "+_comp.getNombre()+" a quien éste designe, a pagar solidariamente la obligación contraída mediante este documento en el caso que no sea pagada por el Paciente o el Responsable del Paciente.", 0, dHeader.size());
