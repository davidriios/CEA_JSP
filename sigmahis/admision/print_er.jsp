...
for (int i=1; i<=12; i++){ 
	...
	sql = "Select count(*) admision from (select a.codigo_paciente as codigoPaciente,a.secuencia as noAdmision, to_char(nvl(a.fecha_ingreso,a.fecha_creacion),'dd/mm/yyyy') as fechaIngreso, a.estado,a.categoria, a.tipo_admision as tipoAdmision, a.compania, a.pac_id as pacId, c.nombre_corto as categoriaDesc, a.centro_servicio as centroServicio, d.descripcion as centroServicioDesc, case when a.categoria=1 and a.hosp_directa='N' then nvl(x.cdsCama,a.centro_servicio) else a.centro_servicio end as area/*es el cds para expediente*/, case when a.categoria=1 and a.hosp_directa='N' then nvl(x.cama,' ') else ' ' end as cama, nvl(trunc(months_between(sysdate,b.f_nac)/12),0) as key from tbl_adm_admision a, tbl_adm_paciente b, tbl_adm_categoria_admision c, tbl_cds_centro_servicio d, (select distinct g.pac_id, g.admision, g.cama, f.unidad_admin as cdsCama  from tbl_adm_cama_admision g,tbl_sal_cama e, tbl_sal_habitacion f where g.compania=e.compania and g.cama=e.codigo and g.habitacion=e.habitacion  and e.habitacion=f.codigo and e.compania=f.compania and g.fecha_final is null and g.hora_final is null) x where  trunc(a.fecha_ingreso)>= to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy')and trunc(a.fecha_ingreso)<=last_day (to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy')) and  a.pac_id=b.pac_id  and a.categoria=c.codigo and a.centro_servicio=d.codigo and a.compania=1 and a.pac_id=x.pac_id(+) and a.estado='I' and a.secuencia=x.admision(+) and a.centro_servicio=3 order by nvl(a.fecha_ingreso,a.fecha_creacion) desc, a.secuencia ) ";
