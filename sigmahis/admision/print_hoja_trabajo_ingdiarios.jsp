...
sql = " select sc.estado_cama, sc.codigo as cama, sc.habitacion,  sh.unidad_admin as centro, cs.descripcion  as descCentro,  sth.precio, sh.estado_habitacion,  getpacts_hojatrabajo(sh.estado_habitacion, sc.estado_cama, sc.codigo) as nombre,  getvip_hojatrabajo(sc.estado_cama, sc.codigo) as vip, a.pac_id, a.secuencia,   getsexo_hojatrabajo(sc.estado_cama, sc.codigo) as sexo,  getedad_hojatrabajo(sc.estado_cama, sc.codigo) as edad,  getmedico_hojatrabajo(sc.estado_cama, sc.codigo) as medico,  getfingreso_hojatrabajo(sc.estado_cama, sc.codigo) as fechaIngreso,  getempresa_hojatrabajo(sc.estado_cama, sc.codigo) as aseguradora,  getprecioalt_hojatrabajo(sc.codigo) as precioAlt,  getprecioalterno_hojatrabajo(sc.codigo) as montoAlt  from tbl_sal_habitacion sh, tbl_sal_cama sc,  tbl_sal_tipo_habitacion sth, tbl_cds_centro_servicio cs, tbl_adm_admision a  where  (sc.compania = sh.compania  and  sc.habitacion = sh.codigo)  and  (sc.compania = sth.compania and  sc.tipo_hab   = sth.codigo) and  sc.estado_cama <> 'I'       and  sh.estado_habitacion <>'I'  and  sh.unidad_admin = cs.codigo(+)  and  sh.unidad_admin not in (887,11) AND sth.estatus = 'A' "+appendFilter+" order by sh.unidad_admin, sc.codigo " ;
...
footer.addBorderCols(" ",0,dHeader.size(),1.5f,0.0f,0.0f,0.0f);
	...
	pc.setFont(7, 1);
		...
		pc.addBorderCols("CAMA",1,1,cHeight,Color.lightGray);
		pc.addBorderCols("PACIENTE",1,1,cHeight,Color.lightGray);
		pc.addBorderCols("VIP / F",1,1,cHeight,Color.lightGray);
		pc.addBorderCols("SEXO",1,1,cHeight,Color.lightGray);
		pc.addBorderCols("PID / ADMISION",1,1,cHeight,Color.lightGray);
		pc.addBorderCols("EDAD",1,1,cHeight,Color.lightGray);
		pc.addBorderCols("MEDICO",1,1,cHeight,Color.lightGray);
...
if ((cdo.getColValue("nombre") != "") && (!cdo.getColValue("nombre").trim().equals("CAMA EN TRAMITE")) && (!cdo.getColValue("nombre").trim().equals("EN MANTENIMIENTO")) &&  (!cama.trim().equals(cdo.getColValue("cama"))))
	...
	if (cdo.getColValue("precioAlt").equals("S"))
		 {
			 montos = Integer.parseInt(cdo.getColValue("montoAlt"));
		 }else{
			 montos = Integer.parseInt(cdo.getColValue("precio"));
			 }

	if (!cdo.getColValue("nombre").trim().equals("CAMA ASIGNADA A + DE 1 PACIENTE"))
		{
	...
	if (!cama.trim().equals(cdo.getColValue("cama")))
		{//valida si se asigna una cama a + de un paciente
			 if (!cdo.getColValue("nombre").trim().equals("CAMA EN TRAMITE"))
				 ...
				 pc.addBorderCols(""+CmnMgr.getFormattedDecimal(montos),2,1,cHeight * 1.56f);
				 pc.addBorderCols(""+cdo.getColValue("cama"),1,1,cHeight * 1.56f);
					 pc.addBorderCols(""+cdo.getColValue("nombre"),0,1,cHeight * 1.56f);
				 pc.addBorderCols(""+cdo.getColValue("vip"),1,1,cHeight * 1.56f);
				 pc.addBorderCols(""+sexo,1,1,cHeight * 1.56f);
				 pc.addCols(cdo.getColValue("pac_id")+"-"+(cdo.getColValue("secuencia")),0,1);
				 pc.addBorderCols(""+cdo.getColValue("edad"),1,1,cHeight * 1.56f);
				 pc.addBorderCols(""+cdo.getColValue("medico"),0,1,cHeight * 1.56f);
				 pc.addBorderCols(""+cdo.getColValue("fechaIngreso"),0,1,cHeight * 1.56f);
				 pc.addBorderCols(""+cdo.getColValue("aseguradora"),0,1,cHeight * 1.56f);
				 }else{
				...
				pc.setFont(7, 0,Color.black);
			pc.addBorderCols("",2,1,cHeight * 1.56f);
			pc.addBorderCols("",1,1,cHeight * 1.56f);
				pc.addBorderCols("",0,1,cHeight * 1.56f);
			pc.addBorderCols("",1,1,cHeight * 1.56f);
			pc.addBorderCols("",1,1,cHeight * 1.56f);
			pc.addBorderCols("",1,1,cHeight * 1.56f);
			pc.addBorderCols("",1,1,cHeight * 1.56f);
