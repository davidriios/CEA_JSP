...
int nCatType = CmnMgr.getCount("select count(*) from (select distinct (b.descripcion||c.descripcion) from tbl_adm_clasif_x_tipo_adm a, tbl_adm_categoria_admision b, tbl_adm_tipo_admision_cia c where a.categoria=b.codigo and a.categoria=c.categoria and a.tipo =c.codigo"+appendFilter+")");

if (request.getMethod().equalsIgnoreCase("GET"))
{
	int maxLines = 55; //max lines of items
	int nItems = al.size() + nCatType; //number of items
	int extraItems = nItems % maxLines; //Se incluye el total de grupos
	...
	String fileNamePrefix = "print_clasificacion";
	...
	pdfHeader(pc, _comp, pCounter, nPages, "ADMISION", "TIPOS DE ADMISION POR CATEGORIA", userName, fecha);
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.setFont(7, 0);
		if (!groupBy.equalsIgnoreCase("[ "+cdo.getColValue("catName")+" ] "+cdo.getColValue("tipoName")))
		{
			pc.createTable();
				pc.addBorderCols("[ "+cdo.getColValue("catName")+" ] "+cdo.getColValue("tipoName"),0,setDetail.size(),0.5f,0.0f,0.0f,0.0f,cHeight);
			pc.addTable();
			lCounter++;
		}
		...
		groupBy = "[ "+cdo.getColValue("catName")+" ] "+cdo.getColValue("tipoName");
	}//for i
