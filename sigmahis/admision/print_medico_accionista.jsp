...
issi.admin.ISSILogger.setSession(session);
	...
	String p_mes  	= request.getParameter("mes7");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String subtitle = "REPORTE DE PACIENTES HOSPITALIZADOS POR MEDICO ACCIONISTAS ";
	...
	if (!p_mes.trim().equals("")){
		...
		pc.addBorderCols("Especialidad ",1,1);
...
if(i!=1) sql += " union all ";
	sql += "SELECT "+i+" mes, em.descripcion as espe, m.primer_apellido||' '||m.primer_nombre as nombremedico, a.categoria as total FROM  tbl_adm_admision a, tbl_adm_categoria_admision c,tbl_adm_medico m, tbl_adm_medico_especialidad e, tbl_adm_especialidad_medica em WHERE a.medico=m.codigo and a.categoria =1 and a.categoria=c.codigo and m.tipo_medico=2 and m.codigo=e.medico and e.especialidad=em.codigo and to_number(to_char(a.fecha_ingreso,'mm')) = "+i+" and to_number(to_char(a.fecha_ingreso,'yyyy')) = "+p_anio+" ";
}	
sqlAll = "Select mes, espe, nombremedico, count(total) tot from ("+sql+") GROUP BY mes, espe, nombremedico ORDER BY tot desc ";
...
/*if (cdo.getColValue("femenino")!= null && !cdo.getColValue("femenino").trim().equals("")){
  ...
  totales += Integer.parseInt(cdo.getColValue("femenino"));
	...
	pc.addCols(" "+cdo.getColValue("espe"),1,1);
...
{
	sql = "SELECT "+p_mes+" mes, em.descripcion as espe, m.primer_apellido||' '||m.primer_nombre as nombremedico, a.categoria as total FROM  tbl_adm_admision a, tbl_adm_categoria_admision c,tbl_adm_medico m, tbl_adm_medico_especialidad e, tbl_adm_especialidad_medica em WHERE a.medico=m.codigo and a.categoria =1 and a.categoria=c.codigo and m.tipo_medico=2 and m.codigo=e.medico and e.especialidad=em.codigo and to_number(to_char(a.fecha_ingreso,'mm')) = "+p_mes+" and to_number(to_char(a.fecha_ingreso,'yyyy')) =  "+p_anio+" ";	
