...
// sql = " select sh.unidad_admin as codSala, cds.descripcion as descSala,  pac.primer_apellido||' '||pac.segundo_apellido||' '||pac.apellido_de_casada||' '||pac.primer_nombre||' '||pac.segundo_nombre as nombrePaciente,  (to_char(aa.fecha_nacimiento,'dd-mm-yyyy')||'('||aa.codigo_paciente||' - '||aa.secuencia||')') as codigoPaciente,  coalesce(pac.pasaporte,pac.provincia||'-'||pac.sigla||'-'||pac.tomo||'-'||pac.asiento)||'-'||pac.d_cedula as cedula,  to_char(aa.fecha_ingreso,'dd/mm/yyyy') as fechaIngreso, to_char(pac.fecha_fallecido,'dd/mm/yyyy') as fechaFallecimiento,  to_char(pac.fecha_nacimiento,'dd/mm/yyyy') as fechaNacimiento, pac.codigo as codPaciente, aa.pac_id as pacId,   getMedicosHonorarios(aa.compania, aa.pac_id, aa2.secuencia) medicosHonorarios  from tbl_adm_admision aa,tbl_adm_paciente pac, tbl_adm_cama_admision aca, tbl_sal_habitacion sh, tbl_cds_centro_servicio cds,  (select a.fecha_nacimiento, a.codigo_paciente, max(a.secuencia) as secuencia from tbl_adm_admision a  where a.estado in ('I','E','A') and a.categoria = 1 group by a.fecha_nacimiento, a.codigo_paciente) aa2,  (select tc.codigo, tc.admision, tc.fecha_nacimiento, tc.pac_codigo, max(tc.hora_inicio) as hora_inicio  from tbl_adm_cama_admision tc group by tc.codigo, tc.admision, tc.fecha_nacimiento, tc.pac_codigo) temp  where   pac.fallecido  = 'S'  and  (aca.codigo   = temp.codigo   and   aca.admision = temp.admision and aca.fecha_nacimiento = temp.fecha_nacimiento and aca.pac_codigo = temp.pac_codigo and   aca.hora_inicio = temp.hora_inicio) and  (aa.secuencia = aa2.secuencia and aa.fecha_nacimiento = aa2.fecha_nacimiento and aa.codigo_paciente = aa2.codigo_paciente) and  (temp.admision = aa2.secuencia and temp.fecha_nacimiento = aa2.fecha_nacimiento and temp.pac_codigo = aa2.codigo_paciente) and  aa2.codigo_paciente = pac.codigo and aa2.fecha_nacimiento = pac.fecha_nacimiento and  aca.habitacion = sh.codigo and sh.unidad_admin = cds.codigo "+appendFilter1+" order by sh.unidad_admin,cds.descripcion, pac.fecha_fallecido asc   ";
