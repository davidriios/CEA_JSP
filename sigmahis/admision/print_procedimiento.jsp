...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String subtitle = "REPORTE DE PROCEDIMIENTOS";
...
for (int i=1; i<=12; i++){ 
	...
	sql = "select  (select count(*)  from TBL_ADM_ADMISION a, tbl_adm_tipo_admision_cia b, tbl_cds_centro_servicio c where  trunc(fecha_ingreso)>= to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy')and trunc(fecha_ingreso)<=last_day (to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy')) AND A.CATEGORIA=B.CATEGORIA and A.TIPO_ADMISION=B.CODIGO and a.centro_servicio = c.codigo and a.tipo_admision = 2 AND a.categoria=3) Ambulatorio,(SELECT COUNT(*) FROM TBL_ADM_ADMISION a, tbl_adm_tipo_admision_cia b, tbl_cds_centro_servicio c WHERE trunc(fecha_ingreso)>= to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy') and trunc(fecha_ingreso)<=last_day (to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy'))AND A.CATEGORIA=B.CATEGORIA AND A.TIPO_ADMISION=B.CODIGO AND a.tipo_admision = 2 AND a.centro_servicio = c.codigo AND a.centro_servicio IN (4,6,8,9,10,11,12,13,15,25) AND a.categoria='1') Hospitalizado  FROM dual"; 
