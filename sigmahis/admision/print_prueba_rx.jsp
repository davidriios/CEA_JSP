...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String subtitle = "REPORTE DE DE PRUEBA DE RADIOLOGIA";
...
for (int i=1; i<=12; i++){ 
	...
	sql = "select  (select count(*)  from tbl_adm_admision  where  trunc(fecha_ingreso)>= to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy')and trunc(fecha_ingreso)<=last_day (to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy')) and centro_servicio=21 and categoria='3') Ambulatorio,(select count(*) from tbl_adm_admision where  trunc(fecha_ingreso)>= to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy') and trunc(fecha_ingreso)<=last_day (to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy')) and centro_servicio=21 and categoria='2') Urgencia,(select count(*) from tbl_adm_admision where  trunc(fecha_ingreso)>= to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy') and trunc(fecha_ingreso)<=last_day (to_date('01/'||lpad("+i+",2,0)||'/"+p_anio+"','dd/mm/yyyy'))  and centro_servicio=21 and categoria='1') Hospitalizado from dual";
