...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
	...
	else if (request.getParameter("anio") != null)
	{
	...
	{
		appendFilter += " and upper(status) like '%"+request.getParameter("status").toUpperCase()+"%'";
  ...
  else
  {
	...
	sql = "SELECT a.anio, a.tipo_compromiso, a.num_doc, a.compania, to_char(a.fecha_documento,'dd/mm/yyyy') fecha_documento, a.status, decode(a.status,'A','APROBADO','N','ANULADO','P','PENDIENTE','R','PROCESADO','T','TRAMITE') desc_status, a.monto_total, nvl(a.cod_proveedor, 0) || ' ' || nvl(b.nombre_proveedor, ' ') nombre_proveedor, nvl(a.cod_almacen, 0) || ' ' || c.descripcion almacen_desc FROM TBL_COM_COMP_FORMALES a, tbl_com_proveedor b, tbl_inv_almacen c where a.cod_proveedor = b.cod_provedor(+) and a.cod_almacen = c.codigo_almacen and a.compania = c.compania and a.tipo_compromiso = 1 and a.compania = "+session.getAttribute("_companyId") + appendFilter+" order by a.anio desc, a.fecha_documento desc, a.num_doc desc";
...
function edit(anio, id, tp)
{
	/*if (tp=='T')
	{
	abrir_ventana('../compras/reg_orden_compra_normal.jsp?mode=edit&id='+id+'&anio='+anio+'&status='+tp);
	}
	else
	{
	abrir_ventana('../compras/reg_orden_compra_normal.jsp?mode=view&id='+id+'&anio='+anio+'&status='+tp);
	 }*/
	abrir_ventana('../compras/reg_orden_compra_normal.jsp?mode=edit&id='+id+'&anio='+anio+'&status='+tp);
}
...
function printList()
{
	abrir_ventana('../compras/print_list_ordencompra_normal.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">
	<authtype type='0'><a href="javascript:add()" class="Link00">[ <cellbytelabel>Registrar Nueva Orden de Compra</cellbytelabel> ]</a></authtype>
	</td>
  </tr>
	<tr>
		<td>
...
<!-- ================================   S E A R C H   E N G I N E S   S T A R T   H E R E   ================================ -->
        ...
        <tr class="TextFilter">
          ...
          <td> 
						<cellbytelabel>B&eacute;squeda Combinada</cellbytelabel>
