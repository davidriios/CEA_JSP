...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
	...
	else if (request.getParameter("codigo_almacen") != null)
	{
		appendFilter += " and upper(a.codigo_almacen) like '%"+request.getParameter("codigo_almacen").toUpperCase()+"%'";

    searchOn = "a.codigo_almacen";
    searchVal = request.getParameter("codigo_almacen");
    searchType = "1";
    searchDisp = "Código Almacén";
	}
  ...
  else
  {
	...
	if(flag.equals("1")){
		sql = "SELECT codigo_almacen, descripcion FROM TBL_INV_ALMACEN a WHERE compania = "+session.getAttribute("_companyId")+appendFilter+" ORDER BY descripcion";
	}
...
function setValues(cod,desc){
	var codAlmacen = '<%=OCDet.getCodAlmacen()%>';

	if(codAlmacen=='') codAlmacen = '0';
	<%
	if(flag.equals("1")){
	%>
		window.opener.document.ordencompra.codigo_almacen.value = cod;
		window.opener.document.ordencompra.desc_codigo_almacen.value = desc;
	<%
		if(OCDet.getOCDetails().size()>0){
		%>
		if(parseInt(codAlmacen)!=cod){
			window.opener.document.ordencompra.clearHT.value="S";
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="COMPRA - SELECCION DE ALMACEN"></jsp:param>
</jsp:include>
...
%>
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')" onClick="javascript:setValues(<%=cdo.getColValue("codigo_almacen")%>,'<%=cdo.getColValue("descripcion")%>')" style="cursor:pointer">
