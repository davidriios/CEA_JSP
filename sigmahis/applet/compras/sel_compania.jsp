...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else
  {
	...
	sql = "SELECT codigo, nombre FROM TBL_SEC_COMPANIA WHERE codigo <> "+session.getAttribute("_companyId")+appendFilter;
	
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) count from TBL_SEC_COMPANIA WHERE codigo <> "+ session.getAttribute("_companyId")+appendFilter);
...
function setValues(cod,desc){
	window.opener.document.requisicion.compania_sol.value = cod;
	window.opener.document.requisicion.desc_compania_sol.value = desc;
	window.close();
}
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="INVENTARIO - SELECCION DE COMPA&Ntilde;&Iacute;A"></jsp:param>
</jsp:include>
...
%>
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="center">
			<a href="javascript:setValues(<%=cdo.getColValue("codigo")%>,'<%=cdo.getColValue("nombre")%>')" class="Link02Bold" onMouseOver="setoverc(this,'Link01Bold')" onMouseOut="setoutc(this,'Link02Bold')"><cellbytelabel>Seleccionar</cellbytelabel></a>
			</td>
		</tr>
<%
}
