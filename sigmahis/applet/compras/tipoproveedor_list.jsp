...
/**
==================================================================================
300001	VER LISTA DE TIPOS DE PROVEEDORES
300002	IMPRIMIR LISTA DE TIPOS DE PROVEEDORES
...
/*if (!(SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"300001") || SecMgr.checkAccess(session.getId(),"300002") || SecMgr.checkAccess(session.getId(),"300003") || SecMgr.checkAccess(session.getId(),"300004"))) throw new Exception("Usted no tiene los suficientes Derechos de Acceso a esta página.");*/
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("codigo") != null)
  {
	...
	appendFilter += " upper(tipo_proveedor) like '%"+request.getParameter("codigo").toUpperCase()+"%'";
    searchOn = "tipo_proveedor";
  ...
  { 
    ...
    searchDisp = "Descripcion";
		descrip    = request.getParameter("descripcion");  // utilizada para mantener la descripción del tipo de proveedor
  }
  ...
  sql = "SELECT tipo_proveedor as codigo, descripcion FROM tbl_com_tipo_proveedor "+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_com_tipo_proveedor "+appendFilter);
...
document.title = 'Tipo Proveedor - '+document.title;

function add()
{
	abrir_ventana('tipoproveedor_config.jsp');
}

function edit(code)
{
	abrir_ventana('tipoproveedor_config.jsp?mode=edit&code='+code);
}

function printList()
{
	abrir_ventana('print_list_tipo_proveedor.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
        <td align="right">
	    <authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel>Registrar Nuevo Tipo Proveedor</cellbytelabel> ]</a></authtype>
	    </td>
    </tr>
	<tr>
		<td>
			<table width="100%" cellpadding="0" cellspacing="0">
			    <tr class="TextFilter">	                    
				    ...
				    <td width="50%"><cellbytelabel>Descripci&oacute;n</cellbytelabel>
					  <%=fb.textBox("descripcion",descrip,false,false,false,40)%>					<%=fb.submit("go","Ir")%>				  </td>
				    <%=fb.formEnd()%>			    </tr>
			</table>
		</td>
	</tr>
