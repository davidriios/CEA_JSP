...
sql = "SELECT a.codigo, a.descripcion, a.cta1||'-'||a.cta2||'-'||a.cta3||'-'||a.cta4||'-'||a.cta5||'-'||a.cta6 as cuenta, b.descripcion as cuentaDesc, decode(a.estado,'A','Activo','I','Inactivo') as estado "
+ " FROM tbl_cja_rubros a, tbl_con_catalogo_gral b"
+ " WHERE a.cta1=b.cta1(+) and a.cta2=b.cta2(+) and a.cta3=b.cta3(+) and a.cta4=b.cta4(+) and a.cta5=b.cta5(+) and a.cta6=b.cta6(+)"+ " and a.compania=b.compania(+) and a.compania="+ (String) session.getAttribute("_companyId")+appendFilter+" "
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_cafeteria_tipomov";
...
for (int j=1; j<=nPages; j++)
{
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("CAJA",1,1);
					pc.addCols("CAFETERÍA - TIPOS DE MOVIMIENTOS", 1, 1);
				pc.addTable();
...
//  				setHeader2.addElement(".10");
				...
				if (al.size()==0) {
						pc.createTable();
							pc.addCols("No existe el Tipo de Movimiento seleccionado",1,5);
						pc.addTable();
						}
				else{
					if (al.size() > 0)
