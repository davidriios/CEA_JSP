...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	float height = 72 * 11f;//11-792 14-1008
	...
	String title = "CAJA";
	String subtitle = "LISTADO DE CAJEROS";
	String xtraSubtitle = "";
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.setVAlignment(0);
		pc.addCols(cdo.getColValue("cod_cajera"),1,1);
		pc.addCols(cdo.getColValue("nombre"),0,1);
		pc.addCols(cdo.getColValue("usuario"),0,1);
		pc.addCols(cdo.getColValue("est"),1,1);

		if ((i % 50 == 0) || ((i + 1) == al.size())) pc.flushTableBody(true);
	}
	if (al.size() == 0) pc.addCols("No existen registros",1,dHeader.size());
	else pc.addBorderCols(al.size()+" Registro(s) en total",0,dHeader.size(),0.0f,0.5f,0.0f,0.0f);
