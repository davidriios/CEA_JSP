...
sql = "SELECT a.centro_servicio as centroCode, b.descripcion as centro, b.tipo_cds as tipoCdsCode, a.descuento, DECODE(a.descuento,'26','26 - JUBILADOS','20','20 - CORTESIAS VARIAS','30','30 - CORTESIAS EMPLEADOS','23','23 - DEDUCIBLES','25','25 - PREVISION SOCIAL','0','0 - OTROS') as descName"
+ " FROM tbl_cja_ctas_descuentos a, tbl_cds_centro_servicio b"
+ " WHERE a.centro_servicio=b.codigo(+) and a.compania="+(String) session.getAttribute("_companyId")+appendFilter+" "
+ " ORDER BY a.centro_servicio, b.descripcion, a.descuento";
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_descuentos";
...
for (int j=1; j<=nPages; j++)
{
			Vector setHeader0=new Vector();
				...
				pc.createTable();
				pc.setFont(9, 1);
				  pc.addCols("Cajas",1,1);
					pc.addCols("MANTENIMIENTO DE DESCUENTOS", 1, 1);
				pc.addTable();
...
//				pc.addTable();
				...
				pc.createTable();
					...
					pc.addBorderCols("Descuento",0);
				pc.addTable();


				if (al.size()==0) {
						pc.createTable();
							pc.addCols("No existe el Descuento seleccionada",1,5);
						pc.addTable();
						}
				else{
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								pc.createTable();
								pc.setFont(7, 0);
									pc.addCols(" "+cdo1.getColValue("centroCode"),0,1);
									pc.addCols(" "+cdo1.getColValue("centro"),0,1);
									pc.addCols(" "+cdo1.getColValue("descName"),0,1);
								pc.addTable();
