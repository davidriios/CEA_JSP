...
sql = "SELECT a.codigo, to_char(a.fecha,'dd/mm/yyyy') as fecha, a.monto_total, decode(a.estatus,'P','Pendiente','C','Cancelada','A','Anulada') as estatus, b.primer_nombre||decode(b.segundo_nombre,null,'',' '||b.segundo_nombre)||decode(b.primer_apellido,null,'',' '||b.primer_apellido)||decode(b.segundo_apellido,null,'',' '||b.segundo_apellido)||decode(b.sexo,'F',decode(b.apellido_de_casada,null,'',' '||b.apellido_de_casada)) as paciente, c.nombre as empresa "
+ " FROM tbl_fac_factura a, tbl_adm_paciente b, tbl_adm_empresa c "
+ " WHERE a.pac_id=b.pac_id and a.cod_empresa=c.codigo and a.compania="+(String) session.getAttribute("_companyId")+appendFilter;
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String folderName = "caja";
		String fileNamePrefix = "print_list_factura_pagos";
...
for (int j=1; j<=nPages; j++)
{
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("Caja",1,1);
					pc.addCols("CONSULTA FACTURAS CON PAGOS", 1, 1);
				pc.addTable();
				...
				pc.createTable();
					...
					pc.addBorderCols("Estatus",1);
					pc.addBorderCols("Monto Total",1);
				pc.addTable();


				if (al.size()==0) {
						pc.createTable();
							pc.addCols("No existe la Factura seleccionada",1,5);
						pc.addTable();
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								pc.setFont(7, 0);
									...
									pc.addCols(" "+cdo1.getColValue("paciente"),0,1);
