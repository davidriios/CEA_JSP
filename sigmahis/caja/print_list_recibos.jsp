...
sql =  "SELECT codigo, ctp_codigo, ctp_anio, decode(anulada,'N','No Anulada','S','Anulada') as anulada, nombre "
+ " FROM tbl_cja_recibos "
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_recibos";
...
for (int j=1; j<=nPages; j++)
{
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("Caja",1,1);
					pc.addCols("CONSULTA DE RECIBOS", 1, 1);
				pc.addTable();
				...
				pc.createTable();
					...
					pc.addBorderCols("No. Transacc.",0);
					pc.addBorderCols("Año",1);
					pc.addBorderCols("Anulada",0);
				pc.addTable();


				if (al.size()==0) {
						pc.createTable();
							pc.addCols("No existe el Recibo seleccionado",1,5);
						pc.addTable();
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								pc.setFont(7, 0);
									...
									pc.addCols(" "+cdo1.getColValue("ctp_codigo"),0,1);
									pc.addCols(" "+cdo1.getColValue("ctp_anio"),1,1);
									pc.addCols(" "+cdo1.getColValue("anulada"),0,1);
								pc.addTable();
