...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
	...
	{
		appendFilter += " and upper(denominacion) like '%"+request.getParameter("name").toUpperCase()+"%'";
  ...
  else
  {
	...
	{
		sql ="select codigo as cargo, denominacion from tbl_pla_cargo  where  compania= "+(String)session.getAttribute("_companyId")+appendFilter+" order by codigo asc";
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="SELECCION DE CARGOS"></jsp:param>
</jsp:include>
...
<%
fb = new FormBean("cargos",request.getContextPath()+request.getServletPath(),FormBean.POST);
%>
<%=fb.formStart(true)%>
<%=fb.hidden("fromPage",request.getContextPath()+request.getServletPath())%>
<%=fb.hidden("toPage",request.getContextPath()+request.getServletPath())%>
...
<%=fb.hidden("sueldoLastLineNo",""+sueldoLastLineNo)%>
<%=fb.hidden("aumenLastLineNo",""+aumenLastLineNo)%>
<%=fb.hidden("bonoLastLineNo",""+bonoLastLineNo)%><%=fb.hidden("tab",tab)%>
...
<table align="center" width="100%" cellpadding="0" cellspacing="1">
	<tr class="TextHeader" align="center">
		...
		<td width="70%"><cellbytelabel>Denominaci&oacute;n</cellbytelabel></td>
		<td width="15%"><%=fb.checkbox("check","",false,false,null,null,"onClick=\"javascript:checkAll('"+fb.getFormName()+"','check',"+al.size()+",this,0)\"","Seleccionar todos los Cargos listados!")%></td>
	</tr>
<%
...
%>
		...
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			<td align="center"><%=cdo.getColValue("cargo")%></td>
			<td><%=cdo.getColValue("denominacion")%></td>
			<td align="center"><%=((fp.equalsIgnoreCase("convencion") && vCargo.contains(cdo.getColValue("cargo"))))?"Elegido":fb.checkbox("check"+i,cdo.getColValue("cargo"),false,false)%></td>
		</tr>
<%
}
%>				
	...
	if (fp.equalsIgnoreCase("convencion"))
	{
		for (int i=0; i<size; i++)
		{
			if (request.getParameter("check"+i) != null)
			{
				...
				cdo.addColValue("denominacion",request.getParameter("denominacion"+i));
				cdo.addColValue("fecha_creacion",cDateTime);   
			  cdo.addColValue("usuario_creacion",(String) session.getAttribute("_userName")); 
			  cdo.addColValue("fecha_inicio",cDateTime); 

				sueldoLastLineNo++;
	
				String key = "";
				if (sueldoLastLineNo < 10) key = "00"+sueldoLastLineNo;
				else if (sueldoLastLineNo < 100) key = "0"+sueldoLastLineNo;
				else key = ""+sueldoLastLineNo;
				cdo.addColValue("key",key);
		
				try
				{
					iSueldo.put(key, cdo);
					vCargo.add(cdo.getColValue("cargo"));
				}
				catch(Exception e)
				{
...
<%
	...
	{
%>
		window.opener.location = '../rhplanilla/convencion_config.jsp?change=1&tab=3&mode=<%=mode%>&id=<%=id%>&motiLastLineNo=<%=motiLastLineNo%>&incenLastLineNo=<%=incenLastLineNo%>&sueldoLastLineNo=<%=sueldoLastLineNo%>&aumenLastLineNo=<%=aumenLastLineNo%>&bonoLastLineNo=<%=bonoLastLineNo%>';
<%
	}
	%>
		window.close();
