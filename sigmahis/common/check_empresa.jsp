...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else
  {
	...
	{
		sql = "SELECT codigo, nombre, estado FROM tbl_adm_empresa WHERE tipo_empresa=1 and estado = 'A'"+appendFilter+" order by nombre";
		al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
		rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_adm_empresa WHERE tipo_empresa=1"+appendFilter);
	}
...
%>
					...
					<td width="50%"><cellbytelabel>Codigo</cellbytelabel>					
...
%>
				...
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
					...
					<td align="center"><%=(vSoc.contains(cdo.getColValue("codigo")))?"Elegido":fb.checkbox("check"+i,cdo.getColValue("codigo"),false,false)%></td>
				</tr>
<%
}
%>	
...
else
{
	...
	for (int i=0; i<size; i++)
	{
		if (request.getParameter("check"+i) != null)
		{
			...
			cdo.addColValue("empresa",request.getParameter("codigo"+i));
			cdo.addColValue("empresaNombre",request.getParameter("nombre"+i));
			socLastLineNo++;

			String key = "";
			if (socLastLineNo < 10) key = "00"+socLastLineNo;
			else if (socLastLineNo < 100) key = "0"+socLastLineNo;
			else key = ""+socLastLineNo;
			cdo.addColValue("key",key);
	
			try
			{
				iSoc.put(key, cdo);
				vSoc.add(cdo.getColValue("empresa"));
			}
			catch(Exception e)
			{
...
<%
	...
	{
%>
	window.opener.location = '../admision/medico_config.jsp?change=1&tab=2&mode=<%=mode%>&id=<%=id%>&espLastLineNo=<%=espLastLineNo%>&socLastLineNo=<%=socLastLineNo%>&ubiLastLineNo=<%=ubiLastLineNo%>';
<%
	}
%>
	window.close();
