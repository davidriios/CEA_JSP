...
{ 
  ...
  else
  {
	...
	if (fp.equalsIgnoreCase("factoresEval"))
	{
		sql = "SELECT codigo, descripcion, valor_min as valorMin, valor_max as valorMax FROM tbl_pla_factores WHERE codigo<>0"+appendFilter+" ORDER BY descripcion";
		al = SQLMgr.getDataList("SELECT * FROM (SELECT rownum as rn, a.* FROM ("+sql+") a) WHERE rn between "+previousVal+" and "+nextVal);
		rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_pla_factores WHERE codigo<>0"+appendFilter);
	}
...
document.title = 'Factores - '+document.title;
</script>
</head>
<body topmargin="0" leftmargin="0" rightmargin="0">
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="SELECCION DE FACTORES"></jsp:param>
</jsp:include>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="1" cellspacing="1">
				<tr class="TextHeader" align="center">
					...
					<td width="15%"><cellbytelabel>Valor Min</cellbytelabel>.</td>
					<td width="15%"><cellbytelabel>Valor M&aacute;s</cellbytelabel>.</td>
					<td width="10%"><%=fb.checkbox("check","",false,false,null,null,"onClick=\"javascript:checkAll('"+fb.getFormName()+"','check',"+al.size()+",this,0)\"","Seleccionar todos los factores listados!")%></td>
				</tr>				
				<%
				...
				<%=fb.hidden("valorMin"+i,cdo.getColValue("valorMin"))%>
				<%=fb.hidden("valorMax"+i,cdo.getColValue("valorMax"))%>
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
					<td><%=cdo.getColValue("codigo")%></td>
					<td><%=cdo.getColValue("descripcion")%></td>
					<td><%=cdo.getColValue("valorMin")%></td>
					<td><%=cdo.getColValue("valorMax")%></td>
					<td align="center"><%=(vFact.contains(cdo.getColValue("codigo")))?"Elegido":fb.checkbox("check"+i,cdo.getColValue("codigo"),false,false)%></td>
				</tr>
				<%
				}
				%>
...
else
{
	...
	for (int i=0; i<size; i++)
	{
		if (request.getParameter("check"+i) != null)
		{
			...
			cdo.addColValue("factor",request.getParameter("codigo"+i));
			cdo.addColValue("descripcion",request.getParameter("descripcion"+i));
			cdo.addColValue("valorMin",request.getParameter("valorMin"+i));
			cdo.addColValue("valorMax",request.getParameter("valorMax"+i));
			
					    
			FactoresEval fa = new FactoresEval();
			    
			fa.setFactor(request.getParameter("codigo"+i));
			...
			String key = "";
			if (factLastLineNo < 10) key = "00"+factLastLineNo;
			else if (factLastLineNo < 100) key = "0"+factLastLineNo;
			else key = ""+factLastLineNo;
			...
			{
				iFact.put(key,fa);
				vFact.add(request.getParameter("codigo"+i));
			}
			catch(Exception e)
			{
...
<%
	...
	{
%>
	window.opener.location = '../rhplanilla/factores_evaluacion_detail.jsp?change=1&mode=<%=mode%>&id=<%=id%>&prov=<%=prov%>&sigla=<%=sigla%>&tomo=<%=tomo%>&asiento=<%=asiento%>&factLastLineNo=<%=factLastLineNo%>';
<%
	}
%>
	window.close();
