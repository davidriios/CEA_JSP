...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (fp.equalsIgnoreCase("convenio_cobertura_centro") || fp.equalsIgnoreCase("convenio_exclusion_centro") || fp.equalsIgnoreCase("convenio_cobertura_detalle") || fp.equalsIgnoreCase("pm_convenio_cobertura_detalle") || fp.equalsIgnoreCase("convenio_exclusion_detalle") || fp.equalsIgnoreCase("pm_convenio_exclusion_detalle") || fp.equalsIgnoreCase("convenio_cobertura_solicitud") || fp.equalsIgnoreCase("convenio_beneficio_new"))
	{
		sql = "select a.codigo, a.descripcion, a.compania from tbl_fac_otros_cargos a where a.activo_inactivo='A' and a.compania="+(String) session.getAttribute("_companyId")+appendFilter+" order by a.descripcion";
		al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
		rowCount = CmnMgr.getCount("select count(*) from tbl_fac_otros_cargos a where a.activo_inactivo='A' and a.compania="+(String) session.getAttribute("_companyId")+appendFilter+"");
	}
...
function setOtrosCargos(i){
    <% if (fp.equalsIgnoreCase("convenio_beneficio_new")){%>
    <%=jsContext%>document.getElementById("codigo_detalle<%=index%>").value = eval('document.otrosCargos.codigo'+i).value;
    <%=jsContext%>document.getElementById("desc_detalle<%=index%>").value = eval('document.otrosCargos.descripcion'+i).value;  
  <%}%>
...
<!-- ================================   S E A R C H   E N G I N E S   S T A R T   H E R E   ================================ -->		
			<table width="100%" cellpadding="1" cellspacing="0">
				<tr class="TextFilter">		
					...
					<%=fb.hidden("secuencia_sol1",secuencia_sol1)%>
					<%=fb.hidden("secuencia_sol2",secuencia_sol2)%>
                    <%=fb.hidden("context",context)%><%=fb.hidden("noResultClose",noResultClose)%>
...
<%
fb = new FormBean("otrosCargos",request.getContextPath()+request.getServletPath(),FormBean.POST);
%>
<%=fb.formStart(true)%>
<%=fb.hidden("fromPage",request.getContextPath()+request.getServletPath())%>
<%=fb.hidden("toPage",request.getContextPath()+request.getServletPath())%>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="1" cellspacing="1">
				<tr class="TextHeader" align="center">
					...
					<td width="75%"><cellbytelabel>Descripci&oacute;n</cellbytelabel></td>
					<td width="10%"><%=fb.checkbox("check","",false,false,null,null,"onClick=\"javascript:checkAll('"+fb.getFormName()+"','check',"+al.size()+",this,0)\"","Seleccionar todos los cargos listados!")%></td>
				</tr>				
				<%
				...
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')" onClick="setOtrosCargos(<%=i%>)">
					<td><%=cdo.getColValue("codigo")%></td>
					<td><%=cdo.getColValue("descripcion")%></td>
					<td align="center"><%=((fp.equalsIgnoreCase("convenio_cobertura_centro") && vCobCD.contains(cdo.getColValue("codigo"))) || (fp.equalsIgnoreCase("convenio_exclusion_centro") && vExclCD.contains(cdo.getColValue("codigo"))) || ( (fp.equalsIgnoreCase("convenio_cobertura_detalle") || fp.equalsIgnoreCase("pm_convenio_cobertura_detalle")) && vCobDet.contains(cdo.getColValue("codigo"))) || (fp.equalsIgnoreCase("convenio_cobertura_solicitud") && vCobDet.contains(cdo.getColValue("codigo")))|| ( (fp.equalsIgnoreCase("convenio_exclusion_detalle") || fp.equalsIgnoreCase("pm_convenio_exclusion_detalle") ) && vExclDet.contains(cdo.getColValue("codigo"))))?"Elegido":fb.checkbox("check"+i,cdo.getColValue("codigo"),false,false)%></td>
				</tr>
<%
}
%>
...
else
{
	...
	if (fp.equalsIgnoreCase("convenio_cobertura_centro"))
	{
		for (int i=0; i<size; i++)
		{
			if (request.getParameter("check"+i) != null)
			{
				...
				cd.setOtrosCargos(request.getParameter("codigo"+i));
	...
	else if (fp.equalsIgnoreCase("convenio_exclusion_centro"))
	{
		for (int i=0; i<size; i++)
		{
			if (request.getParameter("check"+i) != null)
			{
				...
				ed.setOtrosCargos(request.getParameter("codigo"+i));
...
else if (fp.equalsIgnoreCase("convenio_cobertura_solicitud"))
	{
		for (int i=0; i<size; i++)
		{
			if (request.getParameter("check"+i) != null)
			{
				...
				cd.setSecuencia("0");
				cd.setCompania(request.getParameter("compania"+i));
				cd.setCodOtros(request.getParameter("codigo"+i));
				cd.setCodigo(request.getParameter("codigo"+i));
				cd.setDescripcion(request.getParameter("descripcion"+i));
...
<%	
	...
	{
%>
		window.opener.location = '../admision/detalle_cobertura_tipo.jsp?change=1&mode=<%=mode%>&empresa=<%=empresa%>&tipoServicio=<%=tipoServicio%>&centroServicio=<%=centroServicio%>&cobDetLastLineNo=<%=ceDetLastLineNo%>&tipoCds=<%=tipoCds%>&secuencia_cob=<%=secuencia_cob%>&pac_id=<%=pac_id%>&cod_pac=<%=cod_pac%>&solicitud=<%=solicitud%>&admision=<%=admision%>&fecha_nacimiento=<%=fecha_nacimiento%>&secuencia_sol1=<%=secuencia_sol1%>&secuencia_sol2=<%=secuencia_sol2%>&tipoCobertura=<%=tipoCE%>&cobertura=<%=ce%>&index=<%=index%>&cds=<%=centroServicio%>';
<%
	}
%>
	window.close();
