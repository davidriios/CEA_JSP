...
//if (!(SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"500017") || SecMgr.checkAccess(session.getId(),"500018") || SecMgr.checkAccess(session.getId(),"500019") || SecMgr.checkAccess(session.getId(),"500020"))) throw new Exception("Usted no tiene los suficientes Derechos de Acceso a esta página.");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else
  {
	...
	{
		sql = "SELECT codigo, descripcion FROM tbl_adm_ubicacion"+appendFilter+" order by descripcion";
		al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
		rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_adm_ubicacion"+appendFilter);
	}
...
document.title = 'Ubicación Médica - '+document.title;
</script>
</head>
<body topmargin="0" leftmargin="0" rightmargin="0">
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="SELECCION DE UBICACION MEDICA"></jsp:param>
</jsp:include>
...
<%
fb = new FormBean("ubicacion",request.getContextPath()+request.getServletPath(),FormBean.POST);
%>
<%=fb.formStart(true)%>
<%=fb.hidden("fromPage",request.getContextPath()+request.getServletPath())%>
<%=fb.hidden("toPage",request.getContextPath()+request.getServletPath())%>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="1" cellspacing="1">
				<tr class="TextHeader" align="center">
					...
					<td width="10%"><%=fb.checkbox("check","",false,false,null,null,"onClick=\"javascript:checkAll('"+fb.getFormName()+"','check',"+al.size()+",this)\"","Seleccionar todas las ubicaciones listadas!")%></td>
				</tr>				
<%
...
%>
				...
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
					...
					<td align="center"><%=(vUbi.contains(cdo.getColValue("codigo")))?"Elegido":fb.checkbox("check"+i,cdo.getColValue("codigo"),false,false)%></td>
				</tr>
<%
}
%>							
...
else
{
	...
	for (int i=0; i<size; i++)
	{
		if (request.getParameter("check"+i) != null)
		{
			...
			cdo.addColValue("ubicacion",request.getParameter("codigo"+i));
			cdo.addColValue("ubicacionDesc",request.getParameter("descripcion"+i));
			cdo.addColValue("telefono","");
			cdo.addColValue("principal","");
			ubiLastLineNo++;

			String key = "";
			if (ubiLastLineNo < 10) key = "00"+ubiLastLineNo;
			else if (ubiLastLineNo < 100) key = "0"+ubiLastLineNo;
			else key = ""+ubiLastLineNo;
			cdo.addColValue("key",key);
	
			try
			{
				iUbi.put(key, cdo);
				vUbi.add(cdo.getColValue("ubicacion"));
			}
			catch(Exception e)
			{
...
<%
	...
	{
%>
	window.opener.location = '../admision/medico_config.jsp?change=1&tab=3&mode=<%=mode%>&id=<%=id%>&espLastLineNo=<%=espLastLineNo%>&socLastLineNo=<%=socLastLineNo%>&ubiLastLineNo=<%=ubiLastLineNo%>';
<%
	}
%>
	window.close();
