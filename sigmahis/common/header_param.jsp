...
<title><%=java.util.ResourceBundle.getBundle("issi").getString("windowTitle")%></title>
<link rel="shortcut icon" href="<%=request.getContextPath()%>/images/<%=java.util.ResourceBundle.getBundle("issi").getString("icon")%>"/>
...
<link rel="stylesheet" href="<%=request.getContextPath()%>/css/hc_offcanvas_nav.css" type="text/css"/>
...
<script src="<%=request.getContextPath()%>/js/functions_SIGMA.js"></script>
<script src="<%=request.getContextPath()%>/js/hc_offcanvas_nav.js"></script>
<script>
document.onkeyup = function(e) {
    const codeReceived = event.which || event.keyCode;
    if ( e.ctrlKey && codeReceived == 77 ) {
        if ( document.getElementById("topMenuContainer") )
            triggerShortCut(codeReceived);
        else
            parent.triggerShortCut(codeReceived);
    }
};
</script>
