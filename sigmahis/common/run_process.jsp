...
if (anaquelx == null || anaquelx.equals("")) anaquelx = "null";
if (anaquely == null || anaquely.equals("")) anaquely = "null";
...
if (docType.equalsIgnoreCase("ADM")) {
	...
	if (actType.equalsIgnoreCase("3")) {
		...
		if (actType.equalsIgnoreCase("2")){
			...
			tSql.append(", 'CLIENTE_RUC_EDITABLE'),'N') x, ");
			tSql.append(" nvl(v.nombre,f.nombre_cliente) nombre_2, nvl(v.identificacion,f.ruc) identificacion_2, v.e_mail,nvl(v.digito_verificador,f.dv) dv_2,DIRECCION, TELEFONO, CODIGO_UBICACION_FE");
			tSql.append(" FROM  tbl_fac_factura f, VW_DATOS_TIPO_CLIENTE V ");
			tSql.append(" WHERE  v.tipo_cliente (+) = f.cliente_otros ");
			tSql.append(" AND v.codigo_cliente (+)= f.cod_otro_cliente ");
			tSql.append("and    F.COMPANIA = ");
			tSql.append((String) session.getAttribute("_companyId"));
			
	       if (request.getParameter("docNo").equals("POS")){
			       tSql.append(" AND    F.codigo   = (");
				   tSql.append(" SELECT CODIGO FROM TBL_FAC_DGI_DOCUMENTS WHERE COMPANIA = " + ((String) session.getAttribute("_companyId")) + " AND ID = " + docId);
				   tSql.append(")");
	       }else {
			    if (tipo.equalsIgnoreCase("ND") ||tipo.equalsIgnoreCase("NCP") || tipo.equalsIgnoreCase("NC")){
				   tSql.append(" AND    F.codigo   = (");
				   tSql.append(" SELECT COD_REF FROM TBL_FAC_DGI_DOCUMENTS WHERE COMPANIA = " + ((String) session.getAttribute("_companyId")) + " AND ID = " + docId);
				   tSql.append(")");
				   
			   } else{
			       tSql.append(" AND    F.codigo   = (");
				   tSql.append(" SELECT decode(tipo_docto,'FACT',cod_ref,codigo) FROM TBL_FAC_DGI_DOCUMENTS WHERE COMPANIA = " + ((String) session.getAttribute("_companyId")) + " AND ID = " + docId);
				   tSql.append(")");} 
		   }	
			...
			if(cdo!=null && !cdo.getColValue("x").equals("")){
				...
				direccion_fe = cdo.getColValue("DIRECCION");
				telefono_fe = cdo.getColValue("TELEFONO");
				email_fe    = cdo.getColValue("E_MAIL");
				codigo_ubicacion_fe = cdo.getColValue("CODIGO_UBICACION_FE");
				}
			}
	...
	if (actType.equalsIgnoreCase("1")) actDesc = "ACTUALIZAR LIQUIDACION DE RECLAMO";
		docNo = docId;
} else if (docType.equalsIgnoreCase("TOGGLESTATUS")) {//ACTIVAR/INACTIVAR USUARIO
	if (actType.equalsIgnoreCase("N")) actDesc = "UNBLOCK CUENTA USUARIO";
	else if (actType.equalsIgnoreCase("Y")) actDesc = "BLOCK CUENTA USUARIO";
}
...
else if (docType.equalsIgnoreCase("TOGGLESTATUS")) {
...
<%=fb.hidden("estado_art",estado_art)%>
<%=fb.hidden("consigna",consigna)%>
...
<%=fb.hidden("anaquelx",anaquelx)%>
<%=fb.hidden("anaquely",anaquely)%>
...
%>
		...
		<tr class="TextRow01">
			...
			<td align="center"><cellbytelabel>A nombre de: </cellbytelabel> <%=fb.textBox("nombre_2",nombre_2,false,false,false,40,70)%></td>
		</tr>
		<tr class="TextRow01">
		...
		<tr class="TextRow01">
					<td align="center"><cellbytelabel>E-mail: </cellbytelabel> <input type="mail" id="e_mail" name="e_mail" value="<%=email_fe%>" size="50"></td>
		</tr>
		<%
		...
		"     CODIGO AS \"Code\", " +
		"     TO_CHAR(IMPUESTO, 'fm999999999999999990D00') as \"MontoImpuesto\", " + //Precio del m. Si el m no es valorable: informar campo en 0.00 PrecioItem <> Cantidad * (PrecioUnitario - PrecioUnitarioDescuento)
...
//sp_cxp_cierre_trx
		...
		if (actType.equalsIgnoreCase("50") || actType.equalsIgnoreCase("55")) {
			...
			sbSql.append(anio);
			sbSql.append(",");
			sbSql.append(impuesto.trim().equals("")?"null":impuesto);
			sbSql.append(")");
	...
	} else if (docType.equalsIgnoreCase("APP_PM")) //APROBAR SOLICITUD DE PLAN MEDICO
	{
		...
		if (actType.equalsIgnoreCase("7")) {
			...
			sbSql.append(anio);
			sbSql.append(",");
			sbSql.append(anaquelx);
			sbSql.append(",");
			sbSql.append(anaquely);
			sbSql.append(", '");
			sbSql.append(consigna);
	...
	else if (docType.equalsIgnoreCase("CORRECCTA"))
	{
		...
		sbSql.append("update tbl_sec_users set block_status = '");
		sbSql.append(actType);
		sbSql.append("',block_reason='System User Forced', fecha_modificacion = sysdate, usuario_modificacion = '");
		sbSql.append((String)session.getAttribute("_userName"));
		sbSql.append("' where user_id = ");
		sbSql.append(docNo);
		
		if (actType.equalsIgnoreCase("I")) {
			try {
				javax.servlet.http.HttpSession userSession = (javax.servlet.http.HttpSession) SecMgr.getAppUsers(userName);
