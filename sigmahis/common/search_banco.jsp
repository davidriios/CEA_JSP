...
//if (!(SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"900013") || SecMgr.checkAccess(session.getId(),"900014") || SecMgr.checkAccess(session.getId(),"900015") || SecMgr.checkAccess(session.getId(),"900016"))) throw new Exception("Usted no tiene los suficientes Derechos de Acceso a esta página.");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else if (request.getParameter("nombre") != null)
  {
    ...
    searchType = "1";
    searchDisp = "nombre";
		nombre     = request.getParameter("nombre");  // utilizada para mantener el nombre del banco por el que se filtró
  }
  ...
  else
  {
	...
	if (fp.equalsIgnoreCase("asiento") || fp.equalsIgnoreCase("orden_pago"))
	{
		sql = "select a.cod_banco as codigo, a.nombre, a.ruta_transito as rutaTrans, a.compania, b.nombre_banco as banco, a.contacto from tbl_con_banco a, tbl_adm_ruta_transito b where a.ruta_transito = b.ruta and a.compania = "+session.getAttribute("_companyId")+appendFilter+" order by a.nombre";
		al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
		rowCount = CmnMgr.getCount("select count(*) from tbl_con_banco a, tbl_adm_ruta_transito b where a.ruta_transito = b.ruta and a.compania = "+session.getAttribute("_companyId")+appendFilter);
	}
...
{
%>
		window.opener.document.orden_pago.cod_banco.value = eval('document.result.codigo'+k).value;
		window.opener.document.orden_pago.nombre_banco.value = eval('document.result.nombre'+k).value;
<%
}
%>
		window.close();
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="SELECCION DE BANCO"></jsp:param>
</jsp:include>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
		...
		<tr class="TextHeader" align="center">
			...
			<td width="25%"><cellbytelabel>Ruta Tr&aacute;nsito</cellbytelabel></td>
			<td width="35%"><cellbytelabel>Contacto</cellbytelabel></td>
		</tr>				
...
%>
		...
		<%=fb.hidden("contacto"+i,cdo.getColValue("contacto"))%>
