...
if (request.getParameter("searchQuery") != null && !request.getParameter("searchOn").equals("Todos") && (!request.getParameter("searchVal").equals("SV") || (!request.getParameter("searchValFromDate").equals("SVFD") && !request.getParameter("searchValToDate").equals("SVTD"))) && !request.getParameter("searchType").equals("ST"))
  ...
  sql = "SELECT codigo, descripcion FROM TBL_CJA_CAJAS WHERE estado ='A' and compania="+(String) session.getAttribute("_companyId")+appendFilter;
...
document.title = 'Cajas - '+document.title;
function setCaja(k)
{
<% 
   ...
   {  
%>
	window.opener.document.form0.codCaja.value = eval('document.recibo.codigo'+k).value;
	window.opener.document.form0.cajaNombre.value = eval('document.recibo.nombre'+k).value;
<%
   }
   else if (fp.equalsIgnoreCase("cierres"))
   {
%>   
    window.opener.document.formCaja.codCaja.value = eval('document.recibo.codigo'+k).value;
	window.opener.document.formCaja.cajaDesc.value = eval('document.recibo.nombre'+k).value;
<%
   }
%>	
window.close();	
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<%fb = new FormBean("recibo",request.getContextPath()+request.getServletPath(),FormBean.POST);%>
            <%=fb.formStart(true)%>
				...
				<%=fb.hidden("nombre"+i,cdo.getColValue("descripcion"))%>

				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')" onClick="javascript:setCaja(<%=i%>)" style="cursor:pointer">
