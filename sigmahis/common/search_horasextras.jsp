...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else
  {
	...
	if (fp.equalsIgnoreCase("horario_trab"))
	{
		sql = "SELECT codigo, descripcion, factor_multi, detalle_factor FROM tbl_pla_t_horas_ext WHERE codigo<>0 "+appendFilter+" order by codigo";
		al = SQLMgr.getDataList("SELECT * FROM (SELECT rownum as rn, a.* FROM ("+sql+") a) WHERE rn between "+previousVal+" and "+nextVal);
		rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_pla_t_horas_ext WHERE codigo<>0 "+appendFilter);
	}
...
function setHorario(k)
{
<%
	if (fp.equalsIgnoreCase("horario_trab"))
	{
%>
	window.opener.document.form1.tipo_extra.value = eval('document.horasextras.codigo'+k).value;
	window.opener.document.form1.tipo_extra_desc.value = eval('document.horasextras.descripcion'+k).value;
<%
	}
%>	
	window.close();	
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="1" cellspacing="1" class="sortable" id="list">
				<tr class="TextHeader" align="center">
					...
					<td width="20%"><cellbytelabel>Factor Multiplicador</cellbytelabel></td>
				</tr>				
<%
fb = new FormBean("horasextras",request.getContextPath()+"/common/urlRedirect.jsp");
%>
<%=fb.formStart()%>
<%
...
<%=fb.hidden("factorMulti"+i,cdo.getColValue("factor_multi"))%>
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')" onClick="javascript:setHorario(<%=i%>)" style="cursor:pointer">
					<td align="right"><%=cdo.getColValue("codigo")%></td>
					<td><%=cdo.getColValue("descripcion")%></td>
					<td><%=cdo.getColValue("factor_multi")%></td>
				</tr>
<%
}
%>	
