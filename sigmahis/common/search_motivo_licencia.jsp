...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else
  {
	...
	if (fp.equalsIgnoreCase("permisos_empleado"))
	{
		sql = "SELECT codigo, descripcion FROM tbl_pla_motivo_licencia WHERE codigo<>0 "+appendFilter;
		al = SQLMgr.getDataList("SELECT * FROM (SELECT rownum as rn, a.* FROM ("+sql+") a) WHERE rn between "+previousVal+" and "+nextVal);
		rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_pla_motivo_licencia WHERE codigo<>0 "+appendFilter);
	}
...
document.title = 'Motivos de Licencias - '+document.title;
...
<%
	...
	{
%>
		window.opener.document.formPermiso.motivo_lic<%=index%>.value = eval('document.formLicencia.codigo'+k).value;
		window.opener.document.formPermiso.motivoLicDesc<%=index%>.value = eval('document.formLicencia.descripcion'+k).value;
<%
    }
%>	  
	window.close();
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="SELECCION DE MOTIVOS DE LICENCIAS"></jsp:param>
</jsp:include>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="1" cellspacing="1" class="sortable" id="expe">
				...
				<%
				fb = new FormBean("formLicencia",request.getContextPath()+"/common/urlRedirect.jsp");
				%>
				<%=fb.formStart()%>
				<%
