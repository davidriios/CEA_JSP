...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else
  {
	...
	{
		sql = "SELECT codigo, descripcion FROM tbl_pla_tipo_evaluacion WHERE codigo<>0 "+appendFilter;
		al = SQLMgr.getDataList("SELECT * FROM (SELECT rownum as rn, a.* FROM ("+sql+") a) WHERE rn between "+previousVal+" and "+nextVal);
		rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_pla_tipo_evaluacion WHERE codigo<>0 "+appendFilter);
	}	
...
document.title = 'Tipo Evaluación - '+document.title;
...
<%
	...
	{
%>
		window.opener.document.form0.tipoEvaluacion.value = eval('document.tipo.codigo'+k).value;
		window.opener.document.form0.tipoEvaluacionDesc.value = eval('document.tipo.descripcion'+k).value;
<%
    }
%>		
		window.close();
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="SELECCION DE TIPO DE EVALUACION"></jsp:param>
</jsp:include>
...
<%
fb = new FormBean("tipo",request.getContextPath()+"/common/urlRedirect.jsp");
%>
<%=fb.formStart()%>
<%
