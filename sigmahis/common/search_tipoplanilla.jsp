...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else
  {
	...
	{
	sql = "SELECT cod_planilla, nombre from tbl_pla_planilla where compania="+(String) session.getAttribute("_companyId")+appendFilter; 
...
document.title = 'TIPOS DE PLANILLAS - '+document.title;
...
%>
	...
	eval('window.opener.document.formSobretiempo.tipopla'+<%=index%>).value = eval('document.formPlanilla.codigo'+i).value;
	eval('window.opener.document.formSobretiempo.tipoplaDesc'+<%=index%>).value = eval('document.formPlanilla.descripcion'+i).value;
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="SELECCION DE PLANILLAS"></jsp:param>
</jsp:include>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="1" cellspacing="1" class="sortable" id="expe">
				...
				%>
				<%=fb.hidden("codigo"+i,cdo.getColValue("cod_planilla"))%>
				...
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')" onClick="javascript:setTurno(<%=i%>)" style="cursor:pointer">
					<td><%=cdo.getColValue("cod_planilla")%></td>
