...
<%=fb.select("status"+i,"T=TRAMITE,P=PENDIENTE",oc.getStatus())%>
		...
		<%if(fg.equalsIgnoreCase("app")){%>
		<td><%=fb.select("statusReq"+i,"A=APROBADO,R=RECHAZADO","A",false,false, 0,"","","")%></td>
		<%}%>
...
else
{
	...
	for (int i=0; i<size; i++)
	{
		...
		if(request.getParameter("status"+i) != null && request.getParameter("status"+i).trim().equals("N")){
		    if(request.getParameter("statusReq"+i) != null)oc.setStatusReq(request.getParameter("statusReq"+i)); }
