...
function showReporte(isExcel, noFacAgrup)
{
...
var tipo   = eval('document.form0.tipo').value;
	...
	if(!isExcel)abrir_ventana('../compras/print_detalle_facturas.jsp?fp=RDA&compania='+compania+'&almacen='+almacen+'&fDate='+fecha_i+'&tDate='+fecha_f+'&proveedor='+proveedor+'&factura='+factura+'&articulo='+articulo+'&familia='+familia+'&tipoDoc='+tipo);
	else {
    almacen   = almacen || 'ALL';
    ...
    if(!noFacAgrup) abrir_ventana('../cellbyteWV/report_container.jsp?reportName=compras/print_detalle_facturas.rptdesign&pCtrlHeader=true&fp=RDA&compania='+compania+'&almacen='+almacen+'&fDate='+fecha_i+'&tDate='+fecha_f+'&proveedor='+proveedor+'&factura='+factura+'&articulo='+articulo+'&familia='+familia+'&tipoDoc='+tipo);
    else abrir_ventana('../cellbyteWV/report_container.jsp?reportName=compras/print_detalle_facturas_no_agrup_fac.rptdesign&pCtrlHeader=true&fp=RDA&compania='+compania+'&almacen='+almacen+'&fDate='+fecha_i+'&tDate='+fecha_f+'&proveedor='+proveedor+'&factura='+factura+'&articulo='+articulo+'&familia='+familia+'&tipoDoc='+tipo);
	}
...
if(generar=='S')
{
 ...
 if(!isExcel) abrir_ventana('../compras/print_resumen_facturas_consig.jsp?fp=RDA&compania='+compania+'&almacen='+almacen+'&fDate='+fecha_i+'&tDate='+fecha_f+'&cod_prov='+proveedor+'&depto='+depto+'&titulo='+titulo+'&descAlm='+descAlm1);
 else abrir_ventana('../cellbyteWV/report_container.jsp?reportName=compras/print_resumen_facturas_consig.rptdesign&pCtrlHeader=true&compIdParam='+compania+'&almacen='+almacen+'&fDate='+fecha_i+'&tDate='+fecha_f+'&cod_prov='+proveedor+'&depto='+depto+'&titulo='+titulo+'&descAlm='+descAlm1);
}
else {
	if(!isExcel) abrir_ventana('../compras/print_resumen_facturas.jsp?almacen='+almacen+'&fDate='+fecha_i+'&tDate='+fecha_f+'&cod_prov='+proveedor+'&depto='+depto+'&titulo='+titulo+'&descAlm='+descAlm1+'&tipoDoc='+tipo);
	else abrir_ventana('../cellbyteWV/report_container.jsp?reportName=compras/print_resumen_facturas.rptdesign&pCtrlHeader=true&almacen='+almacen+'&fDate='+fecha_i+'&tDate='+fecha_f+'&cod_prov='+proveedor+'&depto='+depto+'&titulo='+titulo+'&descAlm='+descAlm1+'&tipoDoc='+tipo);
}
}
<%}%>
}
...
<%if(fg.trim().equals("RP")){%>
      <script language="javascript">
		...
		<tr class="TextFilter">
			...
			<cellbytelabel>Familia</cellbytelabel><%=fb.textBox("familia","",false,false,false,15)%>
			 ...
			 <cellbytelabel>Art&iacute;culo</cellbytelabel><%=fb.textBox("articulo","",false,false,false,15)%></td>
		</tr>
		<%}%>
		<tr class="TextFilter">
	...
	<%if(fg.trim().equals("RF")||fg.trim().equals("DF")){%>
	...
	<tr class="TextFilter">
			...
			<td><%//=fb.select("tipo","FC=FACTURAS DE CONTADO,FR=FACTURAS A CREDITO,FG=FACTURAS A CONSIGNACION","","S")%>
				<%=fb.select(ConMgr.getConnection(),"select documento, documento||' - '||descripcion from tbl_inv_documento_recepcion /*  */ "+((fg.trim().equals("DF"))?" union all select 'FCFR','FCFR - FACTURAS CONTADO Y CREDITO' from dual ":" where documento not in('NE') ")+" order by 1 ","tipo",((fg.trim().equals("DF"))?"FCFR":""),false,false,0,"T","","","","S")%>
	...
	<%}%>
		...
		<tr class="TextHeader">
			...
			<td>
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;
                <%if(fg.trim().equals("RP") || fg.trim().equals("DF")){%>
                    <%=fb.button("reporteXls","Excel",true,false,null,null,"onClick=\"javascript:showReporte(1)\"")%>
                    <%=fb.button("reporteXlsNoAgrup","Excel (No agrup x fac)",true,false,null,null,"onClick=\"javascript:showReporte(1,1)\"")%>
                <%}%>
