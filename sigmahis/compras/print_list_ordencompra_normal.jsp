...
sql = "SELECT a.anio, a.tipo_compromiso, a.num_doc, a.compania, to_char(fecha_documento,'dd/mm/yyyy') fecha_documento, a.status, a.tipo_compromiso, d.descripcion, to_char(a.monto_total,'9,999,999,990.00') as monto_total, a.numero_factura, to_char(a.fecha_entrega_vencimiento,'dd/mm/yyyy') as fechaVence, decode(substr(a.tipo_pago,0,2),'CR','CREDITO','CO','CONTADO') as tipo_pago, decode(a.status,'A','APROBADO','N','ANULADO','P','PENDIENTE','R','PROCESADO','T','TRAMITE') desc_status, '[ '||nvl(a.cod_proveedor, 0) || '] ' || nvl(b.nombre_proveedor, ' ')as nombre_proveedor, nvl(a.cod_almacen, 0) || ' ' || c.descripcion almacen_desc"
...
+ " a.compania = c.compania  and a.tipo_compromiso = d.tipo_com and a.tipo_compromiso = 1 and a.compania = "+session.getAttribute("_companyId") + appendFilter+" order by a.cod_proveedor, a.anio, a.fecha_documento, a.num_doc";
al = SQLMgr.getDataList(sql);


sql = "SELECT to_char(sum(a.monto_total),'9,999,999,990.00') as total, nvl(a.cod_proveedor, 0)"
+ " FROM TBL_COM_COMP_FORMALES a, tbl_com_proveedor b, tbl_inv_almacen c, tbl_com_tipo_compromiso d"
+ " where a.cod_proveedor = b.cod_provedor(+) and a.cod_almacen = c.codigo_almacen and "
+ " a.compania = c.compania  and a.tipo_compromiso = d.tipo_com and a.tipo_compromiso = 1 and a.compania = "+session.getAttribute("_companyId") + appendFilter+" order by a.cod_proveedor, a.anio, a.fecha_documento, a.num_doc";
tal = SQLMgr.getDataList(sql);
...
//				pc.addTable();
				...
				{
					pc.createTable();
						pc.addCols("No existe la Orden de Compra seleccionada",1,5);
					pc.addTable();
				}
				else
				{
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								no += 1;
							if (!desc.equalsIgnoreCase(cdo1.getColValue("nombre_proveedor")))
							{
							pc.createTable();
							pc.setFont(7, 0);
							 pc.addCols("Proveedor: ",1,2);
							  pc.addCols(" "+cdo1.getColValue("nombre_proveedor"),0,7);
							...
							pc.addTable();
								...
								pc.setFont(7, 0);
									...
									pc.addCols(" "+cdo1.getColValue("monto_total"),2,1) ;
								pc.addTable();
