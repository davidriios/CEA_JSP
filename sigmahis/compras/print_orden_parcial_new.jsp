...
sbSql.append(", nvl(get_sec_comp_param(a.compania,'COM_OC_ADD_BARCODE'),'N') as add_barcode, a.cod_almacen, nvl(a.usuario_aprob,' ') usuario_aprob,to_char(a.fecha_entrega_proveedor,'dd/mm/yyyy') as fecha_entrega_prov, to_char(a.fecha_entrega_vencimiento,'dd/mm/yyyy') as fecha_entrega_venc");
...
sbSql.append(tp);
  ...
  sbSql.append("select ff.no_sol, ff.prov_data, ff.dias_limite_desc, ff.fecha_documento, ff.fecha_entrega_venc, ff.usuario, sum(ff.monto) as sub_total, sum(ff.descuento) as descuento , sum(ff.itbms) as itbm,  sum(ff.monto) - sum(ff.descuento) as sub_desc, sum(ff.monto)-sum(ff.descuento)+sum(ff.itbms) as monto_total,  ff.status, nvl(get_sec_comp_param(ff.compania,'COM_OC_FOOTER'),' ') as observaciones, ff.explicacion, ff.tipoDescuento, ff.porcentaje, ff.usuario_aprob from(");
		...
		sbSql.append(")aa");
	sbSql.append(")ff group by ff.no_sol, ff.prov_data, ff.dias_limite_desc, ff.fecha_documento, ff.fecha_entrega_venc, ff.usuario, ff.status, nvl(get_sec_comp_param(ff.compania,'COM_OC_FOOTER'),' '), ff.explicacion, ff.tipoDescuento,ff.porcentaje, ff.usuario_aprob");
  ...
  sbSql.append("select ff.no_sol, ff.prov_data, ff.dias_limite_desc, ff.fecha_documento, ff.fecha_entrega_venc, ff.usuario, sum(ff.monto) as sub_total, decode(sum(ff.descuento),0,ff.descuentoH,sum(ff.descuento)) as descuento , sum(ff.itbms) as itbm,  sum(ff.monto) - sum(ff.descuento) as sub_desc, sum(ff.monto)-sum(ff.descuento)+sum(ff.itbms) as monto_total,  ff.status, nvl(get_sec_comp_param(ff.compania,'COM_OC_FOOTER'),' ') as observaciones, ff.explicacion, ff.tipoDescuento, ff.porcentaje,ff.usuario_aprob from(");
		sbSql.append("select aa.* ,getnewsaleprice(aa.compania,aa.cod_proveedor, aa.cod_almacen,aa.codarticulo,aa.codflia, aa.codclase,aa.last_purch_price,aa.monto,aa.sale_price) as new_sale_price from(");
			...
			sbSql.append(" and a.compania = e.compania and a.cf_tipo_com = e.tipo_compromiso and a.cf_num_doc = e.num_doc and a.cf_anio = e.anio /*and a.monto_articulo <> getlastprecio (a.compania, e.cod_almacen, a.cod_articulo) and get_incsaleprice(a.compania,a.cod_familia, a.cod_clase,a.cod_articulo)>0*/  order by a.cod_familia, a.cod_clase, a.subclase_id, a.cod_articulo");
		sbSql.append(")aa");
	sbSql.append(")ff group by ff.no_sol, ff.prov_data, ff.dias_limite_desc, ff.fecha_documento, ff.fecha_entrega_venc, ff.usuario, ff.status, nvl(get_sec_comp_param(ff.compania,'COM_OC_FOOTER'),' '), ff.explicacion, ff.tipoDescuento,ff.porcentaje,ff.descuentoH, ff.usuario_aprob");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	pc.createTable(true);
		...
		if (fp.trim().equals("")){
			...
			pc.addInnerTableCols("Fecha: "+cdo.getColValue("fecha_documento")==null?"Fecha: ":"Fecha: "+cdo.getColValue("fecha_documento"),0,2);

			pc.addInnerTableCols("Correo: ",0,1);
			pc.addInnerTableCols(cdo.getColValue("correo"),0,2);
			pc.addInnerTableCols("Fecha de entrega: ",0,2);
			pc.addInnerTableCols(cdo.getColValue("fecha_entrega_venc")==null?"":cdo.getColValue("fecha_entrega_venc"),0,1);
		...
		pc.addBorderCols("Detalle",1,1,0.5f,0.5f,0.0f,0.5f);
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.addInnerTableBorderCols(cdo1.getColValue("especificacion"),1, 1, 0.0f, 0.0f, 0.0f, 0.5f);
