...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	if (!codBarra.trim().equals(""))
	{
		...
		sbFilter.append(" and a.cod_barra = '");
		sbFilter.append(issi.admin.IBIZEscapeChars.forSingleQuots(codBarra).trim());
		sbFilter.append("'");
		codBarra = "";
	}
...
<%=fb.hidden("beginSearch","")%>
			<td>
				...
				<%=fb.textBox("cod_barra",codBarra,false,false,false,15,"ignore",null,"onkeypress=\"allowEnter(event);\", onFocus=\"this.select()\"")%>
			</td>
<%=fb.formEnd()%>
		</tr>
		</table>
