...
==================================================================================
900051	AGREGAR AREA
900052	MODIFICAR AREA
==================================================================================
**/
SecMgr.setConnection(ConMgr);
if (!SecMgr.checkLogin(session.getId())) throw new Exception("Usted está fuera del sistema. Por favor entre al sistema con su nombre de usuario y clave!!!");
if (!(SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"900051") || SecMgr.checkAccess(session.getId(),"900052"))) throw new Exception("Usted no tiene los suficientes Derechos de Acceso a esta página.");
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<tr class="TextRow01">
				...
				<td><%=fb.textBox("descripcion",cdo.getColValue("descripcion"),false,false,false,45)%>&nbsp;&nbsp;Operativo&nbsp;<%=fb.select("operativo","N=No,S=Sí",cdo.getColValue("operativo"))%>&nbsp;&nbsp;Inversi&oacute;n&nbsp;<%=fb.select("inversion","N=No,S=Sí",cdo.getColValue("inversion"))%></td>
			</tr>					
			<tr class="TextRow02">
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/contabilidad/area_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/contabilidad/area_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/contabilidad/area_list.jsp';
<%
	}
%>
	window.close();
