...
/**
==================================================================================
900085	VER LISTA DE TIPOS DE CLIENTES
900086	IMPRIMIR LISTA DE TIPOS DE CLIENTES
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  sql = "SELECT codigo, descripcion, decode(activo_inactivo,'A','Activo','I','Inactivo') as estado FROM tbl_fac_tipo_cliente where compania="+ (String) session.getAttribute("_companyId")+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_fac_tipo_cliente where compania ="+ (String) session.getAttribute("_companyId") +appendFilter);
...
{
   window.opener.document.form1.tipoClteCode.value = code;
   window.opener.document.form1.tipoClte.value = name;
   window.close();
}
</script>
</head>
		...
		<td>
			<table width="100%" cellpadding="0" cellspacing="1">
			    <tr class="TextFilter">		
					...
					<%=fb.hidden("toPage",request.getContextPath()+request.getServletPath())%>
				    <td width="20%">Estado
						<%=fb.select("estado","A=Activo,I=Inactivo","")%>
					<%=fb.submit("go","Ir")%>
					</td>
				    <%=fb.formEnd()%>		
			    </tr>
