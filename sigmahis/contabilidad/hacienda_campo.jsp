...
function addCampo()
{
  abrir_ventana1('../contabilidad/hacienda_campo_popup.jsp?id=<%=id%>&lastLineNo=<%=lastLineNo%>');
}
...
function removeItem(fName,k)
{
	 ...
	 var existe = 0;
	  ...
	  existe = getDBData('<%=request.getContextPath()%>',' count(*) existe','resp_por_campos','cod_clasif= '+code+' and sec_campos = '+subCode+' ','');
 ...
 id = request.getParameter("id");
	...
	catch(Exception e)
	{
    	...
    	//ITEMS.remove(ItemRemoved);  
		 response.sendRedirect("../contabilidad/hacienda_campo.jsp?mode="+mode+"&lastLineNo="+lastLineNo+"&id="+id+"&change=1");
    	return;
		}
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"contabilidad/hacienda_campo.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"contabilidad/hacienda_campo.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/contabilidad/clasificacion_activo.jsp';
<%
	}
%>
	window.close();
