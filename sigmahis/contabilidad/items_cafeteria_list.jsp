...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
	...
	String estado  = "";         // variablea para mantener el valor de los campos filtrados en la consulta
	String codigo  = "";
	String descrip = "";	
 ...
 { 
    if (!request.getParameter("estado").equals("")) 
    {
	   appendFilter = appendFilter+" and a.estado='"+request.getParameter("estado").toUpperCase()+"' ";	
		 estado  = request.getParameter("estado");          // utilizada para mantener el valor del estado del movimiento
    }else{
		   appendFilter=appendFilter+" and upper(a.estado)<>'I'";
			 estado  = request.getParameter("estado");         // utilizada para mantener el estado del movimiento
		 }
 }else{
	    appendFilter=appendFilter+" and upper(a.estado)<>'I'";
			estado  = request.getParameter("estado");         // utilizada para mantener el valor del estado del movimiento
  ...
  if (request.getParameter("codigo") != null)
  {
    ...
    searchType = "1";
    searchDisp = "Código";
		codigo     = request.getParameter("codigo");  // utilizada para mantener el Cód. del movimiento
  }
  ...
  sql = "SELECT a.codigo, a.descripcion, a.cta1||'-'||a.cta2||'-'||a.cta3||'-'||a.cta4||'-'||a.cta5||'-'||a.cta6 as cuenta, b.descripcion as cuentaDesc, decode(a.estado,'A','Activo','I','Inactivo') as estado FROM tbl_cja_rubros a, tbl_con_catalogo_gral b WHERE a.cta1=b.cta1(+) and a.cta2=b.cta2(+) and a.cta3=b.cta3(+) and a.cta4=b.cta4(+) and a.cta5=b.cta5(+) and a.cta6=b.cta6(+) and a.compania=b.compania(+) and a.compania="+ (String) session.getAttribute("_companyId")+appendFilter+" order by a.descripcion";
  al = SQLMgr.getDataList("SELECT * FROM (SELECT rownum as rn, a.* FROM ("+sql+") a) WHERE rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_cja_rubros a, tbl_con_catalogo_gral b WHERE a.cta1=b.cta1(+) and a.cta2=b.cta2(+) and a.cta3=b.cta3(+) and a.cta4=b.cta4(+) and a.cta5=b.cta5(+) and a.cta6=b.cta6(+) and a.compania=b.compania(+) and a.compania="+ (String) session.getAttribute("_companyId")+appendFilter);
...
document.title = 'Tipos de Movimientos - '+document.title;

function add()
{
	abrir_ventana('items_cafeteria_config.jsp');
}

function edit(code)
{
	abrir_ventana('items_cafeteria_config.jsp?mode=edit&code='+code);
}

function printList()
{
	abrir_ventana('../caja/print_list_cafeteria_tipomov.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
        ...
        <%
		  ...
		  //{
        %>
	    <a href="javascript:add()" class="Link00">[ Registrar Nuevo Tipo de Movimiento ]</a>
   	    <%
		  //}
	    %>
	    </td>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="0" cellspacing="1" class="sortable" id="list">
				...
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
					...
					<td><%=cdo.getColValue("cuentaDesc")%></td>
