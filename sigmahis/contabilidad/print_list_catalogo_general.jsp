...
sql = "select * from (SELECT cta1||'-'||cta2||'-'||cta3||'-'||cta4||'-'||cta5||'-'||cta6 as ctaFinanciera, cta1, cta2, cta3, cta4, cta5, cta6, a.descripcion, compania,lado_movim,recibe_mov,b.DESCRIPCION as clasDesc FROM tbl_con_catalogo_gral a,tbl_con_cla_ctas b where compania="+(String) session.getAttribute("_companyId")+appendFilter+" and a.TIPO_CUENTA=b.CODIGO_CLASE) order by ctaFinanciera";
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_catalogo_general";
			     ...
			     setHeader0.addElement(".2");
					...
					pc.createTable();
					  pc.setFont(9, 1);
						...
						pc.addCols("MANTENIMIENTO CATÁLOGO GENERAL", 1,1);
					pc.addTable();
					...
					pc.createTable();
					  pc.addBorderCols("Clasificacion",0);
					  pc.addBorderCols("Código",0);
					  pc.addBorderCols("Descripción",0);
					  pc.addBorderCols("Lado Mov.",0);
					  pc.addBorderCols("Recibe Mov.",0);
					...
					if (al.size()==0)
					 {
					  pc.createTable();
						  pc.addCols("No existe el Código seleccionado");
						pc.addTable();
					 }
					 else
					 {
					    if (al.size()>0)
						 {
							  for(int i= ((maxLines * j) - maxLines); i<(maxLines * j) ;i++)
							    {
									  CommonDataObject cdol = (CommonDataObject) al.get(i);
										no += 1;

										pc.createTable();
										  pc.setFont(7, 0);
										 	pc.addCols(" "+cdol.getColValue("clasDesc"),0,1);
											pc.addCols(" "+cdol.getColValue("ctaFinanciera"),0,1);
											pc.addCols(" "+cdol.getColValue("descripcion"),0,1);
											pc.addCols(" "+cdol.getColValue("lado_movim"),0,1);
											pc.addCols(" "+cdol.getColValue("recibe_mov"),0,1);
										pc.addTable();
