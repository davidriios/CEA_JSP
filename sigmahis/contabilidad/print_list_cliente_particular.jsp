...
sql = "SELECT codigo, descripcion, DECODE(cliente_alquiler,'S','Si','N','No') as alquiler "
+ " FROM tbl_cxc_cliente_particular "
+ " WHERE compania="+(String) session.getAttribute("_companyId")+appendFilter+" order by descripcion";
			...
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("Contabilidad",1,1);
					pc.addCols("MANTENIMIENTO CLIENTES PARTICULAR", 1, 1);
				pc.addTable();
...
//				pc.addTable();
				...
				pc.createTable();
					...
					pc.addBorderCols("Cliente Alquiler",0);
				pc.addTable();


				if (al.size()==0) {
						pc.createTable();
							pc.addCols("No existe el Cliente seleccionado",1,5);
						pc.addTable();
						}
				else{
					if (al.size() > 0)
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								pc.setFont(7, 0);
									...
									pc.addCols(" "+cdo1.getColValue("alquiler"),0,1);
								pc.addTable();
