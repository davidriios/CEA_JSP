...
String estadoContrato = "";  //variable que guarda el estatus del Contrato A=activo I=inactivo
...
sql = "SELECT a.contrato, decode(a.tipo_contrato,'1','Otros Cargos','2','Solo Cargos') as tipo, "
+ " b.descripcion as cliente, decode(a.estado,'A','Activo','I','Inactivo') as estado "
+ " FROM tbl_cxc_contrato_alq a, tbl_cxc_cliente_particular b "
+ " WHERE a.particular=b.codigo and a.compania=b.compania "+appendFilter
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_contrato_alquiler";
			...
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("Contabilidad",1,1);
					pc.addCols("CONTRATOS DE ALQUILERES", 1, 1);
				pc.addTable();
...
//				pc.addTable();
				...
				pc.createTable();
					pc.addBorderCols("Contrato",0);
					pc.addBorderCols("Cliente",0);
					pc.addBorderCols("Tipo Contrato",0);
				...
				if (al.size()==0) {
						...
						pc.createTable();
							pc.addCols("No existe el Contrato seleccionado",1,5);
						pc.addTable();
					...
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								pc.createTable();
								pc.setFont(7, 0);
									pc.addCols(" "+cdo1.getColValue("contrato"),0,1);
									pc.addCols(" "+cdo1.getColValue("cliente"),0,1);
