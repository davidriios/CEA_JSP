...
String familiaArticulo = ""; //utilizada para filtrar la familia artículo.
...
sql =   "SELECT a.compania as ciaCode, a.cod_flia, a.nombre,  b.cta1||'-'||b.cta2||'-'||b.cta3||'-'||b.cta4||'-'||b.cta5||'-'||b.cta6 as cuentaCode, c.descripcion as cuentaDesc "
+ " FROM tbl_inv_familia_articulo a, tbl_inv_cta_x_compania b, tbl_con_catalogo_gral c "
+ " WHERE a.cod_flia=b.cod_familia(+) and a.compania=b.compania_fa(+) "
+ " and b.cta1=c.cta1 and b.cta2=c.cta2 and b.cta3=c.cta3 and b.cta4=c.cta4 and b.cta5=c.cta5 and b.cta6=c.cta6 and "
+ " b.compania_fa=c.compania and a.compania="+(String) session.getAttribute("_companyId")+appendFilter
+ " ORDER BY a.cod_flia, a.nombre";

System.out.println("\n\n ddddddddddddddddddsql="+sql+"\n\n");
al = SQLMgr.getDataList(sql);


sql =   "SELECT distinct(a.cod_flia ) FROM tbl_inv_familia_articulo a, tbl_inv_cta_x_compania b, tbl_con_catalogo_gral c "
+ " WHERE a.cod_flia=b.cod_familia(+) and a.compania=b.compania_fa(+) "
+ " and b.cta1=c.cta1 and b.cta2=c.cta2 and b.cta3=c.cta3 and b.cta4=c.cta4 and b.cta5=c.cta5 and b.cta6=c.cta6 and "
+ " b.compania_fa=c.compania and a.compania="+(String) session.getAttribute("_companyId")+appendFilter
+ " ORDER BY a.cod_flia";
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_ctaasociada_familiaArt";
			...
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("Contabilidad",1,1);
					pc.addCols("CUENTAS DE INTERCOMPAÑIAS ASOCIADAS A FLIAS.", 1, 1);
				pc.addTable();
				...
				pc.createTable();
				  pc.addBorderCols("Familia Art.",1);
				...
				Vector setHeader3=new Vector();
					...
					setHeader3.addElement(".55");
				pc.setNoColumnFixWidth(setHeader3);

				if (al.size()==0)
				{
					pc.createTable();
						pc.addCols("No existe la Familia del Artículo seleccionado",1,5);
					pc.addTable();
				}
				else
				{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
							{
								...
								no += 1;
									 ...
									 // Imprime la Familia de Artículo.
								if (!familiaArticulo.equalsIgnoreCase(cdo1.getColValue("nombre")))
								  {
									pc.createTable();
                      				pc.setFont(7, 1);
										pc.addCols(" "+cdo1.getColValue("cod_flia")+" ["+ cdo1.getColValue("nombre")+" ]",0,1);
										pc.addCols("",0,1);
										pc.addCols("",0,1);
									pc.addTable();
									}

								// Imprime la cuenta relacionada con el artículo
								pc.createTable();
								pc.setFont(7, 0);
									pc.addCols(" "+" ",0,1);
									pc.addCols(" "+cdo1.getColValue("cuentaCode"),0,1);
									pc.addCols(" "+cdo1.getColValue("cuentaDesc"),0,1);
								pc.addTable();

								familiaArticulo = cdo1.getColValue("nombre") ;
