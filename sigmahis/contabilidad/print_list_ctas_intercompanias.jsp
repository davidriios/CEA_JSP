...
String compania = ""; //utilizada para filtrar la compania a la que pertenece la cuenta.
...
sql =   "SELECT a.codigo as ciaCode, a.nombre as cia, "
+ " b.cta1||'-'||b.cta2||'-'||b.cta3||'-'||b.cta4||'-'||b.cta5||'-'||b.cta6 as cuentaCode "
+ " FROM tbl_sec_compania a, tbl_con_cuentas_x_cias b "
+ " WHERE a.codigo=b.compania(+)"+appendFilter
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_ctas_intercompanias";
			...
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("Contabilidad",1,1);
					pc.addCols("CUENTAS DE INTERCOMPAÑIAS ASOCIADAS A CÍA.", 1, 1);
				pc.addTable();
...
//				pc.addTable();
				...
				if (al.size()==0) {
						pc.createTable();
							pc.addCols("No existe la Cuenta asociada a la Compañia",1,5);
						pc.addTable();
					...
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								// Imprime la Compañia
								if (!compania.equalsIgnoreCase(cdo1.getColValue("cia")))
								   {
									   pc.createTable();
										   pc.setFont(7, 1);
											 pc.addCols("Cia: "+cdo1.getColValue("ciaCode")+" ["+cdo1.getColValue("cia")+" ]",0,1);
											 pc.addCols("",0,1);
										 pc.addTable();
									 }

								// Imprime la cuenta relacionada a la compañia
								pc.createTable();
								pc.setFont(7, 0);
									pc.addCols(" "+" ",0,1);
