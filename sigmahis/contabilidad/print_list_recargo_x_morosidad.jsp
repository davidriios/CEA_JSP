...
sql = "SELECT tipo_recargo, descripcion, monto_calculado as montoCal, DECODE(tipo_aplicacion,'1','Mensual','2','Anual') as aplicado" + " FROM tbl_cxc_recargos_alq "
 + " WHERE compania="+(String) session.getAttribute("_companyId")+appendFilter
			...
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("Contabilidad",1,1);
					pc.addCols("RECARGOS POR MOROSIDAD DE ALQUILERES", 1, 1);
				pc.addTable();
...
//				pc.addTable();
				...
				pc.createTable();
					pc.addBorderCols("Tipo Recargo",0);
					pc.addBorderCols("Descripción",0);
					pc.addBorderCols("Aplicado",0);
					pc.addBorderCols("Monto Calculado",1);
				pc.addTable();


				if (al.size()==0) {
						pc.createTable();
							pc.addCols("No existe el Recargo seleccionado",1,5);
						pc.addTable();
					...
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								pc.createTable();
								pc.setFont(7, 0);
									pc.addCols(" "+cdo1.getColValue("tipo_recargo"),0,1);
									pc.addCols(" "+cdo1.getColValue("descripcion"),0,1);
									pc.addCols(" "+cdo1.getColValue("aplicado"),0,1);
									pc.addCols(" "+cdo1.getColValue("montoCal"),2,1);
								pc.addTable();
