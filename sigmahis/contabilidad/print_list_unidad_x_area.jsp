...
String areaCode  = request.getParameter("areaCode"); // para determinar el Area a la que pertenece la Unidad Administrativa
...
sql = "SELECT a.unidad_adm, a.descripcion FROM tbl_con_pres_unidad_x_area a "
 + " WHERE a.area="+areaCode+" and a.compania="+(String) session.getAttribute("_companyId")+appendFilter
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_unidad_x_area";
			...
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("Contabilidad",1,1);
					pc.addCols("MANTENIMIENTO UNIDADES POR ÁREAS", 1, 1);
				pc.addTable();
...
//				pc.addTable();
				...
				pc.addTable();
				 ...
				 pc.createTable();			 // para determinar el Area a la que pertenece la Unidad Administrativa
					pc.setFont(7, 1);
					pc.addCols("Área: "+areaCode+" - [ "+areaDesc+" ]",0,1);
				...
				if (al.size()==0) {
						pc.createTable();
							pc.addCols("No existe la Unidad seleccionada para esta Área",1,5);
						pc.addTable();
