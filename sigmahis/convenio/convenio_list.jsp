...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	if (!codigo.trim().equals("")) { sbFilter.append(" and a.empresa = "); sbFilter.append(codigo); }
	if (!empresa.trim().equals("")) { sbFilter.append(" and a.empresa = "); sbFilter.append(empresa); }
	if (!status.trim().equals("")) { sbFilter.append(" and a.estatus = '"); sbFilter.append(status); sbFilter.append("'"); }
	if (!secuencia.trim().equals("")) { sbFilter.append(" and upper(a.secuencia) like '%"); sbFilter.append(secuencia.toUpperCase()); sbFilter.append("%'"); }
	if (!nombre.trim().equals("")) { sbFilter.append(" and upper(a.nombre) like '%"); sbFilter.append(nombre.toUpperCase()); sbFilter.append("%'"); }

	sbSql.append("select a.empresa, a.secuencia, a.nombre, a.contacto, to_char(a.fecha_inicial,'dd/mm/yyyy') as fechaInicial, nvl(to_char(a.fecha_final,'dd/mm/yyyy'),'---') as fechaFinal, a.estatus, b.nombre as empresaNombre from tbl_adm_convenio a, tbl_adm_empresa b where a.empresa = b.codigo and a.tipo_convenio = 'C'");
	sbSql.append(sbFilter);
	sbSql.append(" order by b.nombre, a.secuencia desc");
...
function add(){abrir_ventana('../convenio/convenio_config.jsp');}
function edit(empresa, secuencia){abrir_ventana('../convenio/convenio_config.jsp?mode=edit&empresa='+empresa+'&secuencia='+secuencia);}
function printList(){abrir_ventana('print_list_convenio.jsp?appendFilter=<%=IBIZEscapeChars.forURL(sbFilter.toString())%>');}
var xHeight=0;
...
<%=fb.hidden("toPage",request.getContextPath()+request.getServletPath())%>
			<td colspan="2">
				...
				<%=fb.select(ConMgr.getConnection(),"select codigo, nombre, codigo from tbl_adm_empresa where tipo_empresa in (1,2,3,5) /*and codigo > 0*/ order by nombre","empresa",empresa,"T")%>COD:<%=fb.intBox("codigo","",false,false,false,10)%>
