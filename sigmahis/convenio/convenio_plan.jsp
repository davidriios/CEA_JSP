...
{
	abrir_ventana1('../common/check_clasificacion_adm.jsp?fp=convenio&mode=<%=mode%>&empresa=<%=empresa%>&secuencia=<%=secuencia%>&tipoPoliza=<%=tipoPoliza%>&tipoPlan=<%=tipoPlan%>&planNo=<%=planNo%>&planLastLineNo=<%=planLastLineNo%>&clasifLastLineNo=<%=clasifLastLineNo%>');
}
...
function showClasif(cTab,categoriaAdm,tipoAdm,clasifAdm,k)
{
	if(eval('document.form1.paquete'+k).checked){top.CBMSG.warning('La configuracion del plan indica que es un Paquete.. Por lo cual no aplica Coberturas / Exclusiones!!!. Verifique.');}else{
	if (<%=pc.getSecuencia()%> == 0) alert('Por favor guarde antes de continuar!');
	else abrir_ventana1('../convenio/convenio_clasif.jsp?tab=<%=tab%>&cTab='+cTab+'&mode=<%=mode%>&empresa=<%=empresa%>&secuencia=<%=secuencia%>&tipoPoliza=<%=tipoPoliza%>&tipoPlan=<%=tipoPlan%>&planNo=<%=planNo%>&categoriaAdm='+categoriaAdm+'&tipoAdm='+tipoAdm+'&clasifAdm='+clasifAdm);
	}
}
function selCotizacion(k){if(eval('document.form1.paquete'+k).checked)abrir_ventana1('../common/sel_cotizacion.jsp?fp=convenio&fg=CON&index='+k);}
function isChecked(k){if(eval('document.form1.paquete'+k).checked){eval('document.form1.selCot'+k).disabled=false;eval('document.form1.precioPaq'+k).readOnly=false;eval('document.form1.benefPaq'+k).disabled=false;eval('document.form1.pagaDifPaq'+k).disabled=false;}else{eval('document.form1.codRegistro'+k).value='';eval('document.form1.precioPaq'+k).value='';eval('document.form1.selCot'+k).disabled=true;eval('document.form1.precioPaq'+k).readOnly=true;eval('document.form1.benefPaq'+k).disabled=true;eval('document.form1.pagaDifPaq'+k).disabled=true;}}
function hidePaq(idx){
	var obj=document.getElementById('paq'+idx);
	if(document.getElementById('pagaPaq'+idx).value=='A'&&obj.style.display=='none')obj.style.display='';else obj.style.display='none';
	newHeight();
}
</script>
</head>
...
<%=fb.hidden("empresaPart",""+pc.getEmpresaPart())%>
				...
				<tr class="TextRow01">
					...
					<td><%=fb.intBox("planNo",pc.getSecuencia(),true,false,true,3,3,"Text10",null,null)%><%=fb.textBox("nombre",pc.getNombre(),true,false,false,40,100,"Text10",null,null)%></td>
					<td align="right"><cellbytelabel>Aplicar Descuentos</cellbytelabel></td>
					<td><%=fb.select("aplica_desc","N=NO APLICA,I=ANTES DE DISTRIBUIR,F=DESPUES DE DISTRIBUIR",pc.getAplicaDesc(),false,false,0,"Text10","","","","")%>
					<%if(pc.getEmpresaPart().trim().equals("S")){%>
					<br>Programa Fidelizacion: 
					<%=fb.select(ConMgr.getConnection(),"select vip as code, descripcion FROM tbl_adm_tipo_paciente order by id","vip",pc.getVip(),false,false,0,"S")%>
					<%}%>
					</td>
				...
				<tr>
					<td colspan="6">
						...
						<tr class="TextHeader01" align="center">
							...
							<td colspan="4"><cellbytelabel>Clasificaci&oacute;n</cellbytelabel></td>
...
%>
						...
						<%=fb.hidden("pcAplicaDesc"+i,pcTemp.getAplicaDesc())%>
						<%=fb.hidden("pcAplicaCo"+i,pcTemp.getAplicaCo())%>
						<%=fb.hidden("pcVip"+i,pcTemp.getVip())%>
<%
}//planConvenio
...
for (int i=1; i<=iClasif.size(); i++)
{
	...
	String stDesc = "A=ABIERTO,C=CERRADO,I=INACTIVO";
	...
	if (cp.getPagaPaq() != null && cp.getPagaPaq().equalsIgnoreCase("A")) displayPaq = "";
	else displayPaq = "none";
%>
						...
						<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')"<%=displayClasif%>>
							...
							<td align="center"><%=fb.select("estado"+i,stDesc,cp.getEstado(),false,false,0,"Text10",null,null)%></td>
						...
						<tr class="TextRow08" id="detail<%=i%>" style="display:none">
							<td colspan="2" valign="top">
								...
								<%=fb.hidden("perdiemDsp"+i,cp.getPerdiemDsp())%>
								...
								<tr class="TextRow08">
									<td>
										...
										<%=fb.decBox("montoClinica"+i,cp.getMontoClinica(),false,false,false,13,"Text10",null,null)%>
										<%=fb.select("tipoValCli"+i,"P=%,M=$",cp.getTipoValCli(),false,false,0,"Text10",null,null)%>
										<%//=fb.select("tipoCobCli"+i,"D=DIARIO,E=EVENTO",cp.getTipoCobCli(),false,false,0,"Text10",null,null)%>
										<%=fb.hidden("tipoCobCli"+i,cp.getTipoCobCli())%>
										...
										<%=fb.hidden("descuentoCli"+i,"")%>
									</td>
								</tr>
								<tr class="TextHeader02">
									<td><%=fb.checkbox("paquete"+i,"S",(cp.getPaquete() != null && cp.getPaquete().equalsIgnoreCase("S")),false,null,null,"onClick=\"javascript:isChecked("+i+")\"")%><cellbytelabel>Paquete</cellbytelabel></td>
								</tr>
								<tr class="TextRow08">
									<td>
										<%=fb.intBox("codRegistro"+i,cp.getCodRegistro(),false,false,true,13,"Text10",null,null)%>
										<%=fb.button("selCot"+i,"PAQUETE",false,false,null,null,"onClick=\"javascript:selCotizacion("+i+")\"")%>
										<cellbytelabel>Precio</cellbytelabel>
										<%=fb.decBox("precioPaq"+i,cp.getPrecioPaq(),false,false,false,13,"10.2",null,null,"")%> 
									</td>
								</tr>
								<tr class="TextRow08">
									<td>
										<cellbytelabel>Pagado por</cellbytelabel>
										<%=fb.select("pagaPaq"+i,"P=PACIENTE,E=EMPRESA,A=AMBOS",cp.getPagaPaq(),false,false,0,"Text10",null,"onChange=\"javascript:hidePaq("+i+")\"",null,"")%>
										<!--<cellbytelabel>Paga Diferencia</cellbytelabel>-->
										<%//=fb.select("pagaDifPaq"+i,"P=PACIENTE,E=EMPRESA",cp.getPagaDifPaq(),false,false,0,"Text10",null,null,null,"")%>
									</td>
								</tr>
								<tr class="TextRow08" id="paq<%=i%>" style="display:<%=displayPaq%>">
									<td>
										<cellbytelabel>% Paciente</cellbytelabel>
										<%=fb.decBox("paqValPac"+i,cp.getPaqValPac(),false,false,false,13,"Text10",null,null)%>
										<%//=fb.select("paqTipoValPac"+i,"P=%,M=$",cp.getPaqTipoValPac(),false,false,0,"Text10",null,null)%>
										<%=fb.hidden("paqTipoValPac"+i,"P")%>
										<cellbytelabel>% Empresa</cellbytelabel>
										<%=fb.decBox("paqValEmp"+i,cp.getPaqValEmp(),false,false,false,13,"Text10",null,null)%>
										<%//=fb.select("paqTipoValEmp"+i,"P=%,M=$",cp.getPaqTipoValEmp(),false,false,0,"Text10",null,null)%>
										<%=fb.hidden("paqTipoValEmp"+i,"P")%>
									</td>
								</tr>
								</table>
							</td>
							<td colspan="2" valign="top">
								...
								<tr class="TextRow08">
									<td>
										...
										<%=fb.hidden("tipoCobPac"+i,cp.getTipoCobPac())%>
										<br>
							...
							<td colspan="4" valign="top">
								<table width="100%" cellpadding="1" cellspacing="0">
								<tr class="TextHeader02">
									<td colspan="3"><cellbytelabel>Pago de la Aseguradora / Empresa</cellbytelabel></td>
								</tr>
								<tr class="TextRow08">
									<td colspan="3">
										<cellbytelabel>Monto</cellbytelabel>
								...
								<tr align="center" height="35">
									<td width="33%" class="TextHeader02" onMouseOver="setoverc(this,'TextHeaderOver')" onMouseOut="setoutc(this,'TextHeader02')" style="cursor:pointer" onClick="javascript:showClasif(0,<%=cp.getCategoriaAdmi()%>,<%=cp.getTipoAdmi()%>,<%=cp.getClasifAdmi()%>,<%=i%>)"><cellbytelabel>COBERTURAS</cellbytelabel></td>
									<td width="33%" class="TextHeader02" onMouseOver="setoverc(this,'TextHeaderOver')" onMouseOut="setoutc(this,'TextHeader02')" style="cursor:pointer" onClick="javascript:showClasif(1,<%=cp.getCategoriaAdmi()%>,<%=cp.getTipoAdmi()%>,<%=cp.getClasifAdmi()%>,<%=i%>)"><cellbytelabel>EXCLUSIONES</cellbytelabel></td>
									<!--<td width="34%" class="TextHeader02" onMouseOver="setoverc(this,'TextHeaderOver')" onMouseOut="setoutc(this,'TextHeader02')" style="cursor:pointer" onClick="javascript:showClasif(2,<%=cp.getCategoriaAdmi()%>,<%=cp.getTipoAdmi()%>,<%=cp.getClasifAdmi()%>,<%=i%>)"><cellbytelabel>DEFINICION DE CALCULOS</cellbytelabel></td>-->
								</tr>
								</table>
							</td>
							<td>&nbsp;</td>
...
else
{
	...
	for (int i=1; i<=planSize; i++)
	{
		...
		pcTemp.setVip(request.getParameter("pcVip"+i));
	...
	pc.setVip(request.getParameter("vip"));
	pc.setEmpresaPart(request.getParameter("empresaPart"));
	...
	for (int i=1; i<=clasifSize; i++)
	{
		...
		cp.setPaquete((request.getParameter("paquete"+i)==null)?"N":"S");
		cp.setCodRegistro(request.getParameter("codRegistro"+i));		
		
		if(cp.getPaquete().trim().equals("S")) {
			cp.setPrecioPaq(request.getParameter("precioPaq"+i));
			cp.setPagaPaq(request.getParameter("pagaPaq"+i));
			cp.setPagaDifPaq(request.getParameter("pagaDifPaq"+i));
		} else {
			cp.setPrecioPaq("");
			cp.setPagaPaq("");
			cp.setPagaDifPaq("");
		}
		if (cp.getPagaPaq().equalsIgnoreCase("A")) {
			cp.setPaqValPac(request.getParameter("paqValPac"+i));
			cp.setPaqTipoValPac(request.getParameter("paqTipoValPac"+i));
			cp.setPaqValEmp(request.getParameter("paqValEmp"+i));
			cp.setPaqTipoValEmp(request.getParameter("paqTipoValEmp"+i));
		} else {
			cp.setPaqValPac("");
			cp.setPaqTipoValPac("");
			cp.setPaqValEmp("");
		...
		if (cp.getTipoCobCli().trim().equals("")) cp.setTipoCobCli("E");
		...
		if (cp.getTipoCobPac().trim().equals("")) cp.setTipoCobPac("E");
		...
		if (cp.getTipoCobEmp().trim().equals("")) cp.setTipoCobEmp("E");
