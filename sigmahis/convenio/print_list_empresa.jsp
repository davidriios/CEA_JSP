...
sql = "select a.codigo, a.nombre, decode(a.estado,'A','ACTIVO','INACTIVO') as estado, a.tipo_empresa, (select descripcion from tbl_adm_tipo_empresa where codigo = a.tipo_empresa) as tipo_descripcion, a.grupo_empresa, (select descripcion from tbl_adm_grupo_empresa where codigo = a.grupo_empresa) as grupo_descripcion from tbl_adm_empresa a"+appendFilter+" order by a.grupo_empresa, a.nombre";
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	pc.addBorderCols("Tipo Config.",1);
	...
	if (al.size() == 0){
		...
		for (int i = 0; i<al.size(); i++){
			...
			if (!groupBy.equalsIgnoreCase(cdo.getColValue("grupo_empresa"))) {
				...
				pc.setFont(7, 1);
				pc.addBorderCols("["+cdo.getColValue("grupo_empresa")+"] "+cdo.getColValue("grupo_descripcion"),0,setDetail.size(),0.5f,0.0f,0.0f,0.0f);
			...
			pc.addCols(cdo.getColValue("tipo_descripcion"),0,1);
