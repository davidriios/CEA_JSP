...
if (usa_fecha_apl_pago == null) usa_fecha_apl_pago = "S";
String vista = "vw_cxc_mov_adm_fa2";
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if(resumido.trim().equals("N") || resumido.trim().equals("A")) sbSql.append(", pac_id, nombre, aseguradora");
  sbSql.append(", aseguradora_desc ");
  sbSql.append(", sum(saldo_ini) saldo_ini,  sum(cargos) as cargos, sum(factura_pac) as factura_pac, sum(factura_emp) as factura_emp, sum(desc_pac) as desc_pac, sum(desc_emp) as desc_emp, sum(paquete) as paquete, sum(ajuste_pac) as ajuste_pac, sum(ajuste_emp) as ajuste_emp ,(((sum(factura_si)+sum(factura_emp)-sum(pagos_aplicados))-sum(pagos_no_aplicados))-sum(ajuste_recibo)+(sum(ajuste_emp))) as saldo  ,(((sum(factura_si_pac)+sum(factura_pac)-sum(pagos_aplicados_pac)-sum(pagos_no_aplicados_pac)))-sum(ajuste_recibos_pac)+(sum(ajuste_pac))) as saldo_pac,sum(pagos_aplicados) pagos_aplicados, sum(pagos_no_aplicados) pagos_no_aplicados, sum(ajuste_recibo) ajuste_recibo, sum(factura_si) factura_si, sum(saldo_cxc) as saldo_cxc,sum(saldo_inicial_pac) as saldo_inicial_pac,sum(factura_si_pac) as factura_si_pac,sum(paquete_pac) as paquete_pac,sum(pagos_aplicados_pac) as pagos_aplicados_pac,sum(ajuste_recibos_pac) as ajuste_recibos_pac,sum(pagos_no_aplicados_pac) as pagos_no_aplicados_pac from (");
    sbSql.append(" select  compania, ");

  if(resumido.trim().equals("N")){
    sbSql.append(" pac_id ");
    sbSql.append(", (select (select nombre_paciente from vw_adm_paciente where pac_id =x.pac_id) from dual) as nombre, aseguradora, (select (select nombre from tbl_adm_empresa where codigo =x.aseguradora) from dual) as aseguradora_desc, 0 saldo_ini,0 as saldo_inicial_pac, ");
  } else if(resumido.trim().equals("A")){
    sbSql.append(" '' pac_id, '' nombre, aseguradora, nvl((select (select nombre from tbl_adm_empresa where codigo =x.aseguradora) from dual), 'N/A') as aseguradora_desc, 0 saldo_ini,0 as saldo_inicial_pac, ");
  }
  else sbSql.append(" '' as  pac_id, '' as nombre, '' aseguradora, ' ' aseguradora_desc, 0 saldo_ini,0 as saldo_inicial_pac, ");
  sbSql.append(" sum(cargos) as cargos,sum(factura_pac) as factura_pac,sum(factura_emp)as factura_emp,sum(desc_pac) as desc_pac,sum(desc_emp) as desc_emp,sum(paquete) as paquete,sum(ajuste_pac) as ajuste_pac,sum(ajuste_emp) as ajuste_emp ,");

 sbSql.append(" 0 as saldo, ");
    ...
    sbSql.append("', 'dd/mm/yyyy') then pagos_aplicados else 0 end) pagos_aplicados");

    sbSql.append(", (SUM (pagos_no_aplicados) + nvl(get_cxc_no_aplicado (x.compania,'");
    sbSql.append(fecha_ini);
    sbSql.append("','");
    sbSql.append(fecha_fin);
    sbSql.append("','TRX',");
    if(resumido.trim().equals("N"))sbSql.append(" aseguradora ");
    else if(resumido.trim().equals("A"))sbSql.append(" aseguradora  ");
    else sbSql.append(" null  ");
    sbSql.append(",'E','");
    sbSql.append(resumido);
    sbSql.append("',null ),0)) as pagos_no_aplicados");

  sbSql.append(", sum(ajuste_recibo) ajuste_recibo, sum(factura_si) factura_si, sum(factura_pac)+sum(factura_emp)+sum(desc_pac) + sum(desc_emp)- sum(paquete) as saldo_cxc ");
  sbSql.append(",sum(factura_si_pac) as factura_si_pac,sum(paquete_pac) as paquete_pac,sum(pagos_aplicados_pac) as pagos_aplicados_pac,sum(ajuste_recibos_pac) as ajuste_recibos_pac, to_number(nvl(get_cxc_no_aplicado (x.compania,'");
    sbSql.append(fecha_ini);
    sbSql.append("','");
    sbSql.append(fecha_fin);
    sbSql.append("','TRX',");
    if(resumido.trim().equals("N"))sbSql.append(" pac_id ");
    else if(resumido.trim().equals("A"))sbSql.append(" null  ");
    else sbSql.append(" null  ");
    sbSql.append(",'P','");
    sbSql.append(resumido);
    sbSql.append("',");
    if(resumido.trim().equals("N"))sbSql.append("  nvl(aseguradora,-100000) ");
    else if(resumido.trim().equals("A"))sbSql.append("  nvl(aseguradora,-100000)  ");
    else sbSql.append(" null  ");
    sbSql.append("),0)) pagos_no_aplicados_pac ");
  ...
  /**/
    sbSql.append(" (sum(factura_emp)+ sum(ajuste_emp)) - ");
    sbSql.append(" SUM (case when fecha_recibo < TO_DATE ('");
    sbSql.append(fecha_ini);
    sbSql.append("', 'dd/mm/yyyy') then pagos_aplicados else 0 end) - (SUM (pagos_no_aplicados)+ nvl(get_cxc_no_aplicado (x.compania,'");
    sbSql.append(fecha_ini);
    sbSql.append("','");
    sbSql.append(fecha_fin);
    sbSql.append("','SI',");
  ...
  /**/
  sbSql.append(" (sum(factura_pac)+ sum(ajuste_pac)) - ");
    sbSql.append(" SUM (case when fecha_recibo < TO_DATE ('");
    sbSql.append(fecha_ini);
    sbSql.append("', 'dd/mm/yyyy')  then pagos_aplicados_pac else 0 end) - (SUM (pagos_no_aplicados_pac)+ to_number(nvl(get_cxc_no_aplicado (x.compania,'");
    sbSql.append(fecha_ini);
    sbSql.append("','");
    sbSql.append(fecha_fin);
    sbSql.append("','SI',");
    ...
    sbSql.append("),0)) ) ");
    sbSql.append(" - sum(ajuste_recibos_pac) + sum(factura_si_pac)  as saldo_inicial_pac, ");


  sbSql.append(" sum(0) as cargos,sum(0) as factura_pac, sum(0) as factura_emp,sum(0) as desc_pac, sum(0) as desc_emp, sum(0) as paquete, sum(0) as ajuste_pac, sum(0) as ajuste_emp ,sum(0) as saldo, sum(0) pagos_aplicados, sum(0) pagos_no_aplicados, sum(0) ajuste_recibo, sum(0) factura_si, sum(0) as saldo_cxc,sum(0) as factura_si_pac, sum(0) as paquete_pac,sum(0) as pagos_aplicados_pac,sum(0) as ajuste_recibos_pac, 0 pagos_no_aplicados_pac  from ");
  sbSql.append(vista);
  sbSql.append(" x where compania =");
  sbSql.append(session.getAttribute("_companyId"));
  ...
  if(resumido.trim().equals("N"))sbSql.append(") group by compania, pac_id, nombre, aseguradora");
  else if(resumido.trim().equals("A"))sbSql.append(") group by compania, pac_id, nombre, aseguradora");
...
if(resumido=='N'){
 abrir_ventana('../cellbyteWV/report_container.jsp?reportName=cxc/rpt_cxc_aux_vs_morosidad.rptdesign&fDesde='+fecha_ini+'&fHasta='+fecha_fin+'&nombre='+nombre+'&codigo='+codigo+'&aseguradora='+aseguradora+'&pCtrlHeader=false');
 } else if(resumido=='A'){
 abrir_ventana('../cellbyteWV/report_container.jsp?reportName=cxc/rpt_cxc_aux_vs_morosidad_res.rptdesign&fDesde='+fecha_ini+'&fHasta='+fecha_fin+'&aseguradora='+aseguradora+'&pCtrlHeader=false');
 }
}
var xHeight=0;
...
<%=fb.formStart(true)%>
     ...
     <tr class="TextHeader" align="center">
      <%if(resumido.trim().equals("N")){%>
      <td width="4%"  rowspan="2"><cellbytelabel>C&oacute;digo</cellbytelabel></td>
      <td width="13%" rowspan="2"><cellbytelabel>Nombre</cellbytelabel></td>
      <%}%>
      <td width="12%" rowspan="2"><cellbytelabel>Aseguradora</cellbytelabel></td>

      <td colspan="8"><cellbytelabel>PACIENTE</cellbytelabel></td>
      <td colspan="8"><cellbytelabel>EMPRESA</cellbytelabel></td>
      <td colspan="4"><cellbytelabel>CXC</cellbytelabel></td>
    ...
    <tr class="TextHeader" align="center">
      ...
      <td width="7%"><cellbytelabel>Ajuste(+-)</cellbytelabel></td>
...
<%
double saldoTotal =0.00,saldo=0.00,saldoAnt=0.00,saldoAntPac=0.00,facturas_pac=0.00,facturas_emp=0.00,desc_pac=0.00,desc_emp=0.00,saldoFin=0.00,ajuste_pac=0.00,ajuste_emp=0.00,cargos=0.00,paquete=0.00, pagos_aplicados = 0.00, pagos_no_aplicados = 0.00, ajuste_recibo = 0.00, tot_pagos_app = 0.00, tot_pagos_no_app = 0.00, tot_ajuste_recibo = 0.00, factura_si = 0.00, tot_factura_si = 0.00,saldo_cxc=0.00,saldoPac=0.00,factura_si_pac=0.00,pagos_aplicados_pac=0.00,pagos_no_aplicados_pac=0.00,ajuste_recibo_pac=0.00,saldoFinPac=0.00;
...
for (int i=0; i<al.size(); i++)
{
  ...
  saldoAntPac += Double.parseDouble(cdo.getColValue("saldo_inicial_pac"));
  factura_si_pac  += Double.parseDouble(cdo.getColValue("factura_si_pac"));
  facturas_pac += Double.parseDouble(cdo.getColValue("factura_pac"));
  ajuste_pac += Double.parseDouble(cdo.getColValue("ajuste_pac"));
  pagos_aplicados_pac  += Double.parseDouble(cdo.getColValue("pagos_aplicados_pac"));
  pagos_no_aplicados_pac  += Double.parseDouble(cdo.getColValue("pagos_no_aplicados_pac"));
  ajuste_recibo_pac  += Double.parseDouble(cdo.getColValue("ajuste_recibos_pac"));
  ...
  saldo_cxc += Double.parseDouble(cdo.getColValue("saldo_cxc"));
  ...
  saldoPac     = Double.parseDouble(cdo.getColValue("saldo_inicial_pac"))+Double.parseDouble(cdo.getColValue("saldo_pac"));

  saldoFin     += Double.parseDouble(cdo.getColValue("saldo_ini"))+Double.parseDouble(cdo.getColValue("saldo"));
  saldoFinPac  += Double.parseDouble(cdo.getColValue("saldo_inicial_pac"))+Double.parseDouble(cdo.getColValue("saldo_pac"));
...
%>
    <tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
     <%if(resumido.trim().equals("N")){%>
      ...
      <td align="right"><%=CmnMgr.getFormattedDecimal(cdo.getColValue("saldo_inicial_pac"))%></td>
      <td align="right"><%=CmnMgr.getFormattedDecimal(cdo.getColValue("factura_si_pac"))%></td>
      ...
      <td align="right"><%=CmnMgr.getFormattedDecimal(cdo.getColValue("pagos_aplicados_pac"))%></td>
      <td align="right"><%=CmnMgr.getFormattedDecimal(cdo.getColValue("pagos_no_aplicados_pac"))%></td>
      <td align="right"><%=CmnMgr.getFormattedDecimal(cdo.getColValue("ajuste_recibos_pac"))%></td>
      <td align="right">
        <%if(saldoPac<0){%><label  class="<%=color%>" style="cursor:pointer"><label class="RedTextBold">&nbsp;&nbsp;<%}%>
      <%=CmnMgr.getFormattedDecimal(saldoPac)%>
        <%if(saldoPac<0){%>&nbsp;&nbsp;</label></label><%}%>
      </td>
      ...
      <td align="right"><%=CmnMgr.getFormattedDecimal(cdo.getColValue("saldo_cxc"))%></td>
      <td align="center">&nbsp;
...
%>
  <tr class="TextHeader02" align="center">
      ...
      <%}else{%>
      <td  align="right">TOTALES PAGINA</td>
      <%}%>

      <td align="right"><%=CmnMgr.getFormattedDecimal(saldoAntPac)%></td>
      <td align="right"><%=CmnMgr.getFormattedDecimal(factura_si_pac)%></td>
      <td align="right"><%=CmnMgr.getFormattedDecimal(facturas_pac)%></td>
      <td align="right"><%=CmnMgr.getFormattedDecimal(ajuste_pac)%></td>
      <td align="right"><%=CmnMgr.getFormattedDecimal(pagos_aplicados_pac)%></td>
      <td align="right"><%=CmnMgr.getFormattedDecimal(pagos_no_aplicados_pac)%></td>
      <td align="right"><%=CmnMgr.getFormattedDecimal(ajuste_recibo_pac)%></td>
      <td align="right"><%=CmnMgr.getFormattedDecimal(saldoFinPac)%></td>
      ...
      <td align="right"><%=CmnMgr.getFormattedDecimal(saldo_cxc)%></td>
