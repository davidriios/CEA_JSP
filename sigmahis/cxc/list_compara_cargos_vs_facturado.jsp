...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	if (!aseguradora.trim().equals("")) { sbFilter.append(" and exists (select null from tbl_adm_beneficios_x_admision where pac_id = f.pac_id and admision = f.admi_secuencia and estado = 'A' and empresa = "); sbFilter.append(aseguradora.toUpperCase()); sbFilter.append(")"); }
	...
	if (!noAdm.trim().equals("")) { sbFilter.append(" and f.admi_secuencia = "); sbFilter.append(noAdm);}
		...
		if (!categoria.trim().equals("") || !tipoAdm.trim().equals("") )
	{
			 sbFilter.append(" and exists (select null from tbl_adm_admision a where a.pac_id=f.pac_id and a.secuencia=f.admi_secuencia and compania=f.compania ");
		...
		if (!diferencia.trim().equals("")){
		sbSql.append(") where (nvl(cargos,0)+nvl(paquete,0)) <> ( nvl(monto_fact_pac,0) + nvl(monto_desc_pac,0)+nvl(monto_fact_emp,0)+nvl(monto_desc_emp,0)) ");
		}
...
function showReporte()
{
	...
	if(document.form0.diferencia.checked==true) diferencia = "S";
		...
		abrir_ventana1('../cellbyteWV/report_container.jsp?reportName=cxc/rpt_cargos_vs_facturado.rptdesign&fDesde='+fechaIni+'&fHasta='+fechaFin+'&aseguradora='+aseguradora+'&pac_id='+pacId+'&aseguradora_desc='+aseguradoraDesc+'&categoria='+categoria+'&noAdm='+noAdm+'&diferencia='+diferencia+'&pCtrlHeader=false&tipoAdm='+tipoAdm);
}
		...
		<tr class="TextFilter" valign="top">
			...
			<td>
				...
				ADM:<%=fb.textBox("noAdm",noAdm,false,false,false,5,"Text10",null,null)%>
					...
					Diferencia:<%=fb.checkbox("diferencia","S",diferencia.equals("S"),false)%>&nbsp;
			</td>
...
<tr>
	<td align="right"><authtype type='0'><a href="javascript:showReporte()" class="Link00">[ <cellbytelabel>Imprimir Lista</cellbytelabel> ]</a></authtype></td>
</tr>
<tr>
...
<%=fb.hidden("index","-1")%>
		<tr class="TextHeader" align="center">
			<td width="11%"><cellbytelabel>Expediente</cellbytelabel></td>
			<td width="15%"><cellbytelabel>Poliza</cellbytelabel></td>
			...
			<td width="6%"><cellbytelabel>Dif. (cargos vs Fact)</cellbytelabel></td>
			<td width="3%">&nbsp;</td>
		</tr>
...
for (int i=0; i<al.size(); i++)
{
	...
	if (i % 2 == 0) color = "TextRow01";
		...
		dif = Double.parseDouble(cdo.getColValue("cargos")) + Double.parseDouble(cdo.getColValue("paquete")) - (Double.parseDouble(cdo.getColValue("monto_fact_pac"))+Double.parseDouble(cdo.getColValue("monto_desc_pac"))+Double.parseDouble(cdo.getColValue("monto_fact_emp"))+Double.parseDouble(cdo.getColValue("monto_desc_emp"))) ;
...
<tr class="TextHeader01">
	<td colspan="13"><%=cdo.getColValue("cat_desc")%></td>
</tr>
<%}%>
		...
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="right"><%=CmnMgr.getFormattedDecimal(dif)%></td>
