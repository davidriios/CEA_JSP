...
if(mostrar_factura == null) mostrar_factura ="";
 ...
 sbSql.append(" select m.tipo_cta, decode(m.tipo_cta,'M','MEDICO','A','ASEGURADORA','J' ,'JUBILADO','P','PARTICULAR','N','JUNTA DIRECTIVA','E','EMPLEADO','O','OTROS CLIENTES','X','DETALLE AUX.') as desc_tipo_cta, decode(m.aseguradora, 0, null, m.aseguradora) aseguradora, decode(m.tipo_cta, 'A', e.nombre, m.nombre || '-' || m.cedula) as desc_empresa, nvl(e.nombre,  'PAGO NO APLICADO') nombre_aseguradora, sum(nvl(m.scorriente,0)) scorriente, sum(nvl(m.s30,0)) s30, sum(nvl(m.s60,0)) s60, sum(nvl(m.s90,0)) s90, sum(nvl(m.s120,0)) s120, sum(nvl(m.s150,0)) s150, sum(nvl(m.scorriente,0) + nvl(m.s30,0) + nvl(m.s60,0) + nvl(m.s90,0) + nvl(m.s120,0) + nvl(m.s150,0)) saldo_actual, m.tipo_cta from tbl_cxc_morosidades m, tbl_adm_empresa e, tbl_fac_factura f where  m.aseguradora = e.codigo(+) and m.cia = f.compania(+) and m.factura = f.codigo(+)");
sbSql.append(" and m.cia = ");
sbSql.append(compania);


	if (tipo_cta.trim().equals("A")) sbSql.append(" and m.tipo_cta in ('A', 'P')");
...
sbSql.append(" group by m.tipo_cta, decode(m.tipo_cta,'M','MEDICO','A','ASEGURADORA','J' ,'JUBILADO','P','PARTICULAR','N','JUNTA DIRECTIVA','E','EMPLEADO','O','OTROS CLIENTES','X','DETALLE AUX.'), m.aseguradora, decode(m.tipo_cta, 'A', e.nombre, m.nombre || '-' || m.cedula), nvl(e.nombre,  'PAGO NO APLICADO'), m.tipo_cta"); 
sbSql.append(" order by 3 asc nulls last, 1, 2, 4 asc");
	...
	PdfCreator pc = new PdfCreator(directory+folderName+"/"+year+"/"+month+"/"+fileName, width, height, isLandscape, sbFooter.toString(), leftRightMargin, topMargin, bottomMargin, headerFooterFont, logoMark, logoPath, statusMark, statusPath, displayPageNo, pageNoFontSize, pageNoLabel, pageNoPoxX, pageNoPosY);
			...
			for (int i=0; i<al.size(); i++)
			{
				CommonDataObject cdo1 = (CommonDataObject) al.get(i);
					...
					if(!groupBy2.equals(cdo1.getColValue("nombre_aseguradora"))){
						pc.setFont(8, 1);
						if(i!=0){
