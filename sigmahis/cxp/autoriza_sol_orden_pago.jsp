...
function reloadPage(unidad_adm){
	window.location = '../cxp/autoriza_sol_orden_pago.jsp?unidad_adm='+unidad_adm;
}
...
function setDetValues(){
	...
	if(documento!='' && fecha !=''){
		window.frames['itemFrame'].location = '../cxp/autoriza_sol_orden_pago_det.jsp?documento='+documento+'&fecha='+fecha;
	}
}
	...
	for(i=0;i<size;i++){
		if(eval('document.orden_pago.estado1_'+i).value=='T' && eval('document.orden_pago.estado_ini'+i).value=='A'){
			if('<%=error_en_permiso%>' == 'S'){
				...
				var monto = parseFloat(replace(eval('document.orden_pago.monto'+i).value, ',', ''));
    ...
    <td class="TableBorder"><table align="center" width="99%" cellpadding="0" cellspacing="1">
		...
		<div id="list_op" width="100%" class="child">
              ...
              <tr class="TextHeader02" >
                ...
                <td align="center" width="31%"><cellbytelabel>Unidad Administrativa</cellbytelabel></td>
              ...
              <tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')" style="cursor:pointer">
                ...
                <td align="center"><%=fb.select("estado1_"+i,"A=Aprobado,T=Autorizado,X=Rechazado", OP.getColValue("estado1"), false, false,0,"text10",null,"onChange=\"javascript:setMotivoRechazo("+i+", this.value);\"")%> </td>
              ...
              <tr id="tr<%=i%>" class="TextRow01" style="display:none">
                <td colspan="7"><cellbytelabel>Motivo del Rechazo</cellbytelabel>:<%=fb.textarea("motivo_rechazado"+i,OP.getColValue("motivo_rechazado"),false,false,false,90,2,"text10",null,"")%></td>
              </tr>
							<%}%>
              ...
              <tr class="" >
                <td colspan="7">
                    ...
                    <tr class="TextHeader02" >
                      ...
                      Tipo Persona:<%=fb.textBox("tipo_persona_desc","",false,false,true,20,"text10",null,"")%>
                      </td>
											<td width="35%">
              ...
              <tr>
                <td colspan="7"><iframe name="itemFrame" id="itemFrame" frameborder="0" align="center" width="100%" height="73" scrolling="no" src="../cxp/autoriza_sol_orden_pago_det.jsp?change=<%=change%>&mode=<%=mode%>&fg=<%=fg%>&fp=<%=fp%>&documento=<%=documento%>"></iframe></td>
              </tr>
            </table></td>
        </tr>
        <tr>
...
else
{
	...
	for(int i=0;i<keySize;i++){
		...
		if(request.getParameter("motivo_rechazado"+i)!=null && !request.getParameter("motivo_rechazado"+i).equals("")) cdo.addColValue("motivo_rechazado", request.getParameter("motivo_rechazado"+i));
...
System.out.println("action......="+request.getParameter("action"));
	if (request.getParameter("action").equals("Guardar")){
		OrdPagoMgr.autorizaSolOrdenPago(al);
	}
%>
<html>
<head>
...
%>
	...
	window.location = '<%=request.getContextPath()%>/cxp/autoriza_sol_orden_pago.jsp?unidad_adm=<%=request.getParameter("unidad_adm")%>';
<%
