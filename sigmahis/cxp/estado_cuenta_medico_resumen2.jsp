...
//saldo_inicial = todo lo facturado + lo ajustado(debito o credito)- lo pagado en cheques.. 
  ...
  sbSql.append(" select x.tipo,x.cod_honorario,x.nombre, x.saldo_inicial, x.facturado,x.ajustado,x.pagado, x.pagado_cont, x.auxiliar, x.ajuste_pago, x.ajuste_cxp, (nvl(x.saldo_inicial,0)+nvl(x.facturado, 0)+ nvl(x.ajustado,0) -nvl(x.pagado,0) -nvl(x.pagado_cont,0)/**/ + nvl(x.ajuste_pago,0)  + nvl(x.auxiliar, 0)/*+ nvl(x.ajuste_cxp, 0)*/ ");
	 
	sbSql.append(") saldo_final from( select 'M' tipo, m.codigo cod_honorario,m.primer_nombre ||decode(m.segundo_nombre,null,'', ' '||m.segundo_nombre)|| ' '||m.primer_apellido||decode(m.segundo_apellido,null,'', ' '||m.segundo_apellido)||decode(m.apellido_de_casada,null,'',' '||m.apellido_de_casada) nombre,nvl(getsaldoiniHon(");
  ...
  sbSql.append((String) session.getAttribute("_companyId"));  
  sbSql.append("),0) ajustado,nvl(( select sum(x.monto_a_pagar) pagado from tbl_con_cheque c,tbl_cxp_orden_de_pago op,tbl_cxp_detalle_orden_pago x where op.compania = ");
  sbSql.append((String) session.getAttribute("_companyId"));  
  sbSql.append(" and op.anio = c.anio and op.num_orden_pago = c.num_orden_pago and op.cod_compania = c.cod_compania_odp and op.anio = x.anio and op.num_orden_pago = x.num_orden_pago and op.cod_compania = x.cod_compania and op.cod_tipo_orden_pago=1 /* and op.cod_medico is not null */ and c.estado_cheque <> 'A' and op.estado = 'A' and op.generado='H' and c.f_emision >= to_date('"); 
  sbSql.append(fecha_ini);
  sbSql.append("', 'dd/mm/yyyy') and c.f_emision <= to_date('");
  sbSql.append(fecha_fin);
  sbSql.append("', 'dd/mm/yyyy') and nvl(x.cod_honorario,x.cod_benef)=m.codigo and x.tipo_hon='H' ), 0) as pagado, nvl((select sum(nvl(h.monto_ajuste,0))-sum(nvl(h.retencion,0)) ajuste_pago from tbl_cxp_hon_det h where nvl(h.cod_real_med,h.cod_medico)=m.codigo and h.tipo_h in ('M','H') and h.fecha >= to_date('");
  sbSql.append(fecha_ini);
  ...
  sbSql.append(fecha_fin);
  sbSql.append("', 'dd/mm/yyyy') and cxp.ref_id = m.codigo and cxp.estado ='R'), 0) ajuste_cxp  from tbl_adm_medico m union all select 'E' tipo, to_char(e.codigo)cod_honorario,e.nombre, nvl(getsaldoiniHon(");
  sbSql.append((String) session.getAttribute("_companyId"));  
  sbSql.append(",'");
  sbSql.append(fecha_ini);
  ...
  sbSql.append("),0)ajustado,nvl(( select sum(x.monto_a_pagar) pagado from tbl_con_cheque c,tbl_cxp_orden_de_pago op,tbl_cxp_detalle_orden_pago x where op.compania = ");
  sbSql.append((String) session.getAttribute("_companyId"));  
  sbSql.append(" and op.anio = c.anio and op.num_orden_pago = c.num_orden_pago and op.cod_compania = c.cod_compania_odp and op.cod_tipo_orden_pago in (1) and op.cod_empresa is not null and c.estado_cheque <> 'A' and op.generado in ('H') and op.num_orden_pago = x.num_orden_pago and op.cod_compania = x.cod_compania and x.cod_honorario=to_char(e.codigo) and x.tipo_hon='E' and c.f_emision >= to_date('");
  sbSql.append(fecha_ini);
  sbSql.append("','dd/mm/yyyy') and c.f_emision <= to_date('");
  sbSql.append(fecha_fin);
  sbSql.append("','dd/mm/yyyy') and op.num_id_beneficiario = to_char(e.codigo)), 0) as pagado, nvl((select sum(nvl(h.monto_ajuste,0))-sum(nvl(h.retencion,0)) ajuste_pago from tbl_cxp_hon_det h where nvl(h.cod_real_med,h.cod_medico)=to_char(e.codigo) and h.tipo_h='E' and h.fecha >= to_date('");
  sbSql.append(fecha_ini);
  sbSql.append("', 'dd/mm/yyyy') and h.fecha <= to_date('");
  sbSql.append(fecha_fin);
  ...
  sbSql.append("', 'dd/mm/yyyy') and cxp.fecha  <= to_date('");
  sbSql.append(fecha_fin);
  sbSql.append("', 'dd/mm/yyyy') and cxp.ref_id = to_char(e.codigo) and cxp.estado ='R'), 0) ajuste_cxp   from tbl_adm_empresa e  where e.tipo_empresa=1 ) x where (nvl(x.facturado,0)+nvl(x.ajustado,0)<> 0 or nvl(x.pagado,0) >0 or nvl(x.pagado_cont,0) >0 /*or nvl(x.ajuste_cxp,0) >0*/ or nvl(x.saldo_inicial,0) <>0 )"); 
 if(!tipo.trim().equals("")){
...
abrir_ventana2('../cellbyteWV/report_container.jsp?reportName=cxp/rpt_cxp_estado_cta_hon_resumen2.rptdesign&codigoParam='+codigo+'&nombreParam='+nombre+'&fIniParam='+fechaini+'&fFinParam='+fechafin+'&tipoParam='+tipo); 
}

function viewEC(code, tipo){var fechaini = document.search01.fecha_ini.value;var fechafin = document.search01.fecha_fin.value;abrir_ventana('../cxp/ver_mov_hon2.jsp?mode=ver&beneficiario='+code+'&tipo='+tipo+'&fechaini='+fechaini+'&fechafin='+fechafin);}
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="0" cellspacing="1">
				<tr class="TextHeader">
					...
					<td width="18%" align="center" ><cellbytelabel>Nombre</cellbytelabel></td>
					<td width="7%" ><cellbytelabel>Saldo Ini.</cellbytelabel></td>
				    <td align="center" width="14%"><cellbytelabel>Facturado</cellbytelabel></td>
				    <td align="center" width="7%" ><cellbytelabel>Ajustado</cellbytelabel></td>
					<td align="center" width="7%" ><cellbytelabel>Retenci&oacute;n/ajuste</cellbytelabel></td>
				    <td align="center" width="6%" ><cellbytelabel>Pagado Hon.</cellbytelabel></td>
				    <td align="center" width="6%" ><cellbytelabel>Pagado Cont.</cellbytelabel></td>
				    <td align="center" width="6%" ><cellbytelabel>Auxiliar</cellbytelabel></td> 
				    <td align="center" width="7%" ><cellbytelabel>Saldo Final</cellbytelabel></td>
					<td width="3%" align="center" ></td>
				</tr>	
				 
				<%
				double saldo = 0.00, saldo_por_pagar = 0.00, saldo_inicial = 0.00;
