...
if(tipo == null) tipo="M";
if(acumHonNoFacturaro == null) acumHonNoFacturaro="S";
...
//saldo_inicial = todo lo facturado + lo ajustado(debito o credito)- lo pagado en cheques.. 
  ...
  sbSql.append(" select x.tipo,x.cod_honorario,x.nombre, x.saldo_inicial, x.facturado,x.ajustado,x.cobrado,x.pagado, x.pagado_cont, x.auxiliar, x.ajuste_pago, x.ajuste_cxp, (x.saldo_inicial - x.cobrado_no_pagado)+x.facturado+x.ajustado - x.cobrado ");
	if(acumHonNoFacturaro.equals("S")){
	sbSql.append("+ x.hon_no_facturado");
	}
	sbSql.append(" por_cobrar, x.cobrado_no_pagado + x.cobrado - x.pagado + x.ajuste_pago por_pagar, (nvl(x.saldo_inicial,0)+nvl(x.facturado, 0)+ nvl(x.ajustado,0) -nvl(x.pagado,0) /*-nvl(x.pagado_cont,0)*/ + nvl(x.ajuste_pago,0)  /*+ nvl(x.auxiliar, 0)+ nvl(x.ajuste_cxp, 0)*/ ");
	if(acumHonNoFacturaro.equals("S")){
	sbSql.append("+ x.hon_no_facturado");
	}
	sbSql.append(") saldo_final, x.hon_no_facturado from (");

	if (tipo.trim().equals("") || tipo.equalsIgnoreCase("M")) {

		sbSql.append("select 'M' tipo, m.codigo cod_honorario,m.primer_nombre ||decode(m.segundo_nombre,null,'', ' '||m.segundo_nombre)|| ' '||m.primer_apellido||decode(m.segundo_apellido,null,'', ' '||m.segundo_apellido)||decode(m.apellido_de_casada,null,'',' '||m.apellido_de_casada) nombre, nvl((select getsaldoinicialHon(");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(",'");
		sbSql.append(fecha_ini);
		sbSql.append("',m.codigo,'M') from dual),0) saldo_inicial, nvl((SELECT SUM (DECODE (b.tipo_transaccion, 'D', -1 * b.cantidad, b.cantidad) * (b.monto + nvl(b.recargo, 0))) monto FROM tbl_fac_transaccion a, tbl_fac_detalle_transaccion b WHERE a.compania = ");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(" and a.fecha  >= to_date('"); 
		sbSql.append(fecha_ini);
		sbSql.append("', 'dd/mm/yyyy') and a.fecha  <= to_date('");
		sbSql.append(fecha_fin);
		sbSql.append("', 'dd/mm/yyyy') AND a.codigo = b.fac_codigo AND a.pac_id = b.pac_id AND a.admi_secuencia = b.fac_secuencia AND a.compania = b.compania AND a.tipo_transaccion = b.tipo_transaccion and b.tipo_cargo = get_sec_comp_param(-1, 'COD_TIPO_SERV_HON') and a.tipo_transaccion in ('H', 'D') and a.pagar_sociedad = 'N' and a.med_codigo = m.codigo  and exists (select null from tbl_fac_factura f where f.estatus  != 'A' and f.pac_id = a.pac_id and f.admi_secuencia = a.admi_secuencia and f.compania = a.compania ) ), 0) facturado,nvl(( select sum (decode (ad.lado_mov, 'D', ad.monto,'C',-1*monto)) from vw_con_adjustment_gral ad,tbl_fac_tipo_ajuste ta,tbl_fac_factura f where (ad.medico = m.codigo and ad.tipo = 'H' and ad.tipo_doc='F') and ta.group_type not in ('D','F') and ta.compania = ad.compania and ta.codigo = ad.tipo_ajuste and ad.fecha_aprob_idx >= to_date('");
		sbSql.append(fecha_ini);
		...
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append("),0) ajustado, nvl((SELECT sum(h.monto) FROM tbl_cxp_hon_det h WHERE h.tipo IN ('M', 'H') AND NVL (h.monto, 0) != 0 and compania = ");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(" and cod_medico = m.codigo and trunc(fecha) >= to_date('");
		sbSql.append(fecha_ini);
		sbSql.append("', 'dd/mm/yyyy') and trunc(fecha) <= to_date('");
		sbSql.append(fecha_fin);
		...
		sbSql.append("', 'dd/mm/yyyy') and a.fecha  <= to_date('");
		sbSql.append(fecha_fin);
		sbSql.append("', 'dd/mm/yyyy') AND a.codigo = b.fac_codigo AND a.pac_id = b.pac_id AND a.admi_secuencia = b.fac_secuencia AND a.compania = b.compania AND a.tipo_transaccion = b.tipo_transaccion and b.tipo_cargo = get_sec_comp_param(-1, 'COD_TIPO_SERV_HON') and a.tipo_transaccion in ('H', 'D') and a.pagar_sociedad = 'N' and a.med_codigo = m.codigo  and not exists (select null from tbl_fac_factura f where f.estatus  != 'A' and f.pac_id = a.pac_id and f.admi_secuencia = a.admi_secuencia and f.compania = a.compania ) ), 0) hon_no_facturado, nvl((SELECT sum(h.monto) FROM tbl_cxp_hon_det h WHERE h.tipo IN ('M', 'H') AND NVL (h.monto, 0) != 0 and compania = ");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(" and cod_medico = m.codigo and h.fecha < to_date('");
		sbSql.append(fecha_ini);
		sbSql.append("', 'dd/mm/yyyy') and (odp_numero is null or exists (select null from tbl_con_cheque c, tbl_cxp_orden_de_pago op WHERE   op.compania = ");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(" AND op.anio = c.anio AND op.num_orden_pago = c.num_orden_pago AND op.cod_compania = c.cod_compania_odp AND op.cod_tipo_orden_pago = 1 AND op.cod_medico IS NOT NULL AND op.generado = 'H' and h.odp_compania = c.cod_compania_odp AND h.odp_anio = c.anio AND h.odp_numero = c.num_orden_pago AND c.estado_cheque <> 'A' AND c.f_emision >= to_date('");
		sbSql.append(fecha_ini);
		sbSql.append("', 'dd/mm/yyyy')))), 0) cobrado_no_pagado from tbl_adm_medico m");

	}

	if (tipo.trim().equals("")) sbSql.append(" union all ");

	if (tipo.trim().equals("") || tipo.equalsIgnoreCase("E")) {

		sbSql.append("select 'E' tipo, to_char(e.codigo)cod_honorario,e.nombre, nvl(getsaldoinicialHon(");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(",'");
		sbSql.append(fecha_ini);
		sbSql.append("',e.codigo,'E'),0)saldo_inicial, nvl((SELECT SUM (DECODE (b.tipo_transaccion, 'D', -1 * b.cantidad, b.cantidad) * (b.monto + nvl(b.recargo, 0))) monto FROM tbl_fac_transaccion a, tbl_fac_detalle_transaccion b WHERE a.compania = ");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(" and a.fecha  >= to_date('"); 
		sbSql.append(fecha_ini);
		sbSql.append("', 'dd/mm/yyyy') and a.fecha  <= to_date('");
		sbSql.append(fecha_fin);
		sbSql.append("', 'dd/mm/yyyy') AND a.codigo = b.fac_codigo AND a.pac_id = b.pac_id AND a.admi_secuencia = b.fac_secuencia AND  a.compania = b.compania AND a.tipo_transaccion = b.tipo_transaccion and b.tipo_cargo = get_sec_comp_param(-1, 'COD_TIPO_SERV_HON') and a.tipo_transaccion in ('H', 'D') and a.pagar_sociedad = 'S' and  a.empre_codigo = e.codigo  and exists (select null from tbl_fac_factura f where f.estatus != 'A' and f.pac_id = a.pac_id and f.admi_secuencia = a.admi_secuencia and f.compania = a.compania) ), 0) facturado,nvl(( select sum (decode (ad.lado_mov, 'D', ad.monto,'C',-1*monto)) from vw_con_adjustment_gral ad,tbl_fac_tipo_ajuste ta,tbl_fac_factura f where (ad.empresa = e.codigo and ad.tipo = 'E' and ad.tipo_doc='F') and ta.group_type not in ('D','F') and ta.compania = ad.compania and ta.codigo = ad.tipo_ajuste and ad.fecha_aprob_idx >= to_date('");
		sbSql.append(fecha_ini);
		sbSql.append("', 'dd/mm/yyyy')and ad.fecha_aprob_idx <= to_date('");
		sbSql.append(fecha_fin);
		sbSql.append("', 'dd/mm/yyyy') and f.estatus <> 'A' and f.compania = ad.compania and f.codigo = ad.factura and ad.compania =");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append("),0) ajustado, nvl((SELECT sum(h.monto) FROM tbl_cxp_hon_det h WHERE h.tipo = 'E' AND NVL (h.monto, 0) != 0 and compania = ");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(" and cod_medico = to_char(e.codigo) and trunc(fecha) >= to_date('");
		sbSql.append(fecha_ini);
		sbSql.append("', 'dd/mm/yyyy') and trunc(fecha) <= to_date('");
		sbSql.append(fecha_fin);
		sbSql.append("', 'dd/mm/yyyy') ),0) cobrado, nvl(( select sum(c.monto_girado) pagado from tbl_con_cheque c,tbl_cxp_orden_de_pago op where op.compania = ");
		...
		sbSql.append("', 'dd/mm/yyyy') and cxp.fecha  <= to_date('");
		sbSql.append(fecha_fin);
		sbSql.append("', 'dd/mm/yyyy') and cxp.ref_id = to_char(e.codigo) and cxp.estado ='R'), 0) ajuste_cxp, nvl((SELECT SUM (DECODE (b.tipo_transaccion, 'D', -1 * b.cantidad, b.cantidad) * (b.monto + nvl(b.recargo, 0))) monto FROM tbl_fac_transaccion a, tbl_fac_detalle_transaccion b WHERE a.compania = ");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(" and a.fecha  >= to_date('"); 
		sbSql.append(fecha_ini);
		sbSql.append("', 'dd/mm/yyyy') and a.fecha  <= to_date('");
		sbSql.append(fecha_fin);
		sbSql.append("', 'dd/mm/yyyy') AND a.codigo = b.fac_codigo AND a.pac_id = b.pac_id AND a.admi_secuencia = b.fac_secuencia AND  a.compania = b.compania AND a.tipo_transaccion = b.tipo_transaccion and b.tipo_cargo = get_sec_comp_param(-1, 'COD_TIPO_SERV_HON') and a.tipo_transaccion in ('H', 'D') and a.pagar_sociedad = 'S' and  a.empre_codigo = e.codigo  and not exists (select null from tbl_fac_factura f where f.estatus != 'A' and f.pac_id = a.pac_id and f.admi_secuencia = a.admi_secuencia and f.compania = a.compania) ), 0) hon_no_facturado, nvl((SELECT sum(h.monto) FROM tbl_cxp_hon_det h WHERE h.tipo = 'E' AND NVL (h.monto, 0) != 0 and compania = ");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(" and cod_medico = to_char(e.codigo) and h.fecha < to_date('");
		sbSql.append(fecha_ini);
		sbSql.append("', 'dd/mm/yyyy') and (odp_numero is null or exists (select null from tbl_con_cheque c, tbl_cxp_orden_de_pago op WHERE op.compania = ");
		sbSql.append((String) session.getAttribute("_companyId"));  
		sbSql.append(" AND op.anio = c.anio AND op.num_orden_pago = c.num_orden_pago AND op.cod_compania = c.cod_compania_odp AND op.cod_tipo_orden_pago = 1 AND op.cod_empresa IS NOT NULL AND op.generado = 'H' and h.odp_compania = c.cod_compania_odp AND h.odp_anio = c.anio AND h.odp_numero = c.num_orden_pago AND c.estado_cheque <> 'A' AND c.f_emision >= to_date('");
		sbSql.append(fecha_ini);
		sbSql.append("', 'dd/mm/yyyy')))), 0) cobrado_no_pagado from tbl_adm_empresa e  where e.tipo_empresa=1");

	}

	sbSql.append(") x where (nvl(x.facturado,0)+nvl(x.ajustado,0)<> 0 or nvl(x.cobrado,0) > 0 or nvl(x.pagado,0) >0 or nvl(x.pagado_cont,0) >0 /*or nvl(x.ajuste_cxp,0) >0*/ or nvl(x.saldo_inicial,0) <>0)"); 
...
abrir_ventana2('../cellbyteWV/report_container.jsp?reportName=cxp/rpt_cxp_estado_cta_hon_resumen_new.rptdesign&codigoParam='+codigo+'&nombreParam='+nombre+'&fIniParam='+fechaini+'&fFinParam='+fechafin+'&tipoParam='+tipo); 
}

function viewEC(code, tipo){var fechaini = document.search01.fecha_ini.value;var fechafin = document.search01.fecha_fin.value;abrir_ventana('../cxp/ver_mov_hon_hnf.jsp?mode=ver&beneficiario='+code+'&tipo='+tipo+'&fechaini='+fechaini+'&fechafin='+fechafin+'&acumHonNoFacturaro=<%=acumHonNoFacturaro%>');}
...
<table align="center" width="99%" cellpadding="1" cellspacing="0"  id="_tblMain">
		...
		<td>
			<table width="100%" cellpadding="0" cellspacing="1">
			    <tr class="TextFilter">		
					...
					<%=fb.hidden("acumHonNoFacturaro", "S")%>
    ...
    <tr>
        <td align="right">
		  		<authtype type='0'><a href="javascript:printListBI()" class="Link00">[ <cellbytelabel>Excel</cellbytelabel> ]</a></authtype>
				</td>
    </tr>
	<tr>
