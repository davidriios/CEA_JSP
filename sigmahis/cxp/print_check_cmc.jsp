...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	float width = 72 * 7.3125f;//526.5 //7.362f;
	float height = 72 * 8.4f;//604.8 //7.323f;
	...
	float ckHeight = 72 * 3.625f;//261
	float ckdHeight = 72 * 4.775f;//343.8
	...
	for (int i=0; i<htCK.size(); i++) {
		...
		pc.createTable(true);
			...
			//- - - - - - - - - - - - - - - - - - - -   C H E C K   - - - - - - - - - - - - - - - - - - - -
			//-----> top space
			pc.addCols(" ",0,dHeader.size(),57.0f);
			...
			pc.addCols(cdoCK.getColValue("dia")+" "+cdoCK.getColValue("mes"),2,2);
			pc.addCols(" ",0,1);
			pc.addCols(cdoCK.getColValue("anio"),0,1);
			pc.addCols(" ",0,1);

			//-----> space between date and name/amount
			pc.addCols(" ",0,dHeader.size(),19.0f);
