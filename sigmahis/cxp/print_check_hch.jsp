...
//java.util.GregorianCalendar gc=new java.util.GregorianCalendar();
	...
	float width = 72 * 8.1f;//612
	float height = 72 * 8.1f;//792
	...
	Vector anexo = new Vector();
		anexo.addElement("14");
		anexo.addElement("65");
		anexo.addElement("120");
		anexo.addElement("120");
		anexo.addElement("80");
		anexo.addElement("110");

	Vector detFinal = new Vector();
		detFinal.addElement("29");
		detFinal.addElement("112");
		detFinal.addElement("112");
		detFinal.addElement("167");
		detFinal.addElement("134");
		detFinal.addElement("29");
	...
	*/
	...
	for (int i=0; i<htCK.size(); i++){
		...
		pc.createTable();
				...
				// por ahora no      pc.addCols(cdoCK.getColValue("num_cheque"),2,4);
				pc.addCols("",2,4);
				pc.addCols(" ",0,2);


				pc.addCols(" ",0,2);
				pc.addCols(cdoCK.getColValue("dia")+" "+cdoCK.getColValue("mes"),1,2);
				pc.addCols("   "+cdoCK.getColValue("anio"),1,2);
				pc.addCols(" ",0,dHeader.size(), 15.0f);

				pc.addCols(" ",0,1);
				pc.addCols(cdoCK.getColValue("beneficiario"),0,2, 28.0F);
				// 29 mayo 2013 pc.addCols(" ",0,1);
				pc.addCols(cdoCK.getColValue("monto_girado_formatted"),0,3);

				pc.addCols(" ",0,dHeader.size(), 5.0f);
		...
		cdoCK.setSql(objOP.getCdo().getSql());
				for (int j=0; j<objOP.getAlDet().size(); j++)
				{
					...
					if (j == 0 && !cdoCK.getColValue("factura").trim().equals("")) {

						pc.addCols(" ",0,1);
						pc.addCols("PARA CANCELAR FACTURA(S) NO. "+cdoCK.getColValue("factura"),0,5);
			pc.addCols(" ",0,1);
						pc.addCols("PAGO A FAVOR DE : "+cdoCK.getColValue("beneficiarioDet"),0,5);
				...
				pc.addCols(CmnMgr.getFormattedDecimal(cdoCK.getColValue("monto_girado")),0,1);
				...
				pc.addCols(cdoCK.getColValue("num_cheque")+"        "+cdoCK.getColValue("ruc"),1,2);
				//pc.addCols(cdoCK.getColValue("ruc"),0,1);
				pc.addCols(" ",1,1);

				if(cdoCK.getColValue("observacion")!=null && !cdoCK.getColValue("observacion").equals("")){
					pc.setFont(8,1);
					pc.addCols(" ",1,1);
					pc.addCols("Nota: "+cdoCK.getColValue("observacion"),0,detFinal.size() - 2);
					pc.addCols(" ",1,1);
				}
