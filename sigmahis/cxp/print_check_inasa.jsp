...
for (int i=0; i<al.size(); i++) {
	...
	sbSql.append("select dck.num_renglon,dck.monto_renglon, dck.cuenta1 || dck.cuenta2 || dck.cuenta3 || dck.cuenta4 cuenta, dck.descripcion, nvl(dck.descripcion, ct.descripcion)||(select (select ' - '||nombre_paciente from vw_adm_paciente where pac_id = z.pac_id) from tbl_fac_factura z where codigo = dck.num_factura and compania = ck.cod_compania) descCuenta, dck.num_factura from ");
	sbSql.append(tableH);
	sbSql.append(" ck, ");
	sbSql.append(tableD);
	...
	sbSql.append("' and ck.tipo_pago = 1 order by ck.num_cheque, dck.num_renglon");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	float width = 72 * 7.375f;
	float height = 72 * 7.375f;
	...
	float ckHeight = 72 * 3.6875f;
	float ckdHeight = 72 * 3.6875f;

	Vector dHeader = new Vector();
		dHeader.addElement("36");
		dHeader.addElement("306");
	...
	for (int i=0; i<htCK.size(); i++) {
		...
		pc.createTable(true);
			...
			pc.addCols(cdoCK.getColValue("dia"),2,1);
			pc.addCols(cdoCK.getColValue("mes"),1,3);
			...
			pc.addCols(" ",0,1);
			pc.addCols(cdoCK.getColValue("monto_girado_formatted"),0,2);
			pc.addCols(" ",0,1);

			//-----> space between name/amount and amount in words
			pc.addCols(" ",0,dHeader.size(),1.0f);
			...
			pc.addCols(cdoCK.getColValue("beneficiario2"),0,4);
			pc.addCols(" ",0,2);

			//-----> bottom space
			pc.setVAlignment(2);
			pc.addCols("_",0,dHeader.size() -1,ckHeight - pc.getTableHeight() - 13.5f);//
			pc.addCols("_",2,1);
			pc.setVAlignment(0);
