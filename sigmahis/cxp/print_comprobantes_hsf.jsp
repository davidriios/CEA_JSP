...
if (!num_ck.equals("")) { sbFilter.append(" and ck.num_cheque= '"); sbFilter.append(num_ck);sbFilter.append("'"); }
...
if(!id_lote.equals("")){sbFilter.append(" and id_lote = "); sbFilter.append(id_lote);}
...
if(tipo.equals("5") && tipo_orden.equals("H") && agrupa_hon.equals("Y")){
	sbSql.append(" and c.tipo_orden in ('M', 'S')");
} else if(tipo.equals("5") && !tipo_orden.equals("")){
	sbSql.append(" and c.tipo_orden = '");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	for (int i=0; i<htCK.size(); i++) {
		...
		pc.createTable(true);
			...
			pc.addCols("COMPROBANTE DE PAGO [ " + tipo_pago_desc+" ]",1,dHeader.size(),cHeight);

			//-----> date
			pc.addCols(" "+" "+" No. Documento: "+cdoCK.getColValue("num_cheque"),0, 3);
			pc.addCols(cdoCK.getColValue("dia")+" "+cdoCK.getColValue("mes") + " " + cdoCK.getColValue("anio"),2,3);
			...
			//-----> amount in words
			pc.addCols(" "+" "+" "+cdoCK.getColValue("palabra"),0,dHeader.size());
	
			//-----> additional name
			//	pc.addCols(" "+" "+" "+cdoCK.getColValue("beneficiario2"),0,dHeader.size());
