...
if (comprobante == null) comprobante = "";
		...
		sql.append("select  r.cta1||'-'||r.cta2||'-'||r.cta3||'-'||r.cta4||'-'||r.cta5||'-'||r.cta6  cuenta, r.cta1,r.cta2,r.cta3,r.cta4,r.cta5,r.cta6,r.lado,sum(nvl(r.valor,0)) monto,c.descripcion from tbl_con_comprobante_cheque r,tbl_con_catalogo_gral c where r.compania = ");
		sql.append((String) session.getAttribute("_companyId"));
			
	if(!fechaIni.trim().equals(""))
	{
		sql.append(" and trunc(r.fecha_desde) >= to_date('");
		sql.append(fechaIni);
		sql.append("','dd/mm/yyyy')");
	}
	if(!fechaFin.trim().equals(""))
	{
		sql.append(" and trunc(r.fecha_hasta) <= to_date('");
		sql.append(fechaFin);
		sql.append("','dd/mm/yyyy')");
	}
	if(!comprobante.trim().equals(""))
	{
		sql.append(" and r.comprobante='");
		sql.append(comprobante);
		sql.append("'");
	}

		
	sql.append(" and c.compania = r.compania and c.cta1 = r.cta1 and c.cta2 = r.cta2 and c.cta3 = r.cta3 and c.cta4 = r.cta4 and c.cta5 = r.cta5 and c.cta6 = r.cta6 group by r.lado,r.cta1,r.cta2,r.cta3,r.cta4,r.cta5,r.cta6,c.descripcion order by r.cta1,r.cta2,r.cta3,r.cta4,r.cta5,r.cta6");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String subtitle = "LIBRO DE CHEQUE";
	if(comprobante.trim().equals("S")) subtitle +=" (COMPROBANTE GENERADO) ";
	else if(comprobante.trim().equals("N")) subtitle +=" (POR GENERAR COMPROBANTE)";
	else  subtitle +=" (TODAS LAS TRANSACCIONES)";
