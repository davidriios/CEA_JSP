...
if(!appendFilter2.trim().equals("")) sql =" select * from ("; 
 sql += "select a.anio, a.id, a.cod_tipo_ajuste, a.monto, to_char(a.fecha,'dd/mm/yyyy') fecha, a.observacion, a.ref_id, a.numero_factura, decode(a.estado,'P','PENDIENTE','R','APROBADO','A','ANULADO') estadoDes, a.estado,nvl(decode(a.destino_ajuste,'H',(select m.primer_nombre || decode(m.segundo_nombre, null, '', ' ' || m.segundo_nombre) || ' ' || m.primer_apellido || decode(m.segundo_apellido, null, '', ' ' || m.segundo_apellido) || decode(m.sexo, 'F', decode(m.apellido_de_casada, null, '', ' ' || m.apellido_de_casada)) from tbl_adm_medico m where m.codigo = to_char(a.ref_id)),'E',(select nombre from tbl_adm_empresa where codigo =a.ref_id),(select c.nombre_proveedor from tbl_com_proveedor c where c.compania=a.compania and c.cod_provedor=to_number(a.ref_id))),'S/NOMBRE') nombre, (select b.descripcion from tbl_cxp_tipo_ajuste b where a.cod_tipo_ajuste = b.cod_tipo_ajuste ) as descripcion, a.destino_ajuste from tbl_cxp_ajuste_saldo_enc a where compania = "+(String) session.getAttribute("_companyId")+appendFilter;
 if(!appendFilter2.trim().equals("")) sql += ") "+appendFilter2;
 
  sql += " order by fecha desc, id desc";


//sql = "select a.anio, a.secuencia, a.secuencia || ' - '||a.anio codigo,a.cod_tipo_ajuste, a.monto, to_char(a.fecha,'dd/mm/yyyy') fecha, a.observacion, a.cod_proveedor, a.numero_factura, decode(a.estado,'P','PENDIENTE','R','APROBADO','A','ANULADO') estadoDes, a.estado, '[ '||a.cod_proveedor||' ] '||c.nombre_proveedor proveedor, b.descripcion from tbl_cxp_ajuste_saldo a, tbl_cxp_tipo_ajuste b, tbl_com_proveedor c where a.cod_tipo_ajuste = b.cod_tipo_ajuste and a.cod_cia = c.compania and a.cod_proveedor = c.cod_provedor and cod_cia = "+(String) session.getAttribute("_companyId")+appendFilter+" order by a.cod_tipo_ajuste, a.anio desc, a.secuencia desc";
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.setFont(7, 0);
		pc.setVAlignment(0);
			pc.addCols(" ["+cdo.getColValue("id")+"] "+cdo.getColValue("anio"),0,1);
