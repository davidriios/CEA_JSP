...
if(unidad_ejec == null){
	cdo = SQLMgr.getData("select codigo from tbl_sec_unidad_ejec where compania = " +  compania);
	unidad_ejec = cdo.getColValue("codigo");
}
if(fg==null) fg = "";
if(fp==null) fp = "";
...
function reloadPage(compania){
	window.location = '../cxp/usuario_x_unid_adm_op.jsp?compania='+compania;
}

function cargar(){
	var compania = document.form1.compania.value;
	var unidad_ejec = document.form1.unidad_ejec.value;
	window.frames['itemFrame'].location = '../cxp/usuario_x_unid_adm_op_det.jsp?compania='+compania+'&unidad_ejec='+unidad_ejec;
}
        ...
        <tr>
          <td class="TableBorder"><table align="center" width="100%" cellpadding="5" cellspacing="0">
              <tr>
                <td><table align="center" width="100%" cellpadding="0" cellspacing="1">
                    ...
                    <tr>
                      <td><table width="100%" cellpadding="1" cellspacing="0">
                          ...
                          <tr class="TextPanel">
                            <td>&nbsp;<cellbytelabel>Usuarios por Unidad Administrativa (Ordenes de Pago)</cellbytelabel></td>
                          </tr>
                          <tr class="TextPanel">
                ...
                <%=fb.select(ConMgr.getConnection(),"select codigo, lpad(codigo, 4, '0')||' - '||descripcion from tbl_sec_unidad_ejec where compania = "+compania+" order by descripcion","unidad_ejec",unidad_ejec,false,false,0, "text10", "", "onChange=\"javascript:cargar();\"")%>
                <%//=fb.button("ir","Ir",false,false,"","","onClick=\"javascript: doSubmit(this.value);\"")%> 
                            </td>
                          </tr>
                        </table></td>
                    </tr>
                    <tr>
                      <td><iframe name="itemFrame" id="itemFrame" frameborder="0" align="center" width="100%" height="300" scrolling="yes" src="../cxp/usuario_x_unid_adm_op_det.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>&compania=<%=compania%>&unidad_ejec=<%=unidad_ejec%>"></iframe></td>
                    </tr>
                    <tr class="TextRow02">
                      <td align="right"> 
...
%>
	alert('<%=errMsg%>');
	window.location = '<%=request.getContextPath()+request.getServletPath()%>?unidad_ejec=<%=request.getParameter("unidad_ejec")%>';
<%
