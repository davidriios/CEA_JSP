...
String beneficiarioView = request.getParameter("beneficiarioView");

if(fg==null) fg = "";
if(fp==null) fp = "";
if(beneficiarioView==null) beneficiarioView = "";
if(beneficiarioView.trim().equals("")) beneficiarioView = beneficiario;
...
function setValues(){
	...
	window.frames['itemFrame'].location = '../cxp/ver_mov_hon_det.jsp?beneficiario=<%=beneficiario%>&tipo=<%=tipo%>&fechaini='+fechaini+'&fechafin='+fechafin+'&format=<%=format%>'+'&acumHonNoFacturaro='+acumHonNoFacturaro;
}
...
{	
	var fechaini = document.form1.fechaini.value;
	var fechafin = document.form1.fechafin.value;
	abrir_ventana2('../cellbyteWV/report_container.jsp?reportName=cxp/rpt_mov_hon.rptdesign&desdeParam='+fechaini+'&hastaParam='+fechafin+'&medicoParam=<%=beneficiario%>&tipoParam=<%=tipo%>&benefNameParam=<%=cdo.getColValue("nombre_beneficiario")%>&format=<%=format%>&ahnfParam=S');
}
function doAction(){xHeight=objHeight('_tblMain');resizeFrame();}
function resizeFrame(){resetFrameHeight(document.getElementById('itemFrame'),xHeight,450);}
                    ...
                    <tr>
                      <td><table width="100%" cellpadding="1" cellspacing="0">
                          ...
                          <tr class="TextHeader02" height="21">
                            <td><cellbytelabel>C&oacute;digo</cellbytelabel>:&nbsp;&nbsp;<%=beneficiarioView%></td>
                            <td><cellbytelabel>Nombre</cellbytelabel>:&nbsp;&nbsp;<%=cdo.getColValue("nombre_beneficiario")%></td>
                          </tr>
                        </table></td>
                    </tr>
                    <tr>
                      <td><iframe name="itemFrame" id="itemFrame" frameborder="0" align="center" width="100%" height="300" scrolling="yes" src="../cxp/ver_mov_hon_det.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>&beneficiario=<%=beneficiario%>&tipo=<%=tipo%>&fechaini=<%=fechaini%>&fechafin=<%=fechafin%>&format=<%=format%>&acumHonNoFacturaro=<%=acumHonNoFacturaro%>"></iframe></td>
                    </tr>
                    <%=fb.formEnd(true)%>
