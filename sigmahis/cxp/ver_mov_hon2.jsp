...
function setValues(){
	...
	window.frames['itemFrame'].location = '../cxp/ver_mov_hon_det2.jsp?beneficiario=<%=beneficiario%>&tipo=<%=tipo%>&fechaini='+fechaini+'&fechafin='+fechafin+'&format=<%=format%>';
}
...
{	
	var fechaini = document.form1.fechaini.value;
	var fechafin = document.form1.fechafin.value;
	abrir_ventana2('../cellbyteWV/report_container.jsp?reportName=cxp/rpt_mov_hon2.rptdesign&desdeParam='+fechaini+'&hastaParam='+fechafin+'&medicoParam=<%=beneficiario%>&tipoParam=<%=tipo%>&benefNameParam=<%=cdo.getColValue("nombre_beneficiario")%>&format=<%=format%>');
}
        ...
        <tr>
          <td class="TableBorder"><table align="center" width="100%" cellpadding="5" cellspacing="0">
                ...
                <td><table align="center" width="100%" cellpadding="0" cellspacing="1">
                    ...
                    <tr>
                      <td><iframe name="itemFrame" id="itemFrame" frameborder="0" align="center" width="100%" height="300" scrolling="yes" src="../cxp/ver_mov_hon_det2.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>&beneficiario=<%=beneficiario%>&tipo=<%=tipo%>&fechaini=<%=fechaini%>&fechafin=<%=fechafin%>&format=<%=format%>&acumHonNoFacturaro=<%=acumHonNoFacturaro%>"></iframe></td>
                    </tr>
                    <%=fb.formEnd(true)%>
