...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	sbFilter.append(" and medico = '");
	sbFilter.append(beneficiario);
	sbFilter.append("' and tipo not in ('PAGOC','AUX')");
	if (acumHonNoFacturaro.equals("N")) sbFilter.append(" and tipo != 'CARGOS'");
	sbFilter.append(" and trunc(fecha) between to_date('");
	sbFilter.append(fechaini);
	sbFilter.append("','dd/mm/yyyy') and to_date('");
	sbFilter.append(fechafin);
	sbFilter.append("','dd/mm/yyyy')");

	if (!trx.equalsIgnoreCase("T")) {
		sbTable.append(", (select nombre_paciente, sum(debito) - sum(credito) as saldo from vw_cxp_mov_hon_hnf where ");
		sbTable.append(sbFilter.substring(4));
		sbTable.append(" group by nombre_paciente) y");
		sbFilter.append(" and z.nombre_paciente = y.nombre_paciente and z.tipo <> 'DIST'");
		if (trx.equalsIgnoreCase("P")) sbFilter.append(" and y.saldo <> 0");
		else if (trx.equalsIgnoreCase("C")) sbFilter.append(" and y.saldo = 0");
	}

	sbSql.append("select tipo, decode(tipo, 'DIST', 'COBRADO', 'AJU', 'AJUSTE', 'AJUA', 'AJUSTE FACTURACION', 'PAGOH', 'PAGOS HONORARIO', 'PAGOC', 'PAGOS CONTABILIDAD', 'CARGOS F', 'FACTURADO', 'AUX', 'AUXILIAR', 'AJUCXP', 'AJUSTE CXP', 'CARGOS NF', 'HONORARIO NO FACT.') || ' ( ' ||z.nombre_paciente || ' )' as tipo_desc, compania, factura, documento, to_char(fecha, 'dd/mm/yyyy') as fecha, debito, credito, distribuido, ref_type, nombre, anio, id_trx, sub_id_trx, z.fecha as fe, z.nombre_paciente from vw_cxp_mov_hon_hnf z");
	...
	if(acumHonNoFacturaro.equals("N")) {
		...
		sbSqlSI.append("select getsaldoinicialhon (");
		sbSqlSI.append((String) session.getAttribute("_companyId"));
		sbSqlSI.append(", '");
		sbSqlSI.append(fechaini);
		sbSqlSI.append("', '");
