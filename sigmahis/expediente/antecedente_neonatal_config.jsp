...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	{
		if (id == null) throw new Exception("El antecedente Neonatal no es válida. Por favor intente nuevamente!");

sql = "select codigo, descripcion, valor, orden, es_default from tbl_sal_factor_neonatal where codigo="+id;
...
<%if (mode.equalsIgnoreCase("add")){%>
document.title="Antecedente Neonatal - Agregar - "+document.title;
<%}else if (mode.equalsIgnoreCase("edit")){%>
document.title="Antecedente Neonatal - Edición - "+document.title;
<%}%>
</script>
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<tr class="TextRow01" >
				...
				<tr class="TextRow01"> 
				<td><cellbytelabel id="3">Valor</cellbytelabel></td>
				<td><%=fb.select("valor","A=ALFANUMERICO,N=NUMERICO",cdo.getColValue("valor"))%></td>							
...
else
{
  ...
  cdo.setTableName("tbl_sal_factor_neonatal");
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/antecedente_neonatal_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/antecedente_neonatal_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/antecedente_neonatal_list.jsp';
<%
	}
