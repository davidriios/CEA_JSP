...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
    ...
    searchType = "1";
    searchDisp = "codigo";
		codigo     = request.getParameter("codigo");       // utilizada para mantener el Código de la Categoría
	}
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");  // utilizada para mantener la Descripción de la Categoría
	}
...
sql = "select codigo, descripcion from tbl_sal_categoria_nota"+appendFilter+" order by codigo";
al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
rowCount = CmnMgr.getCount("select count(*) count from tbl_sal_categoria_nota "+appendFilter);
...
function add()
{
	abrir_ventana('../expediente/cat_nota_enf_config.jsp');
}

function edit(id)
{
	abrir_ventana('../expediente/cat_nota_enf_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../expediente/print_list_cat_nota_enf.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="CATEGORIA DE LAS NOTAS DE ENFERMERIA"></jsp:param>
</jsp:include>
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">
			<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nueva categoria de nota</cellbytelabel> ]</a></authtype>
		</td>
  </tr>
	<tr>
		<td>
