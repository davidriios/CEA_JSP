...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	{
		if (id == null) throw new Exception("El Factor familiar no es válido. Por favor intente nuevamente!");

sql = "select codigo, descripcion, orden, es_default, estado from tbl_sal_ant_med_importantes where codigo="+id;
...
<%if (mode.equalsIgnoreCase("add")){%>
document.title="Factor Famliar- Agregar- "+document.title;
<%}else if (mode.equalsIgnoreCase("edit")){%>
document.title="Factor Famliar - Edición - "+document.title;
<%}%>
</script>
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="EXPEDIENTE - MANTENIMIENTO -FACTOR FAMILIAR"></jsp:param>
</jsp:include>
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<tr class="TextRow01" >
                ...
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    ...
                    Estado:
                    <%=fb.select("estado","A=Activo,I=Inactio",cdo.getColValue("estado"),"")%>
                </td>
			</tr>
...
else
{
    ...
    cdo.setTableName("tbl_sal_ant_med_importantes");
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/exp_ant_medicos_importancia_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/exp_ant_medicos_importancia_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/exp_ant_medicos_importancia_list.jsp';
<%
	}
