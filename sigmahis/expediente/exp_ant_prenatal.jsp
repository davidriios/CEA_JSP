...
function isChecked(k){eval('document.form0.observacion'+k).disabled = !eval('document.form0.valor'+k).checked;if (eval('document.form0.valor'+k).checked)eval('document.form0.observacion'+k).className = 'FormDataObjectEnabled';else eval('document.form0.observacion'+k).className = 'FormDataObjectDisabled';}
function imprimir(){abrir_ventana('../expediente/print_exp_seccion_8.jsp?pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&seccion=<%=seccion%>&desc=<%=desc%>');}
</script>
</head>
...
<%=fb.hidden("fecha_creacion"+i,cdo.getColValue("fecha_creacion"))%>
		<tr class="<%=color%>">
			...
			<td align="center"><%=fb.checkbox("valor"+i,"S",(cdo.getColValue("valor").equalsIgnoreCase("S")),viewMode,null,null,"onClick=\"javascript:isChecked("+i+")\"")%></td>
			<td><%=fb.textarea("observacion"+i,cdo.getColValue("observacion"),false,(!cdo.getColValue("valor").equalsIgnoreCase("S")),viewMode,50,2,2000,null,null,null)%></td>
		</tr>
<%
}
%>
