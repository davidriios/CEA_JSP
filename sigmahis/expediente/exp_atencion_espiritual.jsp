...
function setAtencion(a){var id = eval('document.lista.id'+a).value;	window.location = '../expediente/exp_atencion_espiritual.jsp?modeSec=view&mode=<%=mode%>&seccion=<%=seccion%>&pacId=<%=pacId%>&desc=<%=desc%>&noAdmision=<%=noAdmision%>&id='+id;}
function add(){window.location = '../expediente/exp_atencion_espiritual.jsp?modeSec=add&mode=<%=mode%>&seccion=<%=seccion%>&pacId=<%=pacId%>&desc=<%=desc%>&noAdmision=<%=noAdmision%>&id=0';}
</script>
</head>
