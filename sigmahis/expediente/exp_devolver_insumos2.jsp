...
{
	if(desc == null && iExpSecciones == null){cdo = (CommonDataObject) iExpSecciones.get(seccion);desc = cdo.getColValue("descripcion");}
...
function doAction(){<%if(cda == null || cda.trim().equals("")){%>alert('No Existe Almacen Seleccionado');<%}%>newHeight();}
...
function editMode()
{
	window.location = '<%=request.getContextPath()+request.getServletPath()%>?seccion=<%=seccion%>&modeSec=edit&mode=<%=mode%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&cds=<%=cds%>&cda=<%=cda%>';
}
</script>
</head>
