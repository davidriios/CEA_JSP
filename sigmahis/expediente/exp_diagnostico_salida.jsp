...
function showDiagnosticoList(){ abrir_ventana1('../common/check_diagnostico.jsp?seccion=<%=seccion%>&fp=pSalida&mode=<%=mode%>&modeSec=<%=modeSec%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&desc=<%=desc%>');}
function printExp(){abrir_ventana("../expediente/print_exp_seccion_88.jsp?seccion=<%=seccion%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&desc=<%=desc%>");}
function checkOrden(){var size = document.form0.diagSize.value ;var total =0;var total2 =0;for ( i = 0; i<size; i++ ){if(eval('document.form0.ordenDiag'+i).value != '' && eval('document.form0.ordenDiag'+i).value=='1' && total==0)total++;else if(eval('document.form0.ordenDiag'+i).value != '' && eval('document.form0.ordenDiag'+i).value=='1' && total!=0) total2++;}if ( total==1 ){return true;}else if(total==0){ alert('Debe agregar un diagnostico con prioridad (1)');return false;}}
</script>
</head>
...
<tr><td align="right" colspan="4"><a href="javascript:printExp()" class="Link00">[<cellbytelabel id="1">Imprimir</cellbytelabel>]</a></td></tr>
		  <tr class="TextHeader">
          	<td><cellbytelabel id="2">Diagn&oacute;sticos de Salida</cellbytelabel></td>
          </tr>
		  <tr>
          <td>
            <table width="100%" cellpadding="1" cellspacing="1">
...
%>
				...
				<%
				fb.appendJsValidation("if(!checkOrden()){ error++;}");
