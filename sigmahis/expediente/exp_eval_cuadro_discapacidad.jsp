...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  sql = "SELECT codigo, descripcion FROM tbl_sal_eval_discapacidades WHERE codigo<>0"+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_sal_eval_discapacidades WHERE codigo<>0"+appendFilter);
...
document.title = 'Características de Úlceras x Presión - '+document.title;

function add()
{
	abrir_ventana('eval_cuadro_discapacidad_config.jsp');
}

function edit(code)
{
	abrir_ventana('eval_cuadro_discapacidad_config.jsp?mode=edit&code='+code);
}

function printList()
{
	abrir_ventana('print_list_cuadro_discapacidad.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
        <td align="right">
        	<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nuevo</cellbytelabel> ]</a></authtype>
   	    </td>
    </tr>
	<tr>
		<td>
