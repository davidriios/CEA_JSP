...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  sql = "SELECT codigo, descripcion FROM tbl_sal_eval_geronte WHERE codigo<>0"+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_sal_eval_geronte WHERE codigo<>0"+appendFilter);
...
function add()
{
	abrir_ventana('eval_del_geronte_config.jsp');
}

function edit(code)
{
	abrir_ventana('eval_del_geronte_config.jsp?mode=edit&code='+code);
}

function printList()
{
	abrir_ventana('print_list_eval_del_geronte.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
