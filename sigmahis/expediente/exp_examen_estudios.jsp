...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	if (fp.trim().equalsIgnoreCase("laboratorio"))
	{
		...
		sbSql.append(",'INT_LIS_HOST_RESULTS'),'-') as host_results from dual");//http://144.1.0.3:9080/cellbyte/
		p = SQLMgr.getData(sbSql);
		if (p == null) {
			p = new CommonDataObject();
			p.addColValue("host_results","-");
		}
	...
	/*	if (type.trim().equalsIgnoreCase("previous")) colLabel1 = "Fecha Estudio";
		...
		sql = "select nvl((select obx_segment from tbl_int_result_det where order_no = x.seq_trx and obx_file_type is not null and rownum = 1),z.pac_id||'_'||z.csxp_admi_secuencia||'_'||lpad(x.seq_trx,10,'0')||'.pdf') as result, (select count(*) from tbl_int_result_det where order_no = x.seq_trx and obx_file_type is null) as nresults, x.seq_trx as order_no, z.num_orden, z.codigo, z.cod_solicitud, z.csxp_admi_secuencia, z.csxp_admi_pac_codigo, z.csxp_admi_pac_fec_nac, z.cod_centro_servicio, z.pac_id, nvl(to_char(z.fecha_realizo,'dd/mm/yyyy hh12:mi:ss am'),' ') as fecha_realizo, nvl(to_char(nvl(z.fecha_solicitud,z.fecha_creac),'dd/mm/yyyy'),' ') as fecha_solicitud, nvl(z.codigo_muestra,' ') as codigo_muestra, to_char(z.fecha_creac,'dd/mm/yyyy hh12:mi:ss am') as fecha_creac, nvl(z.cod_procedimiento,' ') as cpt, nvl(y.observacion,y.descripcion) as cpt_desc, nvl(z.comentario_pre,' ') as comentario_pre, z.estado,to_char(z.csxp_admi_pac_fec_nac,'ddmmyyyy') fecha_nac,z.csxp_admi_secuencia admision, z.csxp_admi_pac_codigo pac_codigo from tbl_cds_detalle_solicitud z, tbl_cds_procedimiento y,tbl_adm_admision a,tbl_cds_solicitud x where z.cod_procedimiento=y.codigo(+) and z.pac_id="+pacId+" and a.adm_root="+noAdmision+appendFilter+" and z.pac_id=x.pac_id and z.csxp_admi_secuencia=x.admi_secuencia and Z.COD_SOLICITUD=x.codigo and z.estado <> 'A' and a.pac_id=z.pac_id and z.csxp_admi_secuencia=a.secuencia order by nvl(z.fecha_solicitud,z.fecha_creac) desc, x.seq_trx, z.num_orden";
	}
	...
	/*	colLabel1 = "Fecha Solicitud";
		...
		sql = "select z.codigo, z.cod_solicitud, z.csxp_admi_secuencia, z.csxp_admi_pac_codigo, z.csxp_admi_pac_fec_nac, z.cod_centro_servicio,   z.pac_id, nvl(to_char(z.fecha_realizo,'dd/mm/yyyy hh12:mi:ss am'),' ') as fecha_realizo, nvl(to_char(z.fecha_solicitud,'dd/mm/yyyy'),' ') as fecha_solicitud, nvl(z.codigo_muestra,' ') as codigo_muestra, to_char(z.fecha_creac,'dd/mm/yyyy hh12:mi:ss am') as fecha_creac, nvl(z.cod_procedimiento,' ') as cpt, nvl(y.observacion,y.descripcion) as cpt_desc, x.codigo as cds, x.descripcion as cds_desc, nvl(z.comentario_pre,' ') as comentario_pre, z.estado ,to_char(z.csxp_admi_pac_fec_nac,'ddmmyyy') fecha_nac,z.csxp_admi_secuencia admision, z.csxp_admi_pac_codigo pac_codigo, nvl(z.responseurl,' ') as responseurl from tbl_cds_detalle_solicitud z, tbl_cds_procedimiento y, tbl_cds_centro_servicio x ,tbl_adm_admision a where z.cod_procedimiento=y.codigo(+) and z.cod_centro_servicio=x.codigo and z.pac_id="+pacId+" and a.adm_root="+noAdmision+appendFilter+" and z.estado <> 'A' and  a.pac_id=z.pac_id and z.csxp_admi_secuencia=a.secuencia order by z.fecha_solicitud desc";
	}
...
function imaginalogia(k){abrir_ventana1(eval('document.form0.responseurl'+k).value);}
function showResult(result){<% if (p.getColValue("host_results") != null && !p.getColValue("host_results").equals("-")) { %>abrir_ventana('<%=p.getColValue("host_results")%>'+result);<% } %>}
</script>
</head>
...
<% if (fp.trim().equalsIgnoreCase("laboratorio")) { %>
					<td width="10%"><cellbytelabel>No. Orden</cellbytelabel></td>
<% } %>
					<td width="10%"><cellbytelabel>C&oacute;digo</cellbytelabel></td>
					<td><cellbytelabel>Descripci&oacute;n</cellbytelabel></td>
					<td width="8%"><cellbytelabel>Fecha a realizar</cellbytelabel></td>
					<td width="8%"><cellbytelabel>Fecha Registro</cellbytelabel></td>
					<td width="2%">&nbsp;</td>
				</tr>
<%
String colValue1 = "";
...
%>
				...
				<%=fb.hidden("responseurl"+i,cdo.getColValue("responseurl"))%>

				<tr id="rs<%=i%>" class="<%=color%>" align="center" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
<% if (fp.trim().equalsIgnoreCase("laboratorio")) { %>
					<td><% if (p.getColValue("host_results") != null && !p.getColValue("host_results").equals("-") && !cdo.getColValue("result").trim().equals("")) { %><a href="javascript:showResult('<%=cdo.getColValue("result")%>');" class="Link05"><%=cdo.getColValue("order_no")%>-<%=cdo.getColValue("num_orden")%></a><% } else { %><%=cdo.getColValue("order_no")%>-<%=cdo.getColValue("num_orden")%><% } %></td>
<% } %>
...
{
%>
				<td><% if (!cdo.getColValue("nresults").equals("0")) { %>
				<img src="../images/dwn.gif" onClick="javascript:diFrame('list','9','rs<%=i%>','800','400','0','0','1','DIVExpandRowsScroll',true,'0','<%=request.getContextPath()%>/expediente/results/show_lis_result.jsp?orderNo=<%=cdo.getColValue("order_no")%>&codProcedimiento=<%=cdo.getColValue("cpt")%>&pacId=<%=cdo.getColValue("pac_id")%>&noAdmision=<%=cdo.getColValue("csxp_admi_secuencia")%>',false)" style="cursor:pointer">
				<% } %></td>
<%}else if (fp.trim().equalsIgnoreCase("imagenologia"))
{
%>
<td<% if (!cdo.getColValue("responseurl").trim().equals("")) { %> onClick="javascript:imaginalogia(<%=i%>)" style="cursor:pointer"<% } %>><% if (!cdo.getColValue("responseurl").trim().equals("")) { %><img src="../images/dwn.gif"><% } %></td>
							...
							<div id="ordenMain" width="100%" style="overflow:scroll;position:static;height:200">
								...
								<table width="100%" cellpadding="1" cellspacing="1">
									<tr class="TextRow01" align="left">
										<td>
					<iframe name="iDetalle" id="iDetalle" frameborder="0" align="center" width="100%" height="200" scrolling="no" src=""></iframe>
										</td>
									</tr>
								</table>
