...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
	...
	{
		appendFilter += " where upper(pregunta) like '%"+request.getParameter("descripcion").toUpperCase()+"%'";
...
sql = "select codigo, pregunta, estado, orden, decode(estado,'I','INACTIVA','ACTIVA') estado_desc from tbl_sal_preguntas_cribado "+appendFilter+" order by orden";
al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
rowCount = CmnMgr.getCount("select count(*) count from tbl_sal_preguntas_cribado "+appendFilter);
...
document.title = 'Preguntas Cribado - '+document.title;

function add()
{
	abrir_ventana('../expediente/exp_preguntas_cribado_config.jsp');
}

function edit(id)
{
	abrir_ventana('../expediente/exp_preguntas_cribado_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../expediente/print_preguntas_cribado.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="EXPEDIENTE - MANTENIMIENTO - PREGUNTAS CRIBADO"></jsp:param>
</jsp:include>
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">
			<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nueva pregunta cribado</cellbytelabel> ]</a></authtype>
		</td>
  </tr>
	<tr>
		<td>
...
%>
				...
				<%=fb.hidden("toPage",request.getContextPath()+request.getServletPath())%>
				<td width="50%">
					<cellbytelabel id="3">Pregunta</cellbytelabel>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
		...
		<tr class="TextHeader" align="center">
			<td width="7%"><cellbytelabel id="2">C&oacute;digo</cellbytelabel></td>
			<td width="61%"><cellbytelabel id="3">Pregunta</cellbytelabel></td>
			<td width="7%"><cellbytelabel id="3">Estado</cellbytelabel></td>
			<td width="7%"><cellbytelabel id="3">Orden</cellbytelabel></td>
			<td width="8%">&nbsp;</td>
		</tr>
<%
