...
sql = "select codigo, descripcion, es_default, orden from tbl_sal_factor_prenatal "+appendFilter+" order by codigo";
al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
rowCount = CmnMgr.getCount("select count(*) count from tbl_sal_factor_prenatal "+appendFilter);
...
document.title = 'Factor Prenatal - '+document.title;

function add()
{
	abrir_ventana('../expediente/factor_prenatal_config.jsp');
}

function edit(id)
{
	abrir_ventana('../expediente/factor_prenatal_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../expediente/print_list_factor_prenatal.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">
			<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nuevo Factor Prenatal</cellbytelabel> ]</a></authtype>
		</td>
  </tr>
	<tr>
		<td>
