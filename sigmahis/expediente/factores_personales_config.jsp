...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	{
		if (id == null) throw new Exception("El Tipo factor personal no es válido. Por favor intente nuevamente!");

    sql = "select codigo, descripcion, activar_obs, orden, es_default, grupo, status from tbl_sal_diagnostico_personal where codigo = "+id;
...
<%if (mode.equalsIgnoreCase("add")){%>
document.title="Factores Personales - Agregar - "+document.title;
<%}else if (mode.equalsIgnoreCase("edit")){%>
document.title="Factores Personales - Edición - "+document.title;
<%}%>
</script>
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<tr class="TextRow01" >
				...
				<td><%=fb.textBox("descripcion",cdo.getColValue("descripcion"),true,false,false,50,100)%>
            ...
            <tr class="TextRow01"> 
                <td>&nbsp;<cellbytelabel id="3">Grupo</cellbytelabel></td>
                <td>
                    <%=fb.select("grupo","2=NO PATOLOGICO,1=PATOLOGICO",cdo.getColValue("grupo"),"")%>
...
String saveOption = request.getParameter("saveOption"); //N=Create New,O=Keep Open,C=Close
  ...
  cdo.setTableName("tbl_sal_diagnostico_personal");
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/factores_personales_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/factores_personales_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/factores_personales_list.jsp';
<%
	}
