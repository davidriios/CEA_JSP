...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else
  {
	...
	sql = "select codigo, descripcion, decode(activar_obs,'N','NO','S','SI') as activado, decode(grupo,1,'PATOLOGICO','NO PATOLOGICO') as grupo_desc, grupo, orden, es_default, decode(status,'A','ACTIVO','INACTIVO') as status_desc from tbl_sal_diagnostico_personal where 1 = 1 "+appendFilter+" order by grupo, orden";
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) count from tbl_sal_diagnostico_personal where 1 = 1 "+appendFilter);
...
document.title = 'Expediente - Mantenimiento - Factores Personales'+document.title;

function add()
{
	abrir_ventana('../expediente/factores_personales_config.jsp');
}

function edit(id)
{
	abrir_ventana('../expediente/factores_personales_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../expediente/print_list_factores_personales.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="EXPEDIENTE- MANTENIMIENTO - FACTORES PERSONALES"></jsp:param>
</jsp:include>
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">
			<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nuevos factores personales</cellbytelabel> ]</a></authtype>
		</td>
  </tr>
	<tr>
		<td>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
		...
		<tr class="TextHeader" align="center">
			...
			<td width="5%"><cellbytelabel id="8">Orden</cellbytelabel></td>
			<td width="10%"><cellbytelabel id="8">Por defecto</cellbytelabel></td>
...
for (int i=0; i<al.size(); i++)
{
    ...
    if (!grupo.equals(cdo.getColValue("grupo"))){
%>
    <tr class="TextHeader02">
        <td colspan="6">[<%=cdo.getColValue("grupo")%>]&nbsp;<%=cdo.getColValue("grupo_desc")%></td>
    </tr>
<%
    }
%>
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="center"><%=cdo.getColValue("activado")%></td>
			<td align="center"><%=cdo.getColValue("orden")%></td>
			<td align="center"><%=cdo.getColValue("es_default")%></td>
