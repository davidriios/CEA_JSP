...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
    ...
    searchType = "1";
    searchDisp = "Código";
		codigo     = request.getParameter("codigo");       // utilizada para mantener el Código de la Forma de Transporte
	}
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");  // utilizada para mantener la Descripción de la Forma de Transporte
	}
  ...
  else
  {
	...
	sql = "select codigo, descripcion, decode(activar_obs,'N','No','S','Si') as activado from tbl_sal_forma_transporte"+appendFilter;
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) count from tbl_sal_forma_transporte "+appendFilter);
...
document.title = 'Expediente - Informe de casos polisivo -Forma de Transporte - '+document.title;

function add()
{
	abrir_ventana('../expediente/forma_transp_config.jsp');
}

function edit(id)
{
	abrir_ventana('../expediente/forma_transp_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../expediente/print_list_forma_transp.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="EXPEDIENTE- INFORME DE CASO POLICIVO- FORMA TRANSPORTE"></jsp:param>
</jsp:include>
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">
			<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nueva forma de transporte</cellbytelabel> ]</a></authtype>
		</td>
  </tr>
	<tr>
		<td>
