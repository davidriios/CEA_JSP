...
if (request.getParameter("comp")!=null){ comp=request.getParameter("comp"); }
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  {
    appendFilter += " and upper(a.COD_MEDIDA) like '%"+request.getParameter("codigo").toUpperCase()+"%'";
    searchOn = "a.COD_MEDIDA";
  ...
  sql = "SELECT a.COD_MEDIDA as codigo, b.DESCRIPCION FROM TBL_SAL_UMED_X_FACTOR_NEO a, TBL_INV_UNIDAD_MEDIDA b where a.COD_MEDIDA=b.COD_MEDIDA and a.COD_FACTOR="+comp+" "+appendFilter+" ORDER BY a.COD_MEDIDA";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM TBL_SAL_UMED_X_FACTOR_NEO a, TBL_INV_UNIDAD_MEDIDA b where a.COD_MEDIDA=b.COD_MEDIDA and a.COD_FACTOR="+comp+" "+appendFilter+" order by a.COD_MEDIDA");
...
document.title = 'Listado de Anestesia - '+document.title;

function returnValue(i,op)
{ 
    var code = eval('document.listAnestesia.codigo'+i).value;
	var name = eval('document.listAnestesia.descripcion'+i).value; 
	
    switch(op)
	{ 
	   case 1:
	       eval('window.opener.document.form0.medida<%=index%>').value = code;
           eval('window.opener.document.form0.nombre<%=index%>').value = name;
		   window.close();
	   break;
	}  
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="LISTADO DE ANESTESIA"></jsp:param>
</jsp:include>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<%fb = new FormBean("listAnestesia",request.getContextPath()+request.getServletPath(),FormBean.POST);%>
            <%=fb.formStart(true)%>
