...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	else
	{
		...
		sql = "select id, descripcion, tipo, orden, status, evaluable, comentable, eval_values from tbl_sal_parametro where id="+id;
...
<%=fb.hidden("id",id)%>
<%=fb.hidden("id",id)%>
<%fb.appendJsValidation("if(document.form1.tipo.value==''){alert('Por favor seleccione el Tipo de Parámetro!!');document.form1.tipo.focus();error++;}");%>
<%fb.appendJsValidation("if(!checkEvalValues()){alert('Por favor Introduzca los valores de la evaluaciones!!');document.form1.eval_values.focus();error++;}");%>
		...
		<tr class="TextRow01">
			...
			<td><%=fb.select(ConMgr.getConnection(),"select code, code||' - '||description, code from tbl_sal_tipo_parametro where status='A' order by description","tipo",cdo.getColValue("tipo"),false,false,0,"Text10",null,null,null,"S")%></td>
		</tr>
		<tr class="TextRow01">
			<td><cellbytelabel id="5">Orden</cellbytelabel></td>
			<td><%=fb.intBox("orden",cdo.getColValue("orden"),false,false,false,10,2)%></td>
		</tr>
		<tr class="TextRow01">
			<td><cellbytelabel id="6">Eval&uacute;a el Par&aacute;metro</cellbytelabel>?</td>
			<td><%=fb.checkbox("evaluable","S",(cdo.getColValue("evaluable") != null && cdo.getColValue("evaluable").equalsIgnoreCase("S")),viewMode)%></td>
		</tr>
		<tr class="TextRow01">
			<td><cellbytelabel id="7">Detalla el Par&aacute;metro</cellbytelabel>?</td>
        ...
        <tr class="TextRow02">
			<td colspan="2"><cellbytelabel id="8">Por ejemplo si los valores van a ser SI o NO: Introduzca: S=SI,N=NO o un rango de datos: 1,2,3,...</cellbytelabel></td>
		</tr>
        <tr class="TextRow01">
        <td><cellbytelabel id="9">Valores de las evaluaciones</cellbytelabel></td>
