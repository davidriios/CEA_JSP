...
sql = "select codigo, descripcion, estado, orden, grupo, bloquear from tbl_sal_pausa_seguridad_params where codigo="+id+" and tipo = '"+tipo+"' and grupo = '"+grupo+"'";
...
<%if (mode.equalsIgnoreCase("add")){%>
document.title="Preguntas Pre - Operatoria- Agregar- "+document.title;
<%}else if (mode.equalsIgnoreCase("edit")){%>
document.title="Preguntas Pre - Operatoria - Edición - "+document.title;
<%}%>
</script>
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="EXPEDIENTE - MANTENIMIENTO - PREGUNTAS PRE - OPERATORIA"></jsp:param>
</jsp:include>
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
            ...
            <tr class="TextRow01"> 
			   <td>&nbsp;<cellbytelabel id="4">Grupo</cellbytelabel></td>
			   <td colspan="2"><%=fb.select("grupo","A=ADULTO,N=NEONATO",cdo.getColValue("grupo",grupo))%></td>
			</tr>
			<tr class="TextRow01"> 
			   <td>&nbsp;<cellbytelabel id="4">Tipo</cellbytelabel></td>
			   <td colspan="2"><%=fb.select("tipo","VP=VERIFICACIÓN PRE-OPERATORIA,TO=PAUSA DE SEGURIDAD (TIME OUT),SO=VERIFICACION POST OPERATORIA (Sign - Out)",cdo.getColValue("tipo", tipo))%></td>
			</tr>
			<tr class="TextRow01"> 
			...
			<tr class="TextRow01"> 
			   <td>&nbsp;<cellbytelabel id="4">Bloquear</cellbytelabel></td>
			   <td colspan="2"><%=fb.select("bloquear","N=NO,S=SI",cdo.getColValue("bloquear"))%>&nbsp;&nbsp;&nbsp;<b>**(SI = No permite seleccionar en secci&oacute;n)</b></td>
			</tr>
...
else
{
  ...
  cdo.setTableName("tbl_sal_pausa_seguridad_params");
  ...
  cdo.addColValue("bloquear",request.getParameter("bloquear")); 
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/pausa_seguridad_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/pausa_seguridad_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/pausa_seguridad_list.jsp';
<%
	}
