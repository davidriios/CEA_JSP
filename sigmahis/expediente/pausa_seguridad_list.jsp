...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
    ...
    if (request.getParameter("tipo") != null && !request.getParameter("tipo").trim().equals("")){
		appendFilter += " and tipo = '"+request.getParameter("tipo")+"'";    
	    tipo  = request.getParameter("tipo");
	}
    if (request.getParameter("grupo") != null && !request.getParameter("grupo").trim().equals("")){
		appendFilter += " and grupo = '"+request.getParameter("grupo")+"'";    
	    grupo  = request.getParameter("grupo");
	}

    sql = "select codigo, descripcion, estado, orden, tipo, grupo, decode(grupo,'A','ADULTO','N','NENONATO') grupo_desc, decode(bloquear,'S','SI','N','NO') bloquear_desc from tbl_sal_pausa_seguridad_params where descripcion is not null"+appendFilter+" order by grupo, tipo, orden";
...
function add(){
	abrir_ventana('../expediente/pausa_seguridad_config.jsp?tipo=<%=tipo%>&grupo=<%=grupo%>');
}

function edit(id,tipo,grupo){
	abrir_ventana('../expediente/pausa_seguridad_config.jsp?mode=edit&id='+id+'&tipo='+tipo+'&grupo='+grupo);
}
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">
			<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nueva pausa de seguridad</cellbytelabel> ]</a></authtype>
		</td>
  </tr>
	<tr>
		<td>
...
%>
				...
				<td>
                    ...
                    &nbsp;&nbsp;
                    <cellbytelabel id="4">Grupo</cellbytelabel>
					<%=fb.select("grupo","A=ADULTO,N=NEONATO",grupo,"T")%>
                    &nbsp;&nbsp;
                    <cellbytelabel id="4">Tipo</cellbytelabel>
					<%=fb.select("tipo","VP=VERIFICACIÓN PRE-OPERATORIA,TO=PAUSA DE SEGURIDAD (TIME OUT),SO=VERIFICACION POST OPERATORIA (Sign - Out)",tipo,"T")%>
                    &nbsp;&nbsp;
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
		...
		<tr class="TextHeader" align="center">
			...
			<td width="10%"><cellbytelabel id="4">Bloqueado</cellbytelabel></td>
			<td width="7%">&nbsp;</td>
		</tr>
<%
String gGrupo = "";
for (int i=0; i<al.size(); i++)
{
    ...
    if (!gGrupo.equalsIgnoreCase(cdo.getColValue("grupo"))){
%>
    <tr class="TextHeader01">
        <td colspan="6"><%=cdo.getColValue("grupo_desc")%></td>
    </tr>
<%}%>    
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="center"><%=cdo.getColValue("bloquear_desc")%></td>
			<td align="center">
