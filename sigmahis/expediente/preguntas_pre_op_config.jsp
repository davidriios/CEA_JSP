...
sql = "select codigo, descripcion, estado, orden from tbl_sal_pregunta where codigo="+id;
...
else
{
  ...
  cdo.setTableName("tbl_sal_pregunta");
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/preguntas_pre_op_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/preguntas_pre_op_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/preguntas_pre_op_list.jsp';
<%
	}
