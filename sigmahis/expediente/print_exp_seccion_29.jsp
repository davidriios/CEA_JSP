...
if (tipoTrx == null) tipoTrx = "C";

cdop = SQLMgr.getPacData(pacId, noAdmision);

  sql.append("select distinct p.num_devolucion solicitud_no, a.almacen, b.descripcion||' - '||a.almacen as almacen_desc, to_char(p.fecha, 'dd/mm/yyyy') fecha, to_char(p.fecha, 'hh12:mi am') hora, t.cod_familia, t.cod_clase, t.cod_articulo, t.cantidad, ar.descripcion, p.observacion observaciones, p.usuario_creacion uc,p.anio,p.adm_secuencia admision,p.estado,decode(p.estado,'N','-RECHAZADA','')descEstado FROM tbl_sec_cds_almacen a,tbl_inv_almacen b,tbl_inv_detalle_paciente t,tbl_inv_devolucion_pac p,tbl_inv_articulo ar where p.compania = t.compania and  p.compania = b.compania and  p.num_devolucion =t.num_devolucion and p.anio = t.anio_devolucion and p.codigo_almacen = b.codigo_almacen and a.almacen = b.codigo_almacen and p.pac_id=");
  sql.append(pacId);
  ...
  sql.append(" and p.origen ='EXP' and t.cod_familia =ar.cod_flia and t.cod_clase=ar.cod_clase and t.cod_articulo=ar.cod_articulo and t.compania=ar.compania order by p.anio,p.adm_secuencia,p.num_devolucion ");

if(fg.trim().equals("USOS"))
{
sql = new StringBuffer();
sql.append("select to_char(cu.fecha,'dd/mm/yyyy') fecha,to_char(cu.fecha, 'hh12:mi am') hora,dcu.cod_uso, su.descripcion, dcu.cantidad_uso cantidad, dcu.precio, dcu.secuencia_uso solicitud_no, dcu.renglon, su.tipo_servicio,cu.codigo_almacen,cu.usuario_creacion uc,cu.observaciones,cu.anio,cu.adm_secuencia admision,''descEstado,cu.estado from tbl_sal_cargos_usos cu, tbl_sal_cargos_det_usos dcu, tbl_sal_uso su where cu.compania = ");
sql.append(session.getAttribute("_companyId"));
...
sql.append(" and cu.tipo = '");
sql.append(tipoTrx);
sql.append("' and cu.sop ="); 
