...
sql =    "SELECT  "
+ "(SELECT count(*) FROM tbl_sal_detalle_orden_med "
+ " WHERE pac_id=a.pac_id AND secuencia=a.secuencia ) AS ordmeds, "
+ " to_char(b.f_nac,'dd/mm/yyyy') AS f_nac, a.codigo_paciente, a.secuencia, "
+ " to_char(a.fecha_ingreso,'dd/mm/yyyy') AS fecha_ingreso, to_char(a.fecha_ingreso,'hh12:mi:ss am') AS hora_ingreso, a.pac_id, "
+ "(b.provincia||decode(b.sigla,null,'','-'||b.sigla)||decode(b.tomo,null,'','-'||b.tomo)||decode(b.asiento,null,'','-'||b.asiento)) as cedula,  "
+ "(b.primer_nombre||decode(b.segundo_nombre,null,' ',' '||b.segundo_nombre)||decode(b.primer_apellido,null,' ',' '||b.primer_apellido)||decode(b.segundo_apellido,null,' ',' '||b.segundo_apellido)||decode(b.sexo,'F',decode(b.apellido_de_casada,null,' ',' '||b.apellido_de_casada))) AS nombre_paciente "
+ " FROM tbl_adm_admision a, vw_adm_paciente b "
+ " WHERE b.codigo=a.codigo_paciente AND b.pac_id=a.pac_id"+appendFilter
...
for (int j=1; j<=nPages; j++)
{
			Vector setHeader0=new Vector();
				...
				pc.createTable();
				pc.setFont(9, 1);
				  pc.addCols("Expediente",1,1);
					pc.addCols("CUARTO DE URGENCIAS - TRIAGE", 1, 1);
				pc.addTable();
...
//				pc.addTable();
				...
				if (al.size()==0) {
						pc.createTable();
							pc.addCols("No existe el Paciente seleccionado",1,5);
						pc.addTable();
					...
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								pc.createTable();
								pc.setFont(7, 0);
									pc.addCols(" "+cdo1.getColValue("f_nac"),0,1);
									pc.addCols(" "+cdo1.getColValue("codigo_paciente"),0,1);
									...
									pc.addCols(" "+cdo1.getColValue("fecha_ingreso"),1,1);
									pc.addCols(" "+cdo1.getColValue("hora_ingreso"),1,1);
								pc.addTable();
