...
String userName = UserDet.getUserName();
		...
		sql = "select codigo, descripcion, decode(status,'A','ACTIVO', 'I', 'INACTIVO') status_desc from tbl_sal_signo_vital where 1 = 1 "+appendFilter+" order by codigo";
...
int no2 = 0;
			for (int j=1; j<=nPages; j++)
			{
				...
				pc.addTable();
					...
					pc.createTable();
						pc.setFont(7, 1);
						pc.addCols("No Existen Signos Vitales Registrados.",1,2);
					pc.addTable();
					...
					for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
					{
							...
							pc.addCols(cdo1.getColValue("status_desc"), 1,1);
							pc.addTable();
							if ((i + 1) == nItems)
							{
		   						pc.createTable();
					...
					if (al.size() == 0) {
              pc.createTable();
							pc.addCols("0 Registro en total",0,5);
							pc.addTable();
					}
