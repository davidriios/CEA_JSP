...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
	...
	if (!descrip.trim().equals("")){
		appendFilter += " and upper(descripcion) like '%"+descrip.toUpperCase()+"%'";
	}
	if (!estado.trim().equals("")){
		appendFilter += " and upper(estado) = '%"+estado.toUpperCase()+"%'";
	}
...
sql = "select codigo, descripcion, decode(estado, 'A','ACTIVO', 'I', 'INACTIVO') estado_desc, orden from tbl_sal_infeccion where 1 = 1 "+appendFilter+" order by orden";
al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
rowCount = CmnMgr.getCount("select count(*) count from tbl_sal_infeccion "+appendFilter);
...
document.title = 'Control de procedimiento invasivo - '+document.title;

function add()
{
	abrir_ventana('../expediente/proc_invasivo_config.jsp');
}

function edit(id)
{
	abrir_ventana('../expediente/proc_invasivo_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../expediente/print_list_proc_invasivo.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">
			<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nuevo procedimiento invasivo</cellbytelabel> ]</a></authtype>
		</td>
  </tr>
	<tr>
		<td>
