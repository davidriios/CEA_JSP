...
/*
sql = "select codigo, descripcion from tbl_cds_centro_servicio where estado in ('A') and interfaz IN('BDS','LIS') order by 1";
...
sql = "select codigo, descripcion from tbl_cds_centro_servicio where estado = 'A' and sol_interfaz_lis is not null order by descripcion";
al = SQLMgr.getDataList(sql);
for(int i=0;i<al.size();i++){
	CommonDataObject b = (CommonDataObject) al.get(i);
	solicitado_por = b.getColValue("codigo");
	break;
}*/
...
CommonDataObject p = (CommonDataObject) SQLMgr.getData("select nvl(get_sec_comp_param(-1,'EXP_SOL_LIS_CDS_EXPANDED'),'Y') as cds_expanded, nvl(get_sec_comp_param(-1,'EXP_SOL_LIS_CDS_REQ'),'-') as cds_req, nvl(get_sec_comp_param("+(String) session.getAttribute("_companyId")+",'EXP_SOL_LIS_FECHA_REQ'),'S') as fecha_req from dual");
if (p == null) {
...
function doSubmit(baction){document.form1.baction.value = baction;var estado ='';if(baction=='Solicitar Estudio')var req=reqInfo(false);else var req=reqInfo(true);if(baction=='Generar Cargo'){if(req.pacId!=''){estado = getDBData('<%=request.getContextPath()%>','estado','tbl_adm_admision','pac_id='+req.pacId+' and secuencia='+req.admCorte+' ','');}}var descEstado='';if(estado =='I')descEstado=' INACTIVA ';else if(estado =='N')descEstado=' ANULADA ';if((baction=='Generar Cargo' && (estado !='I' && estado !='N'))||baction=='Cancelar Estudio'){form1BlockButtons(true);}if(baction=='Generar Cargo'){if(estado !='I' && estado !='N'){window.frames['itemFrame'].doSubmit();}else {alert('La Admision se encuentra en estado:'+descEstado+'. No puede Registrar Cargos !!!');}}else window.frames['itemFrame'].doSubmit();}
function printOrder(value){var req=reqInfo(false);var cdsSel = document.form1.cdsSel.value; if(value =='1')abrir_ventana2('../expediente/print_sol_imagenologia.jsp?fg=LAB&interfaz=LIS&pacId='+req.pacId+'&noAdmision='+req.admisionRoot+'&fecha=<%=fecha%>&fechaHasta=<%=fechaHasta%>&area=<%=area%>&estado=<%=estado%>&cdsSel='+cdsSel);}
function reqInfo(showAlert){if(showAlert==undefined){showAlert=true;}var idx=document.form1.regChecked.value;var isValid=false;var pacId='';var admisionRoot='';var admCorte='';var admision='';var codSolicitud='';var codigo='';if(idx.trim()==''){if(showAlert)alert('Por favor seleccione el Estudio!');}else{pacId=document.form1.pacId.value;admisionRoot=document.form1.admision.value;admCorte=document.form1.admCorte.value;admision=document.form1.admision.value;codSolicitud=document.form1.cod_solicitud.value;codigo=document.form1.codigo.value;isValid=true;}return{isValid:isValid,idx:idx,pacId:pacId,admisionRoot:admisionRoot,admision:admision,codSolicitud:codSolicitud,codigo:codigo,admCorte:admCorte};}
...
var xHeight=0;
function doAction(){document.form1.regChecked.value='';loaded=true;checkPendingOM();xHeight=objHeight('_tblMain');resizeFrame();}
...
<table align="center" width="99%" cellpadding="5" cellspacing="1" id="_tblMain">
	<tr>
		...
		<table align="center" width="100%" cellpadding="0" cellspacing="1">
				...
				<%=fb.hidden("fg",fg)%>
				<%=fb.hidden("clearHT","")%>
				<%=fb.hidden("regChecked",""+regChecked)%>
				<%=fb.hidden("pacId","")%>
				...
				<%=fb.hidden("cdsSel","")%>
				<tr class="TextRow02">
					<td colspan="3"><cellbytelabel id="1">Solicitud de Laboratorios</cellbytelabel></td>
				</tr>
				<tr class="TextRow01">
					<td class="TableBottomBorder" colspan="3">
						<table width="100%">
							<tr>
								...
								Estado:<%=fb.select("estado","S=PENDIENTE,T=CARGO GENERADO,A=ANULADA",estado,false,false,0,"Text10",null,null,null,"")%>
								<cellbytelabel id="3">&Aacute;reas</cellbytelabel>:&nbsp;
								<%=fb.select(ConMgr.getConnection(), "select codigo, descripcion from tbl_cds_centro_servicio where estado in ('A') and interfaz in('LIS','BDS') order by 1", "area", area, false, false, 0, "T")%>&nbsp;

								<%
								sql = "select codigo, descripcion from tbl_cds_centro_servicio where estado = 'A' and sol_interfaz_lis is not null order by descripcion";
								%>
								 <cellbytelabel id="4">Solicitado por</cellbytelabel>:
								<%=fb.select(ConMgr.getConnection(), sql, "solicitado_por", solicitado_por, false, false, 0, "T")%>
								<%=fb.checkbox("incluir_admision", "Y", (incluir_admision.equals("Y")), false)%>
				...
				<tr>
					<td colspan="3">
<iframe name="itemFrame" id="itemFrame" align="center" width="100%" height="0" scrolling="yes" frameborder="0" border="0"  src="../expediente/reg_sol_lab_item.jsp?fecha=<%=fecha%>&fechaHasta=<%=fechaHasta%>&area=<%=area%>&solicitado_por=<%=solicitado_por%>&cdsExpanded=<%=p.getColValue("cds_expanded")%>&cdsReq=<%=p.getColValue("cds_req")%>&estado=<%=estado%>&incluir_admision=<%=incluir_admision%>"></iframe>
					</td>
				</tr>
		<!--<tr>
			<td colspan="3">
			<div id="_cMain" class="Container">
				...
				<tr class="TextRow01">
					<td><cellbytelabel id="6">Informaci&oacute;n que se desea obtener / Sospecha Diagn&oacute;stica</cellbytelabel></td>
					<td><cellbytelabel id="7">Comentarios</cellbytelabel></td>
					<td>&nbsp;</td>
				</tr>
				<tr class="TextRow01">
				...
				<tr class="TextRow01">
					<td>
						<%=fb.button("print_orden","Imprimir por Area Seleccionada",false,false,"","","onClick=\"javascript:printOrder(1)\"")%>
												<!---->
				...
				<tr class="TextRow01">
					<td colspan="3"><cellbytelabel id="8">Creado</cellbytelabel>:&nbsp;
...
else
{
	...
	if(request.getParameter("baction")!=null && (request.getParameter("baction").equalsIgnoreCase("Generar Cargo") || request.getParameter("baction").equalsIgnoreCase("Cancelar Estudio"))){
		...
		codSolicitud = request.getParameter("cod_solicitud");
		codigo = request.getParameter("codigo");
		seqTrx = request.getParameter("seqTrx");
	} else {
		response.sendRedirect("../expediente/reg_sol_lab.jsp?mode="+mode+ "&change=1&type=2&fecha="+fecha+"&fechaHasta="+fechaHasta+"&area="+area+"&solicitado_por="+solicitado_por+"&estado="+estado+"&incluir_admision="+incluir_admision);
		return;
	}
%>
<html>
...
{
<%
if (request.getParameter("baction").equalsIgnoreCase("Cancelar Estudio"))
{
%>
	window.close();
<%
...
%>
	alert('<%=errMsg%>');
	abrir_ventana2('../expediente/print_hoja_trabajo_lab.jsp?fg=LAB&interfaz=LIS&pacId=<%=pacId%>&noAdmision=<%=admision%>&seqTrx=<%=seqTrx%>');

<%
	} else throw new Exception(errMsg);
}
%>
	window.location = '<%="../expediente/reg_sol_lab.jsp"%>?fecha=<%=fecha%>&fechaHasta=<%=fechaHasta%>&area=<%=area%>&solicitado_por=<%=solicitado_por%>&estado=<%=estado%>&incluir_admision=<%=incluir_admision%>';
}
</script>
</head>
