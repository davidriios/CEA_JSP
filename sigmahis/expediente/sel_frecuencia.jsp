...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  sql = "select codigo, descripcion from tbl_sal_frecuencia  "+appendFilter+" ORDER BY codigo";
...
{ 
    var code = eval('document.listFrecuencia.codigo'+i).value;
	var name = eval('document.listFrecuencia.descripcion'+i).value; 
	
    switch(op)
	{ 
	   case 1:
	       eval('window.opener.document.form0.frecuencia<%=index%>').value = code;
           eval('window.opener.document.form0.descFrecuencia<%=index%>').value = name;
		   window.close();
	   break;
	}  
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<%fb = new FormBean("listFrecuencia",request.getContextPath()+request.getServletPath(),FormBean.POST);%>
            <%=fb.formStart(true)%>
