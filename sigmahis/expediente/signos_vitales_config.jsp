...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	{
		if (id == null) throw new Exception("El Tipo signo vital no es válido. Por favor intente nuevamente!");

        sql = "select codigo, descripcion, orden, status from tbl_sal_signo_vital where codigo="+id;
...
<%if (mode.equalsIgnoreCase("add")){%>
document.title="Signos Vitales- Agregar- "+document.title;
<%}else if (mode.equalsIgnoreCase("edit")){%>
document.title="Signos Vitales - Edición - "+document.title;
<%}%>
</script>
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="EXPEDIENTE - MANTENIMIENTO -SIGNOS VITALES"></jsp:param>
</jsp:include>
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<tr class="TextRow01" >
                ...
                <td>
                    Orden:&nbsp;<%=fb.textBox("orden",cdo.getColValue("orden"),true,false,false,5,2)%>
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    Estado:&nbsp;
...
else
{
  ...
  cdo.setTableName("tbl_sal_signo_vital");
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/signos_vitales_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/signos_vitales_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/signos_vitales_list.jsp';
<%
	}
