...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	{
		if (id == null) throw new Exception("El Tipo de Tècnica no es válido. Por favor intente nuevamente!");

sql = "select id, nombre, descripcion,status from tbl_sal_tecnica_evaluacion where id="+id;
...
<%if (mode.equalsIgnoreCase("add")){%>
document.title="Tipo de Tecnica - Agregar- "+document.title;
<%}else if (mode.equalsIgnoreCase("edit")){%>
document.title="Tipo de Tecnica - Edición - "+document.title;
<%}%>
</script>
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="TIPO DE TECNICA"></jsp:param>
</jsp:include>
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<tr class="TextRow01" >
				...
				<td><%=fb.textBox("nombre",cdo.getColValue("nombre"),true,false,false,60,99)%></td>
			</tr>
			<tr class="TextRow01" >
...
else
{
  ...
  cdo.setTableName("tbl_sal_tecnica_evaluacion");
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/list_tecnicas.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/list_tecnicas.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/list_tecnicas.jsp';
<%
	}
