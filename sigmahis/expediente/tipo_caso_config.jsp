...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	{
		if (id == null) throw new Exception("El Tipo de caso no es válido. Por favor intente nuevamente!");

sql = "select codigo, descripcion, activar_obs from tbl_sal_tipo_caso where codigo="+id;
...
<%if (mode.equalsIgnoreCase("add")){%>
document.title="Tipos de casos - Agregar - "+document.title;
<%}else if (mode.equalsIgnoreCase("edit")){%>
document.title="Tipos de casos - Edición - "+document.title;
<%}%>
</script>
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="EXPEDIENTE - INFORME DE CASOS POLICIVOS - TIPOS DE CASOS"></jsp:param>
</jsp:include>
...
else
{
  ...
  cdo.setTableName("tbl_sal_tipo_caso");
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/tipo_caso_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/tipo_caso_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/tipo_caso_list.jsp';
<%
	}
