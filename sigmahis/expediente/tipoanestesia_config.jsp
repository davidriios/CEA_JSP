...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	{
		if (code == null) throw new Exception("El Tipo de Anestesia no es válido. Por favor intente nuevamente!");

		sql = "SELECT codigo, descripcion FROM tbl_sal_tipo_anestesia WHERE codigo="+code;
...
<%if(mode.equalsIgnoreCase("add")){%>
document.title=" Tipo Anestesia Agregar - "+document.title;
<%}else if(mode.equalsIgnoreCase("edit")){%>
document.title=" Tipo Anestesia Edición - "+document.title;
<%}%>
</script>
</head>
...
else
{
  ...
  cdo.setTableName("tbl_sal_tipo_anestesia");
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/tipoanestesia_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/tipoanestesia_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/tipoanestesia_list.jsp';
<%
	}
