...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("codigo") != null)
  {
    ...
    searchType = "1";
    searchDisp = "Código";
		codigo     = request.getParameter("codigo");   // utilizada para mantener el Código de la Anestesia
  }
  else if (request.getParameter("descripcion") != null)
  {
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");   // utilizada para mantener la Descripción de la Anestesia
  }
  ...
  sql = "SELECT codigo, descripcion FROM tbl_sal_tipo_anestesia WHERE codigo<>0"+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_sal_tipo_anestesia WHERE codigo<>0"+appendFilter);
...
document.title = 'Tipos de Anestesias - '+document.title;

function add()
{
	abrir_ventana('tipoanestesia_config.jsp');
}

function edit(code)
{
	abrir_ventana('tipoanestesia_config.jsp?mode=edit&code='+code);
}

function printList()
{
	abrir_ventana('print_list_tipoanestesia.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
        <td align="right">
	       <authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nuevo Tipo de Anestesia</cellbytelabel> ]</a></authtype>
	    </td>
    </tr>
	<tr>
		<td>
