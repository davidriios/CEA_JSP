...
sql = "SELECT a.codigo, a.descripcion , b.codigo as subtipocod,decode(a.estatus,'I','Inactivo','A','Activo') estatus, b.cod_tipo_ORDENVARIOS, b.descripcion as subdieatdesc, b.observacion FROM TBL_CDS_ORDENMEDICA_VARIOS a, TBL_CDS_OM_VARIOS_SUBTIPO b WHERE  a.codigo=b.cod_tipo_ORDENVARIOS(+) "+appendFilter+" order by a.descripcion, a.codigo, b.codigo";

  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  
  
  rowCount = CmnMgr.getCount("SELECT count(*) FROM TBL_CDS_ORDENMEDICA_VARIOS a, TBL_CDS_OM_VARIOS_SUBTIPO b WHERE  a.codigo=b.cod_tipo_ORDENVARIOS "+appendFilter);
...
function add()
{
	abrir_ventana('tipos_ordenmedica_varios_config.jsp');
}

function edit(id)
{
	abrir_ventana('tipos_ordenmedica_varios_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('print_list_tipos_ordenvarios.jsp');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
        <td align="right">
        <authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nuevo Tipo de OrdenMedica Varios</cellbytelabel> ]</a></authtype>
	    </td>
    </tr>
	<tr>
		<td>
			<table width="100%" cellpadding="0" cellspacing="1">
				...
				<%fb = new FormBean("search03",request.getContextPath()+"/common/urlRedirect.jsp");%>
					...
					<%=fb.hidden("fromPage",request.getContextPath()+request.getServletPath())%>
					<%=fb.hidden("toPage",request.getContextPath()+request.getServletPath())%>
				    <td width="50%"><cellbytelabel id="4">Estatus</cellbytelabel><%=fb.textBox("estatus","",false,false,false,40)%>
					<%=fb.submit("go","Ir")%>
					</td>
				    <%=fb.formEnd()%>	
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="0" cellspacing="1" class="sortable" id="list">
				<tr class="TextHeader">
					...
					<td width="15%"><cellbytelabel id="4">Estatus</cellbytelabel></td>
					<td width="5%">&nbsp;</td>
				</tr>				
				<%
				String dieta = "";
				...
				<tr class="TextHeader01">
					<td colspan="4"><%=cdo.getColValue("descripcion")%></td>
				</tr>
				<%
					}
				%>
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
					<td><%=cdo.getColValue("subtipocod")%></td>
					<td><%=cdo.getColValue("subdieatdesc")%></td>
					<td><%=cdo.getColValue("estatus")%></td>
					<td align="center">
							<authtype type='4'><a href="javascript:edit(<%=cdo.getColValue("codigo")%>)" class="Link02Bold" onMouseOver="setoverc(this,'Link01Bold')" onMouseOut="setoutc(this,'Link02Bold')"><cellbytelabel id="5">Editar</cellbytelabel></a></authtype>
					</td>
				</tr>
				<%
