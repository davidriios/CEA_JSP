...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	{
		if (id == null) throw new Exception("El Tipo de tratamiento no es válido. Por favor intente nuevamente!");

sql = "select codigo, descripcion, activar_obs, estado from tbl_sal_tratamiento where codigo="+id;
...
<%if (mode.equalsIgnoreCase("add")){%>
document.title="Tratamiento - Agregar - "+document.title;
<%}else if (mode.equalsIgnoreCase("edit")){%>
document.title="Tratamiento - Edición - "+document.title;
<%}%>
</script>
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="EXPEDIENTE - MANTENIMIENTO - TRATAMIENTOS"></jsp:param>
</jsp:include>
...
else
{
  ...
  cdo.setTableName("tbl_sal_tratamiento");
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/tratamiento_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/tratamiento_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/tratamiento_list.jsp';
<%
	}
