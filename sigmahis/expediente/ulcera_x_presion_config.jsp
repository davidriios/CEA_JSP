...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	{
		if (code == null) throw new Exception("El Úlcera por Presión no es válido. Por favor intente nuevamente!");

		sql = "SELECT codigo, descripcion FROM tbl_sal_concepto_ulcera WHERE codigo="+code;
...
<%if(mode.equalsIgnoreCase("add")){%>
document.title=" Caracteristicas de Úlcera x Presión Agregar - "+document.title;
<%}else if(mode.equalsIgnoreCase("edit")){%>
document.title=" Caracteristicas de Úlcera x Presión Edición - "+document.title;
<%}%>
</script>
</head>
...
else
{
  ...
  cdo.setTableName("tbl_sal_concepto_ulcera");
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/ulcera_x_presion_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/ulcera_x_presion_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/ulcera_x_presion_list.jsp';
<%
	}
