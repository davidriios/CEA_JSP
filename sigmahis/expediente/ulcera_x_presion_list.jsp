...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  sql = "SELECT codigo, descripcion FROM tbl_sal_concepto_ulcera WHERE codigo<>0"+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_sal_concepto_ulcera WHERE codigo<>0"+appendFilter);
...
function add()
{
	abrir_ventana('ulcera_x_presion_config.jsp');
}

function edit(code)
{
	abrir_ventana('ulcera_x_presion_config.jsp?mode=edit&code='+code);
}

function printList()
{
	abrir_ventana('print_list_ulcera_x_presion.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
        <td align="right">
        	<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nueva &Uacute;lcera</cellbytelabel> ]</a></authtype>
   	    </td>
    </tr>
	<tr>
		<td>
