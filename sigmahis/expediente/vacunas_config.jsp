...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	{
		if (code == null) throw new Exception("La Vacuna no es válida. Por favor intente nuevamente!");

		sql = "SELECT codigo, descripcion, codigo_vacuna as vacunaCode FROM tbl_sal_vacuna WHERE codigo="+code;
...
<%if(mode.equalsIgnoreCase("add")){%>
document.title=" Tipos de Vacunas Agregar - "+document.title;
<%}else if(mode.equalsIgnoreCase("edit")){%>
document.title=" Tipos de Vacunas Edición - "+document.title;
<%}%>
</script>
</head>
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<%=fb.hidden("code",code)%>
				...
				<tr class="TextRow01">
					<td><cellbytelabel id="2">C&oacute;digo Vacuna</cellbytelabel></td>
					<td><%=fb.textBox("vacunaCode",cdo.getColValue("vacunaCode"),false,false,false,60,20)%></td>
				</tr>							
				<tr class="TextRow01">
...
else
{
  ...
  cdo.setTableName("tbl_sal_vacuna");
  cdo.addColValue("descripcion",request.getParameter("descripcion"));
  if (request.getParameter("vacunaCode") != null) cdo.addColValue("codigo_vacuna",request.getParameter("vacunaCode"));
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/expediente/vacunas_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/expediente/vacunas_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/expediente/vacunas_list.jsp';
<%
	}
