...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("codigo") != null)
  {
    ...
    searchType = "1";
    searchDisp = "Código";
		codigo     = request.getParameter("codigo");     // utilizada para mantener el Código de la Vacuna
  }
  else if (request.getParameter("descripcion") != null)
  {
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");   // utilizada para mantener la Descripción de la Vacuna
  }
  ...
  sql = "SELECT codigo, descripcion, codigo_vacuna as vacunaCode FROM tbl_sal_vacuna WHERE codigo<>0"+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_sal_vacuna WHERE codigo<>0"+appendFilter);
...
document.title = 'Tipo de Vacunas - '+document.title;

function add()
{
	abrir_ventana('vacunas_config.jsp');
}

function edit(code)
{
	abrir_ventana('vacunas_config.jsp?mode=edit&code='+code);
}

function printList()
{
	abrir_ventana('print_list_vacunas.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
      <td align="right">
        <authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nuevo Tipo de Vacuna</cellbytelabel> ]</a></authtype>
	    </td>
    </tr>
	<tr>
		<td>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="0" cellspacing="1" class="sortable" id="list">
				<tr class="TextHeader">
					...
					<td width="15%"><cellbytelabel id="8">C&oacute;digo Vacuna</cellbytelabel></td>
				...
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
					...
					<td><%=cdo.getColValue("vacunaCode")%></td>
