...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	if (!status.trim().equals("")) appendFilter += " status='"+status.toUpperCase()+"'";
	if (!tipo.trim().equals("")) appendFilter += " and tipo_liquido='"+tipo.toUpperCase()+"'";
	...
	sql = "select codigo, descripcion, decode(tipo_liquido,'I','INGRESO','E','EGRESO','M','MEDICAMENTO','D','DIABETICA') as tipo, decode(status,'A','ACTIVO','I','INACTIVO',status) as statusDesc from tbl_sal_via_admin where codigo is not null"+(!appendFilter.equals("")?" and "+appendFilter:appendFilter)+" order by 3";
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) count from tbl_sal_via_admin where codigo is not null"+(!appendFilter.equals("")?" and "+appendFilter:appendFilter));
...
document.title = 'Expediente - Mantenimiento - Via de Administración - '+document.title;

function add(){abrir_ventana('../expediente/vias_ingreso_egreso_config.jsp');}
function edit(id){abrir_ventana('../expediente/vias_ingreso_egreso_config.jsp?mode=edit&id='+id);}
function printList(){abrir_ventana('../expediente/print_list_vias_ingreso_egreso.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="EXPEDIENTE - MANTENIMIENTO - VIA ADMINISTRACION"></jsp:param>
</jsp:include>
<table align="center" width="99%" cellpadding="0" cellspacing="0">
<tr>
	<td align="right">
		<authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nueva V&iacute;a de Administraci&oacute;n</cellbytelabel>]</a></authtype>
	</td>
</tr>
<tr>
	<td>
...
<%=fb.hidden("toPage",request.getContextPath()+request.getServletPath())%>
			...
			<td width="20%">
				<cellbytelabel id="3">Tipo</cellbytelabel>
				<%=fb.select("tipo","I=INGRESO,E=EGRESO,M=MEDICAMENTO,D=DIABETICA",tipo,false,false,0,"Text10",null,null,null,"T")%>
			</td>
			<td width="25%">
				<cellbytelabel id="4">C&oacute;digo</cellbytelabel>
				<%=fb.intBox("codigo",codigo,false,false,false,15,"Text10",null,null)%>
			</td>
			<td width="35%">
...
%>
		<tr class="TextHeader01">
			<td colspan="4" align="center"><%=cdo.getColValue("tipo")%></td>
		</tr>
<%
	}
%>
