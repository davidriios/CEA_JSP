...
if (!viewMode && iDiag.size() > 0 ) mode = "edit";
...
function doAction(){<%if (request.getParameter("type") != null){%>showDiagnosticoList();<%}%>}
function showDiagnosticoList(){
var f = $("#fecha_creacion_diag").val() || '<%=fechaCreacionDiag%>';
var t = $("#hora_creacion_diag").val() || '<%=horaCreacionDiag%>';
abrir_ventana1('../common/check_diagnostico.jsp?fp=pIngreso&mode=<%=mode%>&modeSec=<%=modeSec%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&desc=<%=desc%>&exp=3&tab=<%=tab%>&fecha_creacion_diag='+f+'&hora_creacion_diag='+t);}
function printExp(){abrir_ventana("../expediente3.0/print_exp_seccion_89.jsp?pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&seccion=<%=seccion%>&desc=<%=desc%>");}
...
$(function(){
  ...
  $("a[href='#enfermedad_notificable']").click(function(e){
    ...
    url = "../expediente3.0/exp_enfermedad_notificable.jsp?pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&fg=I&seccion=<%=seccion%>&desc=<%=desc%>";
...
<ul class="nav nav-tabs" role="tablist">
    <li role="presentation" class="<%=active0%>">
        <a href="#datos_de_salida" aria-controls="datos_de_salida" role="tab" data-toggle="tab"><b>Datos de ingreso</b></a>
    </li>
     ...
     <table cellspacing="0" class="table table-small-font table-bordered">
         <%
           ...
           if (request.getParameter("fecha_creacion_diag") == null && request.getParameter("hora_creacion_diag") == null) {
              cdoD = SQLMgr.getData("select to_char(a.fecha_creacion,'dd/mm/yyyy') as fecha_creacion_diag, to_char(a.fecha_creacion, 'hh12:mi:ss am') as hora_creacion_diag from tbl_adm_diagnostico_x_admision a where a.admision="+noAdmision+" and a.pac_id="+pacId+" and a.tipo = 'I' order by a.orden_diag");
        ...
        <tr class="bg-headtabla" align="center">
          ...
          <td width="5%"><%=fb.submit("addDiagnostico","+",false,viewMode,null,null,"onClick=\"javascript:__submitForm(this.form, this.value)\"","Agregar Diagnósticos")%></td>
        </tr>
            ...
            <tr class="TextRow01">
              ...
              <td align="center"><%=fb.intBox("ordenDiag"+i,cdo.getColValue("orden_diag"),false,false,viewMode,2,"form-control input-sm", null,null)%></td>
              <td align="center"><%=fb.submit("rem"+i,"x",true,viewMode,null,null,"onClick=\"javascript:removeItem('"+fb.getFormName()+"',"+i+");  __submitForm(this.form, 'X')\"","Eliminar Diagnóstico")%></td>
            </tr>
			<%}%>
<%
}
...
function editMode()
{
	window.location = '<%=request.getContextPath()+request.getServletPath()%>?seccion=<%=seccion%>&modeSec=edit&mode=<%=mode%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&desc=<%=desc%>&exp=3&tab=<%=tab%>&codigo=<%=codigo%>';
}
</script>
</head>
