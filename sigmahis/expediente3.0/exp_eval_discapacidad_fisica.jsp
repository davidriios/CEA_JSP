...
if (request.getMethod().equalsIgnoreCase("GET")){
    ...
    iHashEval.clear();
    sql="select to_char(fecha,'dd/mm/yyyy') as fecha, observacion from tbl_sal_eval_discapacidad where pac_id="+pacId+" and secuencia="+noAdmision+" order by to_date(fecha,'dd/mm/yyyy') desc";
...
function setEvaluacion(k){var fecha_e = eval('document.form0.fecha_evaluacion'+k).value ;window.location= '../expediente3.0/exp_eval_discapacidad_fisica.jsp?modeSec=view&mode=<%=mode%>&seccion=<%=seccion%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&fecha_eval='+fecha_e+'&desc=<%=desc%>';}
...
function getTotal() {
  ...
  for (var i = 0; i<<%=al.size()%>; i++) {
    var val = $("input:radio[name='aplicar"+i+"'][value='S']:checked").length;
    tot += val;
  }
  if (tot > 0) {
    parent.CBMSG.alert("Por favor comunicarse con el médico para llevar el plan de cuidado discapacidad física.");
  }
  $("#total").val(tot); 
}

function printDatos(){var fecha = document.form0.fecha.value;abrir_ventana1('../expediente3.0/print_eval_discapacidad_fisica.jsp?pacId=<%=pacId%>&desc=<%=desc%>&noAdmision=<%=noAdmision%>&seccion=<%=seccion%>&fecha_eval='+fecha);}
function printDatosTodos(){abrir_ventana1('../expediente3.0/print_eval_discapacidad_fisica.jsp?pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&seccion=<%=seccion%>&desc=<%=desc%>');}
function add(){window.location = "../expediente3.0/exp_eval_discapacidad_fisica.jsp?desc=<%=desc%>&pacId=<%=pacId%>&seccion=<%=seccion%>&noAdmision=<%=noAdmision%>&modeSec=add&mode=<%=mode%>&fecha_eval=<%=fecha_eval%>";}
</script>
</head>
...
<table class="table table-small-font table-bordered table-striped table-custom-2">
    ...
    <tr>
        <td class="bg-headtabla"><div class="pull-left"><b>LISTADO DE EVALUACIONES DE RIESGO</b></div></td>
    </tr>
</table>
...
<thead>
    <tr class="bg-headtabla2" >
        ...
        <th style="vertical-align: middle !important;"><cellbytelabel>Observaci&oacute;n</cellbytelabel></th>
   </tr>
</thead>

<%if(appendFilter.equals("1") && !op.trim().equals("0")){%>
<%=fb.hidden("fecha_evaluacion0",cDateTime.substring(0,10))%>
<tr style="cursor:pointer " onClick="javascript:setEvaluacion(0)">
    <td><%=cDateTime.substring(0,10)%></td>
    <td><cellbytelabel>Evaluaci&oacute;n actual</cellbytelabel></td>
</tr>
<%
} 
...
<%=fb.hidden("action"+i,cdo.getColValue("fechaUp"))%>
<%=fb.hidden("seleccionado"+i,cdo.getColValue("seleccionar"))%>

<tr>
    <td><%=cdo.getColValue("descripcion")%></td>
    <td align="center"><%=fb.radio("aplicar"+i,"S",(cdo.getColValue("seleccionar").equalsIgnoreCase("S")),false,viewMode,"__aplicar",null,"")%></td>
    <td align="center"><%=fb.radio("aplicar"+i,"N",(cdo.getColValue("seleccionar").equalsIgnoreCase("N")),false,viewMode,"",null,"")%></td>
    <td>
        <%=fb.textarea("observacion2"+i,cdo.getColValue("observacion"),false,false,viewMode,50,1,2000,"form-control input-sm","width:100%",null)%>
    </td>
</tr>
<%}%>
</table>
...
else
{
	...
	cdo = new CommonDataObject();
    cdo.setTableName("tbl_sal_eval_discapacidad");
    ...
    if (modeSec.equalsIgnoreCase("add")) {
        ...
        cdo.addColValue("usuario_modificacion", (String) session.getAttribute("_userName"));
        cdo.setAction("U");
        cdo.setWhereClause(" pac_id = "+pacId+" and secuencia = "+noAdmision+" and fecha = to_date('"+request.getParameter("fecha")+"', 'dd/mm/yyyy')");
    }
    
    int selected = 0;
    for (int i=1; i<=size; i++){
        CommonDataObject cdo2 = new CommonDataObject();
        cdo2.setTableName("tbl_sal_eval_discapa_det");
    
		if (request.getParameter("aplicar"+i)!= null && request.getParameter("aplicar"+i).equalsIgnoreCase("S")){
        
            if (request.getParameter("seleccionado"+i) != null && request.getParameter("seleccionado"+i).equalsIgnoreCase("S")) {
           ...
           if (request.getParameter("seleccionado"+i) != null && request.getParameter("seleccionado"+i).equalsIgnoreCase("S")) {
                cdo2.setWhereClause("pac_id = "+request.getParameter("pacId")+" and secuencia = "+request.getParameter("noAdmision")+" and fecha_up = to_date('"+request.getParameter("fechaUp"+i)+"', 'dd/mm/yyyy') and cod_eval = "+request.getParameter("codigo"+i));
                cdo2.setAction("D");
                selected++;
            }
        }
