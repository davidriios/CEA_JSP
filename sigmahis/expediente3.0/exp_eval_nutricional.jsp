...
function add()
{ 
	window.location = '../expediente3.0/exp_eval_nutricional.jsp?mode=add&seccion=<%=seccion%>&pacId=<%=pacId%>&desc=<%=desc%>&noAdmision=<%=noAdmision%>&code=0';
}
...
function setNutricion(k)
{
		...
		window.location = '../expediente3.0/exp_eval_nutricional.jsp?mode=edit&seccion=<%=seccion%>&pacId=<%=pacId%>&desc=<%=desc%>&noAdmision=<%=noAdmision%>&code='+code;
}
function doAction()
{
	//showHide(0);
