...
cdo = SQLMgr.getData("select sexo, edad, edad_mes, nvl(get_sec_comp_param("+companyId+", 'SAL_PED_EDAD'), 0) edad_ped, (select formulario from tbl_sal_nota_eval_enf_urg where pac_id = p.pac_id and admision = a.secuencia) formulario, nvl(get_sec_comp_param("+companyId+", 'SAL_MOSTRAR_ESCALAS'), '0,0') show_escalas from vw_adm_paciente p, tbl_adm_admision a where p.pac_id = "+pacId+" and p.pac_id = a.pac_id and a.secuencia = "+noAdmision);

if ( cdo!= null && cdo.getColValue("formulario") != null && cdo.getColValue("formulario").equals("") ) {
  showOthers = false;
}
...
formulario = cdo.getColValue("formulario","14");
showEscalas = (cdo.getColValue("show_escalas","0,0")).contains("1");

sectionId = "108";

// temp hack
showOthers = true;
String xtraSections = ""; // usada para recuperar
...
if (showOthers) {
    ...
    if (edad <= edadPed ) {
        if (showEscalas) {
          sectionId += ",119,84,118,101,116,91";
          xtraSections = "119,84,118,101,116,91";
        }
    } else {
        // adultos
        if (showEscalas) {
          sectionId += ",84,112,40,116,91";
          xtraSections = "84,112,40,116,91";
        }
    }
} 
                        ...
                        <td>
                            <div id="accordion">
                              ...
                              <%for (int i=0; i<al.size(); i++){
                                ...
                                String xtraQS = "desc="+cdo.getColValue("descripcion")+"&pacId="+pacId+"&seccion="+cdo.getColValue("codigo")+"&noAdmision="+noAdmision+"&mode="+cdo.getColValue("actionMode")+"&cds="+cds+"&defaultAction="+cdo.getColValue("editable")+"&medico="+medico+"&from=salida_pop&estado="+estado+"&xtraSections="+xtraSections;
                              %>
                              ...
                              <div id="cb-accordion-content<%=i%>">
                                <iframe id="sec<%=i%>" name="sec<%=i%>" width="100%" scrolling="yes" frameborder="0" src="<%//=cdo.getColValue("path")%><%//=xtraQS%>"></iframe>
