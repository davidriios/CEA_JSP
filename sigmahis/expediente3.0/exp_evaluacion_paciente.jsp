...
sql=" select a.codigo, a.tipo_evaluacion, to_char(a.fecha,'dd/mm/yyyy') fecha, a.diag_pre_evaluacion codDiagPre, a.hallazgo, a.recomendacion, a.medico codMedico, a.tecnica, a.premedicacion, a.anestesia, a.indicacion, a.rx_tac_torax /*,a.documento*/, decode(a.documento,null,' ','"+java.util.ResourceBundle.getBundle("path").getString("expedientedocs").replaceAll(java.util.ResourceBundle.getBundle("path").getString("root"),"..")+"/'||a.documento) as documento,  b.descripcion descAnestesia, coalesce(c.observacion,c.nombre) descDiagPre, d.primer_nombre||decode(d.segundo_nombre,null,'',' '||d.segundo_nombre)||' '||d.primer_apellido||decode(d.segundo_apellido,null,'',' '||d.segundo_apellido)||decode(d.sexo,'F',decode(d.apellido_de_casada,null,'',' '||d.apellido_de_casada)) as nombre_medico,nvl(a.sub_tipo,' ') sub_tipo from tbl_sal_evaluacion a ,tbl_sal_tipo_anestesia b ,tbl_cds_diagnostico c , tbl_adm_medico d  where a.anestesia = b.codigo(+) and a.diag_pre_evaluacion = c.codigo(+) and a.medico = d.codigo(+) and a.codigo = "+code+" and a.tipo_evaluacion = '"+fg+"' /* and  a.pac_id="+pacId+" and a.admision="+noAdmision+"  */ ";
...
function doAction(){
	...
	abrir_ventana1('../common/check_diagnostico.jsp?seccion=<%=seccion%>&mode=edit&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&code=<%=code%>&tab=<%=tab%>&fg=<%=fg%>&fp=informes&muestraLastLineNo=<%=muestraLastLineNo%>&diagPostLastLineNo=<%=diagPostLastLineNo%>');
	<%}%>
}
function imprimir()
{
//var fecha = document.form0.fecha.value;
abrir_ventana1('../expediente/print_evaluacion_paciente.jsp?pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&code=<%=code%>&fg=<%=fg%>&seccion=<%=seccion%>&desc=<%=desc%>');
}
function showAnestesiaList(){
    abrir_ventana1('../expediente/list_anestesia.jsp?id=3');
}
function showDiagList(){
	abrir_ventana1('../common/search_diagnostico.jsp?fp=informes');
}
function setEvaluacion(k){
    var code = eval('document.listado.codigo'+k).value;
    window.location = '../expediente3.0/exp_evaluacion_paciente.jsp?fg=<%=fg%>&mode=view&seccion=<%=seccion%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&desc=<%=desc%>&code='+code;
}
function add()
{
window.location = '../expediente3.0/exp_evaluacion_paciente.jsp?fg=<%=fg%>&mode=add&seccion=<%=seccion%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&desc=<%=desc%>&code=0';
...
<table align="center" width="100%" cellpadding="0" cellspacing="0">
	...
	<tr class="TextRow01">
				 ...
				 <%=fb.hidden("desc",desc)%>
						<tr class="TextRow02">
							<td width="25%">&nbsp;<cellbytelabel id="1">Listado de Evaluaciones</cellbytelabel></td>
							<td width="75%" align="right"><%if(!code.equals("0")){%><a href="javascript:add()" class="Link00">[ <cellbytelabel id="2">Agregar Evaluaci&oacute;n</cellbytelabel> ]</a><%}%><a href="javascript:imprimir()" class="Link00">[Imprimir]</a> </td>
						</tr>

						<tr class="TextHeader">
							<td><cellbytelabel id="3">Fecha</cellbytelabel></td>
							<td><cellbytelabel id="4">M&eacute;dico</cellbytelabel></td>
						</tr>
<%
...
<div class = "dhtmlgoodies_aTab">
				...
				%>
				<tr class="TextRow01">
						<td><cellbytelabel id="18">Documento</cellbytelabel></td>
						<td colspan="3">
						<%=fb.fileBox("documento",cdo.getColValue("documento"),false,false,40)%>
						
						
						<% if (!hasImage && !doc.trim().equals("")){ %>		
							<img src="../images/search.gif" id="scan" width="20" height="20" onClick="javascript:abrir_ventana('../common/abrir_ventana.jsp?fileName=<%=cdo.getColValue("documento")%>')" style="cursor:pointer;" title="AA <%=doc%>"/>
							</td>
						<%}%>
