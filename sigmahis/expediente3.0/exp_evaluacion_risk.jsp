...
function doAction(){isChecked();checkViewMode();}
function setEvaluacion(code){window.location = '../expediente3.0/exp_evaluacion_risk.jsp?modeSec=view&mode=<%=mode%>&fg=<%=fg%>&seccion=<%=seccion%>&desc=<%=desc%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&cds=<%=cds%>&id='+code;}
function add(){window.location = '../expediente3.0/exp_evaluacion_risk.jsp?modeSec=add&mode=<%=mode%>&fg=<%=fg%>&seccion=<%=seccion%>&pacId=<%=pacId%>&desc=<%=desc%>&noAdmision=<%=noAdmision%>&cds=<%=cds%>&id=0';}
