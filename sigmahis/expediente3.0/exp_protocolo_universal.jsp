...
function getAneste(){var medico=eval('document.form0.anestesiologo').value;var medDesc ='';if(medico!=undefined && medico !=''){medDesc=getDBData('<%=request.getContextPath()%>','primer_nombre||decode(segundo_nombre,null,\'\',\' \'||segundo_nombre)||\' \'||primer_apellido||decode(segundo_apellido,null,\'\',\' \'||segundo_apellido)||decode(sexo,\'F\',decode(apellido_de_casada,null,\'\',\' \'||apellido_de_casada))','tbl_adm_medico','codigo=\''+medico+'\'','');if(medDesc!=''){eval('document.form0.nombre_anestesiologo').value=medDesc;}else{alert('El Codigo no Existe Verifique!');eval('document.form0.anestesiologo').value='';eval('document.form0.anestesiologo').focus();eval('document.form0.nombre_anestesiologo').value ='';}}}
function getPediatra(){var medico=eval('document.form0.pediatra').value;var medDesc ='';if(medico!=undefined && medico !=''){medDesc=getDBData('<%=request.getContextPath()%>','primer_nombre||decode(segundo_nombre,null,\'\',\' \'||segundo_nombre)||\' \'||primer_apellido||decode(segundo_apellido,null,\'\',\' \'||segundo_apellido)||decode(sexo,\'F\',decode(apellido_de_casada,null,\'\',\' \'||apellido_de_casada))','tbl_adm_medico','codigo=\''+medico+'\'','');if(medDesc!=''){eval('document.form0.nombre_pediatra').value=medDesc;}else{alert('El Codigo no Existe Verifique!');eval('document.form0.pediatra').value='';eval('document.form0.pediatra').focus();eval('document.form0.nombre_pediatra').value ='';}}}
function medicoList(){abrir_ventana1('../common/search_medico.jsp?fp=pUniversal');}
function instrumentistaList(){abrir_ventana1('../common/search_medico.jsp?fp=pUniversalIns');}
...
function setEvaluacion(code){window.location = '../expediente3.0/exp_protocolo_universal.jsp?modeSec=view&mode=<%=mode%>&seccion=<%=seccion%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&id='+code;}
function add(){window.location = '../expediente3.0/exp_protocolo_universal.jsp?modeSec=add&mode=<%=mode%>&seccion=<%=seccion%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&id=0';}
</script>
<style type="text/css">
<!--
.style1 {color: #000000}
...
<tr class="TextRow01">
				 ...
				 <%=fb.hidden("baction","")%>
						<tr class="TextRow02">
							...
							<td align="right"><%if(!mode.trim().equals("view")){%><a href="javascript:add()" class="Link00">[ <cellbytelabel id="2">Agregar Evaluaciones</cellbytelabel> ]</a><%}%></td>
						</tr>
...
<%=fb.hidden("aplicar11",prop.getProperty("aplicar11"))%>
		...
		<tr class="TextRow01">
			...
			<%//=fb.checkbox("aplicar6","S",(prop.getProperty("aplicar6").equalsIgnoreCase("S")),viewMode,null,null,"")%></td>
		</tr>
		<tr class="TextRow02">
		...
		<tr class="TextRow01">
			...
			<td colspan="2"><%//=fb.checkbox("aplicar16","S",(prop.getProperty("aplicar16").equalsIgnoreCase("S")),viewMode,null,null,"")%>
											...
											<%=fb.radio("aplicar16","NA",(prop.getProperty("aplicar16").equalsIgnoreCase("NA")),viewMode,false)%>N/A</td>
		...
		<tr class="TextRow02">
			...
			<td colspan="2"><%//=fb.checkbox("aplicar17","S",(prop.getProperty("aplicar17").equalsIgnoreCase("S")),viewMode,null,null,"")%>
											...
											<%=fb.radio("aplicar17","NA",(prop.getProperty("aplicar17").equalsIgnoreCase("NA")),viewMode,false)%>N/A</td>
</td>
		</tr>
		<tr class="TextRow01">
			...
			<td colspan="2"><%//=fb.checkbox("aplicar18","S",(prop.getProperty("aplicar18").equalsIgnoreCase("S")),viewMode,null,null,"")%>
											...
											<%=fb.radio("aplicar18","NA",(prop.getProperty("aplicar18").equalsIgnoreCase("NA")),viewMode,false)%>N/A</td>
		...
		<tr class="TextRow01">
			...
			<td colspan="6">
					<%=fb.textBox("cod_medico",prop.getProperty("cod_medico"),false,false,false,10,null,null,"onChange=\"javascript:getMedico()\"")%>
					<%=fb.textBox("nombre_medico",prop.getProperty("nombre_medico"),false,viewMode,true,60)%>
					<%=fb.button("procedimiento","...",true,(viewMode),null,null,"onClick=\"javascript:medicoList()\"","seleccionar Medico")%></td>
		</tr>
		<tr class="TextRow01">
			<td colspan="2"><cellbytelabel id="45">Anestesi&oacute;logo</cellbytelabel></td>
			<td colspan="6">
					<%=fb.textBox("anestesiologo",prop.getProperty("anestesiologo"),false,false,false,10,null,null,"onChange=\"javascript:getAneste()\"")%>
		...
		<tr class="TextRow01">
			...
			<td colspan="6">
					<%=fb.textBox("pediatra",prop.getProperty("pediatra"),false,false,false,10,null,null,"onChange=\"javascript:getPediatra()\"")%>
		...
		<tr class="TextRow01">
			...
			<td colspan="6">
					<%=fb.textBox("instrumentista",prop.getProperty("instrumentista"),false,false,false,10,null,null,"onChange=\"javascript:getPediatra()\"")%>
					<%=fb.textBox("nombre_instrumentista",prop.getProperty("nombre_instrumentista"),false,viewMode,true,60)%>
					<%=fb.button("pediat","...",true,(viewMode),null,null,"onClick=\"javascript:instrumentistaList()\"","seleccionar Instrumentista")%></td>
		</tr>
		<tr class="TextRow01">
...
else
{
	...
	prop.setProperty("alergias",request.getParameter("alergias"));
