...
sbSql.append(session.getAttribute("_companyId"));
sbSql.append(",'SAL_TOMADOR_TRIAGE'),'-') as tomador_triage");
...
//System.out.println("modeSec ========"+modeSec+" **** fp ==="+fp+"   fg =*************************************   fg =============="+fg+"    fc ========================="+fc);
	...
	if (fp.equalsIgnoreCase("agregar")||modeSec.trim().equals("add")) sql +=" and to_date(decode(observacion,'CONNEX',fecha,fecha_creacion),'dd/mm/yyyy hh12:mi:ss am') = to_date('"+cDateTime+"','dd/mm/yyyy hh12:mi:ss am') ";
		...
		if (!fg.equalsIgnoreCase("SV") && fc != null && !fc.trim().equals("")) {
	...
	if(fp.trim().equals("agregar")) sql += " and a.status = 'A' ";
	sql += " order by a.orden ";
	System.out.println("-------------->SQL TRIAGE ESI="+sql);
	...
	if (fg.trim().equalsIgnoreCase("TSV") && (cdoParam.getColValue("solo_un_triage","-").equals("S")||cdoParam.getColValue("solo_un_triage","-").equals("Y"))) {
		int tot = CmnMgr.getCount("select * from tbl_sal_signo_paciente where tipo_persona = 'T' and pac_id = "+pacId+" and secuencia = "+noAdmision+" and status = 'A'");
		onlyOneTriage = tot > 0;
	}
%>
...
function add(){window.location = '../expediente3.0/exp_triage_esi.jsp?modeSec=add&mode=<%=mode%>&fg=<%=fg%>&seccion=<%=seccion%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&fp=agregar&desc=<%=desc%>&seccion=<%=seccion%>';}
...
function setDefault(){}
function setTriage1(h){var fecha = eval('document.form0.fechaR'+h).value ;var hora = eval('document.form0.horaR'+h).value ;var fc = eval('document.form0.fc'+h).value ;window.location = '../expediente3.0/exp_triage_esi.jsp?modeSec=view&mode=<%=mode%>&seccion=<%=seccion%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&fc='+fc+'&fg=<%=fg%>&desc=<%=desc%>&fp=view';}
...
function chkViajado(obj){
	console.log("viajado", obj.value);
	enableDisable('lugar_viaje', obj.value);
}
</script>
</head>
...
<tr>
	...
	<div id="detListado" class="child">
		...
		<tr class="TextRow02">
			...
			<%if(!fp.trim().equals("agregar")){%><a class="Link00" href="javascript:printExp()">[ <cellbytelabel>Imprimir</cellbytelabel> ]</a><%}%>
			</td>
		</tr>
...
&nbsp;&nbsp;Observación:&nbsp;<%=fb.textarea("vomitoObs", sp.getVomitoObs(), false, false, viewMode, 0, 1, "", "width:75%", "")%></td></td>
				...
				<td width="35%" colspan="3"><%=fb.select("dolor","N=No,S=Si",sp.getDolor(),false,viewMode,0,null,null,"onChange=\"javascript:setEscala(this.value)\"")%>
...
%>
						<tr class="TextRow03">
							<td colspan="6">
							Usted ha viajado en las &uacute;ltimas 2 semanas?
							<%=fb.radio("viajado","S",(sp.getViajado().equals("S")),false,false, "", "", "onClick='javascript:chkViajado(this)'")%><cellbytelabel>S&iacute;</cellbytelabel>
							<%=fb.radio("viajado","N",(sp.getViajado().equals("N")),false,false, "", "", "onClick='javascript:chkViajado(this)'")%><cellbytelabel>No</cellbytelabel>
							<%=fb.textBox("lugar_viaje", sp.getLugarViaje(), false, false, (sp.getViajado()!=null && sp.getViajado().equals("N")), 50,100,"slider",null,null,null,false, "")%>
							</td>
						</tr>
						</table>
					</td>
...
else
{
	...
	if(request.getParameter("viajado")!=null && !request.getParameter("viajado").equals("")){
		spa.setViajado(request.getParameter("viajado"));
		if(spa.getViajado().equals("S")){
			spa.setLugarViaje(request.getParameter("lugar_viaje"));
		} else spa.setLugarViaje("");
	}
