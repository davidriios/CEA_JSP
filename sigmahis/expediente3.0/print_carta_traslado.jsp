...
if (desc == null) desc = "";
	...
	sql.append("select to_char(a.fecha_creacion,'dd/mm/yyyy hh12:mi:ss am') as fecha_solicitud, d.descripcion||decode(es_otro,'Y',' : '||otras_condiciones) as cond , t.transferido_a transferido_a, '' telefonos, '' direccion, t.acompaniante_transf, p.descripcion as parentesco, t.coordinador, t.personal_translado, t.proveedor, to_char(t.fecha_seguimiento,'dd/mm/yyyy') fecha_seguimiento, to_char(t.hora_seguimiento,'hh12:mi:ss am') hora_seguimiento, t.condicion_seguimiento, (select descripcion from tbl_cds_centro_servicio where codigo = t.area_coordinador and rownum = 1) area_coordinador from tbl_sal_adm_salida_datos a , tbl_sal_cons_incap_transf t,tbl_sal_transferencia_params d, tbl_pla_parentesco p where a.pac_id= t.pac_id and a.secuencia=t.admision and t.cond_transf = d.id and t.tipo = 'T' and parentesco_acompaniante = p.codigo(+) and a.pac_id = ");
	sql.append(pacId);
	...
	sql.append("select distinct a.* from (select to_char(rn.fecha_nota,'dd/mm/yyyy') as fechanota, to_char(rn.hora,'hh12:mi:ss am') as hora, nvl(to_char(rn.fecha,'dd/mm/yyyy'),' ') as fecha, nvl(to_char(rn.hora_r,'hh12:mi am'),' ') as horar, nvl(rn.temperatura,' ') as temperatura, nvl(rn.p_arterial,' ') as parterial, nvl(rn.respiracion,' ') as respiracion, to_char(rn.fecha_creacion,'dd/mm/yyyy hh12:mi:ss am') as fechacreacion, rn.estado,ne.estado estadonota,to_date(to_char(rn.fecha_nota,'dd/mm/yyyy'),'dd/mm/yyyy') as fecha_orden, to_date(to_char(rn.hora,'hh12:mi:ss am'),'hh12:mi:ss am') as hora_orden, rn.fecha as fecha_det, to_date(to_char(rn.hora_r,'hh12:mi:ss am'),'hh12:mi:ss am') as hora_det,rn.fcard, rn.pcard, rn.peso, rn.talla from tbl_sal_resultado_nota rn, tbl_sal_notas_enfermeria ne where rn.pac_id=");
	sql.append(pacId);
	sql.append(" and rn.secuencia=");
	sql.append(noAdmision);
	sql.append(" and rn.pac_id=ne.pac_id and rn.secuencia=ne.secuencia and rn.fecha_nota=ne.fecha and to_date(to_char(rn.hora,'hh12:mi:ss am'),'hh12:mi:ss am')=to_date(to_char(ne.hora,'hh12:mi:ss am'),'hh12:mi:ss am') and ne.no_hemodialisis is null /*and rn.fecha_creacion >= (select fecha_transferencia from tbl_sal_cons_incap_transf where pac_id = rn.pac_id and admision = rn.secuencia and tipo = 'T' and rownum = 1)*/	) a order by a.fecha_orden desc, a.hora_orden desc, a.fecha_det, a.hora_det");
	...
	//table body
		...
		for (int sv = 0; sv<alS.size(); sv++){
		   cdoX = (CommonDataObject)alS.get(sv);
		    pc.addCols(cdoX.getColValue("parterial"),1,1);
			pc.addCols(cdoX.getColValue("fcard"),1,1);
			pc.addCols(cdoX.getColValue("respiracion"),1,1);
			pc.addCols(cdoX.getColValue("temperatura"),1,1);
			pc.addCols("",1,6);
		}
