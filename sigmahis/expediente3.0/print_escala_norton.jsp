	...
	for (int i=0; i<al.size(); i++)
	{
		...
		{ // groupBy
			    if (i != 0)
				 ...
				 pc.addCols("#: "+cdo.getColValue("id"),0,dHeader.size());
				...
				if(!fg.trim().equals("SG")&&!fg.trim().equals("BR"))pc.addCols("Menos de 12 es paciente de alto riesgo  ",0,dHeader.size());
				else if(fg.trim().equals("BR"))pc.addCols("Menos de 16 es paciente de alto riesgo  ",0,dHeader.size());
		...
		pc.createInnerTable();
			...
			for (int j=0; j<al2.size(); j++)
			{
				...
				if(j==al2.size()-1)
				{
					...
					if (fg.trim().equals("NO")){	
						...
						if (total < 16 )pc.addCols("ALTO RIESGO",0,1,Color.red);
						else if (total>=13&&total<=14) pc.addCols("MODERADO RIESGO",0,1,Color.orange);
						else if (total>=16&&total<=18) pc.addCols("BAJO RIESGO",0,1,Color.green);
						else if (total>=18) pc.addCols("NO RIESGO",0,1,Color.blue);
					 }
					else pc.addCols(" ",0,1);
			...
			ArrayList alInt = SQLMgr.getDataList("select id.cod_intervencion, ipd.id_escala, id.codigo, id.descripcion , id.mostrar_checkbox, decode(ipd.cod_interv_det,null,'N','S') aplicado,(SELECT ip.OBSERVACION FROM TBL_SAL_INTERVENCION_PACIENTE ip WHERE ip.PAC_ID = ipd.pac_id AND ip.ADMISION = ipd.ADMISION AND ipd.COD_INTERVENCION = ip.COD_INTERVENCION AND IPD.ID_ESCALA = ip.ID_ESCALA AND IPD.TIPO = ip.TIPO ) AS OBSERVACION, (SELECT i.descripcion from TBL_SAL_INTERVENCION i WHERE i.TIPO = ipd.TIPO AND i.CODIGO = ipd.COD_INTERVENCION) intervencion_desc from tbl_sal_intervencion_det id, tbl_sal_intervencion_pac_det ipd where /*id.cod_intervencion = "+intCode+" and*/ id.cod_intervencion = ipd.cod_intervencion and id.codigo = ipd.cod_interv_det and ipd.pac_id = "+pacId+" and ipd.admision = "+noAdmision+" and ipd.id_escala = "+cdo.getColValue("id")+" and ipd.tipo = '"+fg+"' and ipd.cod_interv_det is not null order by id.cod_intervencion, ipd.id_escala, id.CODIGO ");
			pc.setFont(10,1);
			pc.setVAlignment(0);
			...
			if (alInt.size() > 0) {
				pc.setFont(10,1);
				pc.addCols(" ",0,dHeader.size());
				pc.addCols("INTERVENCIONES",1,dHeader.size());
			}

			for (int it = 0; it<alInt.size(); it++) {
			  CommonDataObject cdoIt = (CommonDataObject) alInt.get(it);
			  
			  if (!gInt.equals(cdoIt.getColValue("cod_intervencion") + "-" + cdoIt.getColValue("id_escala"))) {
				observ = cdoIt.getColValue("observacion"," ");	
				
				pc.addCols(" ",0,dHeader.size());
				pc.setFont(10,1);
				pc.addCols(cdoIt.getColValue("intervencion_desc"),0,dHeader.size()); 
				pc.addCols("   Observación: "+cdoIt.getColValue("observacion"," "),0,dHeader.size());

				gInt = "";
			  }
			  
			  pc.setFont(10,0);
			  pc.addCols(cdoIt.getColValue("descripcion"),0,dHeader.size());

              gInt = cdoIt.getColValue("cod_intervencion") + "-" + cdoIt.getColValue("id_escala");		  
			}
