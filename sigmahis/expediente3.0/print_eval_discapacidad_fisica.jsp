...
tblMain.addElement("3");
tblMain.addElement("3");
tblMain.addElement("40");
...
sql = "select a.codigo as codigo, a.descripcion as descripcion, b.observacion as observacion , nvl(b.seleccionar,'N') as seleccionar, to_char(b.fecha_up,'dd/mm/yyyy') as fechaUp, b.cod_eval as codEval , (select h.observacion from tbl_sal_eval_discapacidad h where b.pac_id = h.pac_id and b.secuencia = h.secuencia and b.fecha_up = h.fecha) as observ_header from tbl_sal_eval_discapacidades a, tbl_sal_eval_discapa_det b where a.codigo=b.cod_eval(+) and b.pac_id(+)= "+pacId+" and b.secuencia(+) = "+noAdmision+" and to_date(to_char(fecha_up(+),'dd/mm/yyyy'),'dd/mm/yyyy') = to_date('"+filter+"','dd/mm/yyyy') ORDER BY a.codigo";
...
for (int i=1; i<=al.size(); i++){
    ...
    if (i == 1) {
      ...
      pc.addCols(cdo.getColValue("observ_header"),0,3);
