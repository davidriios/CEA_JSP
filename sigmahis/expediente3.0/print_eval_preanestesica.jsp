...
+" c.cod_anestesiologo as codAnestesiologo, nvl(c.nombre_anestesiologo, (SELECT '['||c.cod_anestesiologo||'] '||a.primer_nombre||decode(a.segundo_nombre,null,'',' '||a.segundo_nombre) ||a.primer_apellido||decode(a.segundo_apellido,null,'',' '||a.segundo_apellido)||decode(a.sexo,'F',decode(a.apellido_de_casada,null,'',' '||a.apellido_de_casada)) FROM TBL_ADM_MEDICO a WHERE a.codigo = c.cod_anestesiologo)) as nombreAnestesiologo, nvl((select '['||c.procedimiento||'] '||decode(observacion , null , descripcion,observacion) as descripcion FROM tbl_cds_procedimiento where codigo = c.procedimiento), c.procedimiento) as procedimiento, nvl((SELECT '['||c.cirujano||'] '||a.primer_nombre||decode(a.segundo_nombre,null,'',' '||a.segundo_nombre) ||a.primer_apellido||decode(a.segundo_apellido,null,'',' '||a.segundo_apellido)||decode(a.sexo,'F',decode(a.apellido_de_casada,null,'',' '||a.apellido_de_casada)) FROM TBL_ADM_MEDICO a WHERE a.codigo = c.cirujano),c.cirujano) as cirujano, to_char(c.fecha,'dd/mm/yyyy') as fechaEvaluacion, to_char(c.fecha,'hh12:mi am') as hora, get_idoneidad(usuario_creacion, 1) usuario_creacion, get_idoneidad(usuario_modif, 1)usuario_modificacion, to_char(FECHA_CREACION, 'dd/mm/yyyy hh12:mi am') fecha_creacion, to_char(FECHA_MODIF , 'dd/mm/yyyy hh12:mi am') fecha_modificacion "
...
//{
	...
	{//for1
      ...
      if (!groupBy.trim().equalsIgnoreCase(cdo.getColValue("codEval"))){
		...
		pc.addCols(" Eval. #: "+cdo.getColValue("codEval"," "),0,dHeader.size());
		pc.addCols(" ",0,dHeader.size());
		
		pc.addCols("Fecha Creac.: "+cdo.getColValue("fecha_creacion"," "),0,3);
		pc.addCols("Usuario: "+cdo.getColValue("usuario_creacion"," "),0,3);
		if (!cdo.getColValue("usuario_modificacion"," ").trim().equals("")) {
			pc.addCols("Fecha Modif.: "+cdo.getColValue("fecha_modificacion"," "),0,3);
			pc.addCols("Usuario: "+cdo.getColValue("usuario_modificacion"," "),0,3);
		}
		pc.addCols(" ",0,dHeader.size());
		...
		for (int j=0; j<al2.size(); j++){
		    ...
		    {//if 2
		  	   pc.addBorderCols(cdo2.getColValue("descripcion"),0,3);
			   ...
			   if (cdo2.getColValue("respuesta").trim().equalsIgnoreCase("N"))
			        ...
			        pc.addBorderCols(cdo2.getColValue("observacion"),0,1);
		    } else
		    {
	    	  pc.addBorderCols(cdo2.getColValue("descripcion")+" :",0,2);
