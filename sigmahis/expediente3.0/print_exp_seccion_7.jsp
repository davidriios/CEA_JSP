...
sql="SELECT a.codigo AS cod_antecedente, a.descripcion, nvl(b.valor,' ') AS valor, nvl(b.observacion,' ') as observacion, get_idoneidad(usuario_creacion, 1) usuario_creacion, get_idoneidad(usuario_modificacion, 1) usuario_modificacion, to_char(FECHA_CREACION, 'dd/mm/yyyy hh12:mi am') fecha_creacion, to_char(FECHA_MODIFICACION , 'dd/mm/yyyy hh12:mi am') fecha_modificacion from TBL_SAL_DIAGNOSTICO_FAMILIAR a, TBL_SAL_ANTECEDENTE_FAMILIAR b where a.CODIGO=b.ANTECEDENTE AND b.PAC_ID = "+pacId+" and nvl(b.admision,"+noAdmision+") = "+noAdmision+" order by a.orden"; /*a.codigo desc*/
