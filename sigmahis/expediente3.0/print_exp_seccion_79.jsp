...
cdop = SQLMgr.getPacData(pacId, noAdmision);
 ...
 if ( !id.equals("0") ){
  ...
  sql = "select id, to_char(fecha,'dd/mm/yyyy hh12:mi:ss am') fechaOrden,codigo,escala,valor,insulina, nvl(tipo,0) tipo_insulina, decode(tipo,1,'RAPIDA',2,'LENTA') tipo_insulina_desc,  (select descripcion from tbl_sal_via_admin where codigo = via) as via from tbl_sal_esquema_insulina where  pac_id="+pacId+" and admision="+noAdmision+appendFilter;
...
//{
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		if ( !groupByid.trim().equals(cdo.getColValue("id")) ){
            ...
            pc.addBorderCols("VIA ADMINISTRACION: "+cdo.getColValue("via", " "),0,2,Color.gray);
            
            pc.addBorderCols("Obs.: "+cdo.getColValue("insulina"),0,4,Color.gray);
