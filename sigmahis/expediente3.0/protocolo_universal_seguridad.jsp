...
{
window.location = '../expediente3.0/protocolo_universal_seguridad.jsp?mode=view&fg=<%=fg%>&seccion=<%=seccion%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&desc=<%=desc%>&cds=<%=cds%>&id='+code;
}
function add()
{
window.location = '../expediente3.0/protocolo_universal_seguridad.jsp?mode=add&fg=<%=fg%>&seccion=<%=seccion%>&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&desc=<%=desc%>&cds=<%=cds%>&id=0';
}
...
<tr class="TextRow01">
				 ...
				 <%=fb.hidden("baction","")%>
						<tr class="TextRow02">
							...
							<%}%>
							<%if(!mode.trim().equals("view")){%><a href="javascript:add()" class="Link00">[ <cellbytelabel id="2"><cellbytelabel>Agregar Pausa Quir&uacute;gica</cellbytelabel></cellbytelabel> ]</a><%}%></td>
						</tr>
...
function editMode()
{
	window.location = '<%=request.getContextPath()+request.getServletPath()%>?seccion=<%=seccion%>&mode=edit&pacId=<%=pacId%>&noAdmision=<%=noAdmision%>&fg=<%=fg%>&id=<%=id%>&desc=<%=desc%>&cds=<%=cds%>';
}
</script>
</head>
