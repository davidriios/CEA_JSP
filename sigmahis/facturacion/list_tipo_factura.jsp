...
//if (!(SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"100005") || SecMgr.checkAccess(session.getId(),"100006") || SecMgr.checkAccess(session.getId(),"100007") || SecMgr.checkAccess(session.getId(),"100008"))) throw new Exception("Usted no tiene los suficientes Derechos de Acceso a esta página.");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
    ...
    searchType = "1";
    searchDisp = "Nombre";
		descrip    = request.getParameter("descripcion");   // utilizada para mantener la descripción del tipo de factura
	}
  ...
  else
  {
	...
	sql = "select codigo, descripcion from tbl_fac_tipo_factura"+appendFilter+" order by descripcion";
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) from tbl_fac_tipo_factura"+appendFilter);
...
document.title = 'Tipo de Factura - '+document.title;

function add()
{
	abrir_ventana('../facturacion/reg_tipo_factura.jsp');
}

function edit(id)
{
	abrir_ventana('../facturacion/reg_tipo_factura.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../facturacion/print_list_tipo_factura.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">&nbsp;
<%
//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"100007"))
//{
%>
			<a href="javascript:add()" class="Link00">[ <cellbytelabel>Registrar Nuevo Tipo de Factura</cellbytelabel> ]</a>
<%
//}
%>
		</td>
...
%>
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="center">
<%
//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"100008"))
//{
%>
