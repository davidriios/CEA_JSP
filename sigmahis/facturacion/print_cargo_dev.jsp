...
String tipoTransaccion = request.getParameter("tipoTransaccion");//utilizado cuando hace check al guardar cargo/devolucion
...
if(!articulo.equals("")){sbSqlFilter.append(" and upper(xx.descripcion) like '%");sbSqlFilter.append(articulo.toUpperCase());sbSqlFilter.append("%'");}
...
sbSql.append(" and a.codigo = b.fac_codigo and a.pac_id = b.pac_id and a.admi_secuencia = b.fac_secuencia and a.compania = b.compania and a.tipo_transaccion = b.tipo_transaccion and b.cantidad - nvl(b.difpaq_cantidad,0) <> 0");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	if (!isCurrTrx) {
		...
		pc.setNoColumnFixWidth(dHeader);
		pc.createTable("paquete",true,0,leftRightMargin * 2);
...
function filtrarImpresion(tipoServ, fDesde, fHasta, articulo){
	window.location = '<%=request.getContextPath()+request.getServletPath()%>?noSecuencia=<%=noSecuencia%>&citasSopAdm=<%=citasSopAdm%>&pacId=<%=pacId%>&tipoServ='+tipoServ+'&fDesde='+fDesde+'&fHasta='+fHasta+'&articulo='+articulo;
}
function buscar(){
...
var facCodigo = document.formPrinted.facCodigo.value;
var cdsFilter = document.formPrinted.cdsFilter.value;
window.location = '<%=request.getContextPath()+request.getServletPath()%>?noSecuencia=<%=noSecuencia%>&citasSopAdm=<%=citasSopAdm%>&pacId=<%=pacId%>&tipoServ='+tipo_servicio+'&fDesde='+fDesde+'&fHasta='+fHasta+'&articulo='+articulo+'&facCodigo='+facCodigo+'&cdsFilter='+cdsFilter;
}

function _printCargos(){
abrir_ventana('<%=request.getContextPath()+request.getServletPath()%>?noSecuencia=<%=noSecuencia%>&pacId=<%=pacId%>&trx=<%=facCodigo%>&printOF=S&cp&citasSopAdm=<%=citasSopAdm%>','printCurrTrx');
//openWin('<%=request.getContextPath()+request.getServletPath()%>?noSecuencia=<%=noSecuencia%>&pacId=<%=pacId%>&trx=<%=facCodigo%>&printOF=S&cp','printCurrTrx');
}
</script>
</head>
<body>
...
<tr class="TextRow02">
	<td align="center" class="TableBorder">
		Tipo de Servicio:
		<%=fb.select(ConMgr.getConnection(), "select codigo, descripcion from tbl_cds_tipo_servicio where compania = " + (String) session.getAttribute("_companyId"), "tipo_servicio", tipoServ, false, false, 0, "text10", "width:150px", "", "Tipo de Servicio", "T")%>
		Fecha de Cargo:
		...
		<jsp:param name="hintPos" value="left" />
		</jsp:include>
		Art&iacute;culo:
		<%=fb.textBox("articulo",articulo,false,false,false,20)%>
		Trn./Cargo:
		<%=fb.textBox("facCodigo",facCodigo,false,false,false,10)%>
		<%=fb.button("_filtrar","Filtrar",false,false,null,null,"onClick=\"javascript:buscar();\"")%>
		<%=(al.size() > 0 && !facCodigo.trim().equals(""))?fb.button("_print_cargos","Imprimir Cargos",false,false,null,null,"onClick=\"javascript:_printCargos();\""):""%>
