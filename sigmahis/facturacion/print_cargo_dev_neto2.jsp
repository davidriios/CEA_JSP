...
sbSql.append(" b.tipo_cargo, decode(b.centro_servicio,get_sec_comp_param(a.compania,'CDS_HON'),a.med_codigo,null) as med_codigo, a.empre_codigo, coalesce((select coalesce(observacion,descripcion) from tbl_cds_procedimiento where codigo = b.procedimiento),(select descripcion from tbl_fac_otros_cargos where compania = b.compania and codigo = b.otros_cargos),(select descripcion from tbl_cds_producto_x_cds where codigo = b.cds_producto and cod_centro_servicio = b.centro_servicio),(select bb.descripcion from tbl_sal_habitacion aa, tbl_cds_centro_servicio bb where aa.unidad_admin = bb.codigo and aa.codigo = b.habitacion and aa.compania = b.compania),(select descripcion from tbl_sal_uso where codigo = b.cod_uso and compania = b.compania),(select descripcion from tbl_inv_articulo where compania = b.compania and cod_articulo = b.inv_articulo),(select primer_nombre||' '||primer_apellido from tbl_adm_medico where codigo = a.med_codigo),(select nombre from tbl_adm_empresa where codigo = a.empre_codigo)) as descripcion, b.monto, nvl(b.recargo, 0) recargo, b.procedimiento, b.habitacion, b.cds_producto, b.cod_uso, b.otros_cargos, b.cod_paq_x_cds, b.art_familia || '-' || b.art_clase || '-' || b.inv_articulo articulo, sum(decode(b.tipo_transaccion,'D',-coalesce(b.difpaq_cantidad,b.cantidad),0)) cant_dev, sum(decode(b.tipo_transaccion,'H',coalesce(b.difpaq_cantidad,b.cantidad),0)) cant_hon, sum(decode(b.tipo_transaccion,'C',coalesce(b.difpaq_cantidad,b.cantidad),0)) cant_cargo, (sum(decode(b.tipo_transaccion,'H',coalesce(b.difpaq_cantidad,b.cantidad),0)) + sum(decode(b.tipo_transaccion,'C',coalesce(b.difpaq_cantidad,b.cantidad),0))+sum(decode(b.tipo_transaccion,'D',-coalesce(b.difpaq_cantidad,b.cantidad),0))) cantidad, to_char(max(decode(b.tipo_transaccion,'C',b.fecha_cargo,null)),'dd/mm/yyyy') fecha_cargo, max(decode(b.tipo_transaccion,'C',b.fecha_cargo,null)) f_cargo from tbl_fac_transaccion a, tbl_fac_detalle_transaccion b where a.pac_id = ");
sbSql.append(pacId);
...
sbSql.append(sbSqlFilter.toString());
sbSql.append(" and a.codigo = b.fac_codigo and a.pac_id = b.pac_id and a.admi_secuencia = b.fac_secuencia and a.compania = b.compania and a.tipo_transaccion = b.tipo_transaccion and (nvl(b.ref_type,'-') <> 'PAQ' or nvl(b.difpaq_cantidad,0) > 0) group by b.compania, ");
if(cdsDet.trim().equals("S"))sbSql.append(" b.centro_servicio, ");
else sbSql.append(" a.centro_servicio, ");
sbSql.append(" b.tipo_cargo, decode(b.centro_servicio,get_sec_comp_param(a.compania,'CDS_HON'),a.med_codigo,null), a.med_codigo, a.empre_codigo/*, b.descripcion*/, b.monto, nvl(b.recargo, 0), b.procedimiento, b.habitacion, b.cds_producto, b.cod_uso, b.otros_cargos, b.cod_paq_x_cds, b.art_familia, b.art_clase, b.inv_articulo/*, b.fecha_cargo*/");

sbSql.append(") x, tbl_cds_centro_servicio c where x.centro_servicio = c.codigo and x.cantidad != 0");

sbSql.append(") z order by 3, 2, 13 asc");//cds, tipo_transaccion, f_cargo
...
sbSql.append(" and a.codigo = b.fac_codigo and a.pac_id = b.pac_id and a.admi_secuencia = b.fac_secuencia and a.compania = b.compania and a.tipo_transaccion = b.tipo_transaccion group by b.compania, ");
if(cdsDet.trim().equals("S"))sbSql.append("b.centro_servicio");
else sbSql.append("a.centro_servicio");
...
sbSql.append(" and a.codigo = b.fac_codigo and a.pac_id = b.pac_id and a.admi_secuencia = b.fac_secuencia and a.tipo_transaccion = b.tipo_transaccion and b.tipo_cargo = '01' and (nvl(b.ref_type,'-') <> 'PAQ' or nvl(b.difpaq_cantidad,0) > 0) group by b.compania, b.centro_servicio, b.procedimiento, b.otros_cargos, b.cds_producto, b.habitacion, b.cod_uso, b.inv_articulo, a.med_codigo, a.empre_codigo order by 1,2");
