...
sbSql = new StringBuffer();
sbSql.append("select pd.id, pd.renglon, pd.descripcion, nvl(pd.cantidad,0) as cantidad, nvl(pd.descuento,0) as descuento, pd.tipo_des, pd.other1, pd.other2, usuario_creacion, to_char(fecha_creacion,'dd/mm/yyyy hh12:mi:ss am') as fecha_creacion, usuario_modificacion, to_char(fecha_modificacion,'dd/mm/yyyy hh12:mi:ss am') as fecha_modificacion, nvl(monto,0) as monto, nvl(total,0) as total, nvl(totalDesc,0) as totalDesc, nvl(impuesto,0) as impuesto, nvl(totalImp,0) as totalImp from tbl_fac_cotizacion_det pd where id = ");
sbSql.append(id);
sbSql.append("order by pd.renglon asc");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String title = (cdoEnc.getColValue("reg_type").trim().equals("COT"))?"PRESUPUESTO":"PAQUETE";
	...
	pc.createTable();
		...
		pc.addBorderCols("DESCRIPCION",1,4);
		pc.addBorderCols("CANTIDAD",1);
		pc.addBorderCols("MONTO",1);
		pc.addBorderCols("% DESC.",1);
		pc.addBorderCols("TOTAL DESC.",1,1);
		pc.addBorderCols("IMPUESTO",1);
		pc.addBorderCols("TOT. IMPUESTO",1);
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.addCols(CmnMgr.getFormattedDecimal("###,###,##0.00",cdo.getColValue("monto")),2,1);
		pc.addCols(CmnMgr.getFormattedDecimal("###,###,##0.00",cdo.getColValue("descuento")),2,1);
		pc.addCols(CmnMgr.getFormattedDecimal("###,###,##0.00",cdo.getColValue("totalDesc")),2,1);
		pc.addCols(CmnMgr.getFormattedDecimal("###,###,##0.00",cdo.getColValue("impuesto")),2,1);
		pc.addCols(CmnMgr.getFormattedDecimal("###,###,##0.00",cdo.getColValue("totalImp")),2,1);
		...
		totalDescuento += Double.parseDouble(cdo.getColValue("totalDesc"));
		totalItbms += Double.parseDouble(cdo.getColValue("totalImp"));
	...
	else
	{
		...
		pc.addBorderCols(" TOTALES:  ",0,5,0.0f,0.5f,0.0f,0.0f);
		pc.addBorderCols(" "+CmnMgr.getFormattedDecimal("###,###,##0.0000",total),2,1,0.0f,0.5f,0.0f,0.0f);
		pc.addBorderCols(" ",2,1,0.0f,0.5f,0.0f,0.0f);
		pc.addBorderCols(" "+CmnMgr.getFormattedDecimal("###,###,##0.0000",totalDescuento),2,1,0.0f,0.5f,0.0f,0.0f);
		pc.addBorderCols(" ",2,1,0.0f,0.5f,0.0f,0.0f);
		pc.addBorderCols(" "+CmnMgr.getFormattedDecimal("###,###,##0.0000",totalItbms),2,1,0.0f,0.5f,0.0f,0.0f);
		pc.addBorderCols(" "+CmnMgr.getFormattedDecimal("###,###,##0.0000",totalTot),2,1,0.0f,0.5f,0.0f,0.0f);
	}
