...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	footer.createTable();
		...
		footer.addCols("FACTURA POR COMPUTADORA AUTORIZADA SEGUN RESOLUCION NO. 201-1213 DEL 18 DE MAYO DE 2004",0,1);
		footer.addCols("TIMBRES QUE CORRESPONDEN AL PRESENTE DOCUMENTO SON PAGADOS POR DECLARACION SEGUN LEY 61 DEL 27 DE DIC. DE 2002",0,1);
		footerHeight = footer.getTableHeight();

	Vector vInvoice = new Vector();
		vInvoice.addElement(".12");
		vInvoice.addElement(".32");
	...
	Vector vContent = new Vector();
		...
		vContent.addElement(".60");
		vContent.addElement(".15");
		vContent.addElement(".10");

	Vector vTotal = new Vector();
		vTotal.addElement(".60");
		vTotal.addElement(".40");
	...
	for (int i=0; i<alFac.size(); i++)
	{
		...
		PdfCreator pc = new PdfCreator(directory+folderName+"/"+year+"/"+month+"/"+fileName, width, height, isLandscape, sbFooter.toString(), leftRightMargin, topMargin, bottomMargin, headerFooterFont, logoMark, logoPath, (fac.getColValue("estatus").equals("A")?true:false), statusPath, displayPageNo, pageNoFontSize, pageNoLabel, pageNoPoxX, pageNoPosY, footer.getTable());
...
/* * * * * * * * * * * * * * *   E N C A B E Z A D O   F A C T U R A   * * * * * * * * * * * * * * */
		...
		pc.createTable("invoice",false,0,leftRightMargin*2,width);
			...
			pc.setFont(headerFontSize,0);
			pc.addCols(fac.getColValue("poliza"),0,2);
			pc.setFont(headerFontSize,1);
			pc.addCols("CERTIFICADO:",0,1);
			pc.setFont(headerFontSize,0);
			...
			pc.setFont(headerFontSize,1);
			pc.addCols(fac.getColValue("doble_msg"),0,7);
...
/* * * * * * * * * * * * * * *   T O T A L E S   * * * * * * * * * * * * * * */
		...
		pc.createTable("total",false,15,0.0f,148.5f);
			pc.setFont(headerFontSize,1);
			pc.addCols("SUB-TOTAL",2,1);
			pc.setFont(headerFontSize,0);
...
System.out.println("------------"+facTotal.getColValue("aplica_copago")+">"+mPac+" -- COPAGO="+facTotal.getColValue("montoCopago"));
				...
				pc.setFont(headerFontSize,1);
				pc.addCols("MONTO PACIENTE",2,1);
				pc.setFont(headerFontSize,0);
				pc.addCols(CmnMgr.getFormattedDecimal(mPac),2,1);
			}

			pc.setFont(headerFontSize,1);
			pc.addCols("TOTAL FACTURA",2,1);
			pc.setFont(headerFontSize,0);
			/*if (fac.getColValue("facturar_a").equalsIgnoreCase("E")){
			totalFactura=Double.parseDouble(facTotal.getColValue("monto_total"))+Double.parseDouble(fac.getColValue("total_honorarios"));
			pc.addCols(CmnMgr.getFormattedDecimal(totalFactura),2,1);}
			else*/ pc.addCols(CmnMgr.getFormattedDecimal(facTotal.getColValue("monto_total")),2,1);
...
/* * * * * * * * * * * * * * *   G R A N   T O T A L   * * * * * * * * * * * * * * */
		pc.setNoColumnFixWidth(vTotal);
		pc.createTable("grand_total",false,15,0.0f,148.5f);
			pc.setVAlignment(2);
			pc.setFont(headerFontSize,1);
...
/* * * * * * * * * * * * * * *   O B S E R V A C I O N E S   * * * * * * * * * * * * * * */
		...
		/*	pc.addCols("GASTOS NO CUBIERTOS O NO ELEGIBLE",0,1);
			...
			//pc.addCols(fac.getColValue("doble_msg"),0,2);
		commentHeight = pc.getTableHeight();


/* * * * * * * * * * * * * * *   H O N O R A R I O S   * * * * * * * * * * * * * * */
		sbSql = new StringBuffer();
		sbSql.append("select coalesce((select nvl(reg_medico,codigo) from tbl_adm_medico where codigo=medico),to_char(med_empresa)) as cod_soc_med, centro_servicio, descripcion, ");
		//if (fac.getColValue("facturar_a").equalsIgnoreCase("E")) sbSql.append("sum(nvl(monto,0) + nvl(descuento,0) + nvl(descuento2,0))");

		//else sbSql.append("sum(nvl(monto,0) + nvl(descuento,0) + nvl(descuento2,0) + nvl(monto_paciente,0))");
		sbSql.append(" sum(nvl(monto,0) + nvl(descuento,0) + nvl(descuento2,0) + nvl(monto_paciente,0)) as monto_bruto, sum(nvl(monto,0))-nvl(getCopagoDet(compania,'");
		sbSql.append(fac.getColValue("codigo"));
		...
		//sbSql.append(" sum(nvl(descuento,0)) as monto_desc
		...
		pc.createTable("honorario",false,0,0.0f,425.5f);
			pc.setFont(headerFontSize,1);
			pc.addBorderCols("HONORARIOS",0,2);
			pc.addBorderCols("CARGO",1,1);
			pc.addBorderCols(((fac.getColValue("facturar_a").equalsIgnoreCase("E"))?"PAC. DED. + %":"DESCUENTO"),1,1);
		...
		for (int j=0; j<alHon.size(); j++)
		{
			...
			pc.setFont(contentFontSize,0);
			pc.addCols(hon.getColValue("cod_soc_med"),0,1);
			pc.addCols(hon.getColValue("descripcion"),0,1);
			...
			pc.addBorderCols("TOTALES...",2,2,0.0f,0.5f,0.0f,0.0f);
			pc.addBorderCols(CmnMgr.getFormattedDecimal(tBruto),2,1,0.0f,0.5f,0.0f,0.0f);
			pc.addBorderCols(CmnMgr.getFormattedDecimal(tDesc),2,1,0.0f,0.5f,0.0f,0.0f);
			pc.addBorderCols(CmnMgr.getFormattedDecimal(tNeto),2,1,0.0f,0.5f,0.0f,0.0f);
...
/* * * * * * * * * * * * * * *   D E T A L L E S   F A C T U R A   * * * * * * * * * * * * * * */
		...
		pc.createTable(true);
			...
			pc.setFont(headerFontSize,1);
			pc.resetVAlignment();
			pc.addBorderCols("C O D I G O",1,1,0.5f,0.5f,0.5f,0.5f);
			pc.addBorderCols("D E S C R I P C I O N",1,2,0.5f,0.5f,0.0f,0.5f);
			pc.addBorderCols("M O N T O",1,1,0.5f,0.5f,0.0f,0.5f);
		pc.setTableHeader(3);
		...
		//if (fac.getColValue("tipo_factura").equals("0"))
		{
			...
			*/
			sbSql.append("select z.cds, z.descripcion, sum(z.monto + z.descuento + z.descuento2 + z.monto_paciente) as monto from (select a.centro_servicio as cds, /*decode(a.tipo_cobertura,'CO','COPAGO',(select descripcion from tbl_cds_centro_servicio where codigo = a.centro_servicio)) as */descripcion, nvl(a.monto,0) as monto, nvl(a.descuento,0) as descuento, nvl(a.descuento2,0) as descuento2, nvl(a.monto_paciente,0) as monto_paciente from tbl_fac_detalle_factura a where a.fac_codigo = '");
			sbSql.append(fac.getColValue("codigo"));
			sbSql.append("' and a.compania = ");
			sbSql.append(fac.getColValue("compania"));
			sbSql.append(" and a.tipo = 'C' and a.imprimir_sino = 'S') z group by z.cds, z.descripcion");
		...
		for (int j=0; j<alDet.size(); j++)
		{
			...
			pc.setFont(contentFontSize,0);
			pc.addBorderCols(det.getColValue("cds"),0,1,0.0f,tBorder,0.5f,0.5f);
			pc.addBorderCols(det.getColValue("descripcion"),0,2,0.0f,tBorder,0.0f,0.5f);
			pc.addBorderCols(CmnMgr.getFormattedDecimal(det.getColValue("monto")),2,1,0.0f,tBorder,0.0f,0.5f);
		...
		if (alDet.size() == 0)
		{
			tempHeight = pc.getTableHeight();
			pc.setFont(contentFontSize,0);
			pc.addBorderCols(" ",0,1,0.0f,0.5f,0.5f,0.5f);
			pc.addBorderCols("No existen registros",0,2,0.0f,0.5f,0.0f,0.5f);
			pc.addBorderCols(" ",2,1,0.0f,0.5f,0.0f,0.5f);
			detailHeight = pc.getTableHeight() - tempHeight;
			tDetailHeight += detailHeight;
			if (tDetailHeight > availHeight)
			{
...
/* * * * * * * * * * * * * * *   R E L L E N O   * * * * * * * * * * * * * * */
			...
			pc.addBorderCols(" ",0,1,0.5f,0.0f,0.5f,0.5f,detailHeight);
			pc.addBorderCols(" ",0,2,0.5f,0.0f,0.0f,0.5f);
