...
sql =  "SELECT a.COMPANIA, a.CODIGO, a.EXPLICACION,to_char(a.fecha,'dd/mm/yyyy')as fecha,decode(a.TIPO_DOC,'F', 'FACTURA', 'R','RECIBO') as TIPO_DOC,a.TIPO_AJUSTE, a.RECIBO, a.TOTAL, b.descripcion "
+ " FROM TBL_FAC_NOTA_AJUSTE a, tbl_fac_tipo_ajuste b "
+ " where  a.tipo_ajuste=b.codigo and a.compania=b.compania and a.compania="+ (String) session.getAttribute("_companyId")+" "+appendFilter
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_notas_ajustes";
...
for (int j=1; j<=nPages; j++)
{
			Vector setHeader0=new Vector();
				...
				pc.createTable();
				pc.setFont(9, 1);
				  pc.addCols("Facturación",1,1);
					pc.addCols("TRANSACCIONES NOTAS DE AJUSTES", 1, 1);
				pc.addTable();
...
//				pc.addTable();
				...
				pc.createTable();
					...
					pc.addBorderCols("Recibo",0);
					pc.addBorderCols("Monto",1);
				pc.addTable();

			//	 pc.createTable();
			//		pc.setFont(7, 1);
			//		pc.addCols("Área: "+repCode,0,1);
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								pc.setFont(7, 0);
									...
									pc.addCols(" "+cdo1.getColValue("tipo_doc"),0,1);
									pc.addCols(" "+cdo1.getColValue("descripcion"),0,1);
									pc.addCols(" "+cdo1.getColValue("explicacion"),0,1);
