...
com = (Company) sbb.getSingleRowBean(ConMgr.getConnection(),sql,Company.class);
		  ...
		  sql = "SELECT a.tipo_servicio as tipoServCode, b.descripcion as tipoServ, a.categoria as catCode, c.descripcion as cat, a.cds_codigo as centroCode, d.descripcion as centro, e.descripcion as tipo,a.desc_individual,a.monto  FROM tbl_fac_porcentaje_desc a, tbl_cds_tipo_servicio b, tbl_adm_categoria_admision c, tbl_cds_centro_servicio d, tbl_fac_tipo_descuento e WHERE a.tipo_servicio=b.codigo and a.categoria=c.codigo and a.cds_codigo=d.codigo and a.tipo_descuento=e.codigo and a.compania=e.compania and a.compania="+ (String) session.getAttribute("_companyId")+appendFilter+" and a.tipo_descuento="+tipoCode+" order by d.descripcion, b.descripcion, c.descripcion";
al = SQLMgr.getDataList(sql);
 	...
 	if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_tipodescPorcentaje";
...
int no2 = 0;
			for (int j=1; j<=nPages; j++)
			{
				...
				Vector setValD=new Vector();
					...
					setValD.addElement(".37");
					setValD.addElement(".16");
					setValD.addElement(".14");
				...
				pc.createTable();
					pc.setFont(9, 1);
					pc.addCols("PORCENTAJES - CENTRO DE SERVICIO",1,2);
					pc.addTable();
				pc.createTable();
			        pc.setFont(7, 1);
				...
				pc.createTable();
					...
					pc.addBorderCols("Desc. Individual",1);
				...
				pc.addTable();
					...
					pc.createTable();
						pc.setFont(7, 1);
						pc.addCols("No Existen Porcentajes Registrados.",1,2);
					pc.addTable();
					...
					for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
					{
							...
							pc.createTable();
							pc.setFont(7, 1);
							pc.addCols(" "+cdo1.getColValue("centro"), 0,1);
							pc.addCols(" "+cdo1.getColValue("tipoServ"), 0,1);
							pc.addCols(" "+cdo1.getColValue("cat"), 0,1);
							pc.addCols(" "+cdo1.getColValue("desc_individual"), 1,1);
							pc.addCols(" "+cdo1.getColValue("monto"), 1,1);
							pc.addTable();
							if ((i + 1) == nItems) break;
					}//End For
					if((no2+2)<=maxLines)
