...
com = (Company) sbb.getSingleRowBean(ConMgr.getConnection(),sql,Company.class);
		   ...
		   sql = "SELECT codigo, descripcion, decode(activo_inactivo,'A','Activo','I','Inactivo') as estado,porcentaje_recargo as recargo,impuesto FROM tbl_fac_tipo_cliente where compania="+ (String) session.getAttribute("_companyId")+appendFilter+" order by descripcion";
al = SQLMgr.getDataList(sql);
 	...
 	if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_tipoClientes";
...
int no2 = 0;
			for (int j=1; j<=nPages; j++)
			{
				...
				pc.addTable();
					...
					pc.createTable();
						pc.setFont(7, 1);
						pc.addCols("No Existen Clientes Registrados.",1,2);
					pc.addTable();
					...
					else
					{
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
							...
							pc.addCols(" "+cdo1.getColValue("recargo"), 1,1);
							pc.addCols(" "+cdo1.getColValue("impuesto"), 1,1);
