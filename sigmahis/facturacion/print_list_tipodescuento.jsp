...
com = (Company) sbb.getSingleRowBean(ConMgr.getConnection(),sql,Company.class);
		 ...
		 sql = "SELECT codigo, descripcion, cta1||'-'||cta2||'-'||cta3||'-'||cta4||'-'||cta5||'-'||cta6 as cuenta, valor, decode(tipo_valor,null,' ','P','PORCENTAJE','M','MONEDA',tipo_valor) as tipo FROM tbl_fac_tipo_descuento WHERE compania="+(String) session.getAttribute("_companyId")+appendFilter+" order by descripcion";
al = SQLMgr.getDataList(sql);
 	...
 	if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_tipodescuento";
...
int no2 = 0;
			for (int j=1; j<=nPages; j++)
			{
				...
				pc.createTable();
					pc.setFont(9, 1);
					pc.addCols("TIPO DE DESCUENTOS",1,2);
					pc.addTable();
				pc.createTable();
			        pc.setFont(7, 1);
				...
				pc.addTable();
					...
					pc.createTable();
						pc.setFont(7, 1);
						pc.addCols("No Existen Tipo de Descuentos Registrados.",1,2);
					pc.addTable();
					...
					else
					{
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
							...
							pc.addCols(" "+cdo1.getColValue("cuenta"), 1,1);
							pc.addCols(" "+cdo1.getColValue("valor"), 1,1);
