...
{
	sql = "select get_sec_comp_param("+(String) session.getAttribute("_companyId")+",'COD_TIPO_SERV_HON') as param_value,nvl(get_sec_comp_param("+(String) session.getAttribute("_companyId")+",'CHECK_DISP'),'S')as valida_dsp, nvl(get_sec_comp_param("+(String) session.getAttribute("_companyId")+",'CHECK_MONTO_HON'),'S')as valida_monto_hon from dual";
	cdoHon = SQLMgr.getData(sql);
	if(cdoHon!=null && !cdoHon.getColValue("param_value").equals("")) codTSHon = cdoHon.getColValue("param_value");
	if(cdoHon ==null){cdoHon =new CommonDataObject();cdoHon.addColValue("valida_dsp","S");cdoHon.addColValue("valida_monto_hon","S");}
...
document.form1.tipoTransaccion.value =parent.document.form0.tipoTransaccion.value;
	...
	var codProv   = ""
	var codHonorario = '';
	var noDoc =  parent.document.form0.noDocumento.value;
	...
	abrir_ventana1('../common/sel_servicios_x_centro.jsp?mode=<%=mode%>&fg='+fg+'&fp=cargo_dev_pac&cs='+cs+'&tipo_cds='+tipo_cds+'&reporta_a='+reporta_a+'&v_empresa='+empresa+'&edad='+edad+'&clasificacion='+clasif+'&incremento='+inc+'&tipoInc='+tipoInc+'&tipoTransaccion='+tipoTrans+'&cat='+cat+'&admiSecuencia='+admSec+'&fechaNac='+fechaNac+'&codPaciente='+codPac+'&codProv='+codProv+'&pacId='+pacId+'&almacen='+almacen+'&codHonorario='+codHonorario+'&noDoc='+noDoc);}
	<%
	} else if(type!=null && type.equals("2")){
	%>
	if(tipoTrans=='D') parent.searchBoletaList();
	<%
	}
	%>
	calc();
...
%>
	...
	if (isNaN(document.form1.cantidad<%=i%>.value) || ((document.form1.cantidad<%=i%>.value != '') && ((parseFloat(document.form1.cantidad<%=i%>.value) % 1) != 0))){
		...
		document.form1.cantidad<%=i%>.select();
		return false;
	<%if(fg.equals("HON")&& cdoHon.getColValue("valida_monto_hon").trim().equals("S")){%>
	} else if (parseFloat(document.form1.monto<%=i%>.value) == 0.00 && action=='Guardar'){
		iCounter++;
		top.CBMSG.warning('El monto no puede ser igual a 0!');
		document.form1.monto<%=i%>.select();
		return false;
	<%}%>	
	}
	if (parseInt(document.form1.compSize<%=i%>.value,10) == 0 && action=='Guardar' && parseInt(document.form1.cds_producto<%=i%>.value,10) == 73){
		iCounter++;
...
function calMonto(j, k){
	...
	if(isNaN(cantidad) || isNaN(monto)){
		...
		if(tipoTransaccion=='D' && cantidad > (cant_cargo-cant_devolucion)/* && ((cs != 125 && cs != 126 && cs != 901 && fg == 'PAC')||(cs != 125 && cs != 126 && fg == 'FH'))*/){
		 ...
		 if(tipoTransaccion=='H' && monto == 0 && cdoHon.getColValue("valida_monto_hon").trim().equals("S")){
			top.CBMSG.warning('El monto no puede ser igual a 0...,VERIFIQUE!');
			eval('document.form1.monto'+j).value = '';
			eval('document.form1.monto'+j).select();
			return false;
		} else {
...
<tr class="TextHeader" align="center">
	...
	<td><%//=fb.intBox("cantidady","1",false,false,true,10,null,null,"onChange=\"javascript:calMonto('y','c')\"")%></td>
...
<%if(mode.trim().equals("add") && !tipoTransaccion.trim().equals("D")){%>
	<jsp:include page="../common/calendar.jsp" flush="true">
		<jsp:param name="noOfDateTBox" value="1" />
	...
	<td colspan="<%=Integer.parseInt(colspan)-_substract%>" align="right"><%if( imprimirCargo.trim().equals("S")){%>
		Imprimir Cargos al Guardar?&nbsp;&nbsp
...
<tr class="TextHeader" align="center">
	...
	<td><%=fb.intBox("cantidadx","0",false,false,false,10,null,null,"onChange=\"javascript:calMonto('x','c')\"")%></td>
...
for (int i=0; i<fTranCarg.size(); i++)
{
	...
	if(fg.equals("HON")&&tipoTransaccion.equals("C"))readonly = false;
...
<tr class="<%=color%>" align="center" <%=fdtc%>>
	...
	<td><%=fb.intPlusZeroBox("cantidad"+i,ad.getCantidad(),(fg.equals("HON"))?true:false,false,viewMode,5,null,null,"onChange=\"javascript:calMonto("+i+",'c')\"")%></td>
	<td><%=fb.decPlusZeroBox("monto"+i,ad.getMonto(),(fg.equals("HON"))?true:false,false,readonly,10,10.2, "", "", "onChange=\"javascript:calMonto("+i+",'p')\"")%></td>
	<td><%=fb.decBox("total"+i,ad.getMontoTotal(),false,false,true,10, 10.2)%></td>
...
<tr class="TextRow02" align="center">
	<td colspan="2" align="right"><%=fb.button("btnImpresionCargoDev","Imprimir Detalle de Cargos",false,false,null,null,"onClick=\"javascript:printCargoDev()\"")%></td>
...
else
{
	...
	FTransDet.setNoDocumento(request.getParameter("noDocumento").trim());
	...
	if(request.getParameter("pagar_sociedad") !=null && request.getParameter("pagar_sociedad").equals("S")){ FTransDet.setPagarSociedad("S");FTransDet.setMedCodigo("");}
	else{ FTransDet.setPagarSociedad("N");FTransDet.setEmpreCodigo("");}
	...
	if(!dl.equals("") || clearHT.equals("S") || (request.getParameter("addx")!=null && request.getParameter("addx").equals("+")) || ((tipoTransaccion!= null && tipoTransaccion.trim().equals("H"))&& fg.equals("HON"))&&!request.getParameter("baction").equalsIgnoreCase("Guardar")){
		response.sendRedirect("../facturacion/reg_cargo_dev_det.jsp?mode="+mode+ "&change=1&type=2&fg="+fg);
		return;
	}

	if(request.getParameter("baction")!=null && request.getParameter("baction").equals("Agregar Cargos")){
		response.sendRedirect("../facturacion/reg_cargo_dev_det.jsp?mode="+mode+"&id="+id+"&change=1&type=1&fg="+fg);
		return;
	}
