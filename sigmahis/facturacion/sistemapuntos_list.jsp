...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("descripcion") != null && !request.getParameter("descripcion").trim().equals(""))
  {
	...
	descripcion    = request.getParameter("descripcion");   // utilizada para mantener la descripción del sistema de puntos
  }
  sql = "SELECT codigo, descripcion FROM tbl_fac_cat_pto_hna WHERE compania ="+(String) session.getAttribute("_companyId")+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_fac_cat_pto_hna WHERE compania ="+(String) session.getAttribute("_companyId")+appendFilter);
...
document.title = 'Sistema de Puntos HNA - '+document.title;

function add()
{
	abrir_ventana('../facturacion/sistemapuntos_config.jsp');
}

function edit(code)
{
	abrir_ventana('../facturacion/sistemapuntos_config.jsp?mode=edit&code='+code);
}

function printList()
{
	abrir_ventana('../facturacion/print_list_sistemapuntos.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
function addPuntos(codCat)
{
    abrir_ventana('../facturacion/puntos_x_servicios_list.jsp?codCat='+codCat);
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
        <td align="right"><authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel>Registrar Nuevo Sistema de Puntos</cellbytelabel> ]</a></authtype></td>
    </tr>
	<tr>
		<td>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="0" cellspacing="1" class="sortable" id="list">
				...
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
					...
					<td align="center">
					<authtype type='4'><a href="javascript:addPuntos(<%=cdo.getColValue("codigo")%>)" class="Link02Bold" onMouseOver="setoverc(this,'Link01Bold')" onMouseOut="setoutc(this,'Link02Bold')">Add Puntos</a></authtype></td>
