...
/**
==================================================================================
600005	VER LISTA DE TIPOS DE OTROS CARGOS
600006	IMPRIMIR LISTA DE TIPOS DE OTROS CARGOS
600007	AGREGAR TIPO DE OTROS CARGOS
600008	MODIFICAR TIPO DE OTROS CARGOS
==================================================================================
**/
SecMgr.setConnection(ConMgr);
if (!SecMgr.checkLogin(session.getId())) throw new Exception("Usted está fuera del sistema. Por favor entre al sistema con su nombre de usuario y clave!!!");
//if (!(SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"600005") || SecMgr.checkAccess(session.getId(),"600006") || SecMgr.checkAccess(session.getId(),"600007") || SecMgr.checkAccess(session.getId(),"600008"))) throw new Exception("Usted no tiene los suficientes Derechos de Acceso a esta página.");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	   ...
	   if (!request.getParameter("estado").equals("")) 
	   {
		  appendFilter = appendFilter+" and activo_inactivo='"+request.getParameter("estado").toUpperCase()+"' ";	
			estado  = request.getParameter("estado");    // utilizada para mantener el estado del cargo por el cual se filtró
	          ...
	          appendFilter=appendFilter+" and upper(activo_inactivo)<>'I'";
						estado = request.getParameter("estado"); // utilizada para mantener el estado del cargo por el cual se filtró
			}
	}else{
	       appendFilter=appendFilter+" and upper(activo_inactivo)<>'I'";
  ...
  sql = "SELECT codigo, descripcion, decode(activo_inactivo,'A','Activo','I','Inactivo') as estado FROM tbl_fac_tipo_otros where compania="+ (String) session.getAttribute("_companyId")+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_fac_tipo_otros where compania ="+(String) session.getAttribute("_companyId") +appendFilter);
...
document.title = 'Tipos Otros Cargos - '+document.title;

function add()
{
	abrir_ventana('tiposotros_config.jsp');
}

function edit(code)
{
	abrir_ventana('tiposotros_config.jsp?mode=edit&code='+code);
}

function printList()
{
	abrir_ventana('print_list_tiposotros.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
function addOtros(tipoCode)
{
    abrir_ventana('otroscargos_list.jsp?tipoCode='+tipoCode);
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
        <td align="right">
        <%
//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"600007"))
		//  {
        %>
	    <a href="javascript:add()" class="Link00">[ <cellbytelabel>Registrar Nuevo Tipo de Otro Cargo</cellbytelabel> ]</a>
   	    <%
		//  }
	    %>
	    </td>
    ...
    <tr>
        <td align="right">
		<%
//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"600006"))
		//  {
		%>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="0" cellspacing="1">
				...
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
					...
					<td align="center"><a href="javascript:addOtros(<%=cdo.getColValue("codigo")%>)" class="Link02Bold" onMouseOver="setoverc(this,'Link01Bold')" onMouseOut="setoutc(this,'Link02Bold')"><cellbytelabel>Otros Cargos</cellbytelabel></a></td>
					<td align="center">
					<%
					//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"600008"))
					//{
					%>
