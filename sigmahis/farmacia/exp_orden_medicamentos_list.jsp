...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	if (cdoP.getColValue("cds_far").equals("-")) throw new Exception("El parámetro de centro de farmacia (CDS_FAR) no está definido!");
...
if(excep)throw new Exception("Opcion Solo Para Compañia Hospital!");
	...
	<%=fb.hidden("other2"+i, cdo.getColValue("other2"))%>
		...
		<tr class="<%=color%>">
			...
			<td align="center"><%=cdo.getColValue("fecha_creacion")%></td>
					<%if(mode.equals("aprobar") || mode.equals("recibir")||mode.equals("rechazar")||mode.equals("despachar")){%>
			<td  align="center" rowspan="2"><%=fb.checkbox("chk"+i,""+i,(mode.equals("despachar"))?false:true,false,"text10",null,"onClick=\"javascript:checkDespachar("+i+")\"",(mode.trim().equals("aprobar"))?"AL SELECCIONAR, ESTARÁ DESPACHANDO Y GENERANDO CARGO AL PACIENTE":"")%></td>
			<%
			}
			%>
		</tr>
