...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	sbSql.append(",'CDS_FAR') as cds, get_sec_comp_param(");
	sbSql.append(compania);
	sbSql.append(",'FAR_USA_INSUMOS') as far_usar_insumos, nvl(get_sec_comp_param(");
	sbSql.append(compania);
	sbSql.append(",'SAL_ADD_CANTIDAD_OMMEDICAMENTO'),'N') as addCantidad from dual");
	CommonDataObject cdoInsumos = SQLMgr.getData(sbSql.toString());
...
function reloadPage()
{
	...
	else window.location= '../farmacia/exp_sol_pacientes_det.jsp?fecha='+fecha+'&fechaHasta='+fechaHasta+'&fg=<%=fg%>&estado=<%=estado%>&cds=<%=cds%>&orden=<%=orden%>&pacBarcode='+pacBarcode+'&paciente='+paciente+'&timer=<%=timer%>&setFecha=<%=setFecha%>&cdsExpanded=<%=cdsExpanded?"Y":"N"%>';
