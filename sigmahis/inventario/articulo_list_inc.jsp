...
function add(){abrir_ventana('../inventario/articulo_config.jsp?fg=<%=fg%>');}
function edit(familyCode, classCode, id,subClassCode,productId){abrir_ventana('../inventario/articulo_config.jsp?mode=view&id='+id+'&familyCode='+familyCode+'&classCode='+classCode+'&subClassCode='+subClassCode+'&productId='+productId+'&fg=<%=fg%>');}
...
<%=fb.hidden("size",""+al.size())%>
		<tr class="TextRow02">
					<td colspan="8" align="right"><%=fb.submit("saveU","Guardar",true,false)%></td>
		</tr>
		<tr class="TextHeader" align="center">
			...
			<td width="5%">&nbsp;</td>
			<td width="7%">% Incremento</td>
			<td width="4%"><%=fb.checkbox("check","",false,false,null,null,"onClick=\"javascript:checkAll('"+fb.getFormName()+"','cambiar',"+al.size()+",this,0)\"","Seleccionar todos los registros listados!")%></td>
		</tr>
<%
String familyClass = "";
...
%>
		<tr class="TextHeader01">
			<td colspan="8">[<%=item.getFamilyName()%>] <%=item.getClassName()%></td>
		</tr>
<%
	}
%>

	<%=fb.hidden("articulo"+i,item.getItemCode())%>
	<%=fb.hidden("refCode"+i,item.getRefCode())%>
	<%=fb.hidden("action"+i,item.getFg())%>
	<%=fb.hidden("description"+i,item.getDescription())%>
		<tr class="<%=color%>">
			...
			<td align="center">
				<authtype type='50'><a href="javascript:edit(<%=item.getFamilyCode()%>,<%=item.getClassCode()%>,<%=item.getItemCode()%>,<%=item.getSubClassCode()%>,<%=item.getProductId()%>)" class="Link02Bold" onMouseOver="setoverc(this,'Link01Bold')" onMouseOut="setoutc(this,'Link02Bold')">Ver</a></authtype>
			</td>
			<td align="center"><%=fb.decBox("incremento"+i,item.getSalePrice(),false,false,false,5,"10.2",null,null,"")%></td>
			<td align="center"><%=fb.checkbox("cambiar"+i,"S",false,false ,null,null,"")%></td>
		</tr>
<%
...
%>
		<tr class="TextRow02">
			<td colspan="8" align="right"><%=fb.submit("saveB","Guardar",true,false)%></td>
		</tr>
	<%=fb.formEnd(true)%>
		</table>
		</div>
...
else
{
		...
		for (int i=0; i<size; i++)
	{
		...
		if (request.getParameter("cambiar"+i) != null)
		{
		CommonDataObject cdo = new CommonDataObject();
		cdo.setTableName("tbl_far_precio_x_articulo");
		cdo.setWhereClause("cod_articulo="+request.getParameter("refCode"+i));

		cdo.setKey(i);
		cdo.setAction(request.getParameter("action"+i));
		cdo.addColValue("fecha_mod","sysdate");
		cdo.addColValue("usuario_mod",(String) session.getAttribute("_userName"));
		cdo.addColValue("porc_inc",request.getParameter("incremento"+i));
		cdo.addColValue("cod_articulo",request.getParameter("refCode"+i));
		cdo.addColValue("descripcion",request.getParameter("description"+i));
		if(request.getParameter("action"+i).trim().equals("I"))
		{
	...
	if (al.size() == 0)
	{
		...
		cdo.setTableName("tbl_far_precio_x_articulo");
		cdo.setWhereClause("cod_articulo = -99 and compania=-1");
...
%>
	...
	window.location = '../inventario/articulo_list_inc.jsp?fg=<%=fg%>&familyCode=<%=familyCode%>&classCode=<%=classCode%>&estado=<%=estado%>&consignacion=<%=consignacion%>&venta=<%=venta%>&subclase=<%=subclase%>&code=<%=codigo%>&name=<%=descrip%>&barcode=<%=barcode%>&itbm=<%=itbm%>';
