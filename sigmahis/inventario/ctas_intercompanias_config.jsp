...
{   	
	sql = "SELECT nombre as cia FROM tbl_sec_compania WHERE codigo="+compId;
	...
	{   
	   ...
	   HashCta.clear();
	   VKey.clear();
 	   sql = "SELECT a.cta1, a.cta2, a.cta3, a.cta4, a.cta5, a.cta6, a.cta1||'-'||a.cta2||'-'||a.cta3||'-'||a.cta4||'-'||a.cta5||'-'||a.cta6 as cuentaCode, b.descripcion as cuenta, a.cta1||'¦'||a.cta2||'¦'||a.cta3||'¦'||a.cta4||'¦'||a.cta5||'¦'||a.cta6||'¦'||a.compania as keyCta FROM tbl_con_cuentas_x_cias a, tbl_con_catalogo_gral b WHERE a.cta1=b.cta1 and a.cta2=b.cta2 and a.cta3=b.cta3 and a.cta4=b.cta4 and a.cta5=b.cta5 and a.cta6=b.cta6 and a.compania=b.compania and a.compania="+compId;	
...
document.title = 'Cuentas de Intercompañias Asociadas a Compañias - '+document.title;

function addCuenta()
{
   abrir_ventana1('ctas_intercompanias2_config.jsp?compId=<%=compId%>&lastLineNo=<%=lastLineNo%>')
} 
</script>
</head>
<body topmargin="0" leftmargin="0" rightmargin="0">
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="INVENTARIO - MANTENIMIENTO - CTAS. INTERCOMPAÑIA ASOC. A CÍA."></jsp:param>
</jsp:include>
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<%=fb.hidden("keySize",""+HashCta.size())%>			
				...
				<tr class="TextRow02">
				    <td colspan="3"><%=compId%> - <%=cdo.getColValue("cia")%></td>
					<td align="right">
					<%
...
{     
   ...
   {	
	  ...
	  cdo3.setTableName("tbl_con_cuentas_x_cias");
	  cdo3.setWhereClause("compania="+compId);	 
	 
	  cdo3.addColValue("compania",compId);
	  ...
	  { 
		 ...
		 HashCta.remove(key);
		 response.sendRedirect("../inventario/ctas_intercompanias_config.jsp?compId="+compId+"&lastLineNo="+lastLineNo+"&change=1");
		 return;
	  } 					 	
   }
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/inventario/ctas_intercompanias_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/inventario/ctas_intercompanias_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/inventario/ctas_intercompanias_list.jsp';
<%
	}
%>
	window.close();
