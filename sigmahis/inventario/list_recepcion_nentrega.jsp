...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
	...
	*/
	...
	{
		appendFilter += " and upper(a.anio_recepcion||' '||a.numero_documento||' '||to_char(a.fecha_documento,'dd/mm/yyyy')||' '||a.cod_proveedor||' '||a.codigo_almacen) like '%"+request.getParameter("fields").toUpperCase()+"%'";
    searchOn = "";
	...
	else if (request.getParameter("searchQuery") != null && !request.getParameter("searchOn").equals("Todos") && (!request.getParameter("searchVal").equals("SV") || (!request.getParameter("searchValFrom").equals("SVF") && !request.getParameter("searchValTo").equals("SVT"))) && !request.getParameter("searchType").equals("ST"))
  {
    ...
    {
			appendFilter += " and upper(a.anio_recepcion||' '||a.numero_documento||' '||to_char(a.fecha_documento,'dd/mm/yyyy')||' '||a.cod_proveedor||' '||a.codigo_almacen) like '%"+searchVal+"%'";
    }
  }
  else
  {
...
{
	abrir_ventana('../inventario/reg_recepcion_nentrega.jsp?mode=view&id='+id+'&anio='+anio_recepcion);
}

function printList()
{
	abrir_ventana('../inventario/print_list_recepcion_nentrega.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}

function reporte(anio_recepcion, id)
{
	abrir_ventana('../inventario/print_recep_nota_entrega.jsp?numero='+id+'&anio='+anio_recepcion);
}
</script>
</head>
...
%>
			...
			<td align="center">
		  <authtype type='2'> <a href="javascript:reporte(<%=cdo.getColValue("anio_recepcion")%>,<%=cdo.getColValue("numero_documento")%>)" class="Link02Bold"><img src="../images/print_analysis.gif" width="18" height="18" border="0"></a></authtype>
			</td>
		</tr>
<%
}
