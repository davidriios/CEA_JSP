...
//if (!(SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"0"))) throw new Exception("Usted no tiene los suficientes Derechos de Acceso a esta página.");
...
String nextVal = ""+recsPerPage, previousVal = "1", searchQuery, searchOn = "SO", searchVal = "Todos", searchType = "ST", searchDisp = "SD", searchValDisp = "Todos", searchValFromDate = "SVFD", searchValToDate = "SVTD";
  ...
  else
  {
	...
	sql="select codigo, descripcion from tbl_cds_maletin"+appendFilter+" order by descripcion";
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_cds_maletin"+appendFilter);
...
document.title = 'Inventario - Creación de Maletín - '+document.title;

function add()
{
abrir_ventana('../inventario/maletin_config.jsp');
}

function edit(id)
{
abrir_ventana('../inventario/maletin_config.jsp?mode=edit&id='+id);
}

function  printList()
{
abrir_ventana('print_list_maletin.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
//{
%>
		<authtype type='3'><a href="javascript:add()" class="Link00">[ Registrar Nuevo Malet&iacute;n ]</a></authtype>
<%
//}
%>
		</td>
...
<table align="center" width="100%" cellpadding="0" cellspacing="1" class="sortable" id="malet">
