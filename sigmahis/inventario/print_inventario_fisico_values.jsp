...
if(soloDif.trim().equals("S")){sbFilter.append(" and df.cantidad_contada - df.cantidad_sistema <> 0 ");}
	   ...
	   CommonDataObject cdoH = (CommonDataObject)SQLMgr.getData("select cf.almacen, cf.anio, cf.codigo_anaquel codigoAnaquel, cf.consecutivo, to_char(cf.fecha_conteo,'dd/mm/yyyy')fechaConteo, cf.observaciones, nvl(cf.asiento_sino,'N')asientoSino, decode(cf.estatus,'P','PENDIENTE (POR ACTUALIZAR)','A','ACTIVO (ULTIMO CONTEO ACTUALIZADO)','I','INACTIVO (INV.ANTERIORES YA ACTUALIZADOS)','N','ANULADO (ANULAR INVENTARIO FISICO)') estatus,al.descripcion descAlmacen ,aa.descripcion descAnaquel from tbl_inv_conteo_fisico cf,tbl_inv_almacen al,tbl_inv_anaqueles_x_almacen aa where cf.compania= "+(String) session.getAttribute("_companyId")+" and al.codigo_almacen = cf.almacen and al.compania = cf.compania and aa.codigo_almacen = cf.almacen and aa.compania = cf.compania and aa.codigo=cf.codigo_anaquel and cf.consecutivo = "+consecutivo+" and cf.anio = "+anio+" and cf.almacen = "+almacen+" and codigo_anaquel = "+anaquely);
	   if (cdoH==null) cdoH = new CommonDataObject();

		sbSql.append("select  df.cf1_anio  cf1Anio, df.almacen cod_almacen, df.anaquel cod_anaquel,df.cf1_consecutivo||'-'||df.cf1_anio consecutivo, df.cod_familia codFamilia, df.cod_clase codClase, df.cod_familia ||'-'|| df.cod_clase ||'-'|| df.cod_articulo cod_articulos, df.cod_familia ||'-'|| df.cod_clase ||'-'|| df.cod_articulo codigo ,nvl(df.cantidad_sistema,0) cantidad_sistema, nvl(df.cantidad_contada,0) cantidad_contada, nvl(df.observaciones,' ') observaciones, nvl(df.cantidad_sistema,0) disponible,nvl(df.precio,0)precio,a.descripcion desc_articulo,a.cod_medida as codMedida, to_char(nvl(df.cantidad_sistema,0)* nvl(df.precio,0),'999999990.00') valExistencia, a.cod_barra ,nvl(df.cantidad_contada,0) - nvl(df.cantidad_sistema,0) as dif_cantidad, (df.cantidad_contada* nvl(i.precio,0)) - (df.cantidad_sistema* nvl(i.precio,0)) diferencia, nvl(df.precio,0) * nvl(df.cantidad_contada,0) as monto_fisico from tbl_inv_detalle_fisico df,tbl_inv_inventario i, tbl_inv_articulo a where df.cf1_anio = "+anio+" and df.cf1_consecutivo= "+consecutivo+"  /* and df.cod_familia = i.art_familia and df.cod_clase   = i.art_clase */ and df.cod_articulo =i.cod_articulo /* and a.cod_flia = df.cod_familia and a.cod_clase   = df.cod_clase */ and a.cod_articulo =df.cod_articulo and a.compania = i.compania and i.compania =  "+(String) session.getAttribute("_companyId")+" and i.codigo_almacen = df.almacen and df.almacen ="+almacen+" order by a.descripcion" );
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String subtitle = "INVENTARIO FISICO VALORIZADO";
	String xtraSubtitle = "";
	...
	pc.createTable();
		...
		pc.setFont(fontSize,1);
		pc.addBorderCols("Descripción Artículo",1,3);
		pc.addBorderCols("Costo Promedio",1,2);
		pc.addBorderCols("C Física",1,2);
		pc.addBorderCols("Valor Físico",1,2);
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.addCols(" "+cdo1.getColValue("precio"), 2,2);
		pc.addCols(" "+cdo1.getColValue("cantidad_contada"), 2,2);
		pc.addCols(" "+CmnMgr.getFormattedDecimal(cdo1.getColValue("monto_fisico")), 2,2);
		//pc.addCols(" --- ", 2,3);
	...
	if (al.size() == 0) pc.addCols("No existen registros",1,dHeader.size());
		 ...
		 //pc.addCols("$"+CmnMgr.getFormattedDecimal("###,###,##0.00",total),2,1);
		  pc.addCols("$"+CmnMgr.getFormattedDecimal("###,###,##0.00",totalFisico),2,2);
		 }
	pc.addTable();
	pc.close();
