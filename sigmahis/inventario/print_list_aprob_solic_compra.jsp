...
sql="SELECT a.requi_anio, a.requi_numero, a.compania, to_char(a.requi_fecha, 'dd/mm/yyyy') requi_fecha, a.estado_requi, decode(a.estado_requi,'A','Aprobado','P','Pendiente') desc_estado_requi, a.usuario_creacion, a.fecha_creacion, a.usuario_modificacion, a.fecha_modificacion, NVL(a.observaciones,' ') observaciones, NVL(a.monto_total,0) monto_total, NVL(a.subtotal,0) subtotal, NVL(a.itbm,0) itbm, nvl(a.activa,' ') activa, nvl(a.unidad_administrativa,0) unidad_administrativa, nvl(a.codigo_almacen,0) codigo_almacen, NVL(a.especificacion, ' ') especificacion, b.descripcion"
+ " FROM TBL_INV_REQUISICION a, tbl_inv_almacen b "
+ " where a.compania="+(String) session.getAttribute("_companyId")+" and a.codigo_almacen = b.codigo_almacen and a.compania = b.compania and a.estado_requi ='P' "+appendFilter+" order by 1 desc,2 desc";
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_aprob_solic_compra";
			...
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("INVENTARIO",1,1);
					pc.addCols("APROBACIÓN DE SOLICITUD DE COMPRA", 1, 1);
				pc.addTable();
					...
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								pc.createTable();
								pc.setFont(7, 0);
									pc.addCols(" "+cdo1.getColValue("requi_anio"),1,1);
									pc.addCols(" "+cdo1.getColValue("descripcion"),0,1);
									pc.addCols(" "+cdo1.getColValue("requi_numero"),1,1);
									pc.addCols(" "+cdo1.getColValue("desc_estado_requi"),1,1);
									pc.addCols(" "+cdo1.getColValue("requi_fecha"),1,1);
								pc.addTable();
