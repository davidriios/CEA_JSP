...
if (appendFilter == null) appendFilter = "";
	...
	sql="select cod_flia||'-'||cod_clase||'-'||cod_articulo articulo, cod_hna, descripcion from tbl_inv_articulo where cod_hna is not null order by descripcion";
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String fileNamePrefix = "print_list_articulos_hna";
	...
	pc.createTable();
		pc.setFont(7, 1);
		pc.addBorderCols("Descripción del Artículo",1);
		pc.addBorderCols("Cod. Artículo",1);
		pc.addBorderCols("Cod. AXA",1);
	pc.addTable();
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.setFont(7, 0);
		pc.createTable();
			pc.addCols(" "+cdo1.getColValue("descripcion"),0,1,cHeight);
			pc.addCols(" "+cdo1.getColValue("articulo"),0,1,cHeight);
			pc.addCols(" "+cdo1.getColValue("cod_hna"),1,1,cHeight);
		pc.addTable();
		lCounter++;
		...
		if (lCounter >= maxLines)
		{
			...
			pdfHeader(pc, _comp, pCounter, nPages, "INVENTARIO", "CODIGOS DE MEDICAMENTOS DE LA FARMACIA Y SU CORRESPONDIENTE CODIGO DE AXA", userName, fecha);
			pc.setNoColumnFixWidth(setDetail);
	...
	{
		pc.createTable();
			pc.addCols("TOTAL FINAL.....   "+al.size(),0,setDetail.size());
		pc.addTable();
	}
