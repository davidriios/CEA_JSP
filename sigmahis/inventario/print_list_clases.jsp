...
al = SQLMgr.getDataList(sql);
	...
	sql="select distinct(b.nombre) from tbl_inv_clase_articulo a, tbl_inv_familia_articulo b where a.compania=b.compania and a.cod_flia=b.cod_flia and a.compania="+(String) session.getAttribute("_companyId")+appendFilter+" order by b.nombre";
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_clasess";
...
for (int j=1; j<=nPages; j++)
{
			Vector setHeader0=new Vector();
				...
				{
					pc.createTable();
						pc.addCols("No existe la Clase seleccionada",1,5);
					pc.addTable();
				}
				else
				{
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								// Imprime Categoría||Tipo de Admisión sólo cuando cambian.
								if (!catTipo.equalsIgnoreCase("["+cdo1.getColValue("family")+"] "))
								{
							    pc.createTable();
								pc.setFont(7, 1);
									pc.addCols("[ "+cdo1.getColValue("family")+" ] ",0,1);
									pc.addCols("",0,1);
								pc.addTable();
								no += 1;
								}
								...
								pc.createTable();
								pc.setFont(7, 0);
									pc.addCols(" "+cdo1.getColValue("code"),0,1);
									pc.addCols(" "+cdo1.getColValue("name"),0,1);
								pc.addTable();

								catTipo="["+cdo1.getColValue("family")+"] ";
