...
if (appendFilter == null) appendFilter = "";
    ...
    sql = "SELECT a.codigo_almacen as almaCode, a.descripcion as alma, b.flia FROM tbl_inv_almacen a, (SELECT a.compania, a.familia as fliaCode, b.nombre as flia, a.almacen as almaCode FROM tbl_inv_parametro_inv a, tbl_inv_familia_articulo b WHERE a.familia=b.cod_flia and a.compania=b.compania) b WHERE a.codigo_almacen=b.almaCode(+) and a.compania=b.compania(+) and a.compania="+(String) session.getAttribute("_companyId")+appendFilter+" ORDER BY a.codigo_almacen, a.descripcion, b.flia";

System.out.println("\n\n ddddddddddddddddddsql="+sql+"\n\n");
al = SQLMgr.getDataList(sql);


    sql = "SELECT distinct(a.codigo_almacen) FROM tbl_inv_almacen a, (SELECT a.compania, a.familia as fliaCode, b.nombre as flia, a.almacen as almaCode FROM tbl_inv_parametro_inv a, tbl_inv_familia_articulo b WHERE a.familia=b.cod_flia and a.compania=b.compania) b WHERE a.codigo_almacen=b.almaCode(+) and a.compania=b.compania(+) and a.compania="+(String) session.getAttribute("_companyId")+appendFilter+" ORDER BY a.codigo_almacen";
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_cta_familiaInv";
			...
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("Contabilidad",1,1);
						pc.addCols("CUENTAS ASOCIADAS POR FAMILIA DE ALMACEN", 1, 1);
				pc.addTable();
				...
				else
				{
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
							...
							if (!familiaArticulo.equalsIgnoreCase(cdo1.getColValue("almaCode")))
							  {
								pc.createTable();
                      			pc.setFont(7, 1);
									pc.addCols(" "+cdo1.getColValue("almaCode")+" - "+cdo1.getColValue("alma"),0,1);
									pc.addCols("",0,1);
								pc.addTable();
							  }
								...
								pc.setFont(7, 0);
									pc.addCols(" ",0,1);
									pc.addCols(" "+cdo1.getColValue("flia"),0,1);
								pc.addTable();

								familiaArticulo = cdo1.getColValue("almaCode") ;
