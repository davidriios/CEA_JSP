...
//	sql="select codigo as code, descripcion as name, compania, estatus from tbl_sal_uso where compania="+(String) session.getAttribute("_companyId")+appendFilter+" order by descripcion";
...
al = SQLMgr.getDataList(sql);
   ...
   sql = "SELECT distinct(a.codigo) FROM tbl_sec_compania a, tbl_con_cuentas_x_cias b, tbl_con_catalogo_gral c WHERE a.codigo=b.compania(+) and (b.compania=c.compania(+) and b.cta1=c.cta1(+) and b.cta2=c.cta2(+) and b.cta3=c.cta3(+) and b.cta4=c.cta4(+) and b.cta5=c.cta5(+) and b.cta6=c.cta6(+)) "+appendFilter+" ORDER BY a.codigo";
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		int extraItems = (nItems + al2.size()) % maxLines;
		...
		String fileNamePrefix = "print_list_cuentasasociadas_cia";
			...
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("INVENTARIO",1,1);
					pc.addCols("CUENTAS DE INTERCOMPAÑIAS ASOCIADAS A CIA", 1, 1);
				pc.addTable();
				...
				Vector setHeader2=new Vector();
					setHeader2.addElement(".35");
					setHeader2.addElement(".65");
					...
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								if (!catTipo.equalsIgnoreCase("["+cdo1.getColValue("ciaCode")+"] "))
								{
									pc.createTable();
									pc.setFont(7, 0);
										pc.addCols(" "+cdo1.getColValue("ciaCode")+" - "+cdo1.getColValue("cia"),0,1);
										pc.addCols(" ",0,1);
									pc.addTable();
								}
								...
								pc.createTable();
								pc.setFont(7, 0);
									pc.addCols("       "+cdo1.getColValue("cuentaCode"),0,1);
									pc.addCols(" "+cdo1.getColValue("nomCuenta"),0,1);
								pc.addTable();

								catTipo="["+cdo1.getColValue("ciaCode")+"] ";

								if (cdo1.getColValue("nomCuenta") != "") no2 +=1;

									if ((i + 1) == nItems)
									{
									 pc.createTable();
										 //pc.addCols(al.size()+" Registros en total",0,5);
										 pc.addCols(no2+" Registros en total",0,5);
									 pc.addTable();
									break;
									}
						}//End For
