...
sql=  "SELECT a.anio_recepcion, a.numero_documento, a.estado, decode(a.estado,'A','ANULADO','R','RECIBIDO') desc_estado, nvl(to_char(a.fecha_documento,'dd/mm/yyyy'), to_char(sysdate,'dd/mm/yyyy')) fecha_documento, a.cod_proveedor, a.codigo_almacen, b.nombre_proveedor, c.descripcion almacen_desc "
+ " FROM tbl_inv_recepcion_material a, tbl_com_proveedor b, tbl_inv_almacen c "
+ " where a.cod_proveedor = b.cod_provedor and a.codigo_almacen = c.codigo_almacen and a.compania = c.compania and a.compania = "+session.getAttribute("_companyId") + " and a.fre_documento = 'FR' " + appendFilter
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String fileNamePrefix = "print_list_recepcion_sin_oc";
			...
			Vector setHeader0=new Vector();
				...
				pc.setFont(9, 1);
				  pc.addCols("INVENTARIO",1,1);
					pc.addCols("RECEPCIÓN SIN ORDEN DE COMPRA", 1, 1);
				pc.addTable();
				...
				pc.createTable();
					...
					pc.addBorderCols("No.Recepción",1);
				...
				if (al.size()==0) {
						pc.createTable();
							pc.addCols("No existe la Orden seleccionada",1,5);
						pc.addTable();
						}
				else{
					if (al.size() > 0)
					{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
						{
						    CommonDataObject cdo1 = (CommonDataObject) al.get(i);
								...
								pc.createTable();
								pc.setFont(7, 0);
									pc.addCols(" "+cdo1.getColValue("anio_recepcion"),0,1);
									pc.addCols(" "+cdo1.getColValue("numero_documento"),0,1);
									pc.addCols(" "+cdo1.getColValue("fecha_documento"),0,1);
									pc.addCols(" "+cdo1.getColValue("cod_proveedor")+ " " + cdo1.getColValue("nombre_proveedor"),0,1);
									pc.addCols(" "+cdo1.getColValue("codigo_almacen")+ " " + cdo1.getColValue("almacen_desc"),0,1);
									pc.addCols(" "+cdo1.getColValue("desc_estado"),0,1);
								pc.addTable();
