...
var CBFingerprintSdk = (function () {
    function CBFingerprintSdk() {
        var _instance = this;
        ...
        this.sdk.onQualityReported = function (e) {
            sampleQuality((e.quality));
        }

    }

    CBFingerprintSdk.prototype.startCapture = function () {
    ...
    CBFingerprintSdk.prototype.stopCapture = function () {
    ...
    CBFingerprintSdk.prototype.getInfo = function () {
        var _instance = this;
        return this.sdk.enumerateDevices();
    };

    CBFingerprintSdk.prototype.getDeviceInfoWithID = function (uid) {
        var _instance = this;
        return  this.sdk.getDeviceInfo(uid);
    };

    
    return CBFingerprintSdk;
})();

//

function deviceDisconnected() {            
   $(fpOptions.fpStatus).attr('src', fpOptions.imgs.disconnected)
}

function deviceConnected() {            
   $(fpOptions.fpStatus).attr('src', fpOptions.imgs.connected)
}

function invalidCommunication() {            
   $(fpOptions.fpStatus).attr('src', fpOptions.imgs.invalid)
}

function sampleQuality(quality) {
  //var indicator = quality
  if (quality == 0) $(fpOptions.fpStatus).attr('src', fpOptions.imgs.valid);
  else $(fpOptions.fpStatus).attr('src', fpOptions.imgs.invalid);
...
function sampleAcquired(s){
            ...
            // Get samples from the object - get 0th element of samples as base 64 encoded PNG image         
                ...
                var samples = JSON.parse(s.samples);
                var rawJson = samples[0];
                var sampleData = Fingerprint.b64UrlTo64(rawJson.Data);
                var decodedData = JSON.parse(Fingerprint.b64UrlToUtf8(sampleData));
                var base64Data = "data:application/octet-stream;base64," + Fingerprint.b64UrlTo64(decodedData.Data);
                localStorage.setItem("wsq", base64Data);
                
                console.log("JSON 0 =", rawJson);
...
function showMessage(message){
    ...
    x = state ? state.querySelectorAll("#status") : [];
    if(x.length != 0){
...
function onDeviceInfo(id, element){
    var myDeviceVal = fp.getDeviceInfoWithID(id);
...
function IeVersionInfo() {
  ...
  // Quick and dirty fp for Microsoft Edge
  else if (document.documentMode || /Edge/.fp(navigator.userAgent))
    return 12;
