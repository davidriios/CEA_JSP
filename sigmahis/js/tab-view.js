  ...
  ************************************************************************************************************/
  ...
  function showTab(parentId,tabIndex)
  {
    ...
    //adjust iframe height from iframe that contains the tab
    if(!top.window.frames['content']&&adjustDivWithIFrameHeight) adjustDivWithIFrameHeight('iSecciones');
    //adjust iframe height from tab that contains the iframe
    if(!top.window.frames['content']&&adjustHeight) adjustHeight(tabIndex);

    for(var z=0;z<functionsName.length;z++){
      var valorX = functionsName[z];
      var indiceX = valorX.substring(0,valorX.indexOf('='));
      var functionName = valorX.substring(valorX.indexOf('=')+1,valorX.length);
      if(parseInt(indiceX)==tabIndex){
        eval(functionName);
      }
    }
    _activeTab=tabIndex;
  ...
  function initTabs(mainContainerID,tabTitles,activeTab,width,height,closeButtonArray,additionalTab, functions,disableTabs)
  {
    if(!closeButtonArray)closeButtonArray = new Array();
    if(!functions)functions = new Array();
    if(!disableTabs)disableTabs = new Array();
    ...
    if(!additionalTab || additionalTab=='undefined'){
      ...
      //set all div with same width size
      /*if(MSIE){
        for (var i=0; i<dhtmlgoodies_tabObj[mainContainerID].getElementsByTagName('DIV').length; i++)
          dhtmlgoodies_tabObj[mainContainerID].getElementsByTagName('DIV')[i].style.width = width;
      }*/
  }else{
    ...
    {
      for(var d=0;d<disableTabs.length;d++)
      {
          if(no == disableTabs[d]) {if(activeTab == no)existe=true;at++; break;}
          else {atActivo=no;}
      }
      if(at == no)break;
    }

    if(!existe){ atActivo= activeTab; }
    ...
    for(var no=0;no<tabTitles.length;no++){
      ...
      var aMouseOver=rolloverTab;
      var aMouseOut=rolloutTab;
      var aClick=tabClick;

      activeTab=atActivo;

      for(var d=0;d<disableTabs.length;d++){if(no==disableTabs[d]){aMouseOver='';aMouseOut='';aClick='';aTab.className='tabInactiveDisabled';break;}}

      aTab.onmouseover = aMouseOver;
      aTab.onmouseout = aMouseOut;
      aTab.onclick=aClick;
      ...
      if((navigatorVersion && navigatorVersion<6) || (MSIE && !strictDocType)){
        ...
        if(aMouseOver=='')aTab.style.cursor='help';
      } // End IE5.x FIX
