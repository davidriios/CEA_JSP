...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (!codigo.trim().equals("")){
    appendFilter += " and id_oset_param like '%"+request.getParameter("codigo")+"%'";
  }
  ...
  sql = "SELECT id_oset_param codigo, descripcion, decode(estado,'A','ACTIVO','INACTIVO') status FROM tbl_oset_param WHERE id_oset_param <> 0"+appendFilter+" order by id_oset_param";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_oset_param WHERE id_oset_param <> 0"+appendFilter);
...
function add(){
	abrir_ventana('exp_oset_params_config.jsp');
}

function edit(code){
	abrir_ventana('exp_oset_params_config.jsp?mode=edit&code='+code);
}

function addDetail(code, desc){
	abrir_ventana('exp_oset_detalles_config.jsp?id_oset_param_code='+code+'&id_oset_param_desc='+desc+'&id_oset_h1=1');
}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="ORDERSET - MANTENIMIENTO"></jsp:param>
</jsp:include>

<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
        <td align="right">
          <authtype type='3'><a href="javascript:add()" class="Link00">[ <cellbytelabel id="1">Registrar Nuevo Par&aacute;metros</cellbytelabel> ]</a></authtype>
   	    </td>
    </tr>
	<tr>
		<td>
