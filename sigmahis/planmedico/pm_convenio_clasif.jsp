...
function showServiceList(cTab,k)
{
	...
	if (cTab == 0)
	{
		...
		fp = 'pm_convenio_cobertura';
	}
	else if (cTab == 1)
	{
		tipo = eval('document.form1.tipoExclusion'+k).value;
		fp = 'pm_convenio_exclusion';
	}
...
<%if (c.getSecuencia() != null && !c.getSecuencia().equals("0") || (c.getSecuencia() != null && c.getSecuencia().equals("0") && c.getCodigo() != null && !c.getCodigo().trim().equals(""))) {%>
				...
				<%}else{%> <cellbytelabel>CENTRO DE SERVICIO</cellbytelabel>
		...
		<tr class="TextRow08" id="detail0-<%=i%>" style="display:none">
			<td colspan="7">
				...
				<tr class="TextHeader02">
					<td width="45%"><cellbytelabel>Descuento de Cl&iacute;nica</cellbytelabel></td>
					<td width="55%" colspan="2"><cellbytelabel>Pago de Paciente</cellbytelabel></td>
				...
				<tr class="TextRow08">
					<td valign="top">
						...
						<%=fb.select("tipoValCli"+i,"P=%",c.getTipoValCli(),false,false,0,"Text10",null,null)%>
					</td>
					...
					<!--
						...
						<%=fb.hidden("limiteEvento"+i,c.getLimiteEvento())%>
						<%=fb.hidden("montoMargenLimite"+i,c.getMontoMargenLimite())%>
						<%=fb.hidden("cantidadProc"+i,c.getCantidadProc())%>
					...
					<%=fb.hidden("montoEmp"+i,c.getMontoEmp()) %>
					<%=fb.hidden("tipoValEmp"+i,c.getTipoValEmp()) %>
					<%=fb.hidden("tipoCobEmp"+i,c.getTipoCobEmp()) %>
					<%=fb.hidden("montoEmpExc"+i,c.getMontoEmpExc()) %>
					<%=fb.hidden("tipoValEmpExc"+i,c.getTipoValEmpExc()) %>
...
%>
		...
		<tr class="TextRow08" id="detail1-<%=i%>" style="display:none">
			<td colspan="5">
				...
				<tr class="TextHeader02">
					<td width="45%"><cellbytelabel>Descuento de Cl&iacute;nica</cellbytelabel></td>
					<td width="55%"><cellbytelabel>Pago de Paciente</cellbytelabel></td>
					<!--<td width="35%"><cellbytelabel>Pago de Aseguradora</cellbytelabel></td>-->
				</tr>
				<tr class="TextRow08">
					<td valign="top">
						...
						<%=fb.select("tipoValCli"+i,"P=%",e.getTipoValCli(),false,false,0,"Text10",null,null)%>
					</td>
					<td valign="top" rowspan="2">
						<cellbytelabel>Monto</cellbytelabel>
						...
						<%=fb.select("tipoCobPac"+i,"E=EVENTO",e.getTipoCobPac(),false,false,0,"Text10",null,null)%>
						...
						<cellbytelabel>Diferencia</cellbytelabel></span>
					</td>
					<!--<td valign="top" rowspan="2">
						<cellbytelabel>Monto</cellbytelabel>
					...
					<%=fb.hidden("montoEmp"+i,e.getMontoEmp())%>
					<%=fb.hidden("tipoValEmp"+i,e.getTipoValEmp())%>
					<%=fb.hidden("tipoCobEmp"+i,e.getTipoCobEmp())%>
				</tr>
				<tr>
...
<tr class="TextHeader" align="center">
	...
	<td width="78%">Descripci&oacute;n</td>
	<td width="5%">Orden</td>
	<td width="2%"><%=fb.submit("addDefinicion","+",true,viewMode,null,null,"onClick=\"javascript:setBAction('"+fb.getFormName()+"',this.value)\"","Agregar Definición de Cálculos")%></td>
</tr>
<%
...
<tr class="<%=color%>"<%=displayDef%>>
	...
	<td align="center"><%=fb.submit("rem"+i,"X",true,viewMode,null,null,"onClick=\"javascript:removeItem('"+fb.getFormName()+"','"+i+"')\"","Eliminar Definición de Cálculos")%></td>
</tr>
<%
}
...
initTabs('dhtmlgoodies_tabView1',Array('Coberturas','Exclusiones','Definición de Cálculos'),<%=cTab%>,'100%','');
</script>
