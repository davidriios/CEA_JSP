...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	if (mode.equalsIgnoreCase("add"))
	{
		...
		CommonDataObject cdo = SQLMgr.getData("select codigo, nombre from tbl_adm_empresa where codigo = "+empresa);
		conv.setNombreEmpresa(cdo.getColValue("nombre"));
	}
	else
	{
...
{
	abrir_ventana1('../common/check_tipoPlan_x_tipoPoliza.jsp?fp=pm_convenio&mode=<%=mode%>&empresa=<%=empresa%>&secuencia=<%=secuencia%>&planLastLineNo=<%=planLastLineNo%>');
}

function displayPlan(tipoPoliza,tipoPlan,planNo)
{
	setFrameSrc('iDetalle','../planmedico/pm_convenio_plan.jsp?tab=1&mode=<%=mode%>&empresa=<%=empresa%>&secuencia=<%=secuencia%>&tipoPoliza='+tipoPoliza+'&tipoPlan='+tipoPlan+'&planNo='+planNo);
}
...
function printPlanDet(tipoPol,tipoPlan,idPlan){
   abrir_ventana("../cellbyteWV/report_container.jsp?reportName=planmedico/rpt_convenio_detallado.rptdesign&tipoPol="+tipoPol+"&tipoPlan="+tipoPlan+"&idPlan="+idPlan+"&idEmp=<%=empresa%>&idConv=<%=secuencia%>&pCtrlHeader=false");
}
</script>
</head>
					...
					<td>
						...
						<tr class="TextRow01">
							...
							<td colspan="3">
								...
								<%//=fb.hidden("empresa",""+conv.getEmpresa())%>
								<%//=fb.hidden("nombreEmpresa",conv.getNombreEmpresa())%>
						<tr class="TextRow01">
				...
				<tr id="panel11">
					<td class="TextRow01">
						...
						<tr class="TextHeader" align="center">
							...
							<td width="23%"><cellbytelabel>Nombre</cellbytelabel></td>
							<td width="5%"><cellbytelabel>C&oacute;d</cellbytelabel>.</td>
							<td width="23%"><cellbytelabel>Nombre</cellbytelabel></td>
							<td width="5%"><cellbytelabel>No</cellbytelabel>.</td>
							<td width="26%"><cellbytelabel>Nombre</cellbytelabel></td>
						</tr>
...
%>
						...
						<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')"<%=(pc.getEstado() != null && pc.getEstado().equalsIgnoreCase("D"))?" style=\"display:none\"":""%>>
							...
							<td align="center">
							  <img src="../images/printer.png" alt="Imprimir detalles del Plan" title="Imprimir detalles del Plan" width="20px" height="20px" onClick="javascript:printPlanDet('<%=pc.getTipoPoliza()%>','<%=pc.getTipoPlan()%>','<%=pc.getSecuencia()%>')">
							</td>
...
<%=fb.hidden("gncLastLineNo",""+gncLastLineNo)%>
				...
				<tr id="panel21">
					<td>
						...
						<tr>
							<td colspan="7">
								<iframe id="iDetalleGNC" name="iDetalleGNC" width="100%" height="25" scrolling="no" frameborder="0" src="../planmedico/pm_add_gnc.jsp?tab=2&mode=<%=mode%>&empresa=<%=empresa%>&secuencia=<%=secuencia%>&gncLastLineNo=<%=gncLastLineNo%>"></iframe>
							</td>
						</tr>
<%
...
else
{
	...
	else if (tab.equals("1")) //PLANES
	{
		...
		for (int i=1; i<=planSize; i++)
		{
			...
			System.out.println("aplica_desc"+i+"="+request.getParameter("aplica_desc"+i));
			System.out.println("aplica_co"+i+"="+request.getParameter("aplica_co"+i));
			System.out.println("_________________________________________________________________________");
...
<%
	if (tab.equals("0"))
	{
		if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/planmedico/pm_convenio_list.jsp"))
		{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/planmedico/pm_convenio_list.jsp")%>';
<%
		}
		else
		{
%>
	window.opener.location = '<%=request.getContextPath()%>/planmedico/pm_convenio_list.jsp';
<%
		}
	}
