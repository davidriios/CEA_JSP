...
function doAction(){
	document.formTarjeta.tab.value = parent.document.form1.tab.value;
...
function ver(id_solicitud, id, mode){
	var iFrameName = '', page = '';
	iFrameName='iFrameNotas';
	page = '../planmedico/reg_tarjeta_cta_det.jsp?id_solicitud=<%=id_solicitud%>&id='+id+'&mode='+mode+'&id_cliente=<%=id_cliente%>';
    ...
    <tr>
			<td class="TableBorder">
				<table align="center" width="100%" cellpadding="1" cellspacing="1">
						...
						<!--<div id="list_op" width="100%" style="overflow;position:absolute">-->
							<table width="99%" border="0" align="center">
								<tr class="TextHeader">
									...
									<td align="center" width="6%"><%=fb.button("addClte","Agregar",false,viewMode,"text10","","onClick=\"javascript:ver("+id_solicitud+", 0, 'add');\"")%></td>
								</tr>
								<%
								key = "";
								...
								<tr class="<%=color%>" align="center">
									...
									<td align="left"><%=(!cdo.getColValue("num_tarjeta_cta").equals("")?CmnMgr.getDecryptToShow(cdo.getColValue("num_tarjeta_cta")):"")%></td>
									...
									<td align="center"><authtype type='1'><a href="javascript:ver(<%=cdo.getColValue("id_solicitud")%>, <%=cdo.getColValue("id")%>, 'view')" class="Link02Bold">Ver</a></authtype></td>
									<td align="center"><authtype type='4'><%if((mode.equals("edit") || (mode.equals("add") && cdo.getColValue("usuario_creacion").equals((String) session.getAttribute("_userId")))) && !cdo.getColValue("estado").equals("I") && !cdo.getColValue("estado").equals("R")){%><a href="javascript:ver(<%=cdo.getColValue("id_solicitud")%>, <%=cdo.getColValue("id")%>, 'edit')" class="Link02Bold">Editar</a><%}%></authtype></td>
								</tr>
								<%}%>
							</table>
						</div>
					...
					<tr class="TextRow01">
						<td colspan="4">
						<iframe name="iFrameNotas" id="iFrameNotas" frameborder="0" align="center" width="100%" height="280" scrolling="yes" src="../planmedico/reg_tarjeta_cta_det.jsp?mode=<%=mode%>&id_solicitud=<%=id_solicitud%>&id=<%=id%>"></iframe>
						</td>
					</tr>			
				</table>
			</td>
