...
{
	sbSql.append("select (select m.primer_nombre || decode(m.segundo_nombre, null, '', ' ' || m.segundo_nombre) || ' ' || m.primer_apellido || decode(m.segundo_apellido, null, '', ' ' || m.segundo_apellido) || decode(m.sexo, 'F', decode(m.apellido_de_casada, null, '', ' ' || m.apellido_de_casada)) from tbl_pm_cliente m where exists (select null from tbl_pm_solicitud_contrato c where c.id = ");
	sbSql.append(id_contrato);
	sbSql.append(" and c.id_cliente = m.codigo)) nombre_id_contrato, '01/'||to_char(sysdate, 'mm/yyyy') fecha_ini, to_char(last_day(sysdate), 'dd/mm/yyyy') fecha_fin from dual");
...
function setValues(){
	...
	window.frames['itemFrame'].location = '../planmedico/ver_mov_cierre_mes_det.jsp?id_contrato=<%=id_contrato%>&tipo=<%=tipo%>&fechaini='+fechaini+'&fechafin='+fechafin+'&format=<%=format%>';
}
...
{	
	var fechaini = document.form1.fechaini.value;
	var fechafin = document.form1.fechafin.value;
	abrir_ventana2('../cellbyteWV/report_container.jsp?reportName=cxp/rpt_mov_hon_hnf.rptdesign&desdeParam='+fechaini+'&hastaParam='+fechafin+'&medicoParam=<%=id_contrato%>&tipoParam=<%=tipo%>&benefNameParam=<%=cdo.getColValue("nombre_id_contrato")%>&format=<%=format%>'+'&ahnfParam='+acumHonNoFacturaro);
}
                    ...
                    <tr>
                      <td><table width="100%" cellpadding="1" cellspacing="0">
                          ...
                          <tr class="TextHeader02" height="21">
                            <td><cellbytelabel>C&oacute;digo</cellbytelabel>:&nbsp;&nbsp;<%=id_contrato%></td>
                            <td><cellbytelabel>Nombre</cellbytelabel>:&nbsp;&nbsp;<%=cdo.getColValue("nombre_id_contrato")%></td>
                          </tr>
                        </table></td>
                    </tr>
                    <tr>
                      <td><iframe name="itemFrame" id="itemFrame" frameborder="0" align="center" width="100%" height="300" scrolling="yes" src="../planmedico/ver_mov_cierre_mes_det.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>&id_contrato=<%=id_contrato%>&tipo=<%=tipo%>&fechaini=<%=fechaini%>&fechafin=<%=fechafin%>&format=<%=format%>"></iframe></td>
                    </tr>
                    <%=fb.formEnd(true)%>
