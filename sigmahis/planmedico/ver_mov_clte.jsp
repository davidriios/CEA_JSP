...
document.title = 'PLAN MEDICO - '+document.title;

function setValues(){
	var fechaini = document.form1.fechaini.value;
	var fechafin = document.form1.fechafin.value;
	window.frames['itemFrame'].location = '../planmedico/ver_mov_clte_det.jsp?id_beneficiario=<%=id_beneficiario%>&fechaini='+fechaini+'&fechafin='+fechafin;
}

function printMovimiento()
{	
	var id_beneficiario = document.form1.id_beneficiario.value;
	var fechaini = document.form1.fechaini.value;
	var fechafin = document.form1.fechafin.value;
	abrir_ventana2('../cellbyteWV/report_container.jsp?reportName=cxp/saldo_proveedor.rptdesign&fechaDesdeParam='+fechaini+'&fechaHastaParam='+fechafin+'&provParam='+id_beneficiario+'&provName=<%=nombre%>');
}
                    ...
                    <tr>
                      ...
                      <td><table width="100%" cellpadding="1" cellspacing="0">
                          ...
                          <tr class="TextHeader02" height="21">
                            <td><cellbytelabel>C&oacute;digo</cellbytelabel>:&nbsp;&nbsp;<%=id_beneficiario%></td>
                            <td><cellbytelabel>Nombre</cellbytelabel>:&nbsp;&nbsp;<%=nombre%></td>
                          </tr>
                        </table></td>
                    </tr>
                    <tr>
                      <td><iframe name="itemFrame" id="itemFrame" frameborder="0" align="center" width="100%" height="300" scrolling="yes" src="../planmedico/ver_mov_clte_det.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>&id_beneficiario=<%=id_beneficiario%>&fechaini=<%=fechaini%>&fechafin=<%=fechafin%>"></iframe></td>
                    </tr>
                    <%=fb.formEnd(true)%>
