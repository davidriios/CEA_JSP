...
if (request.getMethod().equalsIgnoreCase("GET")){
	...
	sbSql.append("select oc.descripcion ref_desc, f.doc_id, to_char(f.doc_date, 'dd/mm/yyyy') fecha, f.printed_no factura, f.client_id, f.client_name||' ('||observations||' )' as client_name, decode(f.doc_type, 'NCR', -f.net_amount, f.net_amount) net_amount, decode(oc.refer_to, 'EMPL', (select num_empleado from tbl_pla_empleado e where to_char(emp_id) = f.client_id), f.client_id) num_empleado, oc.refer_to, (select nombre from tbl_cja_cajera where cod_cajera = f.cod_cajero and compania = f.company_id) nombre_cajero, decode(f.doc_type, 'FAC', f.other3, f.doc_id) codigo_ref,decode(f.doc_type, 'FAC', 'FACP', 'NCR', 'NCP', 'NDB', 'NDP')as tipoDocto, f.doc_type, f.tipo_factura,nvl((select max(ruc) from tbl_cxc_cliente_particular where compania = f.company_id and to_char(codigo) = f.client_id),' ') as clientRuc,f.status,decode(f.status,'O','ACTIVA','A','ANULADA','F','FACTURADA',f.status) as desc_estado,f.centro_servicio as cds,f.client_ref_id as refId,(select almacen from tbl_fac_proforma_items where doc_id=f.doc_id and rownum=1) as wh from tbl_fac_proforma f, tbl_fac_tipo_cliente oc where f.client_ref_id = oc.codigo and f.company_id = oc.compania and oc.compania = ");
    ...
    if(request.getParameter("tipoClte") != null){
...
document.title = 'Proforma - '+document.title;
...
function anular(id){showPopWin('../process/pos_anular_proforma.jsp?&id='+id,winWidth*.65,_contentHeight*.75,null,null,'');} 
 
function facturar(id,cds,wh,refId,referTo,clientId){showPopWin('../pos/facturar.jsp?proformaId='+id+'&cds='+cds+'&almacen='+wh+'&refId='+refId+'&referTo='+referTo+'&clienteId='+clientId,winWidth*.98,_contentHeight*.90,null,null,'');} 
</script>
</head>
		...
		<!-- ================================   S E A R C H   E N G I N E S   S T A R T   H E R E   ================================ -->
			<table width="100%" cellpadding="1" cellspacing="0">
				<tr class="TextFilter">
					...
					C&oacute;digo:
					<%=fb.textBox("codigo", codigo, false, false, false, 10, 40, "text12", "", "", "", false, "", "")%>
					Nombre:
		...
		<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			<table align="center" width="100%" cellpadding="0" cellspacing="1">
				...
				<tr class="SpacingTextBold Text14">
				 ...
				 %>
				<tr class="SpacingTextBold Text14">
					<td align="center" colspan="6"><%=(cdo.getColValue("tipo_factura").equals("CR")?"VENTAS A CREDITO":"VENTAS A CONTADO")%></td>
				</tr>
				 <%
				 }
				%>
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
					...
					<td align="right"><%=cdo.getColValue("desc_estado")%></td>
					<td align="center"><%if(cdo.getColValue("status").trim().equals("O")){%><authtype type='51'><a href="javascript:facturar('<%=cdo.getColValue("doc_id")%>',<%=cdo.getColValue("cds")%>,<%=cdo.getColValue("wh")%>,<%=cdo.getColValue("refId")%>,'<%=cdo.getColValue("refer_to")%>','<%=cdo.getColValue("client_id")%>')" class="LinksTextwhite" onMouseOver="setoverc(this,'LinksTextblack')" onMouseOut="setoutc(this,'LinksTextwhite')">Facturar</a></authtype>&nbsp;&nbsp;<%}%>
					</td>
					<td align="center">
					<%if(cdo.getColValue("status").trim().equals("O")){%><authtype type='50'><a href="javascript:anular('<%=cdo.getColValue("doc_id")%>')" class="LinksTextwhite" onMouseOver="setoverc(this,'LinksTextblack')" onMouseOut="setoutc(this,'LinksTextwhite')">Anular</a></authtype><%}%>
					</td>
				</tr>
				<%
