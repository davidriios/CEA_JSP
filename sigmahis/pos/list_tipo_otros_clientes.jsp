...
if (request.getMethod().equalsIgnoreCase("GET")){
  ...
  String nextVal = ""+recsPerPage, previousVal = "1", searchQuery, searchOn = "SO", searchVal = "Todos", searchType = "ST", searchDisp = "SD", searchValDisp = "Todos", searchValFromDate = "SVFD", searchValToDate = "SVTD";
	...
	sbSql.append("select id, descripcion, to_char(fecha_creacion, 'dd/mm/yyyy') fecha_creacion, usuario_creacion, estado, observacion, decode(estado, 'A', 'Activo', 'I', 'Inactivo') estado_desc from tbl_cxc_tipo_otro_cliente where compania = "+session.getAttribute("_companyId")+"  ");
	if(!UserDet.getUserProfile().contains("0")){sbSql.append(" and id > 0 ");}
...
function add(){
	abrir_ventana('../pos/reg_tipo_otro_cliente.jsp?mode=add');
}

function edit(id){
	abrir_ventana('../pos/reg_tipo_otro_cliente.jsp?mode=edit&id='+id);
}

function printList(){
	//abrir_ventana('../pos/print_reg_tipo_otro_cliente.jsp?appendFilter=<%=issi.admin.IBIZEscapeChars.forURL(sbFilter.toString())%>');
}
</script>
</head>
