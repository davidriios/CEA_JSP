...
sbSql.append("), 'NA') desc_articulo_gral from dual");
...
function selCliente(){
	...
	else showPopWin('../pos/sel_otros_cliente.jsp?fp=proforma&mode=<%=mode%>&tipo_factura='+tipo_factura+'&tipo_pos='+tipo_pos+'&ref_id='+ref_id+'&Refer_To='+refer_to,winWidth*.99,_contentHeight*.99,null,null,'');
}
  ...
  if(codigo) {
    if(cds != "") {
       ...
       CBMSG.warning("Usted está cambiando el Centro de Servicio por lo cual se reiniciará la transacción!", {
         cb: function(r){
            if (r=='Ok') window.location = '../pos/proforma.jsp?cds='+codigo+'&familia=<%=familia%>&almacen=<%=almacen%>&artType=<%=artType%>&tipo_pos=<%=tipo_pos%>';
         }
       });
    }
  }
...
<tr class="TextHeader">
	...
	<input type="hidden" name="tipo_factura" id="tipo_factura" value="CO"> 
Fecha:
				...
				<% if (docType2Btn){%>
				   ...
				   <%if(tipoDocto.equals("")){%>
				      <authtype type="53">
				      <a id="btn_fac" name="btn_fac" onClick="javascript:showList('FAC');" class="btn_red_link">PROFORMA.</a></authtype>
					 
				   <%} else if(tipoDocto.equals("F")){%>
				     <authtype type="53">
				      <a id="btn_fac" name="btn_fac" onClick="javascript:showList('FAC');" class="CellbyteBtn">PROFORMA.</a></authtype>
				   <%}%>
				<%}else{%>
				...
				<%if(tipoDocto.equals("")){%>
				<option value="FAC">PROFORMA</option> 
				<%} else if(tipoDocto.equals("F")){%>
				<option value="FAC">V</option> 
				<%}%>
	...
	<td align="right">&nbsp; <!-- <authtype type="50"><a href="javascript:printProforma();" class="btn_red_link">Proforma</a></authtype>-->
