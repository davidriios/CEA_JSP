...
if (request.getMethod().equalsIgnoreCase("GET")){
	...
	sbSql.append(session.getAttribute("_companyId"));
	sbSql.append(",'CHECK_DISP'),'S') as checkDisp, nvl(get_sec_comp_param(");
	sbSql.append(session.getAttribute("_companyId"));
	sbSql.append(",'POS_PROF_REBAJA_INV'),'S') as rebajaInv,case when nvl(get_sec_comp_param(");
	sbSql.append(session.getAttribute("_companyId"));
	sbSql.append(",'CHECK_DISP'),'S') = 'S' and nvl(get_sec_comp_param(");
	sbSql.append(session.getAttribute("_companyId"));
	sbSql.append(",'POS_PROF_REBAJA_INV'),'S') ='S' then 'S' else 'N' end as checkAvailability from dual");
...
function addItem(i,tLinea){
	...
	if((refer_to=='EMPL' || refer_to=='EMPO') && size>0 && tipo_factura=='CR' && es_combo_adicional=='N' && '<%=tipo_pos%>' == 'CAF') alert('No puede vender más de un artículo a crédito a un colaborador!');
	else {
		...
		if(tipo_docto=='NCR' && parseInt(eval('document.articles.cantidad'+i).value) < 0){
			...
			if(spn!='' && chkQtyNC(i)){
			var txt = '';
			$.ajax({url:'../pos/proforma_det.jsp?'+spn+'&adding=add&refer_to='+refer_to+'&timestamp='+Date.now(),async:false,timeout:5000,success:function(txt){
...
function setArticles(obj){
	...
	else window.location = '../pos/sel_articles_proforma.jsp?fp=fact_cafeteria&artType='+artType+'&doc_id='+doc_id+'&almacen=<%=almacen%>&familia=<%=familia%>&tipo_pos=<%=tipo_pos%>';
}
...
<%
	...
	if(request.getParameter("addCont")!=null){
		response.sendRedirect(request.getContextPath()+request.getServletPath()+"?mode="+mode+"&id="+id+"&change=1&type=1&fg="+fg+"&fp="+fp);
		return;
	}
%>
<html>
