...
String filter="and b.codigo_clase in ('140','145','160','170') and d.codigo=1 and c.codigo_prin='1' and a.recibe_mov='S'";
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/presupuesto/tipoinversion_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/presupuesto/tipoinversion_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/presupuesto/tipoinversion_list.jsp';
<%
	}
%>
	window.close();
