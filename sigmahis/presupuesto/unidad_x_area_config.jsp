...
function addUnidad()
{
  abrir_ventana2('../presupuesto/area_unidadesadm_list.jsp?id=1');
}
</script>
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<tr class="TextRow01">
				<td><cellbytelabel>Unidad Adm</cellbytelabel>.</td>
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/presupuesto/unidad_x_area_list.jsp?areaCode="+areaCode))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/presupuesto/unidad_x_area_list.jsp?areaCode="+areaCode)%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/presupuesto/unidad_x_area_list.jsp?areaCode=<%=areaCode%>';
<%
	}
%>
	window.close();
