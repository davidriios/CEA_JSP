...
function doSubmit(){
	...
	var x = 'N';
	if(getDBData('<%=request.getContextPath()%>','\'S\'','dual','trunc(sysdate) >= to_date(\''+fecha+'\',\'dd/mm/yyyy\')')=='S' && getDBData('<%=request.getContextPath()%>','estatus', 'tbl_con_estado_meses', 'mes = to_number(to_char(to_date(\''+fecha+'\',\'dd/mm/yyyy\'),\'mm\')) and ano = to_number(to_char(to_date(\''+fecha+'\',\'dd/mm/yyyy\'),\'yyyy\')) and cod_cia = <%=(String) session.getAttribute("_companyId")%>')!='CER') document.form1.submit();
	else CBMSG.warning('No puede correr proceso para fecha superior a la actual, o en meses cerrados!');
}
</script>
</head>
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<%=fb.hidden("estado",estado)%>
				...
				<tr class="TextRow01">
					<td colspan="2" align="center"><cellbytelabe><font class="RedTextBold">Est&aacute; seguro de Correr el Job de Facturas?</font></cellbytelabel></td>
				</tr>
				<tr class="TextRow01">
...
else
{
  ...
  code = request.getParameter("code");
	sql.append("call sp_pm_gen_factura_manual(	'");
	sql.append(request.getParameter("fecha"));
	sql.append("')");
