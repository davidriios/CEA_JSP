...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
	...
	String estado  = "";    // variables para mantener el valor de los campos filtrados en la consulta
	String codigo  = "";
	String descrip = "";
	 ...
	 if (request.getParameter("estado") != null)
	 {
		...
		{
		   appendFilter += appendFilter+" and estado = '"+request.getParameter("estado").toUpperCase()+"'";	
			 estado  =  request.getParameter("estado"); // utilizada para mantener el estado del plan que se filtró
		}else{
				appendFilter+=appendFilter+" and upper(estado)<>'I'";
				estado  =  request.getParameter("estado");  // utilizada para mantener el estado del plan que se filtró
			 }
	 }else{
			 appendFilter+=appendFilter+" and upper(estado)<>'I'";
    ...
    searchDisp = "descripcion";
		descrip    = request.getParameter("descripcion"); // utilizada para mantener la descripción del plan de hospedaje filtrado
	}
  ...
  else
  {
    ...
    searchDisp="Listado";
		...
		sql = "select codigo, descripcion, decode(estado,'A','ACTIVO','I','INACTIVO') as estado from tbl_res_planes WHERE codigo<>0 "+appendFilter;
    al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) count from tbl_res_planes WHERE codigo<>0 "+appendFilter);
...
function add()
{
	abrir_ventana('../residencial/plan_hospedaje_config.jsp');
}

function edit(id)
{
	abrir_ventana('../residencial/plan_hospedaje_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../residencial/print_list_planes_hospedaje.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}

function addHabit(id,desc)
{
    abrir_ventana('../residencial/precio_planhabit_config.jsp?id='+id+'&desc='+desc);
}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="RESIDENCIAL - PLANES DE HOSPEDAJE"></jsp:param>
</jsp:include>
...
//{
%>
			<a href="javascript:add()" class="Link00">[ Registrar Nuevo Plan de hospedaje ]</a>
<%
//}
%>
		</td>
...
%>
			...
			<td>
			<a href="javascript:addHabit(<%=cdo.getColValue("codigo")%>,'<%=cdo.getColValue("descripcion")%>')" class="Link02Bold" onMouseOver="setoverc(this,'Link01Bold')" onMouseOut="setoutc(this,'Link02Bold')">Habitaci&oacute;n</a>
			</td>
		</tr>
<%
}
