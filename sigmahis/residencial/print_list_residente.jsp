...
sql ="SELECT a.provincia, a.sigla, a.tomo, a.asiento, a.provincia||'-'||a.sigla||'-'||a.tomo||'-'||a.asiento as cedula, a.primer_nombre||' '||a.segundo_nombre as nombre, a.primer_apellido||' '||a.segundo_apellido as apellido, b.nacionalidad, a.edad "
+ " FROM tbl_res_residente a, tbl_sec_pais b"
+ " WHERE a.compania="+(String) session.getAttribute("_companyId")+" and a.nacionalidad=b.codigo"+appendFilter;
...
if(request.getMethod().equalsIgnoreCase("GET")) {
		...
		String folderName = "residencial";
		String fileNamePrefix = "print_list_residente";
...
for (int j=1; j<=nPages; j++)
{
			Vector setHeader0=new Vector();
				...
				pc.createTable();
				pc.setFont(9, 1);
				  pc.addCols("Residencial",1,1);
					pc.addCols("RESIDENTES", 1, 1);
				pc.addTable();
...
//				pc.addTable();
	...
	//				setHeader2.addElement(".30");
				...
				pc.createTable();
					...
					pc.addBorderCols("Nacionalidad",1);
				...
				{
					pc.createTable();
						pc.addCols("No existe el Residente seleccionado",1,5);
					pc.addTable();
				}
				else
				{
						...
						for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
							{
								...
								pc.setFont(7, 0);
									...
									pc.addCols(" "+cdo1.getColValue("apellido"),0,1);
