...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
	...
	if (request.getParameter("anio") != null && !request.getParameter("anio").trim().equals(""))
	{
		...
		anio     = request.getParameter("anio");  // utilizada para mantener el Cód. del Tipo de Empleado
	}
...
function add(){abrir_ventana('../rhplanilla/calculo_planilla_acr.jsp');} 
function edit(cod,num,anio,cia){abrir_ventana('../rhplanilla/pago_planilla_acr_list.jsp?cod='+cod+'&num='+num+'&anio='+anio+'&cia='+cia);}
function borrar(cod,num,anio,cia){if(confirm('Está seguro de    B O R R A R    la  Planilla.')){showPopWin('../common/run_process.jsp?fp=DELPLA&actType=51&docType=DELPLA&docId='+cod+' - '+anio+' - '+num+'&docNo='+anio+'&compania=<%=(String) session.getAttribute("_companyId")%>&anio='+anio+'&codPlanilla='+cod+'&numPlanilla='+num,winWidth*.75,winHeight*.65,null,null,'');}else CBMSG.warning('Proceso Cancelado por el usuario.');}
function aprobar(cod,num,anio,cia){if(confirm('Está seguro de    A P R O B A R     la  Planilla.\nQuedará en Estado D E F I N I T I V A .... Desea Continuar!!')){showPopWin('../common/run_process.jsp?fp=DELPLA&actType=52&docType=DELPLA&docId='+cod+' - '+anio+' - '+num+'&docNo='+anio+'&compania=<%=(String) session.getAttribute("_companyId")%>&anio='+anio+'&codPlanilla='+cod+'&numPlanilla='+num,winWidth*.75,winHeight*.65,null,null,'');}else CBMSG.warning('Proceso Cancelado por el usuario.');}
function printList(){abrir_ventana('../rhplanilla/print_list_calculo_planilla.jsp?fp=acreedor&appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');}
var xHeight=0;
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="PLANILLA - CALCULO DE PLANILLA DE ACREEDORES "></jsp:param>
</jsp:include>
<table align="center" width="99%" cellpadding="1" cellspacing="0" id="_tblMain">
  <tr>
    <td align="right"> 
			<authtype type='3'><a href="javascript:add()" class="Link00">[ Registrar Nueva Planilla ]</a> </authtype>
		</td>
  </tr>
	<tr>
		<td>
<!-- ================================   S E A R C H   E N G I N E S   S T A R T   H E R E   ================================ -->
			...
			<tr class="TextFilter">		
				...
				Año:<%=fb.intBox("anio",anio,false,false,false,10)%>&nbsp;&nbsp;&nbsp;
 				...
 				<td width="33%">
				<cellbytelabel id="5">Fecha de Pago:</cellbytelabel>
				<jsp:include page="../common/calendar.jsp" flush="true">
				<jsp:param name="noOfDateTBox" value="1"/>
				<jsp:param name="nameOfTBox1" value="fecha_pago"/>
				...
				<%fb.appendJsValidation("if((document.search01.fecha_pago.value!='' && !isValidateDate(document.search01.fecha_pago.value))){CBMSG.warning('Formato de fecha inválida!');error++;}");%>
				<%=fb.formEnd(true)%>
			</tr>
			</table>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
		...
		<tr class="TextHeader" align="center">
			<td width="15%">Año - No.Planilla</td>
...
for (int i=0; i<al.size(); i++)
{
	...
	if (i % 2 == 0) color = "TextRow01";
		...
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="center"> <authtype type='1'><a href="javascript:edit(<%=cdo.getColValue("codPlanilla")%>,<%=cdo.getColValue("numPlanilla")%>,<%=cdo.getColValue("anio")%>,<%=cdo.getColValue("compania")%>)" class="Link02Bold" onMouseOver="setoverc(this,'Link01Bold')" onMouseOut="setoutc(this,'Link02Bold')">  Ver&nbsp;&nbsp;&nbsp;&nbsp;</a></authtype>
			
			
<%  if (!estado.equalsIgnoreCase(cdo.getColValue("estado")))
 {
%>
    <a  class="Link00" >Planilla&nbsp;</a>
	<a  class="Link00" >Cerrada</a>
	...
	<authtype type='7'><a href="javascript:borrar(<%=cdo.getColValue("codPlanilla")%>,<%=cdo.getColValue("numPlanilla")%>,<%=cdo.getColValue("anio")%>,<%=cdo.getColValue("compania")%>)" class="Link02Bold" onMouseOver="setoverc(this,'Link01Bold')" onMouseOut="setoutc(this,'Link02Bold')">Borrar</a></authtype>
<%}%>
			</td>
		</tr>
