...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  sql = "select codigo, descripcion from tbl_pla_permiso_fdes "+appendFilter;
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_pla_permiso_fdes "+appendFilter);
...
document.title = 'Forma de Descuento Para los Permisos - '+document.title;

function add()
{
	abrir_ventana('descuentopermiso_config.jsp');
}

function edit(id)
{
	abrir_ventana('descuentopermiso_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('print_list_descuentopermiso.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
		...
		//  {
        %>
	 <authtype type='3'>   <a href="javascript:add()" class="Link00">[ Registrar Nueva Forma de Descuento Para los Permisos ]</a></authtype>
   	    <%
		 //}
	    %>
	    </td>
