...
{
	if(change == null && sem_fecha_inicio != null && sem_fecha_final != null && fecha_inicio != null && fecha_final != null){
		sql = "select distinct a.provincia, a.sigla, a.tomo, a.asiento, a.compania, to_char(a.fecha, 'dd/mm/yyyy') fecha, a.codigo_posterior_programa programado, a.codigo_turno_posterior turno, to_char (a.fecha, 'DAY') dia, decode(a.codigo_posterior_programa, 'S', decode(isNumber(a.codigo_turno_posterior), 1, (select to_char(hora_entrada, 'hh:mi') || decode(hora_rec_salida, null, ' / ' || to_char(hora_salida, 'hh:mi'), '-' || to_char(hora_rec_salida, 'hh:mi') || ' / ' || to_char(hora_rec_entrada, 'hh:mi') || '-' || to_char(hora_salida, 'hh:mi')) from tbl_pla_ct_turno where compania = a.compania and codigo = a.codigo_turno_posterior), decode(a.codigo_turno_posterior, 'A', 'AUSENCIA', 'LC', 'LIC. CON SUELDO', 'LS', 'LIBRE SEMANAL', 'LN', 'LIBRE NACIONAL', 'PC', 'PERMISO CON SUELDO', 'PS', 'PERMISO SIN SUELDO', 'HD', 'HORAS DE DESCANSO', 'I', 'INCAPACIDAD', 'LG', 'LIC. POR GRAVIDEZ', 'V', 'VACACIONES', 'RP', 'RIESGO PROFESIONAL', a.codigo_turno_posterior)), decode(isNumber(a.codigo_turno_posterior), 1, (select to_char(hora_entrada, 'hh:mi') || decode(hora_salida_almuerzo, null, ' / ' || to_char(hora_salida, 'hh:mi'), '-' || to_char(hora_salida_almuerzo, 'hh:mi') || ' / ' || to_char(hora_entrada_almuerzo, 'hh:mi') || '-' || to_char(hora_salida, 'hh:mi')) from tbl_pla_horario_trab where compania = a.compania and codigo = a.codigo_turno_posterior), decode(a.codigo_turno_posterior, 'A', 'AUSENCIA', 'LC', 'LIC. CON SUELDO', 'LS', 'LIBRE SEMANAL', 'LN', 'LIBRE NACIONAL', 'PC', 'PERMISO CON SUELDO', 'PS', 'PERMISO SIN SUELDO', 'HD', 'HORAS DE DESCANSO', 'I', 'INCAPACIDAD', 'LG', 'LIC. POR GRAVIDEZ', 'V', 'VACACIONES', 'RP', 'RIESGO PROFESIONAL', a.codigo_turno_posterior))) post_turno_descripcion, to_char(a.fecha, 'DAY', 'NLS_DATE_LANGUAGE=SPANISH') nombre_dia, to_char(a.fecha, 'IW') num_semana, nvl(b.v_cantidad_horas, 0) total_horas from tbl_pla_at_det_dist a, (select x.compania, x.emp_id, to_date(to_char(x.fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy') fecha, sum(nvl(x.cantidad, 0)) v_cantidad_horas from tbl_pla_at_det_dist x where x.emp_id = "+emp_id+" and x.compania = "+(String) session.getAttribute("_companyId")+" and (x.generado = 'N' or x.generado is null) group by x.compania, x.emp_id, to_date(to_char(x.fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy')) b where a.compania = "+(String) session.getAttribute("_companyId")+" and a.emp_id = "+emp_id+" and to_date(to_char(a.fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy') >= to_date('"+fecha_inicio+"', 'dd/mm/yyyy') and to_date(to_char(a.fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy') <= to_date('"+fecha_final+"', 'dd/mm/yyyy') and to_date(to_char(a.fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy') >= to_date('"+sem_fecha_inicio+"', 'dd/mm/yyyy') and to_date(to_char(a.fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy') <= to_date('"+sem_fecha_final+"', 'dd/mm/yyyy') and a.emp_id = b.emp_id(+) and a.compania = b.compania(+) and to_date(to_char(a.fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy') = to_date(to_char(b.fecha(+), 'dd/mm/yyyy'), 'dd/mm/yyyy') order by compania, provincia, sigla, tomo, asiento, to_date(to_char(a.fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy')";
		alTPR = SQLMgr.getDataList(sql);
		System.out.println("SQL DIAS\n"+sql);
	}
%>
<html>
<head>
  ...
  <tr class="TextHeaderOver">
      ...
      <table width="100%" align="center"  cellpadding="1" cellspacing="1" bordercolor="#FFFFFF">
        ...
        <tr class="<%=color%>" align="center" onClick="javascript:parent.window.setMarDetValues(<%=i%>)" style="cursor:pointer">
          ...
          <td align="center"><%=fb.textBox("nombre_dia"+i,cd.getColValue("nombre_dia"),false,false,true,15,"text10","","")%></td>
