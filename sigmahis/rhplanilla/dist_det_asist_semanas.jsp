...
//		sql= "select b.semana, b.anio, decode(b.semana||b.anio,'12013', to_char(next_day(to_date('01/01/' || b.anio, 'dd/mm/yyyy') + ((b.semana -2 ) * 7), 'MONDAY'), 'dd/mm/yyyy'),  to_char(next_day(to_date('01/01/' || b.anio, 'dd/mm/yyyy') + ((b.semana - 2) * 7), 'MONDAY'), 'dd/mm/yyyy')) as ini_week,  decode((  to_date(decode(b.semana||b.anio,'12013',to_char(next_day(to_date('01/01/' || b.anio, 'dd/mm/yyyy') + ((b.semana -2) * 7), 'MONDAY') - 1, 'dd/mm/yyyy'), to_char(next_day(to_date('01/01/' || b.anio, 'dd/mm/yyyy') + ((b.semana - 2 ) * 7), 'MONDAY') + 6, 'dd/mm/yyyy')),'dd/mm/yyyy') - to_date('"+fecha_final+"','dd/mm/yyyy')  ) - abs( to_date('"+fecha_final+"','dd/mm/yyyy') - to_date(decode(b.semana||b.anio,'12013',to_char(next_day(to_date('01/01/' || b.anio, 'dd/mm/yyyy') + ((b.semana -2) * 7), 'MONDAY') - 1, 'dd/mm/yyyy'), to_char(next_day(to_date('01/01/' || b.anio, 'dd/mm/yyyy') + ((b.semana - 2 ) * 7), 'MONDAY') + 6, 'dd/mm/yyyy')),'dd/mm/yyyy')), 0, to_char(to_date('"+fecha_final+"','dd/mm/yyyy'),'dd/mm/yyyy'),decode(b.semana||b.anio,'12013',to_char(next_day(to_date('01/01/' || b.anio, 'dd/mm/yyyy') + ((b.semana -2) * 7), 'MONDAY') - 1, 'dd/mm/yyyy'), to_char(next_day(to_date('01/01/' || b.anio, 'dd/mm/yyyy') + ((b.semana - 2 ) * 7), 'MONDAY') + 6, 'dd/mm/yyyy'))) as end_week from (select distinct (to_number(to_char(fecha, 'IW'))) semana, decode(to_char(to_date(fecha,'dd/mm/yyyy'),'dd/mm'),'31/12',to_number(to_char(fecha, 'yyyy'))+1,to_number(to_char(fecha, 'yyyy'))) anio from tbl_pla_at_det_empfecha where ue_codigo = "+grupo+" and anio = "+anio+" and periodo = decode("+quincena+", 1, ("+mes+" * 2)-1, "+mes+" * 2)/* and fecha >= to_date('"+fecha_inicio+"','dd/mm/yyyy')*/ and fecha  <= to_date('"+fecha_final+"','dd/mm/yyyy')  and compania = "+(String) session.getAttribute("_companyId")+") b order by b.semana";
		...
		System.out.println("SQL SEMANAS\n"+sql);
