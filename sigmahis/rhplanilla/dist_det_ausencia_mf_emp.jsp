...
{
	if(change == null && grupo != null && area != null){
		sql = "select distinct a.emp_id, a.provincia, a.sigla, a.tomo, a.asiento, b.num_empleado, b.primer_nombre || ' ' || decode(b.sexo, 'F', decode(b.apellido_casada, null, b.primer_apellido, decode(b.usar_apellido_casada, 'S', 'DE ' || b.apellido_casada, b.primer_apellido)), b.primer_apellido) nombre_empleado, a.compania, b.rata_hora, decode (a.provincia, 0, ' ', 00, ' ', 10, '0', 11, 'B', 12, 'C', a.provincia) || rpad(decode(a.sigla, '00', '  ', '0', '  ', a.sigla), 2, ' ') || '-' || lpad(to_char(a.tomo), 3, '0') || '-' || lpad(to_char(a.asiento), 6, '0') cedula, nvl(d.total_horas, 0) total_horas, (b.rata_hora * nvl(d.total_horas, 0)) total_pagar from tbl_pla_at_det_dist a, tbl_pla_empleado b, tbl_pla_ct_empleado c, (select emp_id, sum (nvl (cantidad, 0)) total_horas from tbl_pla_at_det_dist where compania = "+(String) session.getAttribute("_companyId")+" and (generado = 'N' or generado is null) and to_date(to_char(fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy') >= to_date('"+fecha_inicio+"', 'dd/mm/yyyy') and to_date(to_char(fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy') <= to_date('"+fecha_final+"', 'dd/mm/yyyy') group by emp_id) d where to_date(to_char(fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy') >= to_date ('"+fecha_inicio+"','dd/mm/yyyy') and to_date(to_char(fecha, 'dd/mm/yyyy'), 'dd/mm/yyyy') <=to_date ('"+fecha_final+"', 'dd/mm/yyyy') and b.emp_id = a.emp_id and b.compania = a.compania and c.emp_id = a.emp_id and c.compania = a.compania and c.grupo = "+grupo+" and c.ubicacion_fisica like "+area+" and b.emp_id = d.emp_id(+) order by b.num_empleado";

		System.out.println("SQL EMPLEADOS=\n"+sql);
...
function doAction(){
	parent.window.setTipoMotivoDiaValuesOnLoad();
}
...
function setDiasValues(i){
	...
	parent.window.setTipoMotivoDiaValues(i);
}
  ...
  <tr class="TextHeaderOver">
      ...
      <table width="100%" align="center"  cellpadding="1" cellspacing="1" bordercolor="#FFFFFF">
        <tr class="TextHeader02" height="21">
          ...
          <td align="center">Total Descontar</td>
          <td align="center">&nbsp;</td>
        </tr>
        <%
        ...
        <tr class="<%=color%>" align="center" onClick="javascript:setDiasValues(<%=i%>)" style="cursor:pointer">
          ...
          <td align="center"><%=CmnMgr.getFormattedDecimal(cdo.getColValue("rata_hora"))%></td>
          <td align="center"><%=CmnMgr.getFormattedDecimal(cdo.getColValue("total_horas"))%></td>
          <td align="center"><%=CmnMgr.getFormattedDecimal(cdo.getColValue("total_pagar"))%></td>
