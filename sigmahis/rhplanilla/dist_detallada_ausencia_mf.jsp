...
function setValues(){
	...
	if(anio!='' && mes !='' && quincena != ''){
		window.location = '../rhplanilla/dist_detallada_ausencia_mf.jsp?anio='+anio+'&mes='+mes+'&fp=<%=fp%>&quincena='+quincena;
	}
}
...
function setTextValues(){
	...
	window.frames['itemFrame1'].location = '../rhplanilla/dist_det_ausencia_mf_emp.jsp?grupo='+grupo+'&area='+uf_codigo+'&fecha_inicio='+fecha_inicio+'&fecha_final='+fecha_final;
}

function setTipoMotivoDiaValues(i){
	var emp_id = window.frames['itemFrame1'].document.getElementById('emp_id'+i).value
	var fecha_inicio = document.form1.fecha_inicio.value;
	var fecha_final = document.form1.fecha_final.value;
	window.frames['itemFrame4'].location = '../rhplanilla/dist_det_ausencia_mf_tipo.jsp?fp=<%=fp%>&emp_id='+emp_id+'&fecha_inicio='+fecha_inicio+'&fecha_final='+fecha_final;
	//var i = getRadioButtonValue(window.frames['itemFrame4'].document.form.rb);
	//var motivo = window.frames['itemFrame4'].document.getElementById('motivo'+i).value;
	//window.frames['itemFrame5'].location = '../rhplanilla/dist_det_ausencia_mf_dia.jsp?emp_id='+emp_id+'&fecha_inicio='+fecha_inicio+'&fecha_final='+fecha_final+'&motivo='+motivo;
}

function setTipoMotivoDiaValuesOnLoad(){
	if(window.frames['itemFrame1'].document.form.rb){
		var i = getRadioButtonValue(window.frames['itemFrame1'].document.form.rb);
		setTipoMotivoDiaValues(i);
	} else {
		window.frames['itemFrame4'].location = '../rhplanilla/dist_det_ausencia_mf_tipo.jsp';
		window.frames['itemFrame5'].location = '../rhplanilla/dist_det_ausencia_mf_dia.jsp';
	}
}

function setDiasDet(i){
	var motivo		= window.frames['itemFrame4'].document.getElementById('motivo'+i).value;
	var fecha_inicio = document.form1.fecha_inicio.value;
	var fecha_final = document.form1.fecha_final.value;
	var emp_id = '';
	...
	window.frames['itemFrame5'].location = '../rhplanilla/dist_det_ausencia_mf_dia.jsp?emp_id='+emp_id+'&motivo='+motivo+'&fecha_inicio='+fecha_inicio+'&fecha_final='+fecha_final;
}

function setDiasDetOnLoad(){
          ...
          <td class="TableBorder"><table align="center" width="100%" cellpadding="5" cellspacing="0">
              <tr>
                <td><table align="center" width="100%" cellpadding="0" cellspacing="1">
                    ...
                    <tr height="250">
                      <td width="100%" colspan="3"><iframe name="itemFrame1" id="itemFrame1" frameborder="0" align="center" width="100%" height="220" scrolling="yes" src="../rhplanilla/dist_det_ausencia_mf_emp.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>"></iframe></td>
                    </tr>
                    <tr>
                      <td colspan="3"><table width="100%" cellpadding="1" cellspacing="0">
                      <tr>
                      <td width="60%"><iframe name="itemFrame4" id="itemFrame4" frameborder="0" align="center" width="100%" height="180" scrolling="yes" src="../rhplanilla/dist_det_ausencia_mf_tipo.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>"></iframe></td>
                      <td width="40%"><iframe name="itemFrame5" id="itemFrame5" frameborder="0" align="center" width="100%" height="180" scrolling="yes" src="../rhplanilla/dist_det_ausencia_mf_dia.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>"></iframe></td>
                    </tr></table></td></tr>
                    <tr class="TextRow02">
