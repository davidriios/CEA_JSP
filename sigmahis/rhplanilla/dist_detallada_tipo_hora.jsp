...
function setValues(){
	...
	if(anio!='' && mes !='' && quincena != ''){
		window.location = '../rhplanilla/dist_detallada_tipo_hora.jsp?anio='+anio+'&mes='+mes+'&fp=<%=fp%>&quincena='+quincena;
	}
}
...
function setTextValues(){
	...
	if(anio!='' && mes !='' && quincena != ''){
	window.frames['itemFrame1'].location = '../rhplanilla/dist_det_tipo_hora_emp.jsp?grupo='+grupo+'&area='+uf_codigo+'&anio='+anio+'&mes='+mes+'&fp=<%=fp%>&quincena='+quincena+'&fecha_inicio='+fecha_inicio+'&fecha_final='+fecha_final;
	}
}

function setTipoExtraValues(i){
	var mes = document.form1.mes.value;
	var anio = document.form1.anio.value;
	var quincena = document.form1.quincena.value;
	checkRadioButton(window.frames['itemFrame1'].document.form.rb, i);
	//window.frames['itemFrame1'].document.form.rb[i].checked=true;
	var emp_id = window.frames['itemFrame1'].document.getElementById('emp_id'+i).value
	window.frames['itemFrame4'].location = '../rhplanilla/dist_det_tipo_hora_extra.jsp?fp=<%=fp%>&emp_id='+emp_id+'&anio='+anio+'&mes='+mes+'&quincena='+quincena;
	window.frames['itemFrame5'].location = '../rhplanilla/dist_det_tipo_hora_dias_det.jsp?emp_id='+emp_id+'&anio='+anio+'&mes='+mes+'&quincena='+quincena;
}

function setTipoHorasDist(i){
	var tipo_hextra		= window.frames['itemFrame4'].document.getElementById('tipo_hextra'+i).value;
	var emp_id = '';
	var mes = document.form1.mes.value;
	var anio = document.form1.anio.value;
	var quincena = document.form1.quincena.value;
	checkRadioButton(window.frames['itemFrame4'].document.form.rb, i);
	if(window.frames['itemFrame1'].document.form.rb){
		var i = getRadioButtonValue(window.frames['itemFrame1'].document.form.rb);
		emp_id = window.frames['itemFrame1'].document.getElementById('emp_id'+i).value
		window.frames['itemFrame5'].location = '../rhplanilla/dist_det_tipo_hora_dias_det.jsp?emp_id='+emp_id+'&anio='+anio+'&mes='+mes+'&quincena='+quincena+'&tipo_he='+tipo_hextra;
	} else {
		window.frames['itemFrame5'].location = '../rhplanilla/dist_det_tipo_hora_dias_det.jsp';
	}
}

function setTipoHorasDistOnLoad(){
	if(window.frames['itemFrame4'].document.form.rb){
		var i = getRadioButtonValue(window.frames['itemFrame4'].document.form.rb);
		setTipoHorasDist(i);
	}
}
          ...
          <td class="TableBorder"><table align="center" width="100%" cellpadding="5" cellspacing="0">
              <tr>
                <td><table align="center" width="100%" cellpadding="0" cellspacing="1">
                    ...
                    <tr>
                      <td colspan="3"><table width="100%" cellpadding="1" cellspacing="0">
                          ...
                          <tr class="TextPanel">
                            ...
                            <%
														if(!anio.equals("") && !mes.equals("")){
															sql = "select c.codigo, c.codigo||'-'||c.descripcion from tbl_pla_ct_grupo c where c.compania = "+(String) session.getAttribute("_companyId")+"";
                            } else {
															sql = "select '' codigo, 'Seleccione' descripcion from dual";
														}
														%>
														<%=fb.select(ConMgr.getConnection(),sql,"grupo",grupo,false,false,0,"text10",null,"onChange=\"javascript:loadXML('../xml/areaXGrupo.xml','uf_codigo','','VALUE_COL','LABEL_COL','"+(String) session.getAttribute("_companyId")+"-'+this.value,'KEY_COL','')\"", "", "S")%>
                            &nbsp;
                    ...
                    <tr height="250">
                      <td width="100%" colspan="3"><iframe name="itemFrame1" id="itemFrame1" frameborder="0" align="center" width="100%" height="220" scrolling="yes" src="../rhplanilla/dist_det_tipo_hora_emp.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>"></iframe></td>
                    </tr>
                    <tr>
                      <td colspan="3"><table width="100%" cellpadding="1" cellspacing="0">
                      <tr>
                      <td width="60%"><iframe name="itemFrame4" id="itemFrame4" frameborder="0" align="center" width="100%" height="180" scrolling="yes" src="../rhplanilla/dist_det_tipo_hora_extra.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>"></iframe></td>
                      <td width="40%"><iframe name="itemFrame5" id="itemFrame5" frameborder="0" align="center" width="100%" height="180" scrolling="yes" src="../rhplanilla/dist_det_tipo_hora_dias_det.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>"></iframe></td>
                    </tr></table></td></tr>
                    <tr class="TextRow02">
                      <td align="right" colspan="3">
											<%//=fb.button("cerrar","Cerrar",true,false,"","","onClick=\"javascript:closeWindow();\"")%>
                      </td>
                    </tr>
                    <%=fb.formEnd(true)%>
