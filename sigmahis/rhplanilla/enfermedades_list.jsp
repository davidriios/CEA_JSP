...
/**
==================================================================================
800057	VER LISTA DE ENFERMEDAD
800058	IMPRIMIR LISTA DE ENFERMEDAD
800059	AGREGAR ENFERMEDAD
800060	MODIFICAR ENFERMEDAD
==================================================================================
**/
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");   // utilizada para mantener la descripción de la Enfermedad
	}
  ...
  else
  {
	...
	sql = "select codigo, descripcion from tbl_pla_enfermedad "+appendFilter+" order by descripcion";
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) count from tbl_pla_enfermedad "+appendFilter);
...
function add()
{
	abrir_ventana('../rhplanilla/reg_enfermedad.jsp');
}

function edit(id)
{
	abrir_ventana('../rhplanilla/reg_enfermedad.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../rhplanilla/print_list_enfermedad.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}

function del(codigo){
	var clientIdentifier = '<%=ConMgr.getClientIdentifier()%>';
	if(confirm('Está seguro(a) que desea eliminar?')){
		if(executeDB('<%=request.getContextPath()%>','call sp_par_del_mantenimiento(\'tbl_pla_enfermedad\',\'codigo\',\''+codigo+'\')','tbl_pla_enfermedad')){
			var msg = getMsg('<%=request.getContextPath()%>', clientIdentifier);
			if(msg!='') alert(msg);
			else alert('Registro eliminado satisfactoriamente!');
			window.location = '../rhplanilla/enfermedades_list.jsp';
		} else {
			var msg = getMsg('<%=request.getContextPath()%>', clientIdentifier);
			alert(msg);
		}
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="PLANILLA - ENFERMEDAD"></jsp:param>
</jsp:include>
...
{
%>
		<authtype type='3'>	<a href="javascript:add()" class="Link00">[ Registrar Nueva Enfermedad ]</a></authtype>
<%
}
%>
		</td>
