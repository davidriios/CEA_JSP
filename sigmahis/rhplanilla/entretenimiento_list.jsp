...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");    // utilizada para mantener la descripción del Entretenimiento
	}
  ...
  else
  {
	...
	sql = "select codigo, descripcion from tbl_pla_entretenimiento "+appendFilter+" order by descripcion";
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) count from tbl_pla_entretenimiento "+appendFilter);
...
function add()
{
	abrir_ventana('../rhplanilla/reg_entretenimiento.jsp');
}

function edit(id)
{
	abrir_ventana('../rhplanilla/reg_entretenimiento.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../rhplanilla/print_list_entretenimiento.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}

function del(codigo){
	var clientIdentifier = '<%=ConMgr.getClientIdentifier()%>';
	if(confirm('Está seguro(a) que desea eliminar?')){
		if(executeDB('<%=request.getContextPath()%>','call sp_par_del_mantenimiento(\'tbl_pla_entretenimiento\',\'codigo\',\''+codigo+'\')','tbl_pla_entretenimiento')){
			var msg = getMsg('<%=request.getContextPath()%>', clientIdentifier);
			if(msg!='') alert(msg);
			else alert('Registro eliminado satisfactoriamente!');
			window.location = '../rhplanilla/entretenimiento_list.jsp';
		} else {
			var msg = getMsg('<%=request.getContextPath()%>', clientIdentifier);
			alert(msg);
		}
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="PLANILLA - ENTRETENIMIENTO"></jsp:param>
</jsp:include>
...
{
%>
	<authtype type='3'>	<a href="javascript:add()" class="Link00">[ Registrar Nuevo Entretenimiento ]</a></authtype>
<%
}
%>
		</td>
