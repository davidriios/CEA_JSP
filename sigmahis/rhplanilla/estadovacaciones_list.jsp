...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("codigo") != null)
  {
    ...
    searchType = "1";
    searchDisp = "Código";
		codigo     = request.getParameter("codigo");    // utilizada para mantener el Código de Estado de Vacaciones
  }
  else if (request.getParameter("descripcion") != null)
  { 
    appendFilter += " Where upper(descricion) like '%"+request.getParameter("descripcion").toUpperCase()+"%'";
    searchOn = "descricion";
    searchVal = request.getParameter("descripcion");
    searchType = "1";
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");  // utilizada para mantener la Descripción del Estado
  }
  ...
  sql = "select codigo, descricion as descripcion from tbl_pla_estado_vac"+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_pla_estado_vac"+appendFilter);
...
document.title = 'Estados de las Vacaciones - '+document.title;

function add()
{
	abrir_ventana('estadovacaciones_config.jsp');
}

function edit(id)
{
	abrir_ventana('estadovacaciones_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('print_list_estadovacaciones.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}

function del(codigo){
	var clientIdentifier = '<%=ConMgr.getClientIdentifier()%>';
	if(confirm('Está seguro(a) que desea eliminar?')){
		if(executeDB('<%=request.getContextPath()%>','call sp_par_del_mantenimiento(\'tbl_pla_estado_vac\',\'codigo\',\''+codigo+'\')','tbl_pla_estado_vac')){
			var msg = getMsg('<%=request.getContextPath()%>', clientIdentifier);
			if(msg!='') alert(msg);
			else alert('Registro eliminado satisfactoriamente!');
			window.location = '../rhplanilla/estadovacaciones_list.jsp';
		} else {
			var msg = getMsg('<%=request.getContextPath()%>', clientIdentifier);
			alert(msg);
		}
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="RECURSOS HUMANOS - ESTADOS DE LAS VACACIONES"></jsp:param>
</jsp:include>
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
		...
		//  {
        %>
	<authtype type='3'>    <a href="javascript:add()" class="Link00">[ Registrar Nuevo Estados de las Vacaciones ]</a></authtype>
   	    <%
		 //}
	    %>
	    </td>
