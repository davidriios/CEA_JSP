...
if(request.getMethod().equalsIgnoreCase("GET")) {
	...
	if (!estado.trim().equals("")) { sbFilter.append(" and a.estado = "); sbFilter.append(estado); }
	if (!descripcion.trim().equals("")) { sbFilter.append(" and exists (select null from tbl_sec_unidad_ejec where compania = a.compania and codigo = nvl(a.ubic_seccion,a.seccion) and upper(descripcion) like '%"); sbFilter.append(descripcion.toUpperCase()); sbFilter.append("%')"); }
	if (!cargo.trim().equals("")) { sbFilter.append(" and exists (select null from tbl_pla_cargo where compania = a.compania and codigo = a.cargo and upper(denominacion) like '"); sbFilter.append(cargo.toUpperCase()); sbFilter.append("%')"); }
	...
	sbSql.append("select * from (select rownum as rn, a.* from (");
		sbSql.append("select a.cedula1 as cedula, a.provincia, a.sigla, a.tomo, a.asiento, a.compania, a.nombre_empleado as nombre, a.primer_nombre, a.primer_apellido, nvl(a.ubic_seccion,seccion) as seccion, a.emp_id as empId, a.estado, a.num_empleado as numEmpleado, to_char(a.fecha_ingreso,'dd/mm/yyyy') as  fechaIngreso, a.ubic_seccion as grupo, a.rata_hora as rata");
		sbSql.append(", nvl((select descripcion from tbl_sec_unidad_ejec where compania = a.compania and codigo = nvl(a.ubic_seccion,a.seccion)),' ') as descripcion");
		sbSql.append(", nvl((select denominacion from tbl_pla_cargo where compania = a.compania and codigo = a.cargo),' ') as denominacion");
		sbSql.append(", nvl((select descripcion from tbl_pla_estado_emp where codigo = a.estado),' ') as estadodesc");
		sbSql.append(" from vw_pla_empleado a where a.compania = ");
		sbSql.append(session.getAttribute("_companyId"));
		sbSql.append(sbFilter);
		sbSql.append(" order by a.ubic_seccion, a.nombre_empleado");
	...
	sbSql = new StringBuffer();
	sbSql.append("select count(*) from vw_pla_empleado a where a.compania = ");
	sbSql.append(session.getAttribute("_companyId"));
	sbSql.append(sbFilter);
...
function  printList(){abrir_ventana('../rhplanilla/print_list_expediente_empleados.jsp?appendFilter=<%=IBIZEscapeChars.forURL(sbFilter.toString())%>');}
function mouseOver(obj,option)
{
	...
	switch(option)
	{
		...
		case 4:msg='Corregir Cédula';break;
...
function goOption(option)
{
	...
	else if(option==0) abrir_ventana('../rhplanilla/expediente_empleado_config.jsp?fp=<%=fp%>');
	else
	{
	...
	else if(option==1)abrir_ventana('../rhplanilla/expediente_empleado_config.jsp?fp=<%=fp%>&mode=edit&prov='+prov+'&sig='+sigla+'&tom='+tomo+'&asi='+asiento+'&emp_id='+empId);
	else if (option==2) abrir_ventana('../rhplanilla/expediente_empleado_config.jsp?fp=<%=fp%>&mode=view&prov='+prov+'&sig='+sigla+'&tom='+tomo+'&asi='+asiento+'&emp_id='+empId);
	...
	else if (option==5)abrir_ventana('../rhplanilla/empl_fallecimiento_detail.jsp?mode=add&provincia='+prov+'&sigla='+sigla+'&tomo='+tomo+'&asiento='+asiento+"&empId="+empId+'&numEmpleado='+numEmp);
	else if (option==6) abrir_ventana('../rhplanilla/editar_cedula.jsp?mode=edit&id='+empId+'&fg=num_empl&fp=<%=fp%>');
	else if (option==7) abrir_ventana('../rhplanilla/reg_sobretiempo_config.jsp?mode=add&prov='+prov+'&sig='+sigla+'&tom='+tomo+'&asi='+asiento+'&grp='+grupo+'&num='+numEmp+'&rath='+rata+"&emp_id="+empId);
	...
	else if (option==10) abrir_ventana('../rhplanilla/registro_transacciones_list.jsp?empId='+empId);
...
<!-- ================================   S E A R C H   E N G I N E S   S T A R T   H E R E   ================================ -->
		...
		<%fb = new FormBean("search01",request.getContextPath()+"/common/urlRedirect.jsp");%>
			...
			<%=fb.hidden("fp",fp)%>
		<tr class="TextFilter">
			<td width="8%"><cellbytelabel>Nombre</cellbytelabel>:</td>
			<td width="20%"><%=fb.textBox("nombre","",false,false,false,25,null,null,null)%></td>
			<td width="8%"><cellbytelabel>C&eacute;dula</cellbytelabel>:</td>
			<td width="24%"><%=fb.textBox("cedula","",false,false,false,13,null,null,null)%></td>
			<td width="10%"><cellbytelabel>Estado</cellbytelabel>:</td>
			<td width="30%"><%=fb.select(ConMgr.getConnection(), "select codigo, descripcion from tbl_pla_estado_emp order by codigo", "estado",estado,false,false,0,"Text10",null,null,null,"T")%></td>
		</tr>
		<tr class="TextFilter">
			<td><cellbytelabel>Cargo</cellbytelabel>:</td>
			<td ><%=fb.textBox("cargo","",false,false,false,25,null,null,null)%></td>
			<td><cellbytelabel>Num. Empleado</cellbytelabel></td>
			<td><%=fb.textBox("numEmpleado","",false,false,false,13,null,null,null)%>ID.<%=fb.textBox("empId","",false,false,false,13,null,null,null)%></td>
