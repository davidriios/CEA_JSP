...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	if(fechaInicio != null && fechaFinal != null && grupo != null ){
		...
		sql=" select distinct (e.emp_id), e.provincia, e.sigla, e.tomo, e.asiento, e.unidad_organi ue_codigo, e.primer_nombre||' '||decode(e.sexo, 'F', decode(e.apellido_casada, null, e.primer_apellido, decode(e.usar_apellido_casada, 'S', 'DE '|| e.apellido_casada, e.primer_apellido)), e.primer_apellido) nombre_empleado, e.unidad_organi, to_char(e.fecha_ingreso, 'dd/mm/yyyy') fecha_ingreso, decode(e.provincia, 0, ' ', 00, ' ', 10, '0', 11, 'B', 12, 'C', e.provincia) || rpad(decode(e.sigla, '00', '  ', '0', '  ', e.sigla), 2, '  ') || '-'|| lpad(to_char(e.tomo), 3, '0')||'-' || lpad(to_char(e.asiento), 6, '0') dsp_cedula, e.estado, e.emp_id, b.descripcion, e.num_empleado , f.emp_id as control, f.fecha_vaca, f.dias_vac, substr(f.periodo_ac,1,4) anio_ac, to_number(substr(f.periodo_ac,5,2)) periodo_ac /*f.anio_ac, f.periodo_ac*/ from tbl_pla_estado_emp b, vw_pla_empleado e, (select  e.emp_id, max(decode(e.estado,'2',to_number(d.anio_ac||to_char(d.periodo_ac,'fm09')),'','')) as fecha_vaca , max(d.dias_vac) dias_vac ,  max(to_char(d.anio_ac)||to_char(d.periodo_ac,'fm09')) periodo_ac  /*max(d.anio_ac) anio_ac, max(to_char(d.periodo_ac,'fm09')) periodo_ac*/ from  tbl_pla_dist_dias_vac d, vw_pla_empleado e where e.emp_id = d.emp_id(+)  and e.compania = "+(String) session.getAttribute("_companyId")+" group by e.emp_id/*, d.dias_vac, d.anio_ac, d.periodo_ac*/ ) f where  e.estado = b.codigo and e.emp_id in (select distinct emp_id from tbl_pla_at_det_dist a where a.compania ="+(String) session.getAttribute("_companyId")+appendFilter+" and (a.trx_generada = 'N' or a.trx_generada is null)) and e.emp_id = f.emp_id";
  ...
  <tr class="TextHeaderOver">
      ...
      <table width="100%" align="center"  cellpadding="1" cellspacing="1" bordercolor="#FFFFFF">
		...
		<%if(cdo.getColValue("fecha_vaca").trim().equalsIgnoreCase("")){%>
		  ...
		  <%=fb.hidden("anioAc"+i, anio)%>
		  <%=fb.hidden("periodoAc"+i, periodo)%>
         ...
         <% } else {%>
		  ...
		  <%=fb.hidden("anioAc"+i, cdo.getColValue("anio_ac"))%>
		  <%=fb.hidden("periodoAc"+i, cdo.getColValue("periodo_ac"))%>
