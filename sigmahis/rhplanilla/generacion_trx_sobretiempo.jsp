...
function setValues(){
	...
	var hasta   = document.form1.fechaTrxHasta.value;
		...
		window.frames['itemFrame'].location = '../rhplanilla/genera_trx_sobretiempo_det.jsp?anio='+anio+'&desde='+desde+'&hasta='+hasta+'&periodo='+periodo+'&quincena='+quincena+'&finicio='+inicio+'&ffinal='+final+'&unidad='+unidad+'&fp=<%=fp%>';
...
<jsp:param name="title" value="GENERACION DE TRANSACCIONES DE SOBRETIEMPO"></jsp:param>
</jsp:include>
...
Fecha de Cierre : &nbsp;<%=fb.textBox("fecha",fecha,true,false,true,15,"text10","","")%>
                    ...
                    <tr>
                      <td><iframe name="itemFrame" id="itemFrame" frameborder="0" align="center" width="100%" height="60%" scrolling="yes" src="../rhplanilla/genera_trx_sobretiempo_det.jsp?fp=<%=fp%>&fg=<%=fg%>&mode=<%=mode%>&inicio=<%=fecha%>&final=<%=fecha%>&unidad=<%=unidad%>&anio=<%=anio%>&mes=<%=mes%>&periodo=<%=periodo%>"></iframe></td>
                    </tr>
                    <tr class="TextRow02">
                      <td align="right">
...
%>
	alert('<%=errMsg%>');
	window.location = '<%=request.getContextPath()+request.getServletPath()%>?mode=add&unidad=<%=unidad%>&inicio=<%=finicio%>&final=<%=ffinal%>&quincena=<%=quincena%>&periodo=<%=request.getParameter("periodo")%>&anio=<%=request.getParameter("anio")%>&mes=<%=request.getParameter("mes")%>';
<%
