...
/**
==================================================================================
800045	VER LISTA DE TIPO DE HABILIDAD
800046	IMPRIMIR LISTA DE TIPO DE HABILIDAD
800047	AGREGAR TIPO DE HABILIDAD
800048	MODIFICAR TIPO DE HABILIDAD
==================================================================================
**/
SecMgr.setConnection(ConMgr);
if (!SecMgr.checkLogin(session.getId())) throw new Exception("Usted está fuera del sistema. Por favor entre al sistema con su nombre de usuario y clave!!!");
//if (!(SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"800045") || SecMgr.checkAccess(session.getId(),"800046") || SecMgr.checkAccess(session.getId(),"800047") || SecMgr.checkAccess(session.getId(),"800048"))) throw new Exception("Usted no tiene los suficientes Derechos de Acceso a esta página.");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");   // utilizada para mantener la descripción de la Habilidad
	}
  ...
  else
  {
	...
	sql = "select codigo, descripcion from tbl_pla_habilidad "+appendFilter+" order by descripcion";
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) count from tbl_pla_habilidad "+appendFilter);
...
function add()
{
	abrir_ventana('../rhplanilla/reg_tipo_habilidad.jsp');
}

function edit(id)
{
	abrir_ventana('../rhplanilla/reg_tipo_habilidad.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../rhplanilla/print_list_tipo_habilidad.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}

function del(codigo){
	var clientIdentifier = '<%=ConMgr.getClientIdentifier()%>';
	if(confirm('Está seguro(a) que desea eliminar?')){
		if(executeDB('<%=request.getContextPath()%>','call sp_par_del_mantenimiento(\'tbl_pla_habilidad\',\'codigo\',\''+codigo+'\')','tbl_pla_habilidad')){
			var msg = getMsg('<%=request.getContextPath()%>', clientIdentifier);
			if(msg!='') alert(msg);
			else alert('Registro eliminado satisfactoriamente!');
			window.location = '../rhplanilla/habilidades_list.jsp';
		} else {
			var msg = getMsg('<%=request.getContextPath()%>', clientIdentifier);
			alert(msg);
		}
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="PLANILLA - TIPO DE HABILIDAD"></jsp:param>
</jsp:include>
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">
<%
//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"800047"))
{
%>
	<authtype type='3'>	<a href="javascript:add()" class="Link00">[ Registrar Nuevo Tipo de Habilidad ]</a></authtype>
<%
}
%>
		</td>
...
<!-- ================================   S E A R C H   E N G I N E S   E N D   H E R E   ================================ -->
  ...
  <tr>
    <td align="right">
<%
//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"800046"))
{
%>
...
%>
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="center">
<%
//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"800048"))
{
%>
