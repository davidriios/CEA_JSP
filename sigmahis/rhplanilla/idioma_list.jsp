...
//if (!(SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"800049") || SecMgr.checkAccess(session.getId(),"800050") || SecMgr.checkAccess(session.getId(),"800051") || SecMgr.checkAccess(session.getId(),"800052"))) throw new Exception("Usted no tiene los suficientes Derechos de Acceso a esta página.");
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  String codigo  = "";          // variables para mantener el valor de los campos filtrados en la consulta
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");  // utilizada para mantener la descripción del Idioma
	} 
  ...
  else
  {
	...
	sql = "select codigo, descripcion from tbl_pla_idioma "+appendFilter+" order by descripcion";
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) count from tbl_pla_idioma "+appendFilter);
...
function add()
{
	abrir_ventana('../rhplanilla/reg_idioma.jsp');
}

function edit(id)
{
	abrir_ventana('../rhplanilla/reg_idioma.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../rhplanilla/print_list_idioma.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}

function del(codigo){
	var clientIdentifier = '<%=ConMgr.getClientIdentifier()%>';
	if(confirm('Está seguro(a) que desea eliminar?')){
		if(executeDB('<%=request.getContextPath()%>','call sp_par_del_mantenimiento(\'tbl_pla_idioma\',\'codigo\',\''+codigo+'\')','tbl_pla_idioma')){
			var msg = getMsg('<%=request.getContextPath()%>', clientIdentifier);
			if(msg!='') alert(msg);
			else alert('Registro eliminado satisfactoriamente!');
			window.location = '../rhplanilla/idioma_list.jsp';
		} else {
			var msg = getMsg('<%=request.getContextPath()%>', clientIdentifier);
			alert(msg);
		}
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="PLANILLA - IDIOMA"></jsp:param>
</jsp:include>
<table align="center" width="99%" cellpadding="1" cellspacing="0">
  <tr>
    <td align="right">
<%
//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"800051"))
{
%>
	<authtype type='3'>		<a href="javascript:add()" class="Link00">[ Registrar Nuevo Idioma ]</a></authtype>
<%
}
%>
		</td>
...
<!-- ================================   S E A R C H   E N G I N E S   E N D   H E R E   ================================ -->
  ...
  <tr>
    <td align="right">
<%
//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"800050"))
{
%>
...
%>
		<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="center">
<%
//if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),"800052"))
{
%>
