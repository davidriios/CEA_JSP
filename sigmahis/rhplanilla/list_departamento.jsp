...
String nextVal = ""+recsPerPage, previousVal = "1", searchQuery, searchOn = "SO", searchVal = "Todos", searchType = "ST", searchDisp = "SD", searchValDisp = "Todos", searchValFromDate = "SVFD", searchValToDate = "SVTD";
  ...
  {
	sql="select codigo, descripcion from tbl_sec_unidad_ejec where compania="+(String) session.getAttribute("_companyId")+appendFilter+" and nivel ='2' order by codigo";
	alcentro = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_sec_unidad_ejec where compania="+(String) session.getAttribute("_companyId")+" and nivel ='2' "+appendFilter);
...
{
%>
document.title = 'Lista de Direccion - '+document.title;
<%}
%>
...
%>
window.opener.document.form0.ubic_depto.value = eval('document.direccioness.codigo'+k).value;
window.opener.document.form0.ubic_depto_desc.value=eval('document.direccioness.descripcion'+k).value;
	<%
	}	else if(fg.equalsIgnoreCase("rrhh"))	{  
	%>
window.opener.document.form0.ubic_depto.value = eval('document.direccioness.codigo'+k).value;
window.opener.document.form0.ubic_depto_desc.value=eval('document.direccioness.descripcion'+k).value;
	<%
...
{  
%>
window.opener.document.form<%=cTab%>.depto_dest.value = eval('document.direccioness.codigo'+k).value;
window.opener.document.form<%=cTab%>.depto_desc.value=eval('document.direccioness.descripcion'+k).value;
<%
} 
%>
window.close();		
