...
{
	window.opener.location = '../rhplanilla/instructores_config.jsp?change=1&tab=3&mode=<%=mode%>&prov=<%=prov%>&sig=<%=sig%>&tom=<%=tom%>&asi=<%=asi%>&cursoLastLineNo=<%=cursoLastLineNo%>&expLastLineNo=<%=expLastLineNo%>&educaLastLineNo=<%=educaLastLineNo%>';
	window.close();
}
</script>
</head>
