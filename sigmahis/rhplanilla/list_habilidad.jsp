...
String nextVal = ""+recsPerPage, previousVal = "1", searchQuery, searchOn = "SO", searchVal = "Todos", searchType = "ST", searchDisp = "SD", searchValDisp = "Todos", searchValFromDate = "SVFD", searchValToDate = "SVTD";
  ...
  if(fp.equalsIgnoreCase("empleado"))
  	{
	sql="select codigo, descripcion from tbl_pla_habilidad where codigo > 0"+appendFilter+"order by descripcion";
...
document.title = 'Lista de Habilidad - '+document.title;
</script>
</head>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			<table align="center" width="100%" cellpadding="1" cellspacing="1" class="sortable" id="hab">
			<tr class="TextHeader">
				<td width="10%">&nbsp;</td>
			...
			<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
				...
				<td align="center"><%=(vcthabilidad.contains(cdo.getColValue("codigo")))?"Elegido":fb.checkbox("check"+i,cdo.getColValue("codigo"),false,false)%></td>
			</tr>
			<%
			}
			%>	
...
{//Post
 ...
 for (int i=0; i<size; i++)
 {
	...
	cdo.addColValue("habilidad",request.getParameter("codigo"+i));
	cdo.addColValue("habilidadName",request.getParameter("descripcion"+i));
	habilidadLastLineNo++;
	
	String key="";
	if(habilidadLastLineNo<10)
	key="00"+habilidadLastLineNo;
	else if(habilidadLastLineNo<100)
	key="0"+habilidadLastLineNo;
	else key=""+habilidadLastLineNo;
	cdo.addColValue("key",key);
	try 
	{
	hthabilidad.put(key,cdo);
	vcthabilidad.add(cdo.getColValue("habilidad"));
	}//end Try
	catch(Exception e)
	{
...
{
	window.opener.location = '../rhplanilla/expediente_empleado_config.jsp?change=1&tab=3&mode=<%=mode%>&emp_id=<%=emp_id%>&prov=<%=prov%>&sig=<%=sig%>&tom=<%=tom%>&asi=<%=asi%>&educaLastLineNo=<%=educaLastLineNo%>&cursoLastLineNo=<%=cursoLastLineNo%>&habilidadLastLineNo=<%=habilidadLastLineNo%>&entrenimientoLastLineNo=<%=entrenimientoLastLineNo%>&idiomaLastLineNo=<%=idiomaLastLineNo%>&enfermedadLastLineNo=<%=enfermedadLastLineNo%>&medidadLastLineNo=<%=medidadLastLineNo%>&reconocimientoLastLineNo=<%=reconocimientoLastLineNo%>&parienteLastLineNo=<%=parienteLastLineNo%>&fg=<%=fg%>&fp=<%=fg1%>';
	window.close();
}
</script>
</head>
