...
System.out.println("idiomaLastLineNo- antes del if-------->"+request.getParameter("idiomaLastLineNo"));
...
String nextVal = ""+recsPerPage, previousVal = "1", searchQuery, searchOn = "SO", searchVal = "Todos", searchType = "ST", searchDisp = "SD", searchValDisp = "Todos", searchValFromDate = "SVFD", searchValToDate = "SVTD";
  ...
  if (request.getParameter("descripcion") != null && !request.getParameter("descripcion").trim().equals(""))
  {
    ...
    descripcion = request.getParameter("descripcion");
  	...
  	if(fp.equalsIgnoreCase("empleado") || fp.equalsIgnoreCase("solicitud"))
	{
	sql="select codigo, descripcion from tbl_pla_idioma where codigo > 0 "+appendFilter+"order by descripcion";
...
document.title = 'Lista de Idiomas - '+document.title;
</script>
</head>
<body topmargin="0" leftmargin="0" rightmargin="0">
<jsp:include page="../common/title.jsp" flush="true">
<jsp:param name="title" value="RECURSOS HUMANOS - MANTENIMIENTO - EXPEDIENTE "></jsp:param>
</jsp:include>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			<table align="center" width="100%" cellpadding="1" cellspacing="1" class="sortable" id="idim">
			<tr class="TextHeader">
				<td width="10%">&nbsp;</td>
			...
			<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
				...
				<td align="center"><%=(vctidioma.contains(cdo.getColValue("codigo")))?"Elegido":fb.checkbox("check"+i,cdo.getColValue("codigo"),false,false)%></td>
			</tr>
			<%
			}
			%>	
...
{//Post
 ...
 for (int i=0; i<size; i++)
 {
	...
	cdo.addColValue("idioma",request.getParameter("codigo"+i));
	cdo.addColValue("nameidioma",request.getParameter("descripcion"+i));
	idiomaLastLineNo++;
	
	String key="";
	if(idiomaLastLineNo<10)
	key="00"+idiomaLastLineNo;
	else if(idiomaLastLineNo<100)
	key="0"+idiomaLastLineNo;
	else key=""+idiomaLastLineNo;
	cdo.addColValue("key",key);
	try 
	{
	htidioma.put(key,cdo);
	vctidioma.add(cdo.getColValue("idioma"));
	}//end Try
	catch(Exception e)
	{
...
{
%>
	window.opener.location = '../rhplanilla/expediente_empleado_config.jsp?change=1&tab=5&mode=<%=mode%>&emp_id=<%=emp_id%>&prov=<%=prov%>&sig=<%=sig%>&tom=<%=tom%>&asi=<%=asi%>&educaLastLineNo=<%=educaLastLineNo%>&cursoLastLineNo=<%=cursoLastLineNo%>&habilidadLastLineNo=<%=habilidadLastLineNo%>&entrenimientoLastLineNo=<%=entrenimientoLastLineNo%>&idiomaLastLineNo=<%=idiomaLastLineNo%>&enfermedadLastLineNo=<%=enfermedadLastLineNo%>&medidadLastLineNo=<%=medidadLastLineNo%>&reconocimientoLastLineNo=<%=reconocimientoLastLineNo%>&parienteLastLineNo=<%=parienteLastLineNo%>';
<%	} else
	{
	%>
	window.opener.location = '../rhplanilla/solicitud_empleado_config.jsp?change=1&tab=5&mode=<%=mode%>&prov=<%=prov%>&sig=<%=sig%>&tom=<%=tom%>&asi=<%=asi%>&anio=<%=anio%>&cons=<%=cons%>&educaLastLineNo=<%=educaLastLineNo%>&cursoLastLineNo=<%=cursoLastLineNo%>&habilidadLastLineNo=<%=habilidadLastLineNo%>&entrenimientoLastLineNo=<%=entrenimientoLastLineNo%>&idiomaLastLineNo=<%=idiomaLastLineNo%>&enfermedadLastLineNo=<%=enfermedadLastLineNo%>&medidadLastLineNo=<%=medidadLastLineNo%>&reconocimientoLastLineNo=<%=reconocimientoLastLineNo%>&parienteLastLineNo=<%=parienteLastLineNo%>&fg=<%=fg%>&fp=<%=fg1%>';
<%
	}
	%>
	window.close();
