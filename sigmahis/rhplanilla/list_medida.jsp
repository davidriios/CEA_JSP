...
String nextVal = ""+recsPerPage, previousVal = "1", searchQuery, searchOn = "SO", searchVal = "Todos", searchType = "ST", searchDisp = "SD", searchValDisp = "Todos", searchValFromDate = "SVFD", searchValToDate = "SVTD";
  ...
  if (request.getParameter("descripcion") != null && !request.getParameter("descripcion").trim().equals(""))
  {
	...
	{
	sql="select codigo, descripcion from tbl_pla_tipo_medida where codigo > 0 "+appendFilter+"order by descripcion";
...
document.title = 'Lista de Medidas - '+document.title;
</script>
</head>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			<table align="center" width="100%" cellpadding="1" cellspacing="1" class="sortable" id="med">
			<tr class="TextHeader">
				<td width="10%">&nbsp;</td>
			...
			<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
				...
				<!---<td align="center"><%=(vctmedidas.contains(cdo.getColValue("codigo")))?"Elegido":fb.checkbox("check"+i,cdo.getColValue("codigo"),false,false)%></td>---->
...
{//Post
 ...
 String currDatec = CmnMgr.getCurrentDate("dd/mm/yyyy");
 for (int i=0; i<size; i++)
 {
	...
	cdo.addColValue("tipo_med",request.getParameter("codigo"+i));
	cdo.addColValue("medidaName",request.getParameter("descripcion"+i));
	cdo.addColValue("fechamed",currDatec);	
	medidadLastLineNo++;
	
	String key="";
	if(medidadLastLineNo<10)
	key="00"+medidadLastLineNo;
	else if(medidadLastLineNo<100)
	key="0"+medidadLastLineNo;
	else key=""+medidadLastLineNo;
	cdo.addColValue("key",key);
	try 
	{
	htmedida.put(key,cdo);
	vctmedidas.add(cdo.getColValue("tipo_med"));
	}//end Try
	catch(Exception e)
	{
...
{
	window.opener.location = '../rhplanilla/expediente_empleado_config.jsp?change=1&tab=7&mode=<%=mode%>&emp_id=<%=emp_id%>&prov=<%=prov%>&sig=<%=sig%>&tom=<%=tom%>&asi=<%=asi%>&educaLastLineNo=<%=educaLastLineNo%>&cursoLastLineNo=<%=cursoLastLineNo%>&habilidadLastLineNo=<%=habilidadLastLineNo%>&entrenimientoLastLineNo=<%=entrenimientoLastLineNo%>&idiomaLastLineNo=<%=idiomaLastLineNo%>&enfermedadLastLineNo=<%=enfermedadLastLineNo%>&medidadLastLineNo=<%=medidadLastLineNo%>&reconocimientoLastLineNo=<%=reconocimientoLastLineNo%>&parienteLastLineNo=<%=parienteLastLineNo%>&fg=<%=fg%>&fp=<%=fg1%>';
	window.close();
}
</script>
</head>
