...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("searchQuery") != null)
  {
    ...
    searchType = "1";
    searchDisp = "Código";
		codigo     = request.getParameter("codigo");   // utilizada para mantener el Código del Motivo de Falta
	}
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");  // utilizada para mantener la descripción de la Falta
	}
  ...
  else
  {
	...
	sql = "select codigo, descripcion from tbl_pla_motivo_falta"+appendFilter+" order by descripcion";
	al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
	rowCount = CmnMgr.getCount("select count(*) count from tbl_pla_motivo_falta "+appendFilter);
...
document.title = 'Recursos Humanos - Motivos de Faltas - '+document.title;

function add()
{
	abrir_ventana('../rhplanilla/motivofalta_config.jsp');
}

function edit(id)
{
	abrir_ventana('../rhplanilla/motivofalta_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../rhplanilla/print_list_motivofalta.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="RECURSOS HUMANOS -  MANTENIMIENTO - MOTIVOS DE FALTAS"></jsp:param>
</jsp:include>
...
//{
%>
		<authtype type='3'>	<a href="javascript:add()" class="Link00">[ Registrar Nuevos Motivos de Faltas ]</a></authtype>
<%
//}
%>
		</td>
