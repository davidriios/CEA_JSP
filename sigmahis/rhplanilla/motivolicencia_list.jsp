...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  if (request.getParameter("codigo") != null)
  {
    ...
    searchType = "1";
    searchDisp = "Código";
		codigo      = request.getParameter("codigo");     // utilizada para mantener el Código de la Licencia
  }
  ...
  { 
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");  // utilizada para mantener la descripción de la Licencia
  }
  ...
  sql = "select codigo, descripcion, clausula from tbl_pla_motivo_licencia "+appendFilter;
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_pla_motivo_licencia "+appendFilter);
...
document.title = 'Motivo de Licencia con Sueldo - '+document.title;

function add()
{
	abrir_ventana('motivolicencia_config.jsp');
}

function edit(id)
{
	abrir_ventana('motivolicencia_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('print_list_motivolicencia.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="RECURSOS HUMANOS - MOTIVOS PARA TOMAR UNA LICENCIA CON SUELDO"></jsp:param>
</jsp:include>
...
<!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
			...
			<table align="center" width="100%" cellpadding="0" cellspacing="1">
				<tr class="TextHeader">
					...
					<td width="40%">Cl&aacute;usula</td>
					<td width="10%">&nbsp;</td>
				</tr>				
				<%
				...
				<tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
					...
					<td><%=cdo.getColValue("clausula")%></td>					
					<td align="center">&nbsp;
					<%
				//	if (SecMgr.checkAccess(session.getId(),"0") || SecMgr.checkAccess(session.getId(),""))
					//{
