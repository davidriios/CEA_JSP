...
{
  abrir_ventana1('../rhplanilla/param_reportes_aumentos.jsp?fecha='+fecha);
}
</script>
</head>
...
for (int i=0; i<al.size(); i++)
{
	...
	if (i % 2 == 0) color = "TextRow01";
		 ...
		 if (!nFecha.equalsIgnoreCase(cdo.getColValue("fecha")))
				 {
				%>
				  
				<tr align="left" bgcolor="#FFFFFF" class="linksblacklight">
        	<td colspan="10" class="TitulosdeTablas"> [<%=cdo.getColValue("tipo_aumento")%>] - [<%=cdo.getColValue("aumDesc")%>] - Generado el : <%=cdo.getColValue("fecha")%></td>
        </tr>
				<%
				}
		...
		<tr id="rs<%=i%>" class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')">
			...
			<td align="right"><%=cdo.getColValue("nuevaRata")%></td>
