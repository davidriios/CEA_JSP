...
String nextVal = ""+recsPerPage, previousVal = "1", searchQuery, searchOn = "SO", searchVal = "Todos", searchType = "ST", searchDisp = "SD", searchValDisp = "Todos", searchValFromDate = "SVFD", searchValToDate = "SVTD";
  ...
  {
    appendFilter += " where upper(cod_planilla) like '%"+request.getParameter("codigo").toUpperCase()+"%'";
    searchOn = "cod_planilla";
  ...
  {
    appendFilter += " where upper(nombre) like '%"+request.getParameter("descripcion").toUpperCase()+"%'";
    searchOn = "nombre";
  ...
  else
  {
	...
	sql="select cod_planilla codigo, nombre from tbl_pla_planilla where compania = "+(String) session.getAttribute("_companyId")+" "+appendFilter+" order by codigo";
	alcentro = SQLMgr.getDataList(sql);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM tbl_pla_planilla where compania = "+(String) session.getAttribute("_companyId")+" "+appendFilter);
...
function returnValue(code, name){
	<%if(fp.equals("ausencia_rrhh") || fp.equals("ausencia")){%>
		window.opener.document.form.cod_planilla_des<%=id%>.value=code; 
		window.opener.document.form.cod_planilla_des_desc<%=id%>.value=name;
	<%}%>
		window.close(); 
}
</script>
    ...
    <td><!-- ================================   S E A R C H   E N G I N E S   S T A R T   H E R E   ================================ -->
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr class="TextFilter">
          ...
          <td width="35%">&nbsp; <%=fb.textBox("codigo","",false,false,false,30,null,null,null)%> <%=fb.submit("go","Ir")%></td>
          <%=fb.formEnd()%>
          <%fb = new FormBean("search02",request.getContextPath()+request.getServletPath());%>
          <%=fb.formStart()%>
          <%=fb.hidden("id",id)%>
          <td width="15%">&nbsp;Descripci&oacute;n</td>
          <td width="35%">&nbsp; <%=fb.textBox("descripcion","",false,false,false,30,null,null,null)%> <%=fb.submit("go","Ir")%> </td>
          <%=fb.formEnd()%> </tr>
      </table>
    ...
    <td class="TableLeftBorder TableRightBorder"><!-- ================================   R E S U L T S   S T A R T   H E R E   ================================ -->
      <table align="center" width="100%" cellpadding="0" cellspacing="1" class="sortable" id="plan">
        ...
        <%
				...
				%>
        <tr class="<%=color%>" onMouseOver="setoverc(this,'TextRowOver')" onMouseOut="setoutc(this,'<%=color%>')" onClick="javascript:returnValue(<%=cdo.getColValue("codigo")%>,'<%=cdo.getColValue("nombre")%>')" style="cursor:pointer">
  ...
  <tr>
    <td class="TableLeftBorder TableBottomBorder TableRightBorder"><table align="center" width="100%" cellpadding="1" cellspacing="0">
        <tr class="TextPager">
          ...
          <%=fb.formEnd()%> </tr>
        <tr>
          <td colspan="4" align="right"><%=fb.button("cancel","Cancelar",true,false,null,null,"onClick=\"javascript:window.close()\"")%> </td>
        </tr>
      </table></td>
  </tr>
</table>
