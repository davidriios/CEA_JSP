...
sql = "select count(*)  as count, sum(a.aumento) as totmonto, sum((nvl(a.sueldo_anterior,0) + nvl(a.aumento,0)) / nvl(a.rata_x_hora,1)) as totnrata, sum(a.sueldo_anterior) as totsant, sum(nvl(a.sueldo_anterior,0) + nvl(a.aumento,0)) as totnsal, nvl(e.ubic_seccion, seccion) seccion from tbl_pla_aumento_cc a, tbl_pla_cargo c, tbl_pla_empleado e, tbl_pla_tipo_aumento f, tbl_pla_estado_emp g, tbl_sec_unidad_ejec u where a.compania = e.compania and  a.emp_id = e.emp_id and  e.cargo = c.codigo and  e.compania = c.compania  and a.compania = u.compania and nvl(e.ubic_seccion,e.seccion) = u.codigo and  a.tipo_aumento	= f.codigo and  a.compania = f.compania and a.compania= "+(String) session.getAttribute("_companyId")+ appendFilter + " and e.estado = g.codigo group by nvl(e.ubic_seccion,seccion) order by 6";
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	pc.createTable();
		...
		pc.addCols("RATA NUEVA",1,1);
		pc.addCols("SALARIO ANTERIOR",1,1);
		pc.addCols("AUMENTO",1,1);
		pc.addCols("NUEVO SALARIO",1,1);
		
		pc.setFont(7, 0);
		pc.addCols("",0,dHeader.size());
			
		pc.setFont(7, 1);
		pc.addCols("FECHA DE AUMENTO :  ",2,2);
		pc.addCols("  "+fechaProc,0,7);
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.addCols(""+cdo2.getColValue("count")+"  AUMENTO(S)  "+" TOTALES POR UNIDAD :    ",2,6);
		//pc.addCols(" "+cdo2.getColValue("totnrata"),2,1);
		...
		pc.setFont(7, 0);
			...
			pc.setFont(7, 0);
		pc.setVAlignment(0);
		pc.addCols("UNIDAD ADMINISTRATIVA :   "+cdo.getColValue("seccion"),1,2);
		pc.addCols(""+cdo.getColValue("unidadDesc"),0,7);	
		...
		pc.addCols(cdo.getColValue("estDesc"),0,1);
		pc.addCols(cdo.getColValue("fechaIngreso"),1,1);
		pc.addCols(""+CmnMgr.getFormattedDecimal(cdo.getColValue("nuevaRata")),2,1);
	...
	pc.addCols(" TOTALES FINALES :  "+al.size()+"  EMPLEADOS : ",2,6);
