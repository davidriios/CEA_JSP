...
sql = " select ac.cod_acreedor codAcreedor, ac.nombre nombreAcreedor, ac.direccion direccion, ac.telefono telefono, ac.fax fax, decode(ac.forma_pago,1,'CHK',2,'ACH','---')  formaPago, decode(ac.tipo_cuenta,'A','AHO','C','COR','P','EMP','---') tipoCuenta, ac.cuenta_bancaria cuentaBanco, decode(ac.estado,'A','Activo','I','Inactivo') estadoAcreedor, ac.ruc, c.nombre descCia from tbl_pla_acreedor ac, tbl_sec_compania c where c.codigo = ac.compania "+appendFilter+" order by ac.nombre, ac.cod_acreedor ";	
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String subtitle = " LISTADO DE ACREEDORES ";
	String xtraSubtitle = "";
	...
	pc.setFont(8, 1);
	pc.addBorderCols("Cod.",1);
	pc.addBorderCols("NOMBRE DEL ACREEDOR",0);	
	pc.addBorderCols("TELÉFONO",1);	
	pc.addBorderCols("DIRECCIÓN",1);
	pc.addBorderCols("RUC",1);	
	pc.addBorderCols("No. CUENTA BANCARIA",1);
	pc.addBorderCols("FORMA PAGO",1);
	pc.addBorderCols("EST.",1);
	...
	for (int i=0; i<al.size(); i++)
	{
		...
		pc.setVAlignment(0);
			...
			pc.addCols(" "+cdo.getColValue("formaPago"),1,1);			
			pc.addCols(" "+cdo.getColValue("estadoAcreedor"),1,1);		
			pc.setFont(7, 0);
