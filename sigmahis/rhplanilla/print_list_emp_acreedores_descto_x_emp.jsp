...
if (fg.trim().equals("DESC")) titulo = " LISTADO DE DESCUENTOS POR EMPLEADO ";
  else titulo = " LISTADO DE SALDOS DE DESCUENTOS POR EMPLEADO " ;
...
if (fg.trim().equals("DESC")) //DESC = Listado de Descuentos por Empleados
  {		
	pc.setFont(8, 1);
	pc.addBorderCols("No. ACREEDOR",1,1,1.0f,1.0f,0.0f,0.0f);
	pc.addBorderCols("NOMBRE DEL ACREEDOR",0,1,1.0f,1.0f,0.0f,0.0f);	
	pc.addBorderCols("        MONTO DEL DESCUENTO",0,1,1.0f,1.0f,0.0f,0.0f);
	pc.addBorderCols(" ",1,1,1.0f,1.0f,0.0f,0.0f);	
  }else 
   {             // Listado de Saldos de Descuentos por Empleados
    pc.setFont(8, 1);
	pc.addBorderCols("No. ACREEDOR",1,1,1.0f,1.0f,0.0f,0.0f);
	pc.addBorderCols("NOMBRE DEL ACREEDOR",0,1,1.0f,1.0f,0.0f,0.0f);	
	...
	//Inicio --Agrupamiento por Empleado
		if (!groupBy.equalsIgnoreCase("[ "+cdo.getColValue("numEmpleado")+" ] "+cdo.getColValue("nombreEmpleado")))
		{ // groupBy
			if (i != 0)
			  {//i-1 
				...
				{
					pc.setFont(7, 1);
					pc.addCols("CANT. DE DESCUENTOS ==> "+" . . . "+pxu,0,2);
					pc.addBorderCols("MONTO TOTAL ==> "+CmnMgr.getFormattedDecimal(totDescuento),2,1,0.0f,1.0f,0.0f,0.0f);
					pc.addCols(" ",0,1);
		...
		pc.addCols(" "+cdo.getColValue("cedula"),0,2);
				pxs++;
				totDescuento = 0;
		}//Final --Agrupamiento por Empleado
	...
	if (fg.trim().equals("DESC")) //DESC = Listado de Descuentos por Empleados
	 {		
	  ...
	  groupBy = "[ "+cdo.getColValue("numEmpleado")+" ] "+cdo.getColValue("nombreEmpleado");
	  pxu++;	   
	  //totDescuento += (Double.parseDouble(cdo.getColValue("descMensual")));
	...
	{	
	  ...
	  pc.setFont(9,0);
	  pc.addCols("TOTAL DE DESCUENTOS POR EMPLEADOS "+" . . . "+pxs,1,dHeader.size());	
	}
	pc.addTable();
	pc.close();
