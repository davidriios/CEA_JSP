...
if (descMenor  == null || descMenor == "") descMenor  = "9999999999.99";
...
sql= " select da.cod_acreedor codAcreedor, ac.nombre nombreAcreedor, e.num_empleado numEmpleado, nvl(e.cedula1,e.cedula_beneficiario) cedula, e.nombre_empleado nombreEmpleado, d.num_documento numDocumento, d.saldo saldo, da.cod_grupo codGrupo, gr.nombre descGrupo, sum(da.monto) montoTotal from tbl_pla_planilla_encabezado pe, tbl_pla_descuento_aplicado da, vw_pla_empleado e, tbl_pla_descuento d, tbl_pla_acreedor ac, tbl_pla_grupo_descuento gr where pe.cod_planilla in (select cod_planilla from planilla where beneficiarios = 'EM') and (da.anio = pe.anio and da.cod_planilla = pe.cod_planilla and da.num_planilla = pe.num_planilla and da.cod_compania = pe.cod_compania) and ( e.emp_id = da.emp_id  and e.compania = da.cod_compania) and (d.emp_id = da.emp_id and d.cod_compania = da.cod_compania and d.num_descuento = da.num_descuento) and (d.cod_acreedor = ac.cod_acreedor) and (d.cod_compania = ac.compania) and gr.cod_grupo = da.cod_grupo "+appendFilter+" group by da.cod_acreedor, ac.nombre, e.num_empleado, nvl(e.cedula1,e.cedula_beneficiario), e.nombre_empleado, d.num_documento, d.saldo, da.cod_grupo, gr.nombre having sum(da.monto) >= "+descMayor+" and sum(da.monto) <= "+descMenor+" order by da.cod_acreedor, e.nombre_empleado ";  
...
if (request.getMethod().equalsIgnoreCase("GET"))
{
	...
	String subtitle = " TOTAL DESCONTADO POR ACREEDOR (INICIO DE DESCTO.)";
	String xtraSubtitle = "";
	...
	pc.addBorderCols("DESCONTADO",1,1,1.0f,1.0f,0.0f,0.0f);	  
	...
	//Inicio --Agrupamiento por Acreedor
		...
		pc.addCols("Acreedor:  "+"[ "+cdo.getColValue("codAcreedor")+" ] "+cdo.getColValue("nombreAcreedor"),0,dHeader.size());		 
			 ...
			 if (!grupoDesc.equalsIgnoreCase(""))
			  {
				pc.addBorderCols(" Grupo de Descuento:  "+cdo.getColValue("descGrupo"),0,dHeader.size(),1.0f,1.0f,0.0f,0.0f);
			  }else pc.addBorderCols(" ",0,dHeader.size(),1.0f,1.0f,0.0f,0.0f);	
		...
		pc.setVAlignment(0);
		 ...
		 totDescuento += (Double.parseDouble(cdo.getColValue("montoTotal")));	  
