...
com = (Company) sbb.getSingleRowBean(ConMgr.getConnection(),sql,Company.class);
		  ...
		  sql = "select a.tipo_reg as Tipo, a.reg_control as Control, a.salario_limite as SalarioLimite, a.cod_compania as cia, e.codigo as cod, e.nombre as namecia, e.num_patronal numpat, e.cedula_juridica cedjur, a.correlativo, a.salario_limite salmin, nvl(a.valor_dependiente,0) dependiente, nvl(a.seg_soc_emp,0) sse, nvl(a.seg_edu_emp,0) see, nvl(a.prima_antig,0) prima, nvl(a.porc_endeudamiento,0) deuda, nvl(a.gr_porc_no_renta,0) grnr, nvl(a.seg_soc_grep_emp,0) ssgre, nvl(a.gr_limite_no_renta,0) grlnr, decode(a.estado,'A','ACTIVA','I','INACTIVA') as estado from tbl_pla_parametros a, tbl_sec_compania e where a.cod_compania=e.codigo " +appendFilter+" order by a.cod_compania";
...
String logoPath = java.util.ResourceBundle.getBundle("path").getString("companyimages")+"/"+((_comp.getLogo() != null && !_comp.getLogo().trim().equals(""))?_comp.getLogo():"blank.gif");
		...
		String fileNamePrefix = "print_parametros";
...
int no2 = 0;
			for (int j=1; j<=nPages; j++)
			{
				...
				pc.createTable();
					pc.setFont(9, 1);
					pc.addCols("PLANILLA",1,2);
					pc.addTable();

				pc.createTable();
					pc.setFont(9, 1);
					pc.addCols("PARAMETROS",1,2);
					pc.addTable();
				pc.createTable();
			        pc.setFont(7, 1);
				...
				pc.createTable();
					pc.setFont(7, 1);
					pc.addBorderCols("Código. Cía",0);
					pc.addBorderCols("Nombre de Compañía",1);
					pc.addBorderCols("Correlativo",0);
					pc.addBorderCols("Salario Mínimo",1);
					pc.addBorderCols("Valor por Depend.",1);
					pc.addBorderCols("Seg. Social",1);
					pc.addBorderCols("Seguro Educ.",1);
					pc.addBorderCols("Prima de Antig.",1);
					pc.addBorderCols("% Deuda",1);
				...
				pc.addTable();
					...
					pc.createTable();
						pc.setFont(7, 1);
						pc.addCols("No Existen Parámetros Registrados.",1,10);
					pc.addTable();
					...
					for (int i=((maxLines * j) - maxLines); i<(maxLines * j); i++)
					{
							...
							pc.createTable();
							pc.setFont(7, 1);
							pc.addCols(" "+cdo1.getColValue("cod"), 0,1);
							pc.addCols(" "+cdo1.getColValue("namecia"), 0,1);
							pc.addCols(" "+cdo1.getColValue("correlativo"), 0,1);
							pc.addCols(" "+cdo1.getColValue("salmin"), 1,1);
							pc.addCols(" "+cdo1.getColValue("dependiente"), 1,1);
							pc.addCols(" "+cdo1.getColValue("sse"), 1,1);
							pc.addCols(" "+cdo1.getColValue("see"), 1,1);
							pc.addCols(" "+cdo1.getColValue("prima"), 1,1);
							pc.addCols(" "+cdo1.getColValue("deuda"), 1,1);
							pc.addCols(" "+cdo1.getColValue("estado"), 1,1);
							...
							pc.createTable();
								pc.addCols(al.size()+" Registros en total",0,10);
							pc.addTable();
