...
if (request.getMethod().equalsIgnoreCase("GET")) {
	...
	if (!lote.equals("")) {
		sbSql.append(" and x.id_lote = ");
		sbSql.append(lote);
	}
	if (incompletos.trim().equals("X")){
			sbSql.append(" /* and (( x.entrada is null or x.salida is null ) or editado ='S')  */ ");
		}
	
	if (!fecha.equals("")) {
		sbSql.append(" and to_date(x.dia||'/'||x.mes||'/'||x.anio,'dd/mm/yyyy') = ");
...
<%=fb.hidden("fecha",""+fecha)%>
		<tr class="TextFilter">
			...
			<td width="18%" align="right">Nombre</td>
		...
		<tr>
			<td colspan="8">
				...
				<tr class="TextHeader" align="center"> 
					...
					<td width="15%" rowspan="2">Entrada</td>
					<td colspan="2">Comida</td>
					<td width="15%" rowspan="2">Salida</td>
					<td width="2%" rowspan="2">&nbsp;</td>
					<td width="6%" rowspan="2">Prog. Turno</td>
					<td width="27%" rowspan="2">Horario Programado o Asignado</td>
					<td width="3%" rowspan="2">&nbsp;</td>
				</tr>
				<tr class="TextHeader" align="center">
  ...
  for (int i=0; i<al.size(); i++) { cdo = (CommonDataObject) al.get(i); String evt = sbEvt.toString(); evt = evt.replaceAll("IDX",""+i); %>
				...
				<tr class="TextRow01" align="center" id="marc<%=i%>"> 
					...
					<jsp:include page="../common/calendar.jsp" flush="true">
										...
										<jsp:param name="nameOfTBox1" value="<%="entrada"+i%>" />
										<jsp:param name="valueOfTBox1" value="<%=cdo.getColValue("entrada")%>" />
					...
					<jsp:include page="../common/calendar.jsp" flush="true">
										...
										<jsp:param name="nameOfTBox1" value="<%="salida"+i%>" />
										<jsp:param name="valueOfTBox1" value="<%=cdo.getColValue("salida")%>" />
										<jsp:param name="jsEvent" value="<%=""%>"/>
										<jsp:param name="onChange" value="<%=""%>"/>
										<jsp:param name="readonly" value="<%="n"%>"/>
										</jsp:include>
...
<%
	...
	for (int i=0; i<size; i++) {
		...
		cdo.addColValue("salida",(request.getParameter("salida"+i).equals(""))?"":request.getParameter("salida"+i));
...
function openMode(){window.location='<%=request.getContextPath()+request.getServletPath()%>?mode=<%=mode%>&grupo=<%=grupo%>&area=<%=area%>&iDate=<%=issi.admin.IBIZEscapeChars.forURL(iDate)%>&fDate=<%=issi.admin.IBIZEscapeChars.forURL(fDate)%>&empId=<%=empId%>&incompletos=<%=incompletos%>&lote=<%=lote%>&fg=<%=fg%>&fecha=<%=fecha%>';}
</script>
</head>
