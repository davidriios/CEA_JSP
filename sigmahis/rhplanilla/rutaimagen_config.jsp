...
if (mode.equalsIgnoreCase("edit"))
	...
	sql =   "Select DISTINCT a.emp_id, a.provincia|| '-' ||a.sigla|| '-' ||a.tomo|| '-' ||a.asiento as cedula, a.firma, a.provincia, a.sigla, a.tomo, a.asiento, a.compania, a.primer_nombre as nombre1, nvl(a.segundo_nombre,' ')  as nombre2, a.primer_apellido as apellido1, nvl(a.segundo_apellido,' ') as apellido2, nvl(a.apellido_casada, ' ') as casada, a.primer_nombre||' '||a.primer_apellido nombre, a.num_empleado as numEmpleado, decode(a.foto,null,' ','"+java.util.ResourceBundle.getBundle("path").getString("fotosimages").replaceAll(java.util.ResourceBundle.getBundle("path").getString("root"),"..")+"/'||a.foto) as foto, a.firma, a.cargo, b.denominacion as nameCargo from tbl_pla_empleado a, tbl_pla_cargo b where a.compania="+(String) session.getAttribute("_companyId")+" and a.compania = b.compania and a.cargo = b.codigo and  a.compania_uniorg="+(String) session.getAttribute("_companyId")+" and  a.emp_id ="+empid;
 ...
 <tr> 
		<td> 
			<table width="100%" cellpadding="1" cellspacing="1" border=1 bordercolor="#d0deea" style="border-collapse:collapse; border-top:1.0pt solid #d0deea; border-bottom:1.0pt solid #d0deea; border-left:1.0pt solid #d0deea; border-right:1.0pt solid #d0deea;">
				...
				<td width="35%"><%=cdo.getColValue("nameCargo")%></td>
				</tr>	
				 <tr class="TextHeader">
				...
				<tr class="TextRow01">
						<td>Firma </td>
						<td><%=fb.textBox("firma",cdo.getColValue("firma"),false,false,false,40)%></td>
						<td>Foto</td>
						<td><%=fb.fileBox("foto",cdo.getColValue("foto"),false,false,20)%></td>
...
else
{
	...
	cdo.addColValue("firma",(String) ht.get("firma"));
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/rhplanilla/rutaimagen_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/rhplanilla/rutaimagen_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/rhplanilla/rutaimagen_list.jsp';
<%
	}
%>
