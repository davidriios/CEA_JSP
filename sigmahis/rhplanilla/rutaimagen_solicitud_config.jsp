...
if (mode.equalsIgnoreCase("edit"))
	...
	sql = "select distinct  provincia|| '-' ||sigla|| '-' ||tomo|| '-' ||asiento as cedula, provincia, sigla, tomo, asiento, compania, primer_nombre as nombre1, nvl(segundo_nombre,' ')  as nombre2, primer_apellido as apellido1, nvl(segundo_apellido,' ') as apellido2, primer_nombre||' '||primer_apellido nombre, decode(foto,null,' ','"+java.util.ResourceBundle.getBundle("path").getString("fotosimages").replaceAll(java.util.ResourceBundle.getBundle("path").getString("root"),"..")+"/'||foto) as foto, to_char(fecha_solicitud,'dd/mm/yyyy') as fechaSol, to_char(fecha_nacimiento,'dd/mm/yyyy') as fecha from tbl_pla_solicitante where compania = "+(String) session.getAttribute("_companyId")+" and anio = "+anio+" and consecutivo = "+cons;
...
document.title=" Ruta de Imagen Soicitante- "+document.title;
<%}else if(mode.equalsIgnoreCase("edit")){%>
document.title=" Ruta de Imagen Solicitante Edición- "+document.title;
<%}%>
</script>
</head>
			...
			<table width="100%" cellpadding="1" cellspacing="1" border=1 bordercolor="#d0deea" style="border-collapse:collapse; border-top:1.0pt solid #d0deea; border-bottom:1.0pt solid #d0deea; border-left:1.0pt solid #d0deea; border-right:1.0pt solid #d0deea;">
				...
				<tr> 
					...
					<table width="100%" cellpadding="1" cellspacing="1" border=1 bordercolor="#d0deea" style="border-collapse:collapse;">				
							 <tr class="TextHeader">
                    <td colspan="4">Datos del Solicitante</td>
                  </tr>						
				<tr class="TextRow01">
				<td width="15%">Nombre</td>
				<td width="29%"><%=cdo.getColValue("nombre")%></td>				
				<td width="21%">Cédula </td>
				<td width="35%"><%=cdo.getColValue("provincia")%>-<%=cdo.getColValue("sigla")%>-<%=cdo.getColValue("tomo")%>-<%=cdo.getColValue("asiento")%></td>
				</tr>	
				<tr class="TextRow01">
				<td width="15%">Fecha de Nacimiento</td>
				<td width="29%"><%=cdo.getColValue("fecha")%></td>
				<td width="21%">Fecha de Solicitud </td>
				<td width="35%"><%=cdo.getColValue("fechaSol")%></td>
				</tr>	
							 <tr class="TextHeader">
...
else
{
	...
	cdo.setTableName("tbl_pla_solicitante");
	...
	cdo.addColValue("anio",(String) ht.get("anio"));	
	cdo.addColValue("consecutivo",(String) ht.get("cons"));
	...
	else
	{
		...
		cdo.setWhereClause("compania="+(String) session.getAttribute("_companyId")+" and provincia="+prov+" and sigla='"+sig+"' and tomo="+tom+" and asiento="+asi+" and anio = "+anio+" consecutivo = "+cons);
...
<%
	if (session.getAttribute("_urlInfo") != null && ((Hashtable) session.getAttribute("_urlInfo")).containsKey(request.getContextPath()+"/rhplanilla/rutaimagen_solicitud_list.jsp"))
	{
%>
	window.opener.location = '<%=(String) ((Hashtable) session.getAttribute("_urlInfo")).get(request.getContextPath()+"/rhplanilla/rutaimagen_solicitud_list.jsp")%>';
<%
	}
	else
	{
%>
	window.opener.location = '<%=request.getContextPath()%>/rhplanilla/rutaimagen_solicitud_list.jsp';
<%
	}
%>
