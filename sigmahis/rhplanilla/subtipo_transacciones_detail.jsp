...
function doSubmit()
{
   ...
   document.formSub.factorMulti.value = parent.document.form0.factorMulti.value;
   document.formSub.clase_trx.value = parent.document.form0.clase_trx.value;
...
<!-- ================================   F O R M   S T A R T   H E R E   ================================ -->
			...
			<%fb.appendJsValidation("if(document.formSub.baction.value!='Guardar')return true;");%>
			<%=fb.formStart(true)%>
			<%=fb.hidden("baction","")%>
			...
			<%=fb.hidden("keySize",""+iTrans.size())%>
			    ...
			    %>
				 <tr class="TextRow01"><%=fb.hidden("key"+i,key)%><%=fb.hidden("remove"+i,"")%><%=fb.hidden("fechaCreacion"+i,sub.getFechaCreacion())%><%=fb.hidden("usuarioCreacion"+i,sub.getUsuarioCreacion())%>
					 ...
					 <td><%=fb.decBox("monto"+i,sub.getMonto(),false,false,false,17,6.2)%>
	  ...
	  if (request.getParameter("baction") != null && request.getParameter("baction").equals("Agregar"))
	  {
		...
		return;
		 ...
		 tip.setDescripcion(request.getParameter("tipoDesc"));
		  tip.setFactorMulti(request.getParameter("factorMulti"));
		  tip.setClaseTrx(request.getParameter("clase_trx"));
