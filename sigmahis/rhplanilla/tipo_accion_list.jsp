...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  else if (request.getParameter("descripcion") != null)
  {
    ...
    searchDisp = "Descripción";
		descrip    = request.getParameter("descripcion");  // utilizada para mantener la descripción de la Acción filtrada
  }
...
sql = "SELECT a.codigo, a.descripcion FROM TBL_PLA_AP_TIPO_ACCION a WHERE a.codigo<>0"+appendFilter+" order by a.codigo";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM TBL_PLA_AP_TIPO_ACCION a WHERE a.codigo<>0"+appendFilter);
...
document.title = 'Recursos Humanos - Tipos de Acciones - '+document.title;

function add()
{
	abrir_ventana('../rhplanilla/tipo_acciones_config.jsp');
}

function edit(id)
{
	abrir_ventana('../rhplanilla/tipo_acciones_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../rhplanilla/print_list_tipoacciones.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="RECURSOS HUMANOS - MANTENIMIENTOS - TIPOS DE ACCIONES"></jsp:param>
</jsp:include>
...
<table align="center" width="99%" cellpadding="1" cellspacing="0">
    <tr>
        ...
        <%
		  ...
		  //{
        %>
	    <authtype type='3'>   <a href="javascript:add()" class="Link00">[ Registrar Nuevo Tipo de Acci&oacute;n ]</a></authtype>
   	    <%
		  //}
	    %>
	    </td>
