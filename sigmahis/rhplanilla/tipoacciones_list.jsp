...
if (request.getMethod().equalsIgnoreCase("GET"))
{
  ...
  sql = "SELECT codigo, descripcion FROM TBL_PLA_TIPO_ACCION_EVALUACION WHERE codigo<>0"+appendFilter+" order by descripcion";
  al = SQLMgr.getDataList("select * from (select rownum as rn, a.* from ("+sql+") a) where rn between "+previousVal+" and "+nextVal);
  rowCount = CmnMgr.getCount("SELECT count(*) FROM TBL_PLA_TIPO_ACCION_EVALUACION WHERE codigo<>0"+appendFilter);
...
function add()
{
	abrir_ventana('../rhplanilla/tipoacciones_config.jsp');
}

function edit(id)
{
	abrir_ventana('../rhplanilla/tipoacciones_config.jsp?mode=edit&id='+id);
}

function printList()
{
	abrir_ventana('../rhplanilla/print_list_tipo_accion.jsp?appendFilter=<%=IBIZEscapeChars.forURL(appendFilter)%>');
}
</script>
</head>
...
<jsp:include page="../common/title.jsp" flush="true">
	<jsp:param name="title" value="RECURSOS HUMANOS - TRANSACCIONES - TIPOS DE ACCIONES"></jsp:param>
</jsp:include>
